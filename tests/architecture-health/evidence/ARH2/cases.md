...
Behavioral evidence: every pinned lane is a real cargo nextest command over a live workspace member crate, and every witness is a real `#[test]`/`#[tokio::test]` function of a real file whose nextest id contains the filter (module-path fragment of the file under its crate directory, joined directly or through the trailing `tests` module). The lanes are the targeted-domain gate commands of this node and were run green on the reviewed candidate; their outcome is pass/fail only — cost belongs to the measurements product, never to a pin.
...
- `pin-command-not-canonical` / `pin-filter-malformed` / `pin-without-witnesses`: a pin's command is exactly `cargo nextest -p <crate> <filter>` with a non-flag filter.
...
- `pin-filter-selects-nothing`: the filter is a substring of the witness's nextest id under module-path semantics (dirty twin swaps the filter to `unrelated_module`).
...
- `gate-cell-unknown`: every gate-cell mechanism is a locked cell id of `performance-gates.toml` (dirty twin cites `B6_INVENTED_CELL`).
- `behavior-lane-unbound` / `behavior-lane-invented`: the behavior dimension's lane list is exactly the characterization product's pinned command set, both directions (dirty twins splice out `stable_key_tests` and append a phantom lane).
- `runner-class-unbound`: the number policy binds the locked `[runner]` class of `performance-gates.toml`.
