...
Clean products validate: the five ARH0 god-module candidates each carry exactly one hotspot contract, the declared import direction equals the import tree measured from the live source (comments stripped; nested `use` groups expanded with prefix inheritance so brace-list items never classify as roots and local `macro_rules!` re-exports name file locals), the layer rules equal the live crate Cargo manifests, constructor anchors and retained/narrowed surface items bind real declarations, state rows name live identifiers with one sole owner, and every declared route has a cutover disposition.
...
- `import-drift`: the declared verter/external/crate-internal sets must equal the measured import tree of the live source — a declared-but-unmeasured import (dirty twin adds `verter_semantic` to the scheduler file) and a measured-but-undeclared one (dirty twin undeclares `verter_language`) both fail, as does a measured import with no dependency in the owning crate's manifest.
...
- `state-without-owner` / `state-owner-missing`: every state row names one sole owner, and `.rs` owner paths exist.
...
- `retained-item-unconsumed`: for `semantic_query` (whose twelve external consumers are the complete cross-crate population, flagged `crossCrateRetainedVerifiedByConsumers`) every retained type must be imported by a declared consumer; the scheduler's retained surface serves the verter_session population, so its proof is scoped and documented instead of asserted.
...
- `cutover-route-missing`: every hotspot declaring narrowing rows must have a register row binding that route to the executing heir (dirty twin drops `ARH1-CUT-4`, uncovering the `semantic_query.rs` bulk narrowing).
