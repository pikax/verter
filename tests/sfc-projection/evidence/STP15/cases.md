...
Changed symbols: `BindingKind` (unchanged variants; `toRefs` whole-result now classifies `Plain`); `WriteRejection::ConstBinding` (new) with `TemplateWriteTarget::immutable` rows and the `write_target` refusal; `push_binding`/`classify_declarator` take declaration mutability (`const` plains refuse writes, all other kinds unchanged); `setter_domain` asserts its span instead of clamping; `BindingUsageSet::from_region_text` (new) collecting template mentions (HTML comments removed), script identifier references, and style `v-bind()` names over `from_authored_references`; `VueProjectionBackend::binding_usage` (new) accounting usage from the admitted carrier's own template/script/style regions with declared names from the read rows; `OptionsMember::setter_domain` (new) carrying the authored Options `set(v: Domain)` annotation into the write domain; `defineProps` rest-spread locals bind readonly rows; type-only imports bind no read row; `FACTORY_EXPORTS` membership cited to the Vue reactivity-core and script-setup API documents plus the pinned `vue@3.6.0-rc.5` surface and checked against the parsed `from 'vue'` imports; `protocol.mjs` requires per-lane `test result` evidence, per-discriminator `FAILED` rejection proof, and five applied twins including the const-plain control.
