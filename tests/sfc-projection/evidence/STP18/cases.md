...
Transaction shape: `const W = new (__VerterUseComponent(Comp, __VerterUseConstructor(Comp)))({ ... })`. The callee is the foreign component contract adapter (see `tests/sfc-projection/evidence/STP19/cases.md`): the component's own contract first, then its signature with an `unknown` attribute index on the props parameter, so TypeScript infers the binder once at the construction from every member, contextually types callbacks, keeps literal types and checks declared keys, while a fallthrough attribute is not an excess-key error (excess-key policy is STP20's obligation). Members follow the runtime property plan: certainly overwritten writes are excluded, `v-bind` object spreads keep their merge position, and the first handler of each listener key is placed after every spread; further listeners, repeated `class`/`style` writes and inline statements are validation-only checks against the specialized instance (`__VerterUseListener` / `__VerterUseProp`). Listener checks read `__VerterUseListener`, which keeps a declared type (an author-written `any` included) and is untyped only for an undeclared key; an event-option key (`.once` / `.capture` / `.passive`) is a check against its own key when declared, else against the unsuffixed listener key the runtime strips the options from. Handler shape comes from the parse: identifiers and member expressions (optional chains included) and function expressions are the handler; any other single expression is the returned body of a `$event` handler, a statement list its block body. Reserved keys are exactly the runtime `isReservedProp` set. Slot, event, model and instance observations read the witness binding. The specialization key digests the logical use and its checking inputs, never source offsets.
