...
- a generic rest constructor that publishes its inferred `args: T` tuple is selected by paired helper overloads before conditional reflection: its binder, inferred tuple and any preceding construct overloads remain on the declared constructor path, rather than treating a constraint-instantiated `any[]` as Vue's non-generic catch-all;
