...
describe("VerterHost", () => {
  ...
  it("routes a TypeScript Svelte carrier to both IDE and runtime surfaces", () => {
    // T4 made TypeScript + legacy script lowering a supported runtime surface.
    // This public NAPI assertion proves both carrier outputs survive the FFI;
    // a Vue-only route would expose `_sfc_main`, while the former pre-T4 route
    // would refuse with `svelte-runtime-unsupported-typescript`.
    const host = new VerterHost();
    host.upsert({
    ...
    expect(main.code).not.toContain("_sfc_main");
...
<template><div>{{ view }}</div></template>`,
  ...
  // The assembled runtime-client map survives the public typed transport
  // with real authored coordinates — a script anchor after a non-BMP
  // character (2 UTF-16 units) maps back to its exact authored
  // line/column, and the same request with the map off returns no map and
  // identical code. JS string indexes are UTF-16 code units, so every
  // column computed below is in the map's own coordinate space.
  it("carries authored anchors through the assembled runtime-client source map", () => {
    const host = new VerterHost();
    const source =
      '<script setup lang="ts">\nconst label = "\u{1F389}"; const count = 1\n</script>\n<template>\n  <div>\u{1F389}{{ count }}</div>\n</template>\n';
    host.upsert({ inputId: "AssemblyMap.vue", fileKind: "vue", source });

    const compile = (runtimeSourceMap: boolean) =>
      host.compileRequest("AssemblyMap.vue", {
        framework: "vue",
        identity: { isProduction: false, forceJs: false },
        products: [{ kind: "runtimeClient", runtimeSourceMap }],
        options: { backend: "inferred", ssr: false, isCustomElement: [], babelParserPlugins: [] },
      });
    const mainOf = (response: any) =>
      response.products
        .flatMap((product: any) => product.nodes ?? [])
        .find((node: any) => node.node.kind === "main");

    const mapped = mainOf(compile(true));
    expect(mapped).not.toBeUndefined();
    const unmapped = mainOf(compile(false));
    expect(unmapped.sourceMap).toBeUndefined();
    expect(unmapped.code).toBe(mapped.code);

    const map = JSON.parse(mapped.sourceMap as string);
    expect(map.sources.every((name: string) => name === "AssemblyMap.vue")).toBe(true);
    expect(map.sourcesContent).toContain(source);

    // Generated side: `count` inside the verbatim `const count = 1` line.
    const anchorIndex = mapped.code.indexOf("const count = 1") + "const ".length;
    const generatedPrefix = mapped.code.slice(0, anchorIndex);
    const generatedLine = generatedPrefix.split("\n").length - 1;
    const generatedColumn = generatedPrefix.length - generatedPrefix.lastIndexOf("\n") - 1;
    // Authored side: line 1, and the column after `🎉` counted as 2 units.
    const authoredLine = 1;
    const authoredColumn = source.split("\n")[1].indexOf("count");

    expect(mappingAt(map.mappings as string, generatedLine, generatedColumn)).toEqual([
      authoredLine,
      authoredColumn,
    ]);
  });
...
/** Resolve the ABSOLUTE authored `[srcLine, srcCol]` of the segment covering
 *  generated `(line, column)` — VLQ deltas for source positions accumulate
 *  across lines, generated columns reset per line. Returns `null` when no
 *  mapped segment covers the position (unmapped generated bytes). */
function mappingAt(mappings: string, line: number, column: number): [number, number] | null {
  const BASE64 = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
  const groups = mappings.split(";");
  let sourceLine = 0;
  let sourceColumn = 0;
  let covering: [number, number] | null = null;
  for (let groupIndex = 0; groupIndex <= line && groupIndex < groups.length; groupIndex++) {
    ...
    for (const raw of group.split(",")) {
      ...
      if (values.length >= 4) {
        ...
        if (groupIndex === line && generatedColumn <= column) {
          covering = [sourceLine, sourceColumn];
        }
      } else if (groupIndex === line && generatedColumn <= column) {
        // A 1-field segment marks generated bytes unmapped from here.
        covering = null;
      }
    }
  }
