...
function extractTarball(tarball: string, destPkgDir: string): void {
  ...
  // Run from the archive directory and pass only its basename. This avoids
  // GNU tar's drive-letter-as-remote ambiguity on Windows without relying on
  // the GNU-only `--force-local` option, which bsdtar does not implement.
  const fwd = (p: string) => p.replace(/\\/g, "/");
  execFileSync("tar", ["-xzf", basename(tarball), "-C", fwd(destPkgDir), "--strip-components=1"], {
    cwd: dirname(tarball),
    stdio: ["ignore", "pipe", "pipe"],
  });
}
...
`,
        ...
        expect(result.hasTransformVueStyle).toBe(true);
        expect(result.hasPrepareStyleForPreprocessor).toBe(true);
