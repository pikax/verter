...
const selectedBundlers =
  selectedBundler === "all" ? bundlers : bundlers.filter((b) => b.name === selectedBundler);

if (selectedBundlers.length === 0) {
  console.error(
    `Unknown bundler: ${selectedBundler}. Valid: ${bundlers.map((b) => b.name).join(", ")}`,
  );
  process.exit(1);
}
...
async function runPlaywright(bundlerName: string): Promise<boolean> {
  try {
    await execaCommand(`npx playwright test --config e2e/playwright.config.ts`, {
      cwd: unpluginDir,
