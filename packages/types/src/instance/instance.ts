...
> = {
  ...
  props: ExtractPropsFromMacro<T["props"]> & ModelToProps<MacroToModelRecord<T["model"]>>;
  attrs: Attrs;
  refs: T["templateRef"];
  emit: MacroToEmitValue<T["emits"]> & ModelToEmits<MacroToModelRecord<T["model"]>>;
...
type MakeInternalInstanceFromNormalisedMacro<
  ...
  DEV extends boolean = false,
> =
  CreateTypedInternalInstanceFromNormalisedMacro<T, Attrs, DEV> extends infer I
...
export type InternalInstanceFromMacro<T, Attrs = {}, DEV extends boolean = false> =
...
export type ToInstanceProps<T, MakeDefaultsOptional extends boolean> =
...
> = {
    ...
    ToInstanceProps<T["props"], MakeDefaultsOptional> &
      ...
      (MacroToModelRecord<T["model"]> extends infer M
        ? (ModelToProps<M> extends infer MP
            ? MakeDefaultsOptional extends true
              ? ExtractOptionalModel<T["model"]> extends infer O extends keyof MP
  ...
  ? unknown extends V
    ? ToInstanceProps<T["props"], true> & ModelToProps<MacroToModelRecord<T["model"]>>
...
export type PublicInstanceFromMacro<
  ...
  AttrsProps extends boolean = false,
  DEV extends boolean = false,
  InternalInstance extends false | MakeInternalInstanceFromNormalisedMacro<any, any> = false,
> =
  NormaliseMacroReturn<T> extends infer NT extends NormalisedMacroReturn<any>
...
> = Omit<
  import("vue").ComponentPublicInstance<
    ...
    T["expose"] extends { object: infer V } ? (unknown extends V ? "" : keyof V) : "",
    {},
    El
  >,
...
export type CreateExportedInstanceFromNormalisedMacro<
  ...
  DEV extends boolean = false,
> = PublicInstanceFromNormalisedMacro<T, Attrs, El, true, DEV> extends infer I ? I : {};
 ...
 * ```
 */
export type SFCPublicInstanceFromMacro<T, Attrs, El extends Element> = PublicInstanceFromMacro<
  T,
  Attrs,
  El,
  false,
 ...
 * ```
 */
export type ExternalPublicInstanceFromMacro<T, Attrs, El extends Element> = PublicInstanceFromMacro<
  T,
  Attrs,
  El,
  true,
