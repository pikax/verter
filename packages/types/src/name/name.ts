 ...
 */
export type CanCapitalize<T extends string> = T extends "-" | "_" | "$" | "" ? "x" : Capitalize<T>;

/**
 * Converts PascalCase strings to kebab-case.
 * Handles transitions between uppercase and lowercase letters.
 */
export type PascalToKebab<T extends string> = T extends `${infer Left}${infer Right}${infer Rest}`
 ...
 */
export type CamelToKebab<T extends string> = T extends `${infer Left}${infer Right}${infer Rest}`
 ...
 */
export type NormalisePascal<T extends string> =
...
type _HyphenateImpl<S extends string, IsStart extends boolean> = S extends `${infer F}${infer R}`
