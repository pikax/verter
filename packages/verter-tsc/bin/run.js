...
function findBinary() {
  ...
  const devBuild = path.join(__dirname, "..", "..", "..", "target", "debug", devBinName);
  if (fs.existsSync(devBuild)) return devBuild;

  // Release build fallback.
  const releaseBuild = path.join(__dirname, "..", "..", "..", "target", "release", devBinName);
...
if (result.error) {
  process.stderr.write(`verter-tsc: failed to start binary '${binary}': ${result.error.message}\n`);
  process.exit(2);
}
