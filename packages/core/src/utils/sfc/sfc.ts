...
export function retrieveHTMLComments(source: string) {
    return Array.from(source.matchAll(COMMENTED_BLOCKS_REGEX)).map(x => ({ start: x.index, end: x.index + x[0].length }))
}
...
export function catchEmptyBlocks(descriptor: SFCDescriptor): SFCBlock[] {
    ...
    for (const it of matches) {
        ...
        const isInComment = commentedBlocks.some(block => {
            return (contentStartIndex >= block.start && contentStartIndex < block.end) ||
...
export function extractBlocksFromDescriptor(descriptor: SFCDescriptor): Array<VerterSFCBlock> {
    ...
    for (let i = 0; i < sfcBlocks.length; i++) {
        ...
        const tagMatch = tagInit.matchAll(BLOCK_TAG_REGEX).next().value as RegExpMatchArray


        const tag = tagMatch.groups.tag
        const content = tagMatch.groups.content


        const tagStartIndex = tagMatch.index + initTagOffset
        const startTagPos: BlockPosition = {
            start: tagStartIndex,
            end: tagStartIndex + tagMatch[0].length
        }


        const tagEnd = source.slice(endTagOffset, nextOffsetEnd)
        const tagEndMatch = tagEnd.matchAll(BLOCK_END_TAG_REGEX).next().value as RegExpMatchArray
