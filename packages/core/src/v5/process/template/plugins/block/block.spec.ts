...
describe("process template plugins narrow", () => {
  ...
  describe("slot", () => {
    ...
    it("v-slot", () => {
      const { result } = parse(`<div v-slot="item" :test="()=>test"><span>test</span></div>`);
      expect(result).toContain(
    ...
    it("v-slot + v-if", () => {
      ...
      expect(result).toContain("if(___VERTER___ctx.item){<div");
      expect(result).toContain(
    ...
    it("<template #slot>", () => {
      const { result } = parse(`<div><template #slot><span>test</span></template></div>`);
      expect(result).toContain(
