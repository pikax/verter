    ...
    if (
      ...
      if (item.parent.init.callee.type === "Identifier") {
        ...
        if (ctx.isSetup) {
          ...
          if (
            handleTypeProps(item.parent.init, item.declarator.start, s, ctx)
          ) {
            this.hasTypedProps = true;
          }
          ...
          if (macroName === "defineModel") {
            ...
            if (
              defineProps.type === "CallExpression" &&
              handleTypeProps(defineProps, item.declarator.start, s, ctx)
            ) {
              this.hasTypedProps = true;
            }
            ...
            s.prependRight(defineProps.start, `const ${pName}=`);
            s.prependLeft(defineProps.end, `;`);
            s.prependLeft(defineProps.start, pName);
  ...
  transformFunctionCall(item, s, ctx) {
    ...
    if (item.name === "defineModel") {
      ...
      if (ctx.isSetup) {
        ...
        if (
          defineProps.type === "CallExpression" &&
          handleTypeProps(defineProps, item.node.start, s, ctx)
        ) {
          this.hasTypedProps = true;
        }
    ...
    if (ctx.isSetup) {
      s.appendRight(item.node.start, `const ${varName}=`);
      ...
      if (handleTypeProps(item.node, item.node.start, s, ctx)) {
        this.hasTypedProps = true;
      }
    } else {
      ctx.items.push({
...
function handleTypeProps(
  ...
  const typeName = ctx.prefix("TYPE_PROPS");
