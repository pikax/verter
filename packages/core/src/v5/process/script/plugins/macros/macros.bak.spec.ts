...
describe("process script plugin script block", () => {
  ...
  describe("setup", () => {
    ...
    describe("defineProps", () => {
      describe.only("with type", () => {
        it("defineProps<{ a: string}>()", () => {
          const { result, context } = parse(`defineProps<{ a: string }>()`);
          const Props = context.prefix("Props");
          const TypeProps = context.prefix("TYPE_PROPS");

          expect(result).toContain(
            `;type ${TypeProps}={ a: string };const ${Props}=defineProps<${TypeProps}>()`
          );

          expect(result).toContain(`${TypeProps}:{} as ${TypeProps}`);
        });
        ...
        it("withDefaults(defineProps<{ a: string}>(), {})", () => {
          ...
          expect(result).toContain(
            `;type ${TypeProps}={ a: string };const ${Props}=defineProps<${TypeProps}>();withDefaults(${Props}, {})`
          );
        ...
        it("const props = withDefaults(defineProps<{ a: string }>(), {})", () => {
          ...
          expect(result).toContain(
            `;type ${TypeProps}={ a: string };const ${Props}=defineProps<${TypeProps}>();const props = withDefaults(${Props}, {})`
          );
          expect(result).toContain(`${TypeProps}:{} as ${TypeProps}`);
