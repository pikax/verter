...
export type $V_ModelToProps<T> = {
  [K in keyof T]: T[K] extends $V_ModelRef<infer C> ? C : T[K] extends $V_ModelRef<infer C> | undefined ? C | undefined : null
}

export type $V_ModelToEmits<T> = {} extends T ? () => any : {
    [K in keyof T]-?: (event: `update:${K & string}`, arg: T[K] extends $V_ModelRef<infer C> ? C : T[K] extends $V_ModelRef<infer C> | undefined ? C | undefined : unknown) => any
}[keyof T]
