...
import { ParseContext, type ParseScriptContext } from "@verter/core";
...
export function processScript(context: ParseContext) {
  ...
  if (context.script == null) {
    s = new MagicString(
      [
        `export function resolve${RenderContextExportName}${generic}() {} {`,
        `return {}`,
        `} `,
      ].join("\n")
    );
    ...
    const prependContent = [
      ...
      } function resolve${RenderContextExportName}${generic}() {`,
      "",
    ];

    const returnContent = ["", "return {} as "];
    ...
    s.append(
      `type ${RenderContextExportName}Type${generic} = ReturnType<typeof resolve${RenderContextExportName}${generic}>` +
