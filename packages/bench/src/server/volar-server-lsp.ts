...
import { createProtocolConnection, ProtocolConnection } from 'vscode-languageserver-protocol';
...
export async function getLanguageServer() {
    if (!connection) {
        ...
        connection.onNotification('tsserver/request', async ([id, command, args]: [string, string, any]) => {
    ...
    return {
        vueserver: {
            ...
            openInMemoryDocument: async (uri: string, languageId: string, content: string) => {
