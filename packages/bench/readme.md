...
**Server Persistence:** Language servers run continuously throughout each benchmark suite without reinitialization. Both `getVolarServer()` and `getVerterServer()` are **singleton functions** that return the same server instance on every call. This means:

- Each benchmark suite initializes servers once in `beforeAll()`
- The `setup()` functions in individual benchmarks call the getters but receive the **same persistent instances**
- Servers remain active throughout all iterations, benefiting from:
  - TypeScript program caching
  - JIT compilation optimizations
  - Warm module resolution
  - Cached virtual file mappings
- This reflects real-world usage where language servers run for hours without restarts

**Warm-up Phase:** Each benchmark suite includes 3 warm-up iterations over 1 second to stabilize V8 optimizations and eliminate cold-start variance.

**Identical LSP Architecture:** Both Volar and Verter benchmarks use LSP+IPC communication (not direct API calls) to ensure architectural parity.

**Fresh vs Cached Tests:**

- **Fresh:** Document opened, operations performed, document closed within each iteration
- **Cached:** Document pre-opened in `beforeAll()`, operations performed on warm state

### Known Differences

**Parser Benchmarks:** Measure different scopes - Verter performs lightweight AST parsing while Volar performs full virtual code generation. Results are informative but not directly comparable.

**didChange Notification:** Verter's LSP wrapper sends an extra `didChange` after `didOpen` which may pre-warm caches. This is noted in the code and reflects potential server architecture differences.
