...
describe("preprocessor result qualification", () => {
  ...
  it("anchors CSS-service diagnostics produced from generated CSS", async () => {
    // The CSS service validates the preprocessor output, whose offsets do not
    // address the authored block. Until that output carries a source map, its
    // ranges must not be shifted as though the generated bytes were authored.
    ...
    const results = await svc.doValidation(URI, inline, 1);
    const blockStart = { line: 0, character: inline.indexOf(".authored") };
