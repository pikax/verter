...
const transpileGate: {
  ...
  /** Per-call results, consumed in order (multi-block documents). */
  sequence?: QualifiedStyleResult[];
} = {};
...
vi.mock("./transpiler", () => ({
  transpile: async () => {
    ...
    if (transpileGate.sequence?.length) return transpileGate.sequence.shift();
...
describe("preprocessor result qualification", () => {
  afterEach(() => {
    transpileGate.result = undefined;
    transpileGate.sequence = undefined;
  });
  ...
  it("never re-parses preprocessed output with the CSS service", async () => {
    // The convergence contract: generated bytes belong to the qualified
    // result's preprocessed stage and are never handed to a second CSS
    // parser. Re-parsing them used to report CSS-service errors anchored
    // wholesale on the block's first position — coordinates that addressed
    // nothing the user wrote. A clean transpile is a clean block.
    const inline = '<style lang="sass">.authored\n  color red\n</style>';
    liveDocuments().splice(0, liveDocuments().length, { uri: { toString: () => URI }, version: 1 });
    transpileGate.result = preprocessed(".generated { color:; }");
    const svc = service((params) => available(params, inline, { dialect: "sass" }));
  ...
  it("joins the qualified record by sealed block token, not list position", async () => {
    // Two preprocessor blocks: the clean first record must not answer for the
    // second block, whose compile error maps into its own authored slice.
    const inline =
      '<style lang="sass">.first\n  color: red\n</style>\n\n<style lang="sass">.second\n  color:\n</style>';
    liveDocuments().splice(0, liveDocuments().length, { uri: { toString: () => URI }, version: 1 });
    transpileGate.sequence = [
      preprocessed(".first { color: red; }"),
      preprocessed("", [
        {
          stage: "authored",
          origin: "processor",
    ...
    const section = (open: number, token: string) => {
      const contentStart = inline.indexOf(">", open) + 1;
      const contentEnd = inline.indexOf("</style>", contentStart);
      const range = (start: number, end: number) => ({ sourceSpaceToken: "s0", start, end });
      return {
        kind: "section",
        markupRootTokens: [],
        section: {
          blockToken: token,
          role: { kind: "style", dialect: "sass", scoped: false, module: "none" },
          openingRange: range(open, contentStart),
          contentRange: range(contentStart, contentEnd),
          closingRange: range(contentEnd, contentEnd + 8),
          fullRange: range(open, contentEnd + 8),
          attributeInsertionAnchor: range(contentStart - 1, contentStart - 1),
          attributes: [],
        },
      };
    };
    const open1 = inline.indexOf("<style");
    const open2 = inline.indexOf("<style", open1 + 1);
    const svc = service((params) => {
      const response = available(params, inline, { dialect: "sass" });
      if (response.kind !== "available") return response;
      response.structure.blocks = [
        section(open1, "style-0"),
        section(open2, "style-1"),
      ] as unknown as typeof response.structure.blocks;
      return response;
    });
    ...
    expect(results).toEqual([
      {
        ...
        diagnostics: [
          {
            // Authored line 1 of the SECOND block's slice is SFC line 5 —
            // the error lands in the block that produced it, not the first.
            range: {
