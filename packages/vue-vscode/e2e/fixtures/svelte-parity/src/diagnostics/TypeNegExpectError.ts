/**
 * @ts-expect-error suite for Svelte: each directive MUST suppress a real error.
 * If the imported surface is `any`, TS2578 (unused @ts-expect-error) fails E2E.
 */
import TypeNegChild from "./TypeNegChild.svelte";
import type { ComponentProps } from "svelte";

type ChildProps = ComponentProps<typeof TypeNegChild>;
...
// @ts-expect-error pick payload is string
export const badPickHandler: PickHandler = (n: number) => {
  void n;
};

// @ts-expect-error change payload is number
