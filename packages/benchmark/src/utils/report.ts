...
export function generateMarkdownReport(report: BenchmarkReport): string {
  ...
  lines.push("");
  lines.push(
    "| Fixture | Size | Vue (ms) | Verter (ms) | Memory | Speedup | Throughput | Status |",
  );
  lines.push(
    "|---------|------|----------|-------------|--------|---------|------------|--------|",
  );
  ...
  for (const fixture of report.fixtures) {
    ...
    lines.push("");
    lines.push(
      `**Speedup:** ${fixture.speedup.toFixed(2)}x (${statusEmoji[fixture.status]} ${fixture.status})`,
    );
    lines.push("");
  }
...
export function generateConsoleOutput(report: BenchmarkReport): string {
  ...
  lines.push(
    `Overall Status: ${statusColor}${summary.overallStatus.toUpperCase()}${statusColors.reset}`,
  );
  lines.push(`Average Speedup: ${summary.avgSpeedup.toFixed(2)}x`);
  lines.push(
    `Passed: ${summary.passed} | Warnings: ${summary.warnings} | Failed: ${summary.failed}`,
  );
  lines.push("");
  ...
  for (const fixture of report.fixtures) {
    ...
    lines.push(
      `  Vue:    ${fixture.vue.stats.mean.toFixed(2)} ms (${fixture.vue.opsPerSec.toFixed(0)} ops/s, ${fixture.vue.throughputMBs.toFixed(2)} MB/s) - ${formatMemory(fixture.vue.stats.heapUsedMB * 1024 * 1024)}`,
    );
    lines.push(
      `  Verter: ${fixture.verter.stats.mean.toFixed(2)} ms (${fixture.verter.opsPerSec.toFixed(0)} ops/s, ${fixture.verter.throughputMBs.toFixed(2)} MB/s) - ${formatMemory(fixture.verter.stats.heapUsedMB * 1024 * 1024)}`,
    );
