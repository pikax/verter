...
console.log(`\n  CPU cores  : ${CPU_COUNT}`);
console.log(
  `  Vize build : ${vizeBuildType.toUpperCase()}${vizeBuildType === "debug" ? " (unoptimized — release build would be faster)" : " (optimized)"}`,
);
...
for (const f of fixtures) {
  ...
  bench.add("vue", () => {
    compileVue(f.source, f.filename);
  });
  bench.add("verter", () => {
    compileVerterHost(host, f.source, f.filename);
  });
  bench.add("vize", () => {
    vize!.compileSfc(f.source, { filename: f.filename });
  });
  ...
  console.log(
    `  ${col(f.name, 24, false)}${col(formatBytes(f.size), 8)}  ${col(formatDuration(vue), 10)}  ${col(formatDuration(verter), 10)}  ${col(formatDuration(vizeMs), 10)}  ${col(vSpeedup, 11)}  ${col(zSpeedup, 9)}`,
  );
}

const avgVerterSpeedup =
  stResults.reduce((s, r) => s + r.vueMean / r.verterMean, 0) / stResults.length;
const avgVizeSpeedup = stResults.reduce((s, r) => s + r.vueMean / r.vizeMean, 0) / stResults.length;

console.log("  " + "─".repeat(88));
console.log(
  `  ${col("AVERAGE", 24, false)}${col("", 8)}  ${col("", 10)}  ${col("", 10)}  ${col("", 10)}  ${col(avgVerterSpeedup.toFixed(1) + "x", 11)}  ${col(avgVizeSpeedup.toFixed(1) + "x", 9)}`,
);
console.log();
...
const fmtRow = (name: string, config: string, ms: number, baseline: number) => {
  ...
  console.log(
    `  ${col(name, 22, false)}  ${col(config, 17, false)}  ${col(formatDuration(ms), 10)}  ${col(fps.toLocaleString(), 10)}  ${col(speedup, 10)}`,
  );
};
...
if (vizeBuildType === "debug") {
  ...
  console.log(
    "       cp target/release/vize_vitrine.dll npm/vize-native/vize-vitrine.win32-x64-msvc.node",
  );
} else {
  console.log(
