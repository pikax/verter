...
// from the typed compile request. A missing carrier (no ideCompanion product, or
// an absent/empty code or source-map) MUST surface carrierContentHash as null —
// NEVER hashed from "", which would let two both-sides-missing runs compare as an
// EQUAL empty hash and slip past the content-equality rail. These specs inject a
// scripted host via the VerterHostFactory seam, so they need no native build (and
// run on the unit-spec job).
describe("axis-A carrier-content rail fails closed on a MISSING IDE carrier", () => {
  ...
  it("the carrier-content pass issues one Vue ideCompanion compileRequest with source maps per SFC", () => {
    ...
    for (const request of requests) {
      ...
      expect(request.identity.filename).toMatch(/\.vue$/);
