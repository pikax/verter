...
function detectBuildType(
  release: string,
  debug: string,
): { bin: string | null; type: "release" | "debug" | "missing" } {
  ...
  return relMtime >= dbgMtime ? { bin: release, type: "release" } : { bin: debug, type: "debug" };
}
...
function fmtResult(r: RunResult, baseline?: RunResult): { time: string; speedup: string } {
  ...
  const speedup =
    baseline && !baseline.skipped && !baseline.timedOut && r.ms > 0
...
for (const proj of PROJECTS) {
  ...
  if (proj.note) {
    ...
    if (!existsSync(nuxtTsconfig)) {
      process.stdout.write(
        `  ${col(proj.name, 20, false)}  !!  .nuxt/ not generated — run 'nuxi prepare' first\n`,
      );
    }
  }
  ...
  const vueTsc = runTool(
    vueTscInfo.bin,
    [...vueTscInfo.args, "--noEmit", "--project", proj.tsconfig],
    proj.root,
  );
  ...
  const row = [col(proj.name, 20, false), col(vueFiles, 5), vueTscTime, verterTime, verterSpeedup];
  if (SHOW_ERRORS) {
    row.push(
...
if (verter.type === "debug")
  warnings.push("verter-tsc: DEBUG build — cargo build --package verter_tsc --release");
