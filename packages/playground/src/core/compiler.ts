...
let oxcTransform: ((filename: string, code: string, options?: TransformOptions) => Promise<TransformResult>) | null = null
...
export async function transpileTS(code: string, filename: string): Promise<{ code: string; errors: string[] }> {
  if (!oxcTransform) {
