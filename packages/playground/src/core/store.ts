...
import { File, type OutputMode, type StoreState, type CompilerOptions, type CompileTiming } from './types'
...
export function useStore(): Store {
  ...
  async function init() {
    ...
    if (savedState) {
      ...
      if (savedState.activeFile && (files.value[savedState.activeFile] || savedState.activeFile === IMPORT_MAP_FILENAME)) {
    ...
    watch(
      () => ({
        files: Object.fromEntries(
          Object.entries(files.value).map(([k, f]) => [k, f.code])
        ),
        activeFile: activeFilename.value,
        outputMode: outputMode.value,
        compilerOptions: { ...compilerOptions },
        importMap: { imports: { ...importMap.imports }, scopes: importMap.scopes ? { ...importMap.scopes } : undefined },
      }),
      (state) => {
