...
The materialize fence (`hot_path_never_calls_materialize_type_expr`) stays RED at 10 — it tracks reverse-materialization SITES (a separate workstream), not this cache-correctness contract; this block must not change that count. The §7 HOT_TERMINAL_SINKS accounting is unaffected (no new sinks). Keep the CORE's sequential no-poison gates, the override-Uncacheable path, and the over-gating positive-control tests green.
