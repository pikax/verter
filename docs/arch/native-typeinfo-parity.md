...
| `KeyOf` | live | `ProjectionReductionContext` (env `T L J`) | `TypeNode` | — | `Singleflight` (keyspace guarded by the planned `KeyspaceBudget` reducer; `ReturnOnly` on overflow) |
| `MappedType` | live | `ProjectionReductionContext` (env `T L J`) | `TypeNode` | — | `Singleflight` (keyspace explosion guarded by the planned `KeyspaceBudget` reducer; `ReturnOnly` on overflow) |
