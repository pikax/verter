...
- tsgo project/file sync registry — `crates/verter_lsp/src/tsgo/project_sync.rs` (the synced-file registry OQ-4 stores `SnapshotId` on); `crates/verter_lsp/src/sync_coordinator.rs`.
...
- `SnapshotId(u128)` — **new** (Phase 4; `verter_session`).
...
- `CompilerLocation` (a coordinate-space + provenance enum: `Generated { span: GeneratedByteRange, snapshot_id }` | `RealSource { span: SourceByteRange, source_hash: Hash16 }`) and `TargetProvenance` (`GeneratedMapping(SnapshotId)` | `HostSource(Hash16)` | `LiveSameFile`) — **new** (Phase 5/6-7; P1-1 — every compiler-derived location carries intrinsic provenance for its coordinate space).
...
- `verter_session` — Phase 3 (anchor production at `IndexedReady` + `TemplateAnalysisState`), Phase 4 (`SnapshotId` on `CachedTsx`/`IdeResponse`, `target_ide_context`/`TargetSourceContext`), Phase 5 (`navigation::definition` core + `TsNavigationBackend` trait), Phase 6/7 (engine generalization).
...
```
Provide only total, lossless conversions (no hidden `as` truncation). NO `From` between source-side and generated-side types (mirrors the existing `verter_span` "no `From` between span types" rule) — in particular **no `From<GeneratedByteRange>` for `SourceByteRange`** so a generated span can never be silently stored as a source span. `LspPosition`/`TsPosition` are distinct so a TSX position can never be passed where a Vue LSP position is expected. `GeneratedByteRange` is the coordinate-space carrier for every generated-TSX nav span (P1-1): backend answers and `CompilerLocation`/`DefinitionTarget` use it (paired with `SnapshotId`) for the generated space and `SourceByteRange` (paired with `Hash16`) for the real-source space — the two never mix.
...
## PHASE 4 — `SnapshotId` (content-addressed TSX identity) + host-sourced target-mapper / source-context loading

### Context (OQ-4 BINDING)
Cross-file targets must be mapped using **the target file's own** source map, loaded from the **host** (not the open-doc registry), valid only when its snapshot matches the TSX snapshot tsgo resolved against. Today target mappers come from `get_position_mapper` (open documents only) so a closed target `.vue` cannot be mapped (→ `Range::default()`), and there is no snapshot identity. OQ-4 binds a **content-addressed** id computed at compile-cache publish — which side-steps P0-2's stale-stability concern entirely.

### Changes

**4A. New `SnapshotId(u128)` (OQ-4) — `crates/verter_session/src/navigation/snapshot.rs` (or `types.rs`).**
```rust
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)] // Copy + Send + Sync
pub struct SnapshotId(pub u128);
```
**OQ-4 derivation (resolves P0-2):** `SnapshotId = Hash128("verter.ide.tsx.v1" || generated_tsx_bytes)` — a content hash of the **actual generated TSX bytes**, computed **once at compile-cache publish** (streamed while materializing the TSX, at `virtual_file_pipeline.rs:1851` where `CachedTsx` is built). Because it hashes the *output bytes*, it is **immune** to the stale-stability hazard (a cross-file dep edit that changes the TSX necessarily changes the bytes → changes the id; an edit that does NOT change the TSX legitimately keeps the id). Warm hits return the **stored** id (O(1) `u128` compare; no per-request rehash, no recompute). The version prefix (`"verter.ide.tsx.v1"`) namespaces the hash so a format change forces a new id space.

**4B. Store `SnapshotId` at the publish point.**
- Add `pub snapshot_id: SnapshotId` to `CachedTsx` (`types.rs:1723-1727`) — populated at construction (`virtual_file_pipeline.rs:1851`) from the streamed hash of `tsx.code`. It flows through `CompileOutputValue.tsx: Option<CachedTsx>` (`cache_runtime/compile_output_node.rs:130`) for free.
- Add `pub snapshot_id: SnapshotId` to `IdeResponse` (`types.rs:1732-1741`) and populate it in `get_ide` (the `IdeResponse` literal at `virtual_file_pipeline.rs:1366-1371`) from `CachedTsx.snapshot_id` (no recompute — read the stored id, available via `CompileOutputNodeFactValidatedSession::peek_tsx`).
- Store the `SnapshotId` on the **tsgo synced-file registry** (`crates/verter_lsp/src/tsgo/project_sync.rs`) when a `.vue.tsx` is synced to tsgo, so a returned `TsDefinitionSpan` can be tagged with the snapshot it answered against (OQ-1 / OQ-4).
...
- `snapshot_id_stable_across_reads` — two `target_ide_context` calls without an edit → equal `SnapshotId`.
- `snapshot_id_changes_on_recompile` — edit `Child.vue` (content that changes the TSX) → `SnapshotId` differs.
- `snapshot_id_stable_when_tsx_unchanged` — **(OQ-4/P0-2 discriminating)** a cosmetic edit that does NOT change the generated TSX bytes keeps the `SnapshotId` equal (content-addressed, not key-addressed).
...
- `target_source_context_for_ts_file` — **(P0-2)** `target_source_context("…/util.ts")` → `Some` with `source`, `line_index`, `source_hash`; no TSX/`SnapshotId` involved.
...
- `snapshot_id_is_content_addressed` (unit): assert `SnapshotId` equals `Hash128("verter.ide.tsx.v1" || code_bytes)` for a known TSX string (pins the OQ-4 derivation; a key-addressed re-derivation fails).
...
**(P1-1 typed coordinate-space + provenance.)** A backend answer is modelled by its **coordinate space AND its provenance token together** — never a span in one space with a stale-stability token from the other. `GeneratedByteRange` (new, Phase 1 — see 1A) pairs with `SnapshotId`; `SourceByteRange` (real source) pairs with `Hash16` (`= [u8;16]`, the real workspace type at `crates/verter_session/src/types.rs:12`). The location-only result is therefore a provenance-bearing enum, NOT a struct with a bare `Option<SnapshotId>`:
```rust
...
pub enum TsDefinitionSpan {
    ...
    GeneratedTsx {
        ...
        snapshot_id: SnapshotId,          // the TSX snapshot tsgo answered against
    },
...
#[async_trait] pub trait TsNavigationBackend {
    async fn definition_at_position(
        &self, tsx_canonical: &str, offset: GeneratedByteOffset, snapshot: SnapshotId,
    ...
    async fn code_actions_at_position(
        &self, tsx_canonical: &str, range: GeneratedByteRange, snapshot: SnapshotId,
...
pub enum TargetProvenance {
    GeneratedMapping(SnapshotId), // span produced by mapping a generated TSX range → .vue source; validate SnapshotId
...
```
**(P0-2 / OQ-4 / P1-1)** `RealSource`/`SfcComponent` carry `provenance: TargetProvenance`: `GeneratedMapping(SnapshotId)` when the `.vue` source span was produced by mapping a generated-TSX range back (the only case needing a TSX snapshot), or `LiveSameFile` for a same-file binding resolved from live analysis (no remote token). A `.ts/.js` target whose real-source span tsgo returned directly is `RealSource` with `provenance = HostSource(Hash16)`. `ExternalDeclaration` (library `.d.ts`/`.vue.d.ts`) carries `source_hash: Hash16` directly, NOT a `SnapshotId`. **(P1-I)** A target is renderable only if its provenance token still matches at render time (`GeneratedMapping` → `target_ide_context(uri).snapshot_id == SnapshotId`; `HostSource`/`ExternalDeclaration` → `target_source_context(uri).source_hash == Hash16`; `LiveSameFile` → always renderable); otherwise it is **DROPPED** (never recomputed-in-place-and-remapped).
...
- `stale_mapper_snapshot_dropped_not_recomputed` — **(P1-I)** force a `SnapshotId` mismatch → the target is DROPPED (assert no location rendered against the stale mapper; assert NO in-place recompute path runs).
...
- New **`CompilerLocation`** (OQ-7 + P1-1) — the common provenance-carrying value every backend (nav or semantic) normalizes into a `DefinitionTarget`. **(P1-1)** It is modelled by coordinate-space + provenance so the "span with two bare `Option`s" hazard is unrepresentable — a generated span ALWAYS carries a `SnapshotId`, a real-source span ALWAYS carries a `Hash16`:
  ...
  pub enum CompilerLocation {
      ...
      Generated { owner_canonical: CanonicalId, span: GeneratedByteRange, snapshot_id: SnapshotId },
...
- `/host-session` skill — `SnapshotId` (content-addressed `Hash128`), `target_ide_context`/`target_source_context`, host-sourced target mappers, snapshot-match-or-DROP rule.
...
| **P0-2** SnapshotId derivation | Content-addressed `Hash128("verter.ide.tsx.v1"||tsx_bytes)` (immune to stale-stability); `.ts/.js/.d.ts` validated via `TargetSourceContext.source_hash`, not TSX id | 4 |
...
| **OQ-4** | `SnapshotId(u128)=Hash128(prefix||tsx_bytes)` at publish on `CachedTsx`/`IdeResponse`/tsgo registry; on every `TsDefinitionSpan`/`CompilerLocation`; non-IDE via `TargetSourceContext`; mismatch⇒DROP | 4, 5, 6/7 |
