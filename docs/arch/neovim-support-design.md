...
- The `editor-neovim` job in `.github/workflows/ci.yml` provisions the fixture and a commit-pinned plenary checkout, consumes the shared `verter-lsp` binary built once by `build-editor-lsp`, and runs the contract on Linux against Neovim `0.12.3`. The job rejects failure tokens, nonzero process status, and output that proves no assertion-bearing test ran. **Two-tier coverage:** the full 9-leg matrix (Neovim `0.11.0` / `0.12.3` / nightly × Linux / macOS / Windows) still runs, but at RELEASE time — the `editor-neovim` job in `release.yml` — where it keeps the `0.11.0` supported-floor check, the nightly early-warning signal for upstream LSP-client changes, and macOS/Windows coverage of the Lua path-handling layer, discharging the Cross-Platform Portability CRITICAL rule. All 9 legs download one of three shared per-OS `verter-lsp` binaries instead of each rebuilding it uncached, which is what the retired standalone `neovim.yml` did. `release-check.yml` rehearses that full matrix without publishing, so the proof is available on demand before a tag is cut.
