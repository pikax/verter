...
- The `editor-neovim` job in `.github/workflows/ci.yml` provisions the fixture and a commit-pinned plenary checkout, consumes the shared `verter-lsp` binary built once by `build-editor-lsp`, and runs the contract on Linux against Neovim `0.12.3`. The job rejects failure tokens, nonzero process status, and output that proves no assertion-bearing test ran. **Linux-only, all three Neovim versions.** The retired standalone `neovim.yml` ran a 9-leg matrix (Neovim `0.11.0` / `0.12.3` / nightly × Linux / macOS / Windows), each leg rebuilding `verter-lsp --release` uncached. `release.yml`'s `editor-neovim` keeps the VERSION axis — the `0.11.0` supported floor, current stable, and nightly as the early-warning signal for upstream LSP-client changes — but runs all three on `ubuntu-latest` only, against one shared `verter-lsp` artifact. `ci.yml` runs a single Linux leg at the stable version per PR. The macOS/Windows matrix entries are left commented in `release.yml` to restore.

**Coverage note:** the Lua config layer is no longer exercised on macOS or Windows, so the Cross-Platform Portability CRITICAL rule is NOT discharged for this client by CI. This is the most exposed of the editor clients, because the Lua layer builds path strings and resolves the binary itself, and Windows genuinely diverges there (backslashes, drive letters, the `.exe` suffix) in a way macOS — being POSIX like Linux — does not. Windows path handling in `editors/nvim` is therefore currently unproven; if any one editor lane is worth paying a Windows leg for, it is this one.
