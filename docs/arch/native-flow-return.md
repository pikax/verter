...
Owning U-block(s): U6
Prerequisites: U2; U4 persistence-only (U4 gates ONLY persistent slice registration — `FlowSliceHashNode` / `FlowSliceLoweredBodyNode` — never the memory substrate)
...
>
> The locked design's **Substrate consumption contract** (C1–C7 + SCC-1…SCC-10) is the SOLE normative
> producer/consumer interface statement — the `FlowReturn`/`ResolveCall` key axes, the split result/carrier
> value model, the evaluation entries, and the acceptance criteria. This subplan REFERENCES and IMPLEMENTS
> that contract; it holds no second normative key/value/interface statement, and any key/value/result shape
> below is an implementation citation of that contract, not an independent authority.
...
- **Prerequisite U4 is PERSISTENCE-ONLY — an artifact-level edge, never a block-level
  gate.** The memory-side `ArtifactNode` runtime already exists (validation,
  admission, singleflight, storage); U4's unique contribution is the sealed
  PERSISTENT capability. The whole memory substrate proceeds without U4: U4 gates
  ONLY the persistent registration of the slice artifacts. The flow slice-hash and
  slice-lowered-body artifacts are **B4-style cache-runtime nodes**
  (`FlowSliceHashNode`, `FlowSliceLoweredBodyNode`), not bespoke
  `FileArtifactStore` side maps; their memory-side registration lands with
  `U6.FLOW_RETURN_SUBSTRATE`, and their PERSISTENT registration is the zero-row
  owed item **`U6.FLOW_SLICE_PERSISTENCE`** (← U4): it blocks no semantic U6
  child block, and the U6 aggregate does not reach terminal ratification until it
  passes.
...
```
U2.QUERY_VALUE_DOMAIN + U2.RELATION_INFER + U4[persistence-only: persistent slice registration, not the memory substrate]
        │
        ▼
U6.FLOW_RETURN_SUBSTRATE   (FunctionBodySkeleton + FunctionFlowGraph + ReturnPathPeeker demand planner + slice nodes + FlowReturn dispatch + audit)
        │  (U6.FLOW_SLICE_PERSISTENCE ← U4: zero-row owed persistent slice registration; blocks no semantic child, gates U6 aggregate terminal ratification only)
        │
...
**`FunctionBodySkeleton`**: an arena-free, shallow
...
is a structural, query-independent index at `IndexedReady` density, built once per
file/function content version from the retained snapshot — eagerly (in / under
`IndexedReady`) or lazily; the locked design's share-vs-split storage condition
settles timing and placement at implementation — and never rebuilt per query or
demand.

**`FunctionFlowGraph`** is a **sparse, arena-free dependence structure built ONCE
per function per content version** from its `FunctionBodySkeleton` — eagerly or
lazily from the retained snapshot (the five share-vs-split checks settle it),
never per query or demand — the same density and the same
...
The NORMATIVE key surface, cache identity, version rooting, satisfaction/backfill semantics, and the sole
warm rail are owned by the locked design (Substrate consumption contract C1 + C7, Decision 2, and the
R6/R21 rules they cite) — this section is the implementation expansion C1 delegates the `demand` / `input`
axis detail to; it states no second normative interface. Implementation composition and actions:

- `FlowReturnContext` carries the five split env hashes as named direct fields, the substitution canonical
  hash, `ProjectionReductionContext`, and `FlowPolicy`. It does NOT carry `ReturnProjectionDemand` or
  `FlowInputContext` — those are the sibling `demand` / `input` key fields, so the full identity is
  `FlowReturnContext + ReturnProjectionDemand + FlowInputContext` with no field duplicated across
  `context` / `demand` / `input`; this same normalized identity IS the re-entry identity on the obligation
  runtime (C1).
- `ReturnProjectionDemand` is the flow-typed `(ProjectionDemand, EvalPolicy)` point (parent §2.10); a
  `Skeleton` (`generic_open = TypeParamShells` + carrier-stop) slice is incomparable to a bound-expansion
  slice. Implementation action: mint `satisfied_projection` as the demand point the compute actually
  materialised and decide warm-hit/backfill through the shared demand-lattice dominance machinery — no
  mode-enum ordering, no flow-private satisfaction logic (the governing rules are C1 + the cache
  runtime's recorded-materialised-point contract).
- Classification: query-identity cache (multi-candidate slot; concurrent variants coexist under one
  `function_slot`-rooted slot, validated by `ReadSetSignature.validate_with_self_roots` against the
  caller's live view). The key stays content-free per R6 (no content hash, `parse_stable_hash`,
  `flow_body_stable_hash`, or `fact_dep_signature`); `flow_body_stable_hash` is content-derived VALUE-side
  identity rooting the `FlowSliceHashNode` artifact node and recorded on the
  `ProgramAnalysisFactRef::FlowBody` warm-validity ref.
- Implementation action (facts): warm validity records through the EXISTING
  `ProgramAnalysisFactRef::FlowBody { function, flow_body_stable_hash }` — the flow value's
  `ReadSetSignature.facts` carries that `FlowBody` rooting plus `MemberPresence`, `Member`,
  `RouteGeneration`, `ExportSurface`, `ModuleAugmentation`, `AmbientGlobal`, `LibIntrinsic`,
  `TypeEnvOptions`, and project-generation facts as read. NO new `ProgramAnalysisFactRef` variant is
  introduced and NO `FlowSlice` ref ever appears in `ReadSetSignature.facts` — C7/SCC-9 permit exactly ONE
  intra-function warm rail. The `FactKey::FlowSlice` fact `{ function_slot, projection_path, slice_hash,
  selected_binding_ids, selected_effect_ids, selected_control_region_ids, closure_summary_ids }` is
  recorded at result-publish SOLELY as a candidate-selection discriminant (schema owner
  `verter_workspace::fact_registry` — see the block Changes); it and its selected IDs are never
  re-derived or consulted as a warm gate. Warm validity is EXACTLY C7's rail: the re-derived live
  `flow_body_stable_hash` (the sole intra-function gate) plus the unioned consumed
  `ReadSetSignature.facts`. `StoreView::validates_program_analysis_domain` FAILS CLOSED on a missing /
  stale / unrooted `FlowBody` rooting — a fail-closed miss recomputes rather than serving a torn result;
  budget / overflow / cycle / cancellation / partial slice results are `ReturnOnly` (parent §6).

### Value domain and the typed result

`SemanticQueryKey::FlowReturn` resolves to `SemanticQueryValue::FlowReturn(
Arc<FlowReturnResult>)` (parent §3). The value model is OWNED by the locked
design's split result/carrier contract (Substrate consumption contract C1) —
referenced here, not restated. Implementation actions: define the SUCCESS carrier
`FlowReturnResult { return_type, can_fall_through, degradation:
Option<FlowReturnDegradation> }` (`FlowReturnDegradation` is owed by
`U6.FLOW_RETURN_SUBSTRATE`; per C1 a degraded success defaults to `ReturnOnly` and
warms only under a later explicit, fact-rooted admission-table row); keep the typed
`FlowReturnFailure` for the no-value classes, routed through `ReturnOnly`; ride the
read-set fact signature, the actual `satisfied_projection`, self roots, and
publication metadata on the published-value carrier (`MemoEntry` /
`PublishedMemoCandidate`), never the domain result; carry the typed
`NonAdmissionReason` on refusals. Predicate / assertion effects
...
— so degraded-but-successful results (`Unknown` with a typed
`FlowReturnDegradation` on the success carrier) ride the
`Ok` arm with the audit record carrying the reason, while a genuine no-value
outcome (the typed `FlowReturnFailure` class — missing function, cancellation,
budget exhaustion, torn state, irrecoverable undecidability) rides the `Err` arm
— the locked design's split result/carrier contract (C1). This is the single
...
Prerequisites: U2.QUERY_VALUE_DOMAIN, U2.RELATION_INFER; U4 persistence-only (an artifact-level edge, not a block-level gate).
Blocked until: U2.QUERY_VALUE_DOMAIN done (the `FlowReturn` / `ResolveCall` key shapes + the typed `SemanticQueryValue` value domain are registered there) and U2.RELATION_INFER done (narrowing and call-argument assignment relate through `Relate`). U4 does NOT block this block: the memory-side `ArtifactNode` runtime already exists (validation, admission, singleflight, storage), and the whole memory substrate — everything this block lands — proceeds without U4. U4 gates ONLY the persistent registration of the slice nodes (`FlowSliceHashNode` / `FlowSliceLoweredBodyNode` as PERSISTENT cache-runtime nodes), recorded as the zero-row owed item `U6.FLOW_SLICE_PERSISTENCE` (← U4): it blocks no semantic U6 child block, and the U6 aggregate does not reach terminal ratification until it passes. This is the keystone U6 block; every other U6 block consumes the skeleton, the peeker, the slice nodes, and the `FlowReturn` dispatch it lands.
...
- `crates/verter_session/src/flow_return/result.rs` + `error.rs` — DEFINE the two U6-extension result types per the locked design's split result/carrier contract (C1): the SUCCESS carrier `FlowReturnResult { return_type, can_fall_through, degradation: Option<FlowReturnDegradation> }` (the `ReadSetSignature`, actual `satisfied_projection`, self roots, and publication metadata ride the published-value carrier — `MemoEntry` / `PublishedMemoCandidate` — never the domain result; a refusal carries its typed `NonAdmissionReason`) and `ResolvedCallResult` (the call-resolution result value the `ResolveCall` arm carries — consumed by U6.CALL_RESOLVE's `build_resolve_call`); `FlowReturnError` (the typed `Err` arm of `AuditedResult`, carrying the `FlowReturnFailure`-class no-value outcomes).
...
- `crates/verter_session/src/cache_runtime/` — register `FlowSliceHashNode` (content-addressed: key carries the per-function **`flow_body_stable_hash`** — the body-sensitive / cosmetic-insensitive hash over `FunctionBodySkeleton` + `FunctionFlowGraph`, NOT `parse_stable_hash`, so `return { b: 1 }` and `return { b: 2 }` key distinct slices — plus `parse_env_hash` + `parser_version` + `function_part` + the `ReturnProjectionDemand` slice identity) and `FlowSliceLoweredBodyNode` (keyed additionally on the slice hash) as `ArtifactNode` impls — memory-side registration on the existing `ArtifactNode` runtime lands HERE; their PERSISTENT registration is the zero-row owed item `U6.FLOW_SLICE_PERSISTENCE` (← U4); `flow_body_stable_hash` is produced once per function content version by the authoritative structural flow-body producer, alongside the `FunctionFlowGraph` (eager-or-lazy timing and storage placement settled by the share-vs-split checks); the lowered-slice node's eviction follows the standard `remove_canonical(canonical_id)` cascade.
- `crates/verter_workspace/src/fact_registry.rs` (the fact-SCHEMA owner — `verter_semantic::facts::registry` is only its compatibility re-export (`pub use verter_workspace::fact_registry::*`) and is NOT a schema surface) — extend `FactKey` with `FlowSlice { function_slot, projection_path, slice_hash, selected_binding_ids, selected_effect_ids, selected_control_region_ids, closure_summary_ids }`, classified into `FactDomain::ProgramAnalysis` (`FactKey::domain()` returns `ProgramAnalysis` for `FlowSlice`) — a candidate-selection DISCRIMINANT only. NO new `FactVersionRef` variant: the version-ref family (`crates/verter_workspace/src/fact_cache.rs`) keeps the landed `ProgramAnalysisFactRef::FlowBody { function, flow_body_stable_hash }` as the SOLE `ProgramAnalysis` warm-validity ref (C7/SCC-9 — exactly one intra-function rail); no `ProgramAnalysisFactRef::FlowSlice` is introduced, and the `FlowSlice` fact never enters `ReadSetSignature.facts`. U6 owns PRODUCING and CONSUMING the flow facts: the `FlowBody` rooting and the `FlowSlice` discriminant are recorded at the result-publish point on every cold rebuild where the slice hash is `Hash(_)`; under `BudgetExceeded` nothing is recorded. Validation is owned and implemented by the landed `StoreView::validates_program_analysis_domain` (`crates/verter_session/src/resolver_store.rs`): it FAILS CLOSED on a missing / stale / unrooted `FlowBody` rooting by re-deriving ONLY the live region's `flow_body_stable_hash` (the SOLE intra-function warm gate — locked design Decision 2 / C7) and revalidating the unioned consumed `ReadSetSignature.facts`; the recorded `slice_semantic_hash` / `selected_*_ids` are NEVER re-derived or consulted as a warm-validity gate.
...
- `crates/verter_session/src/types.rs` (`HostConfig`) — `flow_return_loop_budget: u16` (default 32). NO `flow_return_cycle_sentinel_depth` (or any other depth counter) is introduced: no depth sentinel coexists with the shared re-entry stack on the flow path (locked design Decision 6.1) — flow cycle control is the `FlowReturn` view of the ONE shared obligation/re-entry stack. Disposition per Decision 6.1's escape clause: no `flow_depth` / `depth_sentinel` symbol exists in the tree, and the live depth guards (`MAX_RESOLVE_DEPTH`, `HostConfig.depth_budget`, the relation engine's iterative work budget) are not flow-return-cycle-path counters — recorded here as the required no-op deletion line, with the `no_depth_sentinel_on_flow_return_path` guard (Required new guards below) asserting the absence of any depth counter on the flow-return path regardless. No `flow_return_enabled` flag (a transitional flag would gate a forbidden dual path or be an unused-field stub; incremental delivery is the audited-degraded mechanism).
- `crates/verter_session/src/meta_resolve/projectors/output_sink.rs` (`reduce_published_field_types` high-level driver + the sink-private `reduce_field_type_expr_with_mode` per-field reducer it wraps — both relocated into the terminal `output_sink` sink module; the pure reducible-operator predicate `type_expr_contains_reducible_operator` stays in `projectors/published_reducer.rs`) — the sole admission point: when the per-field reducer encounters `TypeExpr::TypeOf(ValueRef)` whose value root resolves to a **function value** AND the caller wrote `ReturnType<typeof callee>`, the existing builtin-utility dispatch table (where `Pick` / `Omit` / `Required` / `Partial` live) gains a `ReturnType` entry that resolves the callee to a function `SymbolHandle`, dispatches `SemanticQueryKey::FlowReturn`, and returns `FlowReturnResult.return_type`. Path-precise: the call result is materialised ONLY on the consumer-walked path; sibling fields stay shallow.

Deliverables:
- The `FunctionBodySkeleton` (arena-free, no type lowering; built once per content version — eagerly or lazily per the share-vs-split checks, never per query), the per-function `FunctionFlowGraph` built once per content version from it (typed edge classes; value-def / path-write / eval-effect / control-region landed here; no build-time type lowering), the `ReturnPathPeeker` graph demand planner (slice = graph reachability over the `FunctionFlowGraph`, two-frontier rule as edge classes), the `FlowSliceHashNode` / `FlowSliceLoweredBodyNode` cache-runtime nodes (hash-then-lower split), and `FlowSliceIR`.
...
- `WholeFunctionFlowIrNode` and its scanner-backed whole-function evaluation (`crates/verter_session/src/flow_ir.rs`, including its reuse of the scanner's expression lowering) — interim discard-class divergence evidence only (locked design, Substrate consumption contract): REPLACED by the demand-sliced substrate and DELETED in the SAME change. It must not survive as a parallel compatibility engine, and no production reference to it may remain.
- The flow/return-path depth-recursion sentinel — replaced by the `FlowReturn` view of the shared re-entry stack; no depth counter coexists with the shared stack on the flow path (locked design Decision 6.1). Verified against the tree: no such sentinel symbol exists and the live depth guards are off the flow-return cycle path, so this is the required NO-OP deletion line; the `no_depth_sentinel_on_flow_return_path` guard asserts the absence regardless.
...
SemanticQueryKey/facts touched: `FlowReturn` (value domain `FlowReturn(Arc<FlowReturnResult>)`); dispatches into `TypeOf` (callee value resolution), `ProjectPath` / `ProjectMember` (return member projection), `IndexedAccess` (the `ReturnType<typeof f>['x']` thread). Facts recorded: the `ProgramAnalysisFactRef::FlowBody` rooting (the sole `ProgramAnalysis` warm-validity ref) + `FlowSlice` (new — candidate-selection discriminant ONLY: slice identity + selected binding / effect / control-region / closure ids; never in `ReadSetSignature.facts`). Facts read: `MemberPresence`, `Member`, `RouteGeneration`, `ExportSurface`, `ModuleAugmentation`, `AmbientGlobal`, `LibIntrinsic`, `TypeEnvOptions`, project-generation facts. Admission: `FlowSliceBudget`; flow-cycle sentinel `ReturnOnly`; budget / overflow / cycle / cancellation / partial-slice route through `CacheAdmission` non-admission.
...
- `flow_graph_build_is_shallow_interned_no_lowering` — the PART 1 §6.2 perf-hardening guard for the build path: the `FunctionFlowGraph` build uses compact interned IDs (no owned strings / boxed AST pointers as node/edge/slot/path handles) and lowers NO type at build time (asserts inventory/graph construction produces no `TypeExpr` lowering, `Relate` / `Instantiate` or other resolution dispatch, route lookup, or imported-fact production — those happen only when a slice traverses an edge). It does NOT constrain build timing or region-materialization strategy: eager full structural construction and lazy once-per-content-version construction are BOTH conforming — the locked design's five share-vs-split storage checks (each with its own discriminating executable guard, results recorded per SCC-10; U0 registers them) settle timing and placement at implementation, and a per-query / per-demand rebuild is rejected by `function_flow_graph_built_once_per_function_skeleton`, not here. Discriminating fixture: a large function body with a tiny demand slice (`ReturnType<typeof big>["b"]`) lowers no type and charges no resolution dispatch / route lookup / imported-fact production during graph construction; fails if the build lowers types or produces such facts. Lands HERE with the `FunctionFlowGraph` build.
- `no_depth_sentinel_on_flow_return_path` — no depth counter (config field, constant, or local sentinel) participates in flow-return cycle control; the flow path's cycle discipline is the `FlowReturn` view of the ONE shared re-entry stack (locked design Decision 6.1). Separately owed: this guard was deleted by the frozen CALL_RESOLVE branch; the locked design requires it to land WITH this block.
...
- `flow_slice_budget_exceeded_admits_nothing` — forces `FlowSliceBudget` `BudgetExceeded`; asserts the result is RETURNED (not a panic / None), the `SemanticGraphStore` slot is NOT warmed, the `FlowSliceLoweredBodyNode` store has NO entry, the result's `ReadSetSignature.facts` has NO `FlowBody` entry for the function and NO `FlowSlice` discriminant fact is recorded, and the audit payload records the budget-exceeded reason (the three-layer non-admission rule).
- `program_analysis_fact_domain_validates_flow_slice` — the `FlowSlice` fact is classified into `FactDomain::ProgramAnalysis` (`docs/arch/fact-based-cache.md`) as a candidate-selection DISCRIMINANT only, and warm `FlowReturn` validity runs through the landed `StoreView::validates_program_analysis_domain` (`crates/verter_session/src/resolver_store.rs`) over the SOLE `ProgramAnalysisFactRef::FlowBody` rooting, which **FAILS CLOSED** on a missing / stale / unrooted rooting. Discriminating fixture: (a) editing a function body so its `flow_body_stable_hash` changes fails the warm read → recompute, never serves the stale slice; (b) a missing / unrooted `FlowBody` rooting validates `false` (fail-closed), not vacuously `true`; (c) a probe asserting `FlowSlice` / selected-ID participation in warm validity FAILS the build (the sole-rail invariant — locked design C7/SCC-9). Ownership: `verter_workspace` owns the fact schema; `verter_session` owns and implements the validation (landed); U6 (this block) owns producing and consuming the flow facts.
...
- `predicate_signature_without_body_audits_signature_only_outcome` — a declared (signature-only) predicate applied at the call site applies the signature fact and emits `PredicateEffectApplied` even though no body was lowered; the result carries `degradation: None`.
...
Changes (exact files / functions):
- `crates/verter_session/src/semantic_query.rs` — land the `SemanticQueryKey::ResolveCall` enum variant **+** its `SemanticQueryKeySpec` row together, in EXACTLY the final key shape the locked design defines (its §1.1 / Substrate consumption contract C4 — the sole normative key statement, implemented here, every axis immediate): `{ point, callee, call_kind: Call | New | TaggedTemplate | Decorator | JsxFactory, receiver_this, args, explicit_type_args, contextual_result, policy, flow (SEALED-EMPTY until non-empty narrowing lands), context }` (U2 finalized only the slot-identity SHAPE/model the variant reuses; the variant + spec row land HERE so `semantic_query_key_spec_table_equals_enum` stays green incrementally) and its `SemanticQueryValue::ResolvedCall(Arc<ResolvedCallResult>)` mapping; `CallArgKey` distinguishing EAGER type identities from CONTEXT-SENSITIVE expression identities carrying `ProgramPointId`, spread/literal authored form, flow narrowing, substitution-at-typing-time, binder identity, and the resolved contextual-target identity (C4/§1.3).
...
- `crates/verter_semantic/src/analysis/function_program.rs` — land the unified call-site record family `FunctionCallSiteRecord { span, callee, target, args }` (the locked design's C2/SCC-4: the SINGLE record family every call-shaped consumer reads; a second, per-consumer call-site index is FORBIDDEN), with lexical-scope- and hoisting-correct target binding: a nested same-name function SHADOWS outer candidates, direct recursion resolves exactly, and an unserved nested target FAILS CLOSED (a typed miss) — never a fallback to an outer same-name function.
- `crates/verter_session/src/project_semantic_dispatch/flow_return.rs` (the call arm of the shared expression-site evaluation entry — C3/SCC-5) — call-argument acquisition routes through the shared evaluation entry at a bare `ProgramPointId` and threads its TYPED outcome — `Complete(node) | ReturnOnly(reason) | ProvenAbsent` (or equivalent) — with consumed facts/completeness bubbled into the enclosing `ResolveCall` query; no bare `Option<SemanticNodeId>` with a side-channel partial marker, and calls on that path route only through `ResolveCall` (no scanner fallback).
- `crates/verter_session/src/project_semantic_dispatch/dispatch_txn.rs` + `project_semantic_dispatch/return_equation.rs` — the SCC-7 runtime-authority surfaces (locked design C5, items 1–5): ONE `CheckerDispatchTransaction`-owned generic obligation stack/ledger spanning `Relate` / `FlowReturn` / `ResolveCall` (no second relation stack, TLS cycle set, or flow depth sentinel participates); the mixed-domain SCC close through the same lowlink/assumption storage, each node keyed by its full normalized identity (incl. transient inference occurrence/session identity where required); root-fenced commit — SCC-closed members drain onto the root's committing admission as fenced backfill, no member publishes before the component root commits, and any unknown / abandoned / superseded / budgeted / nonconvergent member releases the ENTIRE component without publication; the staged `InferenceSessionState` commit (`Collecting → StagedDeterministic → CommittedDeterministic | Abandoned`) with applicability REPLAY starting from a fresh collecting session, reproducing winner + substitution, and only then permitting the mixed `solve_return_equation` fixpoint and atomic drain. Flow-frame/SCC-close evidence is supplied by `U6.FLOW_RETURN_SUBSTRATE`.
...
- `resolve_call_key_covers_args_this_contextual_type_overload_policy_and_context` — the `ResolveCall` key covers EVERY final axis of the locked design's C4 key (`point`, `callee`, `call_kind`, `receiver_this`, `args` — eager + context-sensitive — `explicit_type_args`, `contextual_result`, overload `policy`, the sealed-empty `flow`, and `context`); two `ResolveCall` keys differing in exactly one hash unequal, and reconstruction routes through the one shared function (the locked design's SCC-6). Lands HERE with the `ResolveCall` variant (its variant must exist on the committed tree for the guard to test it).
...
- The SCC-4 / SCC-5 / SCC-6 / SCC-7 acceptance criteria (locked design, Substrate consumption contract) land here as EXECUTABLE guards/fixtures with the block: the single-`FunctionCallSiteRecord`-family fixtures with lexical shadowing, exact direct recursion, and fail-closed unserved-nested-target assertions (SCC-4); the typed expression-outcome propagation fixture — `Complete | ReturnOnly(reason) | ProvenAbsent` bubbled into the enclosing query (SCC-5); the every-final-axis, one-shared-reconstruction-function, and call-record-retention assertions (SCC-6 — extending `resolve_call_key_covers_args_this_contextual_type_overload_policy_and_context`); and the SCC-7 mixed-domain runtime-authority proof — a REAL `Relate → ResolveCall → FlowReturn → Relate` cycle closing as ONE SCC through the one `CheckerDispatchTransaction` obligation stack/ledger and lowlink/assumption store, with root-fenced commit/whole-component release and fresh-collecting-session staged replay reproducing winner + substitution before the mixed return equation and atomic drain. The two broad guards above (`checker_reentry_graph_spans_flow_call_contextual_narrowing`, `cross_engine_cycle_discharge_admits_only_stable_deterministic_results`) are INSUFFICIENT ALONE for SCC-7 — SCC-7 expressly says so; the mixed-component fixture is the required proof, with flow-frame/SCC-close evidence supplied by `U6.FLOW_RETURN_SUBSTRATE`.
