...
When a `.vue` `<script setup>` (or a template expression) becomes SYNTACTICALLY BROKEN mid-edit, the user perceives that ALL diagnostics vanish. Canonical repro (`D:\dev\temp\verter-demo\templateRef.vue`, observation-only):
