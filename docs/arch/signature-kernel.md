...
| Cold, warm, partial persisted cache, edit/revert, cancelled/retried work, epoch rebuild | Same final logical snapshot gives the same observations and output. |
...
| D12 charter amendment (section 13) | Implemented charters are frozen acceptance records (`APPLICATION.md`). | The enlargement is recorded in `decisions/2026-09-17-signature-kernel-train.md` and by the DAG edge `V0 → D12`; `D12.md` is not rewritten. |
