...
`session_overlay_augmentation_isolated_from_base_meta` (registered under the Declaration Augmentation
row of `crates/verter_session/tests/g_misc0/critical_rules_have_guards.rs`). The formerly-planned
...
overlay-results-do-not-populate-base-caches Cache-Architecture rule (the query-identity consumers'
keys stay content-free; overlay content identity is validated on the VALUE).
...
|---|---|---|
| `TypeInfoParityBlockId` DAG (per-block prereqs, consumed mechanisms, `required_guards`, `verification_labels`) | `crates/verter_session/tests/manifest_data/typeinfo_parity_blocks.rs`, generated by `scripts/gen-typeinfo-ignore-manifest.mjs` | Block contracts derive from the generator, never from §1 prose; the generated table is the sole writer's output (`--check` freshness). |
...
- `node scripts/gen-typeinfo-ignore-manifest.mjs --check` (alias
...
- **Scope (ONE clean cutover — no migrate-twice):**
  ...
  1. Finalize the **`SemanticQueryKey` identity SHAPE once** (the slot-identity
     ...
     **five landed spine variants** of the parent's seven-variant spine in that
     ...
     > Source: the codex step-0 ruling Q2 (Query Keys), recorded in the orchestration ledger.
...
- **Guards:** `node_taxonomy_complete` (the LANDED single enumerating assertion that
  ...
  block-contract row in `gen-typeinfo-ignore-manifest.mjs` carries this exact set); plus
