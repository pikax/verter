...
### 2.2 Backfill / Satisfaction Rule

Cache satisfaction is **materialized-record + directional**, NOT an enum-rank "broader mode dominates" lattice. A candidate serves a request only when some RECORDED materialised `(path, point)` the candidate's compute actually produced dominates the request at the SAME path — `cached_satisfies(MemoEntry.satisfied_projection, requested_point_for_key(key))` — never the candidate's nominal slot/mode and never an enum rank. (A warm hit additionally requires per-candidate `ReadSetSignature.validate_with_self_roots` against the caller's live view.)

Backfill clones a broader entry verbatim into a projection-depth-narrower target slot, gated by the SAME directional `cached_satisfies` check over recorded materialised points. The gate is directional: `Navigate ⊒ Shallow` in the lattice does NOT license serving a `Shallow` shell surface from a carrier-stopping `Navigate` result — the lattice-unsound `Shallow → Navigate` clone is REJECTED. In particular "`Shallow` satisfies `Navigate`" is wrong: a `Shallow` shell carries no carrier-stop materialisation a `Navigate` request needs.

`Skeleton` is isolated — it never satisfies and is never satisfied by another mode (it is the unbound-generic BFS mode whose `TypeParam` shells are not a substitute for a bound projection).

Narrower successful results MUST NOT claim broader work is cached. A whole-surface projection may backfill per-member or per-indexed-access caches ONLY for the members it actually materialised; a narrow member result must not claim sibling-member or whole-surface caches.
...
Semantic resolution for these constructs lives in `verter_session::semantic_query::ProjectSemanticDispatch` via the following `SemanticQueryKey` variants. Env composition is PER-CONTEXT, not uniform (live Option-B identity; R21 — detailed in the closing paragraph of this section): each variant carries a named `*Context` struct holding exactly the split env hashes its value depends on (content-free, R6), plus its content-free slot identity and any `type_args`. There is no uniformly-embedded env envelope and no `resolver_version` in these content-free query-identity keys. The shapes (LIVE where the variant is landed in the enum; FORWARD-PLANNED per block U2 / the unified plan + `u2-query-value-domain-design.md` §413 locked design where marked):

- `SemanticQueryKey::ResolveMergedDeclaration { decl_slot: ResolvedDeclSlotIdentity, type_args: Arc<[SemanticNodeId]>, context: MergedDeclarationContext }` → `SemanticNodeData::MergedDeclaration` (FORWARD-PLANNED U2 variant; `MergedDeclarationContext` carries `{P,R}` + `substitution`). The current interim realization is the `SemanticNodeData::MergedDecl { contributors }` peer-merge carrier reduced inline, pending the dedicated U2 merge reducer.
- `SemanticQueryKey::ResolveDeclarationAugmentation { target: Module | Global, context: DeclarationAnalysisContext }` → `DeclarationAnalysis` value domain (FORWARD-PLANNED generalized U2 variant replacing the former `ResolveModuleAugmentation` slot). The reverse-lookup `AugmentationTargetKey { project_identity, resolve_env_hash, lib_env_hash, population, target }` (on `FileArtifactStore::augmentation_index`) is itself parse-env-free, and the forward-planned `DeclarationAnalysisContext` query CONTEXT carries exactly the matching `{R, L, J}` env axes — `resolve_env_hash`, `lib_env_hash`, `project_identity`. `parse_env_hash` is NOT a key dim: a parse-option / parser-version change is reflected through the VALUE-side body read (the cold compute re-sources the augmenter specifiers/bodies from live parser output and roots the value on the contributing files' `FileWholeHash` self-roots), consistent with the R6 content-free query-identity rule (see `u2-query-value-domain-design.md` §2.2 and the `native-typeinfo-parity.md` §2.2 struct def). The current interim realization is build-time `stitch_module_augmentations` (in `project_semantic_dispatch::build`) folding augmenter contributions into a `MergedDecl` carrier, pending the dedicated U2 augmentation reducer.
- `SemanticQueryKey::ResolveAmbientNamespace { namespace_slot: ResolvedDeclSlotIdentity, type_args: Arc<[SemanticNodeId]>, context: AmbientNamespaceContext }` → `SemanticNodeData::AmbientNamespace` (LIVE; env dims `P R T L J`; `AmbientNamespaceContext` carries forward-declared `parse_env_hash` + `resolve_env_hash` + `mode`).
- `SemanticQueryKey::ResolveOverloadSet { callee: SemanticNodeId, type_args: Arc<[SemanticNodeId]>, context: OverloadSetContext }` → ordered `Arc<[SignatureRef]>` (LIVE; `OverloadSetContext` carries `{R}`).
- `SemanticQueryKey::ResolveEnum { enum_slot: ResolvedDeclSlotIdentity, context: EnumContext }` → first-class enum representation (LIVE; `EnumContext` carries `{R}`).
...
Env composition is PER-CONTEXT, not uniform (live Option-B identity; R21). Each `SemanticQueryKey` variant carries a named `*Context` struct holding exactly the split env hashes that variant's value depends on — never a bundled `project_config_hash`. The U2B.5/6/7 spine surfaces carry `parse_env_hash` FORWARD-DECLARED in addition to `resolve_env_hash` (+ `mode`): `ClassSurfaceContext` and `AmbientNamespaceContext` carry `parse_env_hash` (P, forward-declared for the deferred decorator/heritage/body reducer) + `resolve_env_hash` (R) + `mode`; `ProgramAnalysisContext` carries the five split env hashes (P R T L J) + `substitution`, with `flow` (`FlowNarrowingKey`) / `contextual` (`ContextualTypingKey`) carried as PER-VARIANT key fields on `FlowNarrowingAt` / `ContextualTypeAt`. Every `*Context` is CONTENT-FREE (R6): no content / `parse_stable_hash` / version hash, no `fact_dep_signature`. Guards: `resolve_class_surface_identity_covers_parse_env_axis`, `resolve_ambient_namespace_identity_covers_parse_env_axis` (assert the forward-declared `P` axis is a distinct cache dimension on the spine surfaces) and `semantic_query_key_spec_table_equals_enum` (the spec table tracks the enum after every block).
...
Flow narrowing and contextual typing are not part of the default `resolveSymbolGraph` payload — they require a dedicated query. Each variant carries its own `ProgramAnalysisContext` (the split env hashes P R T L J + `substitution`, per §2.17) plus its per-variant `flow` / `contextual` set-identity key field:
...
| `FileArtifactStore::augmentation_index` | Content-addressed | `AugmentationTargetKey { project_identity, resolve_env_hash, lib_env_hash, population, target }` (`population: AugmentationPopulation {Base, Session(overlay-set fingerprint)}` gives Base/Session overlay isolation). `parse_env_hash`/`type_env_hash` EXCLUDED — augmentation target identity depends on which lib + which resolver paths see the augmenter, not on parsing or strictness options. |
...
| `SemanticGraphStore` query nodes | Query-identity | One key struct per `SemanticQueryKey` variant — see §2.17 / §3.11 for explicit struct definitions. Env-hash inclusion is PER-CONTEXT (R21), not uniform: each variant's named `*Context` carries exactly the split env hashes its value depends on (`resolve_env_hash`, `type_env_hash`, `lib_env_hash`, `project_identity`). The family-keyed variants (`Instantiate.base`, `ResolveMacroPayload.owner`) carry the content-free, env-bearing `ResolvedDeclSlotIdentity` (with the extra `resolve_env_hash` on the per-key `InstantiateContext` / `MacroPayloadContext`) and re-source the file content version at value-compute, so `parse_env_hash` does NOT enter those keys. The U2B.5/6/7 spine surfaces are the exception: `ClassSurfaceContext` / `AmbientNamespaceContext` carry `parse_env_hash` FORWARD-DECLARED, and `ProgramAnalysisContext` carries the full P R T L J + `substitution` (with `flow` / `contextual` as per-variant key fields). All `*Context` structs are CONTENT-FREE (R6): no content / `parse_stable_hash` / version hash, no `fact_dep_signature`. |
| `SemanticGraphStore::relation_memo` | Query-identity | `RelateMemoKey { source: SemanticNodeId, target: SemanticNodeId, relation: RelationKind, policy: RelationPolicy, source_freshness: FreshnessKey, inference_context: Option<InferenceContextKey>, context: RelationContext }`, where `RelationContext` carries the split `R T L J` (`resolve_env_hash`, `type_env_hash`, `lib_env_hash`, `project_identity`) plus `substitution: SubstitutionCanonicalHash` and `projection_reduction: ProjectionReductionContext`. CONTENT-FREE (R6): every field is a content-free key dimension (interned node ids, closed kind/policy/freshness enums, content-free inference context, env hashes) — NO `resolver_version`, NO content/`parse_stable_hash`/version hash, NO `fact_dep_signature`. Version rooting lives on the cached VALUE (the `ReadSetSignature` carrier + `validated_at_generation`), revalidated on every warm hit — never in the key. |
...
| `SemanticQueryKey Env Composition Is Per-Context` (§2.17) | `resolve_class_surface_identity_covers_parse_env_axis`, `resolve_ambient_namespace_identity_covers_parse_env_axis`, `semantic_query_key_spec_table_equals_enum` | 0a |
...
| `resolve_class_surface_identity_covers_parse_env_axis` | `crates/verter_session/tests/g_block/u2b5_class_ns_enum_overload_guards.rs` |
| `resolve_ambient_namespace_identity_covers_parse_env_axis` | `crates/verter_session/tests/g_block/u2b5_class_ns_enum_overload_guards.rs` |
...
> **SUPERSEDED (see the top-of-file banner):** this per-file phase schedule and the `typeinfo_tests_unignore_plan.md` doc-manifest are historical. The authoritative ledger is the 362-row `block_id` partition in [`native-typeinfo-parity.md`](./native-typeinfo-parity.md) §10.4.1, backed by the in-repo `.rs` two-table manifest (`crates/verter_session/tests/typeinfo_ignored_test_manifest.rs`); the binding count is the parser-derived **362** (not 384 — that was the raw `#[ignore]` line count), tracked by `block_id` rather than phase.
...
| `MemberShapeCacheDb` | retired; replaced by `ShapeCacheDb` — the per-member graph-native materialiser cache is now a slot inside `ShapeCacheDb` indexed by `ShapeSubject::MemberValueNode` and built via `ShapeCacheKey::surface_member_value_whole_with_context(scope, &SurfaceMember, ProjectionReductionContext)` (the terminal-context parameter `terminal_context: ProjectionReductionContext`, not a `mode`). `ProjectTypeStore` owns the unified `shape_cache_db: ShapeCacheDb`. The static guard at `crates/verter_session/tests/cases/g_block/block_6i_static_guards.rs::shape_cache_db_replaces_split_caches` asserts `pub struct MemberShapeCacheDb` MUST NOT exist anywhere in the tree. | `crates/verter_session/src/component_meta_caches.rs` (`pub struct ShapeCacheDb`, `pub enum ShapeSubject`, `ShapeCacheKey::surface_member_value_whole_with_context`) |
