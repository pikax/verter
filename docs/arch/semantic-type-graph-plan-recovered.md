...
Semantic resolution for these constructs lives in `verter_session::semantic_query::ProjectSemanticDispatch` via the following `SemanticQueryKey` variants. Env composition is PER-CONTEXT, not uniform (live Option-B identity; R21 — detailed in the closing paragraph of this section): each variant carries a named `*Context` struct holding exactly the split env hashes its value depends on (content-free, R6), plus its content-free slot identity and any `type_args`. There is no uniformly-embedded env envelope and no `resolver_version` in these content-free query-identity keys. The live shapes:

- Same-name declaration merging is realized as the node-data carrier `SemanticNodeData::MergedDecl { contributors }` (reduced by the `MergedDecl` peer-merge reducer), NOT a dedicated `SemanticQueryKey` variant.
- Cross-file ambient augmentation (`declare module` / `declare global`) is realized by build-time `stitch_module_augmentations` (in `project_semantic_dispatch::build`) folding augmenter contributions into a `MergedDecl` carrier, NOT a dedicated query-key variant. The augmenter set is addressed by `AugmentationTargetKey { project_identity, resolve_env_hash, lib_env_hash, population, target }` on `FileArtifactStore::augmentation_index`.
- `SemanticQueryKey::ResolveAmbientNamespace { namespace_slot: ResolvedDeclSlotIdentity, type_args: Arc<[SemanticNodeId]>, context: AmbientNamespaceContext }` → `SemanticNodeData::AmbientNamespace` (env dims `P R T L J`; `AmbientNamespaceContext` carries forward-declared `parse_env_hash` + `resolve_env_hash` + `mode`).
- `SemanticQueryKey::ResolveOverloadSet { callee: SemanticNodeId, type_args: Arc<[SemanticNodeId]>, context: OverloadSetContext }` → ordered `Arc<[SignatureRef]>`.
- `SemanticQueryKey::ResolveEnum { enum_slot: ResolvedDeclSlotIdentity, context: EnumContext }` → first-class enum representation.
