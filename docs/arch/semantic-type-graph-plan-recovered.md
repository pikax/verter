...
- `SemanticQueryKey::ResolveDeclarationAugmentation { target: Module | Global, context: DeclarationAnalysisContext }` → `DeclarationAnalysis` value domain (FORWARD-PLANNED generalized U2 variant replacing the former `ResolveModuleAugmentation` slot). The reverse-lookup `AugmentationTargetKey { project_identity, resolve_env_hash, lib_env_hash, population, target }` (on `FileArtifactStore::augmentation_index`) is itself parse-env-free; the precise env identity of the `DeclarationAnalysisContext` query CONTEXT (specifically whether `parse_env_hash` rides on it, since the cached declaration-analysis value reads parsed augmentation specifiers/bodies) is an OPEN reconciliation tracked in the U2 design docs (`u2-query-value-domain-design.md` §413/§21.2 vs the `native-typeinfo-parity.md` struct def) and settled by the U2 implementer — it is not restated here. The current interim realization is build-time `stitch_module_augmentations` (in `project_semantic_dispatch::build`) folding augmenter contributions into a `MergedDecl` carrier, pending the dedicated U2 augmentation reducer.
