...
- `SemanticQueryKey::ResolveDeclarationAugmentation { target: Module | Global, context: DeclarationAnalysisContext }` → `DeclarationAnalysis` value domain (FORWARD-PLANNED generalized U2 variant replacing the former `ResolveModuleAugmentation` slot; `DeclarationAnalysisContext` carries `{R,L,J}`, with `AugmentationTargetKey` folding `{J,R,L,target}`). The current interim realization is build-time `stitch_module_augmentations` (in `project_semantic_dispatch::build`) folding augmenter contributions into a `MergedDecl` carrier, addressed by `AugmentationTargetKey { project_identity, resolve_env_hash, lib_env_hash, population, target }` on `FileArtifactStore::augmentation_index`, pending the dedicated U2 augmentation reducer.
