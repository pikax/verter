...
| D-41 | **COMPONENT / `<svelte:*>`-SPECIAL NAMED-SLOT FILLER ROUTING (DEFER-NEW, owner: the component/special named-slot filler block — reconcile with the 5f-a component-slot vertical; hard trigger: any change to the component props-object member assembly or the component-family `$$slots` emission, any broadening of component/special-hosted attribute acceptance, AND in any case before declaring the Svelte client runtime feature-complete / RC. Cite: codex scope-consult Q4.)** Official `svelte@5.56.3` ACCEPTS a `slot=`-bearing COMPONENT / `<svelte:*>`-special direct component child (`<Child><Inner slot="foo"/></Child>`) and routes the filler into the parent's `$$slots`; Verter does not emit that filler routing — consuming `slot=` on component/special children is a broad emission rewrite (projection/emission change), outside the unified slot-validation gate's route-scope. TEMPORARY behavior: the unified slot choke-point (`validate_slot_placement` at `classify_node` entry — the sole `slot=` validation authority, run before every per-host accept/fold/prop projection) fails the whole class CLOSED with the typed `Unsupported` refusal (`DynamicAttribute` / diagnostic `svelte-runtime-unsupported-dynamic-attribute`) — never the former quiet-wrong plain-prop projection (`Init{slot:'foo'}`) or `$.attribute_effect` fold. The valid REGULAR-ELEMENT / `<svelte:fragment slot>` static filler placement (the `slot_attr_owners` set) stays accepted. Guards: the unified-gate fail-closed rows (Component direct/nested/dynamic + `<svelte:element>` static/dynamic + `<svelte:component>`/`<svelte:self>`/`<svelte:boundary>`/`<svelte:head>`/standalone-fragment + the invalid-placement Element rows) in `svelte_client_fail_matrix.rs`, the per-`SpecialKind` exhaustive choke-point unit proof (`validate_slot_placement_fails_closed_for_every_attr_bearing_kind`), and the positive filler-accept control (`valid_regular_element_and_fragment_fillers_still_compile`). **Done when:** component/special fillers route into `$$slots` matching pinned `svelte@5.56.3`, and the fail-closed rows are converted to positive goldens in the SAME change. |
...
| **5h** | **Custom elements / `$host()`.** `<svelte:options customElement>` + `$host()` → `$$props.$$host` + module `customElements.define(name, $.create_custom_element(Cmp, props, [], [], { mode }))` + `$.push`/`$.pop($$exports)` getter/setter accessors. | 4, 5f-a | — |
