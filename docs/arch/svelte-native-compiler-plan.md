...
external corpus and lands DEFERRED — after the compiler is feature-complete (so real components compile rather
than mostly-refuse) and BEFORE the performance work (Blocks 12/14), so the optimizer and the perf gate are
validated/benchmarked against real-world cases, not only synthetic fixtures. Execution order:
`5a-5m → I → 6/7/8/9 → 10/11 → RC → 12/14 → 13`. RC is the Svelte slice of a broader per-framework real-world
corpus matrix (Vue: nuxt/ui + PrimeVue + Element Plus + Vuetify; Astro: accessible-astro-components + Starlight
+ the withastro cross-framework example; React: Radix/Ariakit/MUI; Preact: preact + signals; cross-framework:
TanStack — the hardest generics/typings stress); each framework's corpus lands with its adapter, before that
framework's perf work, per the framework-agnostic roadmap.
