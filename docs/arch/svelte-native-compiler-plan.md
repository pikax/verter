...
| Delegated + non-delegated events, legacy modifiers | `$.delegated`/`$.delegate`, `$.event`, `$.preventDefault`/… | 4 (delegated), 5d (regular-element non-delegated + legacy modifiers); special-element global events → 5f |
...
| **5f** | **Components, snippets, special elements, transitions/actions/animations.** Direct component calls + `$.component`, snippet defs / `$.snippet` / direct render, `{@attach}` (`$.attach`), `$.element`, the full special-element set — `<svelte:head>` (`$.head`), `<svelte:document>` (`$.event(…, $.document, …)`), `<svelte:boundary>` (`$.boundary`), `<svelte:self>` (recursive self-call), `<svelte:fragment>` (`$$slots` entry), `<svelte:window>`/`<svelte:body>` — and `$.transition` / `$.action` / `$.animation` (per the §3.2 special-elements rows)… ALSO owns the component-host + special-element BINDINGS (component `bind:this`/`bind:prop`/function-binds consuming 5c's shared bind-operator substrate; `<svelte:window>`/`<svelte:body>`/`<svelte:document>` size/scroll/property/`bind:this` binds), which depend on 5f's own host emission (D-21) and on component instance-import hoisting; 5f depends on 5c for the reusable bind-operator substrate, NOT because 5c shipped component/window binds. ALSO owns the `<svelte:window>`/`<svelte:body>`/`<svelte:document>` GLOBAL EVENT output (`$.event(…, $.window\|$.document\|$.document.body, …)` — emitted via the SAME 5d event-emission substrate, fed the global-host `EventEmitTarget` variants — with NO template / NO clone-root / NO `$.append`: the init-only, no-body root-region module shape) PLUS the `client_surface.rs:865` special-element node-gate opening (window/body/document acceptance) and the `html.rs:672` / `refuse_unsupported_root_region` no-body/init-only root-region machinery. | 4, 5c, 5e | — |
...
Decisions Log as **D-34** (it lands AFTER the compiler is feature-complete — post `5a-5m`; sequence
