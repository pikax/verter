...
| D-17 | **COMPARATOR FEED-FORWARD DEBT (deferred — owner: the first block that admits author `<script>` functions / `$derived` / param-bearing handlers, 5c+).** The AST-structural emit comparator (`expr_sig` / `params_sig` / `module_sig` in `crates/verter_compiler/tests/svelte_client_emit_topology.rs`) — the oracle wired at BOTH gate sites (`emitted_client_topology_matches_official_goldens`, `emitted_codegen_corpus_matches_official_goldens`) — is COMPLETE for the Svelte client subset emitted today but intentionally DROPS a cluster of behavior-bearing axes: param-default `FormalParameter.initializer` (`(a = 1)` ≡ `(a = 2)` ≡ `(a)`); async/generator bits (`r#async` / `generator`); object-property `kind` / `method` / `computed` / `shorthand`; and the discriminant-only statement/declaration fallback (`for` / `while` / `throw` / `switch` / `try` collapse to `Stmt(discriminant)`). **DEFER basis (codex-ruled, unreachable-today):** every dropped axis is UNREACHABLE in the currently-accepted surface — author `<script>` functions reject as `InstanceScriptItem`, param-bearing handlers as `NonDelegatedEvent`, `$derived` as `AdvancedRune`, author control-flow is refused — so no dropped axis is BOTH reachable AND uncovered; the one always-emitted axis (imports) is independently pinned by `emitted_imports_ok` at both gate sites. NOT a comparator weakening today. **CONTRACT:** the FIRST block that ADMITS any of those surfaces (5c+) MUST encode the matching axis in the SAME change — read `FormalParameter.initializer`, `r#async`, `generator`, `ObjectProperty.{kind,method,computed,shorthand}`, and replace the statement/declaration discriminant fallback with a structural encoder. The comparator is the gate's oracle, so a dropped axis that becomes REACHABLE is a silent false-PASS. **Mirror:** the in-source comment at the `expr_sig` definition (the comparator family head) lists the same unreachable-today axes + this same-change contract. |
