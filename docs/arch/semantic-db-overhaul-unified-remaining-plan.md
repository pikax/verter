...
- **Q2 structural-body cache POPULATION** (TEMPORARY debt — tracked in
  `docs/arch/q2-structural-body-cache-deferral.md`). The LANDED Stage-6 declaration-body
  handle-payload / hot-read flip (Option B) landed the `PreparedStructuralBodyCache` + key +
  registry **dead-code-correct but un-wired** (POPULATION deferred): its key `(body_slot, provenance, merge_role)` lacks
  the resolving lowerer's args/env/substitutions/name-resolution/mode/scope dimensions,
  so populating it with resolving-lowered bodies would be UNSOUND. Warm-state reuse is
  already sound via the `SemanticGraphStore` `Instantiate` memo. The cache population, an
  env-independent-or-dimension-carrying key redesign, the Q2 A-vs-B measured perf-consult
  (moot until the cache is populated), and the 3 deferred S4-bump counters all defer to a
  future cache-redesign block (Stage 7-ish). Ruling: two converged codex legs
  (leg A finding 7 + leg B finding 4, recorded in the orchestration ledger),
  CTO-ratified. **SUPERSEDED by the ratified Stage-10 design**: the cache is never
  populated — it DELETES at Stage-10 B2 (with `q2-structural-body-cache-deferral.md`;
  see `stage10-typeexpr-terminal-removal-design.md` §5.5); the correctness-required
  lowered-body memo is the first-class `LowerLocator` query, and any future perf cache
  is a fresh Stage-12 design. The remaining stale q2 references clean up with B2's
  deletion slice.
