...
|---|---|---|
| `TypeInfoParityBlockId` DAG (per-block prereqs, consumed mechanisms, `required_guards`, `verification_labels`) | `crates/verter_session/tests/manifest_data/typeinfo_parity_blocks.rs`, generated by the `gen-typeinfo-manifest` bin | Block contracts derive from the generator, never from §1 prose; the generated table is the sole writer's output (`--check` freshness). |
...
- `cargo run -p verter_session --bin gen-typeinfo-manifest -- --check` (alias
...
- **Guards:** `node_taxonomy_complete` (the LANDED single enumerating assertion that
  ...
  block-contract row in the `gen-typeinfo-manifest` generator carries this exact set); plus
