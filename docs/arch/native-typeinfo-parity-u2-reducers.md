...
- `session_overlay_augmenter_isolated_from_base_index` + `session_overlay_augmentation_isolated_from_base_meta` (the LANDED overlay-aware augmentation guards; unified-plan §0.5.1) — the augmentation index is OVERLAY-AWARE: `AugmentationTargetKey` carries `population: AugmentationPopulation {Base, Session(overlay-set fingerprint)}`; a session scan reads the session's overlay artifacts UNIONED with base under `Session` while base reads stay on `Base`; an unsaved-buffer / overlay edit that adds or removes a `declare module` / `declare global` contributor therefore gets a REAL stitched session answer; overlay augmenters never poison the base index and never cross sessions; there is NO base-only `session.is_none()` assert on this surface (composes with the overlay-results-do-not-populate-base-caches Cache-Architecture rule — the semantic result remains value-side fact validated). These two guards SUPERSEDE the retired fail-closed deliverable `session_overlay_augmentation_fails_closed_until_implemented`: its either/or contract was discharged on the implemented arm, and a fail-closed fixture would now regress landed behavior (a session view gets a stitched answer, not `ReturnOnly`).

Critical-rule guards: implements the parent's `(CRITICAL)` declaration-augmentation / value-domain / `GraphTypeNode`-purity rules plus the declaration-merge-order rule (PART 1 §1.8); the augmentation-identity + value-domain guards above, `declaration_merge_records_binder_overload_augmentation_order_as_facts`, and the landed overlay-aware trio `session_overlay_augmenter_isolated_from_base_index` / `session_overlay_augmentation_isolated_from_base_meta` (unified-plan §0.5.1) are their R6 guards. There is no wire-relocation guard (the relocation was rejected); the landed `node_taxonomy_complete` taxonomy guard pins `GraphTypeNode` kinds 21–25 (including augmentation arms 23/25) as live (the planned `module_augmentation_is_public_graph_state` guard was NOT landed — it does not exist in `crates/`).
