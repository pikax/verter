...
1. **U0 foundations** — the manifest re-derivation to exactly 362
   ...
   per-row `ProofRequirement` model, the proof registry + row-test rail (landed
   under U0-FINISH-B in the locked design's hand-authored pure-data shape), the U0
   row-exact capability→mechanism→proof coverage table (PART 2 §10.4 — the
   U13/U15-gated unbuilt residual, generated from the manifest whose `block_id`
   column comes from the distinct hand-authored parent §10.4.1 row→block_id
   partition — the two are not the same artifact), and the
...
```
U0.MANIFEST_SUBSTRATE  (no U-block prereqs; gates every other block's row-lift via the manifest guard suite — the §10.4 coverage gate joins at U13/U15)
        │
        ▼
U2.QUERY_VALUE_DOMAIN  (typed value domain + 5 landed U2 spine keys [2 augmentation spine keys forward-planned → U2.MODULE_AUGMENTATION] + three U2-landed added keys + ProjectionDemand/EvalPolicy demand lattice + mode presets + finalized shape for U6-landed FlowReturn/ResolveCall + per-block SemanticQueryKeySpec)
        │
...
Blocked until: nothing — U0 is the first block. Every other block in this subplan (and the U6/U3/U8/U10/U11/U13/U15 blocks in the sibling subplans) is blocked until U0 is done, because U0 owns the ledger, the proof registry, the DAG guard, and the (U13/U15-gated) §10.4 coverage gate — the substrate that decides when any block's rows may lift.

Context: The live manifest at `crates/verter_session/tests/typeinfo_ignored_test_manifest.rs` is the two-table 13-column ledger (`IgnoredTestRow { file, function, substrate, capability, organ, owning_u_block, block_id, semantic_queries, proof, status, mechanism_id, consumed_mechanisms, unblocker }` + the closed 7-row `AdditionalProofRow` table) with `EXPECTED_TOTAL_IGNORED_COUNT` DERIVED as `count(IgnoredTestRow where status == Ignored)` (the TOTAL row count — 362 — is guard-pinned by `ignored_test_row_table_holds_exactly_362_rows`; the ignored/lifted split is DERIVED from `IgnoreStatus` and never frozen in this doc); the rows live in `manifest_data/typeinfo_ignored_test_manifest_rows.rs` (verified 362 rows) and the block contracts in `manifest_data/typeinfo_parity_blocks.rs`. The ledger carries the `block_id` / `status` / `proof` / `capability` / `organ` / `owning_u_block` / `semantic_queries` / `mechanism_id` / `consumed_mechanisms` columns, the `TYPEINFO_PARITY_BLOCKS` DAG (each `BlockContractRow` carrying `required_guards` + `verification_labels`), the block-DAG + bijection + per-key-mirror guards, and the §10.4.1 row→block_id partition table (the HAND-AUTHORED authoritative input the generator parses ONLY to assign each `IgnoredTestRow`'s `block_id`). LANDED state (U0-FINISH-A): the substrate extension above — that ledger + DAG + §10.4.1 partition + the `BlockContractRow` contract metadata (`required_guards` + `verification_labels`) + the `SemanticQueryName` mirror — is built on the tree, and `package.json` + `pnpm-lock.yaml` pin the `@typescript/native-preview` npm dependency to `7.0.0-dev.20260526.1`. LANDED state (U0-FINISH-B, built per the LOCKED `docs/arch/u0-oracle-harness-design.md`): the per-row executable `ProofRequirement` registry + row-test wrappers (lifted rows prove via `oracle::run_row` against the checked-in pinned-tsgo snapshots) and the TS7 oracle harness (the pinned-tsgo snapshot generator + the `oracle_core` driver) are built on the tree, with the CI tsgo prewarm and the `tsgo`-forbidden default-test guard (`oracle_tsgo_forbidden.rs::tsgo_not_reachable_from_resolver`) keeping tsgo off the resolver/default-test path. NAMED RESIDUAL (not built): the row-exact capability→mechanism→proof coverage table of PART 2 §10.4 and its checking guard `capability_rows_map_to_expected_query_fact_mechanisms` exist today only as a forward-declared required-guard string on the generated `U13.PROJECTION` / `U15.FINAL_LIFT` block-contract rows — no implementing test fn; the gate is enforced at those blocks' landing. U0 is DONE as the lift substrate (ledger + proof registry + oracle harness); the coverage gate is that one named residual. The FULL U0 block PLAN below (Deliverables / Exit / Verification) is the landed block's design record; per-row lift state derives from the generated manifest + `IgnoreStatus` (the unified remaining plan's §1.1 status authorities), never from this doc. The binding total re-derives to EXACTLY 362 with the manifest parser (the authoritative count; 356 / 371 are stale, 384 is the raw `#[ignore]` line count including macro-body lines — Capability Map preamble). The `SemanticQueryKeySpec` table generator is owned by `U2.QUERY_VALUE_DOMAIN` (which adds the keys it tabulates), NOT U0. Block LANDING is the git/CI protocol (PART 2 §§11–14): branch per block → green CI → three-reviewer LAND → squash-merge with the `Typeinfo-Block:` trailer — there is NO tracked `.cutover-state.typeinfo_parity` cursor, no namespaced xtask, and no crash-recovery substrate for U0 to build. This block exists now because no U2 (or U6) block's rows can flip `Lifted` until they are tracked in the extended ledger and gated by its guard suite (the §10.4 coverage table joins that gate at U13/U15, covering earlier-lifted rows retroactively).

Changes (exact files / functions):
- `crates/verter_session/tests/typeinfo_ignored_test_manifest.rs` — extend the manifest module IN PLACE (no second ledger file). Replace the four-field `IgnoredTestRow` with the parent's field set (PART 2 §10.1): `{ file, function, substrate: TargetSubstrate, capability: TypeInfoCapability, organ: ArchitectureOrgan, owning_u_block: UBlock, block_id: TypeInfoParityBlockId, semantic_queries: &'static [SemanticQueryName], proof: ProofRequirement, status: IgnoreStatus, unblocker }`. Add the separate coverage-only `AdditionalProofRow` (same fields minus `status`/`unblocker`, never in the count/bijection). Add `enum ProofRequirement { Ts7Oracle(OracleId), StructuralGuard(GuardId), NegativeGuard(GuardId), OracleAndGuard{oracle,guard}, RowTestGuard{file,function} }` (NO `NotTsOracleApplicable` arm) and `enum IgnoreStatus { Ignored, Lifted{block_id} }` (binary — no tracked `Verifying`/`lease_id`; the "in-flight, not-yet-landed" state of a block is simply its unmerged branch — PART 2 §10.1 / §11.1). Add `TYPEINFO_PARITY_BLOCKS: &[BlockContractRow]` (`block_id`, `owning_u_block`, `organ`, `prereqs`, `mechanism_id`, `consumed_mechanisms`, `required_guards`, `verification_labels` — there is no `subplan path` field — PART 2 §9), plus per-key `key_owning_block` metadata mapping each `SemanticQueryName` to its owning `block_id` (`ResolveCall → U6.CALL_RESOLVE`, etc.). Add the block-MECHANISM metadata (parent §11.5): a `mechanism_id: MechanismId` (the dominant producer) AND a `consumed_mechanisms: &'static [MechanismId]` list on `IgnoredTestRow`, `AdditionalProofRow`, and `BlockContractRow`, plus a `fn mechanism_owning_block(MechanismId) -> TypeInfoParityBlockId` map naming the single block that owns (produces) each mechanism. Add the guard `typeinfo_parity_block_dag_is_acyclic_and_consumed_keys_and_mechanisms_are_prereqs` (parent §11.5): it builds the block-dependency graph from `TYPEINFO_PARITY_BLOCKS` + the key-owning-block metadata + the mechanism metadata + each `IgnoredTestRow` / `AdditionalProofRow`'s `semantic_queries` and `consumed_mechanisms` lists, and FAILS on (1) a cycle in the block prerequisite graph, (2) a row whose dominant `mechanism_id` owner ≠ its own `block_id`, (3) any row or block-level `consumed_mechanism` whose `mechanism_owning_block` is not itself or a transitive prerequisite, (4) any block or row consuming a key whose owning block is not itself or a transitive prerequisite (retained keys-only check) OR a row whose `block_id` disagrees with the prerequisites implied by its consumed keys, and (5) prints the offending row/block, consumed key/mechanism, owning block, and missing-prerequisite path on failure. It catches the fi08-class deadlock (a `FlowNarrowing`-substrate row whose narrowing siblings live in `U6.NARROW_INVALIDATION`, consuming `PredicateAssertion.assertion_effect_dotted_member_path` owned by `U6.PREDICATE_ASSERTION` when that was not a prerequisite — check 2 forces the row's `block_id` to `U6.PREDICATE_ASSERTION`). Keep the existing carried-forward guards (`every_ignored_typeinfo_test_*`, `every_ignore_reason_meets_minimum_quality_bar`, the reason-equality and per-file partition guards).
- `crates/verter_session/tests/manifest_data/typeinfo_ignored_test_manifest_rows.rs` — regenerate the 362 rows with the extended schema (every row carries `capability`/`organ`/`owning_u_block`/`block_id`/`semantic_queries`/`proof`/`status: Ignored`/`mechanism_id`/`consumed_mechanisms`), re-derived by the manifest parser to exactly 362.
- New generator binaries (each a dedicated `cargo run` target, checked-in output, generated-not-hand-maintained): (1) the proof registry + typed row-test wrapper generator (PART 2 §10.3) mapping each row's `(file::function, ProofRequirement)` to its executable proof artifact and emitting the per-row wrapper invocation; (2) the U0 row-exact coverage-table generator (PART 2 §10.4) mapping every row through `capability → mechanism_id → semantic_queries/facts → ProofRequirement → block_id` (the authoritative `row → block_id` partition is parent §10.4.1). House them under the workspace generator convention (a `cargo run`-wrapped binary invoked by a `pnpm` script, mirroring the existing ts-rs/schemars generator discipline); the corresponding Rust tests only DIFF and FAIL, never write tracked source. (The `SemanticQueryKeySpec` table generator is NOT a U0 deliverable — it lands in `U2.QUERY_VALUE_DOMAIN`, which adds the keys it tabulates; U0 owns only the manifest-derived coverage-table and proof-registry tooling.) STATUS: (1) landed under U0-FINISH-B in the locked design's superseding shape — a hand-authored pure-data registry (`oracle_query_specs.rs`) whose lifted bodies call the shared `oracle_core` driver, with the `oracle_gen` `cargo run` generator producing the checked-in snapshots (no generated registry file); (2) NOT landed — the named coverage-gate residual, enforced at the `U13.PROJECTION` / `U15.FINAL_LIFT` landings (see Context).
...
- `package.json` + `pnpm-lock.yaml` — DONE (U0-FINISH-A): the `@typescript/native-preview` npm dependency is pinned to the exact oracle version `7.0.0-dev.20260526.1`; the floating `"latest"` range was removed and the lockfile resolves identically. DONE (U0-FINISH-B): the oracle row generator landed (deterministic `OracleId` from `(fixture, query, compiler_options_hash, tsgo_version, oracle_schema_version)`, checked-in normalized snapshots stamped with the pinned `oracle_core::identity::TSGO_VERSION`, feature/env-gated regeneration), the guard forbidding `tsgo` execution in runtime/default tests except the gated drift generator landed (`oracle_tsgo_forbidden.rs` — PART 1 §4.4), and CI prewarms the tsgo cache. Honest residual: the gated REGENERATION path discovers the tsgo binary from the npx cache version-agnostically (no spawn-time binary-version assert; the CI prewarm pulls `@latest`) — the pin is enforced on the CONSUMPTION path (hermetic checked-in snapshots, tsgo-forbidden default tests, version-stamped snapshot identity), not yet as a spawn-time assert on the regeneration binary.
...
- The proof registry + row-test wrappers — landed under U0-FINISH-B in the LOCKED design's shape: a hand-authored pure-data registry (`oracle_query_specs.rs`) whose lifted bodies call the shared `oracle_core` driver; the generated checked-in artifacts on that rail are the oracle snapshots (the `oracle_gen` `cargo run` generator). The U0 row-exact coverage table (PART 2 §10.4) did NOT land with U0-FINISH-B — it is the named residual, forward-declared on the `U13.PROJECTION` / `U15.FINAL_LIFT` block-contract rows (see Context). The §10.4.1 row→block_id partition is a DISTINCT, hand-authored input (already landed under U0-FINISH-A) that the future coverage-table generator consumes; it is not itself a generated artifact and must not be equated with the §10.4 coverage table. (The `SemanticQueryKeySpec` table is produced by `U2.QUERY_VALUE_DOMAIN`'s generator, not U0.)
...
- The pinned `@typescript/native-preview` npm dependency (`7.0.0-dev.20260526.1`, in `package.json` + `pnpm-lock.yaml`) — DONE (U0-FINISH-A); CI prewarm + runtime tsgo-version identity (`oracle_core::identity::TSGO_VERSION` pinned on every snapshot), the oracle row generator, and the `tsgo`-forbidden runtime guard (`oracle_tsgo_forbidden.rs`) — DONE (U0-FINISH-B): the pin is enforced on the CONSUMPTION path (hermetic checked-in snapshots, tsgo-forbidden default tests, version-stamped snapshot identity); the gated REGENERATION path's binary discovery stays version-agnostic (no spawn-time assert — the honest residual stated under Changes).
...
- The floating `"@typescript/native-preview": "latest"` range in `package.json` — DONE (removed by the U0-FINISH-A pin to `7.0.0-dev.20260526.1`).
...
Exact test rows lifted: none. U0 lifts no `#[ignore]` rows — it builds the substrate that tracks and gates the lifting of all 362 rows. Its own coverage is the META-coverage: every one of the 362 `IgnoredTestRow`s plus every `AdditionalProofRow` acquires a non-placeholder `mechanism_id`, an executable `ProofRequirement`, and a `semantic_queries`/facts mapping in the §10.4 coverage table — the U13/U15-gated unbuilt residual: this META-coverage acceptance transfers to the `U13.PROJECTION` / `U15.FINAL_LIFT` gates (`every_manifest_row_has_non_placeholder_mechanism_and_executable_proof` / `capability_rows_map_to_expected_query_fact_mechanisms`), NOT a precondition on earlier blocks' row lifts (those ride the landed §10.3 proof rail).

Required new guards (PART 2 §§10.2–10.5, 11.5, 11.8–11.12):
- Proof + coverage: `every_oracle_id_resolves_to_checked_in_snapshot`, `every_guard_or_row_proof_resolves_to_default_suite_test`, `lifted_row_executes_declared_proof`, `every_manifest_row_has_non_placeholder_mechanism_and_executable_proof`, `capability_rows_map_to_expected_query_fact_mechanisms`, `block_rows_cannot_lift_without_complete_coverage` (a branch flipping rows `Lifted` without complete coverage fails CI), `ignored_test_row_table_holds_exactly_362_rows`, `additional_proof_row_table_holds_exactly_7_rows`, `semantic_query_name_mirror_matches_live_tag_set`, `every_block_contract_row_carries_required_guards`, `typeinfo_manifest_files_are_byte_equal_to_regenerated_generator_output`, `key_owning_block_owner_mapping_is_pinned_closed_set` (the U0 `BlockContractRow.required_guards` carries the complete FINISH-A set directly: the universal DAG guard, the two count guards `ignored_test_row_table_holds_exactly_362_rows` / `additional_proof_row_table_holds_exactly_7_rows`, and the four FINISH-A structural guards `semantic_query_name_mirror_matches_live_tag_set` / `every_block_contract_row_carries_required_guards` / `typeinfo_manifest_files_are_byte_equal_to_regenerated_generator_output` / `key_owning_block_owner_mapping_is_pinned_closed_set` — the U0-FINISH-B proof/coverage guards are NOT in required_guards), the source-`#[ignore]` ↔ `Ignored`-rows ↔ `EXPECTED_TOTAL_IGNORED_COUNT` bijection/count guards, `no_landed_typeinfo_block_has_live_ignored_rows`. LANDED-NAME NOTE: the lifted-row proof/snapshot semantics of `every_oracle_id_resolves_to_checked_in_snapshot` / `lifted_row_executes_declared_proof` landed under the LOCKED design's guard names (`registry_entries_bind_to_their_snapshots_and_lifted_rows`, `registry_snapshot_fidelity_engine_is_discriminating`, `lifted_row_audit_query_mode_matches_spec`, `lifted_row_mechanism_trace_matches_manifest`, `lifted_row_trace_engine_is_discriminating` — scoped to LIFTED rows per that design); the all-row coverage pair `capability_rows_map_to_expected_query_fact_mechanisms` / `every_manifest_row_has_non_placeholder_mechanism_and_executable_proof` remains the unbuilt residual forward-declared on the `U13.PROJECTION` / `U15.FINAL_LIFT` rows; `block_rows_cannot_lift_without_complete_coverage` / `every_guard_or_row_proof_resolves_to_default_suite_test` remain unbuilt prose-only forward declarations (they land with the §10.4 coverage gate).
...
Proof requirement: U0 owns no `IgnoredTestRow`; its proof is the suite of structural guards above (each a `StructuralGuard`-class default-suite test), plus the generation-time failure of any generator whose row has no resolvable proof / placeholder mechanism. The coverage gate `block_rows_cannot_lift_without_complete_coverage` is the CI precondition later blocks' row-lifts ride on from the `U13.PROJECTION` / `U15.FINAL_LIFT` landings onward (earlier lifts ride the landed §10.3 proof rail + the manifest guard suite and are covered retroactively).
...
- The proof registry + row-test wrappers (landed under U0-FINISH-B per the LOCKED design — hand-authored pure-data registry + driver-calling lifted bodies) plus the gated oracle-row generator and its checked-in snapshots are green under the landed guard suite (`registry_entries_bind_to_their_snapshots_and_lifted_rows`, the `registry_snapshot_fidelity_*` / `lifted_row_*` guards). The §10.4 row-exact coverage table (consuming the already-landed hand-authored §10.4.1 row→block_id partition) did NOT land with U0-FINISH-B — its "every manifest row across both tables has a non-placeholder `mechanism_id` + executable `ProofRequirement` + capability-consistent `semantic_queries`/facts" acceptance transfers to the `U13.PROJECTION` / `U15.FINAL_LIFT` gates (`capability_rows_map_to_expected_query_fact_mechanisms` / `every_manifest_row_has_non_placeholder_mechanism_and_executable_proof`). (The `SemanticQueryKeySpec` table is verified in `U2.QUERY_VALUE_DOMAIN`, not here.)
...
- `package.json` + `pnpm-lock.yaml` pin the `@typescript/native-preview` npm dependency to the exact oracle version `7.0.0-dev.20260526.1` — DONE (U0-FINISH-A); CI prewarm + runtime tsgo-version identity (`oracle_core::identity::TSGO_VERSION` pinned on every snapshot), the green `tsgo`-forbidden guard, and the oracle generator producing deterministic checked-in snapshots — DONE (U0-FINISH-B): the pin is enforced on the CONSUMPTION path (hermetic checked-in snapshots, tsgo-forbidden default tests, version-stamped snapshot identity); the gated REGENERATION path's binary discovery stays version-agnostic (no spawn-time assert — the honest residual stated under Changes).
...
- The landed oracle-row generator (`oracle_gen`, feature/env-gated) producing checked-in snapshots, verified by the landed registry/wrapper guard suite. (The §10.4 row-exact coverage-table generator is NOT a live verification target — it is the named residual gated at `U13.PROJECTION` / `U15.FINAL_LIFT`; see Context. The registry is hand-authored pure data per the LOCKED design, not a generator output.)
...
Re-entry notes: U0 is idempotent — re-running re-derives 362 and regenerates the manifest files plus the oracle-row snapshots deterministically (the proof registry is hand-authored pure data per the LOCKED design; the §10.4 row-exact coverage table is the unbuilt residual gated at `U13.PROJECTION` / `U15.FINAL_LIFT`). A partial U0 leaves an unmerged branch, never a torn tracked cursor — there is nothing to reconcile (PART 2 §13); pick up the branch (or re-cut one) and continue. The manifest tells exactly which generated artifact is stale (its diff-test fails). Do not hand-edit any generated file; re-run its generator.
...
Context: Two resolver-core foundations are U0-owned ownership surfaces, distinct from the manifest/ledger substrate (`U0.MANIFEST_SUBSTRATE`) and from the U2 value-domain SHAPE + keying contract (the `U2.QUERY_VALUE_DOMAIN` block in this subplan). Both are locked by the design gate `docs/arch/u2-query-value-domain-design.md` and both live in `crates/verter_session/src/resolver_core`. They are recorded here so the U0/U2 boundary stays explicit and no U2 block silently absorbs them.
...
Context: The live `SemanticQueryKey` enum (`crates/verter_session/src/semantic_query.rs`) returns the typed `SemanticQueryValue` value domain and now carries the keys this block adds — the U2-shape keys `ResolveClassSurface` / `ApparentType` / `TemplateLiteralReduce` / `ResolveAmbientNamespace` / `ResolveEnum` / `ResolveOverloadSet` (owned by U2 blocks per `key_owning_block`: U2.CLASS_SURFACES / U2.CLASS_SURFACES / U2.MAPPED_TEMPLATE / U2.MODULE_AUGMENTATION / U2.ENUMS / U2.CLASS_SURFACES — each producer block's "SemanticQueryKey/facts touched" line in this subplan lists its key WITH ITS VALUE DOMAIN; U2.JSX_FOUNDATIONS only CONSUMES `ResolveAmbientNamespace`, and U6.CALL_RESOLVE only CONSUMES `ResolveOverloadSet`) plus the U6-OWNED keys `FlowNarrowingAt` / `ContextualTypeAt` (owned by U6.FLOW_RETURN_SUBSTRATE / U6.CONTEXTUAL_CALLBACK per `key_owning_block` — mirrored in the U0 ledger, not U2-owned) (22 live `SemanticQueryKeyTag::ALL` variants, mirrored + guarded in the U0 ledger); the remaining planned keys `ResolveMergedDeclaration` / `ResolveDeclarationAugmentation` and the U6-landed `FlowReturn` / `ResolveCall` are not yet added. The pre-U2 `Relate` was the bare `{ source, target }` shape, which was an unsound cache identity that this plan upgraded to the full relation identity (PART 1 §2.7). The parent requires (a) the typed `SemanticQueryValue` value-domain layer so flow/contextual keys return `ProgramAnalysis` values and augmentation keys return `DeclarationAnalysis` values rather than type nodes (PART 1 §3); (b) the seven-variant spine — five landed here, two forward-planned (the forward-planned seventh GENERALIZED to `ResolveDeclarationAugmentation` covering Module + Global, owned by `U2.MODULE_AUGMENTATION` (its block in this subplan)) (PART 1 §§2.1–2.2); (c) the three U2-landed added keys' SHAPES — `ResolveClassSurface`, `ApparentType`, `TemplateLiteralReduce` (PART 1 §§2.3–2.6) — with explicit R21/R6-clean per-key `*Context` structs, plus the finalized slot-identity SHAPE/model that the U6-landed `FlowReturn` / `ResolveCall` variants will reuse (their enum variants + spec rows + dispatch behavior land in U6, NOT here); (d) the `Relate` existing-key upgrade to the full relation identity + `InferenceContextKey` (PART 1 §2.7); (e) explicit contexts on every remaining variant (PART 1 §2.8); (f) the generated `SemanticQueryKeySpec` table closing the per-key class by enum/table equality (PART 1 §2.9) — a STANDING per-block invariant (table == enum on every committed tree, validated incrementally after each block) rather than a one-shot "U2 proves all five rows" gate; and (g) the `ProjectionDemand` / `EvalPolicy` **demand lattice** as the PRIMARY semantic-demand + cache-identity dimension carried on every projection / flow `demand` field, with the five mode names (`Identity` / `Navigate` / `Shallow` / `Expanded` / `Skeleton`) as public presets over it (`Skeleton` = `generic_open = TypeParamShells` + carrier-stop, not a special mode) and cache satisfaction by the LANDED materialized-point rail — a warm hit requires a RECORDED materialised `(path, point)` the compute actually produced to dominate the request at the same path (`cached_satisfies(MemoEntry.satisfied_projection, requested_point_for_key(key))`); the demand lattice is the algebra INSIDE that check, never enum order and never a nominal-demand oracle (PART 1 §2.10). This block lands the U2 key/value-domain SHAPE + the demand lattice + presets + the spec table for the keys it adds; the reducer BEHAVIOR lands in the per-reducer blocks, the published-boundary EXACTNESS gating over the landed satisfaction rail lands in U10.RESULT_DB, and the `FlowReturn` / `ResolveCall` rows + variants land in U6. It exists now because every U2 reducer dispatches through these keys, resolves into these value-domain arms, and carries a `(ProjectionDemand, EvalPolicy)` demand point.
...
- The five landed U2 spine keys (the two augmentation spine keys `ResolveMergedDeclaration` / `ResolveDeclarationAugmentation` are forward-planned, owned by `U2.MODULE_AUGMENTATION` (its block in this subplan)), the U2-landed added keys (`ResolveClassSurface`, `ApparentType`, `TemplateLiteralReduce`), the `Relate` full-identity upgrade, and every landed variant's explicit R21/R6-clean context (PART 1 §§2.1–2.8).
- The `ProjectionDemand` / `EvalPolicy` demand lattice + the lattice partial order as the PRIMARY demand + cache-identity dimension, with the five mode names as public presets over it (`Skeleton` = `generic_open = TypeParamShells` + carrier-stop) and cache satisfaction by recorded materialized points under the lattice dominance algebra (`cached_satisfies`), not enum order (PART 1 §2.10).
- The generated, checked-in `SemanticQueryKeySpec` table closing the per-key class by enum/table EQUALITY (PART 1 §2.9).

Legacy deletions:
- The former `ResolveModuleAugmentation` key slot — generalized into `ResolveDeclarationAugmentation { target: Module | Global }` (the narrow module-only key is removed, not kept alongside). FORWARD-PLANNED: neither key is live; this generalization lands with `ResolveDeclarationAugmentation` in `U2.MODULE_AUGMENTATION`, not in the foundation block.
...
- Demand-lattice definition (PART 1 §2.10): `query_modes_are_presets_over_projection_demand_eval_policy` (each of the five mode names resolves to exactly its `(ProjectionDemand, EvalPolicy)` preset; no mode name is a primary key dimension on any cache) and `skeleton_is_typeparamshells_plus_carrier_stop_not_special_mode` (the `Skeleton` preset is exactly `generic_open = TypeParamShells` + carrier-stop, with no special-cased semantic branch keyed on a `Skeleton` mode tag). Satisfaction is the LANDED materialized-point rail, pinned by the landed pair `cache_satisfaction_is_materialized_point_not_nominal_demand` + `backfill_writes_only_recorded_materialized_points` (the formerly-planned `cache_satisfaction_is_demand_lattice_not_enum_order` is RETIRED — written as planned it would reintroduce a weaker nominal-lattice rail); the published-boundary exactness residual lands in U10.RESULT_DB as `result_db_published_boundary_serves_only_recorded_materialized_points`; the dispatch's multi-candidate dominance here must keep the landed pair green.
...
Critical-rule guards: this block implements the parent's `(CRITICAL)` typed-value-domain, per-key-identity, and query-modes-as-demand-lattice-presets (PART 1 §2.10) rules; the value-domain + spec-table + per-key guards above plus the two demand-lattice definition guards (`query_modes_are_presets_over_projection_demand_eval_policy`, `skeleton_is_typeparamshells_plus_carrier_stop_not_special_mode`) ARE their R6 guards (satisfaction is pinned by the LANDED `cache_satisfaction_is_materialized_point_not_nominal_demand` / `backfill_writes_only_recorded_materialized_points` pair; the published-boundary exactness residual lands at U10.RESULT_DB as `result_db_published_boundary_serves_only_recorded_materialized_points`). Any new `(CRITICAL)` rule text added to docs in this change registers its guard here in the same change.
...
Docs updated: update the `/type-resolution` skill's Query Mode Contract / `SemanticQueryKey` surface notes for the five landed U2 spine keys (the two augmentation spine keys forward-planned) + the three U2-landed added keys (`ResolveClassSurface`, `ApparentType`, `TemplateLiteralReduce`) + the finalized slot-identity shape for the U6-landed `FlowReturn` / `ResolveCall` + the forward-planned generalized `ResolveDeclarationAugmentation` + the typed `SemanticQueryValue` value domain + the `ProjectionDemand` / `EvalPolicy` demand lattice with the five mode names as presets (satisfaction by recorded materialized points, the lattice as its algebra — not enum order) + the per-block `SemanticQueryKeySpec` table; update the `/type-cache-architecture` skill for the upgraded `Relate` identity + `InferenceContextKey` and the per-key R21/R6-clean `*Context` discipline.
...
- `crates/verter_session/tests/typeinfo_ignored_test_manifest.rs` (+ `manifest_data/`) — register the NEW `mapped_modifiers.rs::mapped_modifier_minus_optional_preserves_explicit_undefined_on_required_property` `AdditionalProofRow` (coverage-only, block_id `U2.MAPPED_TEMPLATE`, `ProofRequirement::StructuralGuard(mapped_minus_optional_preserves_explicit_undefined_on_required_property)` as its EVENTUAL proof requirement — on the current tree it is emitted as a `RowTestGuard { file, function }` forward-declaration placeholder, see the Proof-requirement note below); it is NOT one of the 362 `IgnoredTestRow`s and is excluded from the count + bijection (PART 2 §10.1).
...
- `crates/verter_protocol/proto/verter/v1/typeinfo.proto` — NO wire change. The merge/augmentation wire home is the EXISTING closed contract: `GraphTypeNode` kinds 21–25 (`GraphMergedDeclaration`, `GraphAmbientModule`, `GraphModuleAugmentation` (23), `GraphAmbientNamespace`, `GraphGlobalAugmentation` (25)) REMAIN the wire home. A proposal to relocate merge/augmentation to a distinct `DeclarationAnalysisGraph` wire message was **rejected** — the correct home already exists in the closed contract, so adding the message would be dead duplicate surface (see `/type-resolution` → Merge/augmentation WIRE domain). The `DeclarationAnalysis` value domain (below) is the in-process value-side counterpart; it is NOT a wire relocation.
...
- No wire change: the merge/augmentation wire home stays the existing `GraphTypeNode` kinds 21–25; the `DeclarationAnalysisGraph` relocation is rejected (not landed).
...
- No `GraphTypeNode` arm is retired: `module_augmentation` (23) / `global_augmentation` (25) REMAIN the wire home (the `DeclarationAnalysisGraph` relocation was rejected). The §8 exporter `TypeNode::ModuleAugmentation` DTO stays over the existing kinds 21–25, pinned by the landed `node_taxonomy_complete` taxonomy guard (which pins arms 23/25 as live). (The planned `module_augmentation_is_public_graph_state` guard was NOT landed — it does not exist in `crates/`; `node_taxonomy_complete` is the live pin.)
...
- NO wire-relocation guard: the merge/augmentation wire home stays the existing `GraphTypeNode` kinds 21–25, so the augmentation arms (23/25) remain valid `GraphTypeNode` members and are NOT required to be retired or relocated (the `DeclarationAnalysisGraph` relocation was rejected). The `GraphTypeNode`-purity guards must classify kinds 21–25 as legitimate merge/augmentation arms, not flag them for retirement.
- `session_overlay_augmenter_isolated_from_base_index` + `effective_export_set_session_view_stitches_overlay_augmenter` + `no_effective_export_set_base_only_session_assert` (the LANDED overlay-aware augmentation guards; unified-plan §0.5.1) — the augmentation index is OVERLAY-AWARE: `AugmentationTargetKey` carries `population: AugmentationPopulation {Base, Session(overlay-set fingerprint)}`; a session scan reads the session's overlay artifacts UNIONED with base under `Session` while base reads stay on `Base`; an unsaved-buffer / overlay edit that adds or removes a `declare module` / `declare global` contributor therefore gets a REAL stitched session answer; overlay augmenters never poison the base index and never cross sessions; there is NO base-only `session.is_none()` assert on this surface (composes with the overlay-results-do-not-populate-base-caches Cache-Architecture rule — the query-identity `EffectiveExportSetScope` stays content-free, overlay content identity is validated on the VALUE). These three guards SUPERSEDE the retired fail-closed deliverable `session_overlay_augmentation_fails_closed_until_implemented`: its either/or contract was discharged on the implemented arm, and a fail-closed fixture would now regress landed behavior (a session view gets a stitched answer, not `ReturnOnly`).

Critical-rule guards: implements the parent's `(CRITICAL)` declaration-augmentation / value-domain / `GraphTypeNode`-purity rules plus the declaration-merge-order rule (PART 1 §1.8); the augmentation-identity + value-domain guards above, `declaration_merge_records_binder_overload_augmentation_order_as_facts`, and the landed overlay-aware trio `session_overlay_augmenter_isolated_from_base_index` / `effective_export_set_session_view_stitches_overlay_augmenter` / `no_effective_export_set_base_only_session_assert` (unified-plan §0.5.1) are their R6 guards. There is no wire-relocation guard (the relocation was rejected); the landed `node_taxonomy_complete` taxonomy guard pins `GraphTypeNode` kinds 21–25 (including augmentation arms 23/25) as live (the planned `module_augmentation_is_public_graph_state` guard was NOT landed — it does not exist in `crates/`).
...
Exit acceptance: all 11 rows lift and pass; module + global augmentation facts resolve to the `DeclarationAnalysis` value domain (never the `TypeNode` value domain — though their wire projection still lands on the augmentation `GraphTypeNode` arms 23/25); merged declarations / ambient modules / ambient namespaces remain value-bearing object surfaces; the two `import_attribute_*` simulated-JSON `typeof import` shapes resolve through the merged/ambient surfaces; the `AugmentationTargetKey` env is derived solely from context (no public constructor can create a target/context env mismatch); the augmentation `GraphTypeNode` arms (kinds 21–25) REMAIN the wire home (the `DeclarationAnalysisGraph` relocation is rejected, not landed); a session/overlay augmentation query resolves through the overlay-aware `Session` population (overlay ∪ base) to a real stitched answer — overlay augmenters never poison the base index and never cross sessions, and no base-only `session.is_none()` assert exists on the surface (`session_overlay_augmenter_isolated_from_base_index`, `effective_export_set_session_view_stitches_overlay_augmenter`, `no_effective_export_set_base_only_session_assert`).

Verification commands:
- `cargo test --package verter_session` module-features / augmentation tests.
- `cargo test --package verter_protocol` typeinfo proto/TS freshness + taxonomy guards (the wire surface is unchanged — kinds 21–25 stay); `cargo test --package verter_session --test g_block typeinfo_graph_taxonomy` and the validation guards.
- `cargo test --package verter_session --test typeinfo_ignored_test_manifest` (manifest guard suite).
- The block's lifted-row proofs via the generated wrapper.
- Full workspace gate (as U0); `node scripts/gen-corpus-audit-tests.mjs` if audit fixtures change.

Docs updated: keep the §8 exporter `TypeNode::ModuleAugmentation` DTO and the §3 `module_augmentation = 23` / `global_augmentation = 25` type-value arms over the EXISTING `GraphTypeNode` kinds 21–25, pinned by the landed `node_taxonomy_complete` taxonomy guard (the `DeclarationAnalysisGraph` relocation is rejected, not landed; the planned `module_augmentation_is_public_graph_state` guard was NOT landed — it does not exist in `crates/`); add `SemanticQueryValue::DeclarationAnalysis` as the in-process value-domain counterpart; update the `/type-cache-architecture` skill's `augmentation_index` / `AugmentationTargetKey` notes and the `/type-resolution` merged/ambient-surface notes for the generalized `ResolveDeclarationAugmentation`.

Re-entry notes: idempotent. There is NO wire change in this block (the `DeclarationAnalysisGraph` relocation was rejected; the merge/augmentation wire home stays the existing `GraphTypeNode` kinds 21–25), so no schema-version bump and no TS-binding regeneration are required for the wire surface. If partial, the manifest shows which `module_features` rows remain `#[ignore]`.
...
Proof requirement: per-row — the nine `IgnoredTestRow`s are TS7-oracle-pinned (`Ts7Oracle`) for the resolved JSX shapes; the six `AdditionalProofRow`s are `OracleAndGuard` (oracle + the named existing-query JSX guard) as their EVENTUAL proof requirement — on the current tree these six (with the seventh, mapped, companion) are all emitted as `ProofRequirement::RowTestGuard { file, function }` forward-declaration placeholders (built + enforced by the manifest test); U0-FINISH-B's proof registry assigns the executable kinds. Consumed by each row's / additional-row's generated wrapper.
...
- `cargo test --package verter_session --test typeinfo_ignored_test_manifest` (manifest guard suite + the `AdditionalProofRow` non-participation in the count/bijection).
...
In every case the parent's §10.4.1 partition (the HAND-AUTHORED authoritative
row→`block_id` map the generator parses to assign each row's `block_id`; distinct from
the PART 2 §10.4 row-exact coverage table — the named residual that survived
U0-FINISH-B's landing, not yet built) is the authority: each row
maps to exactly one `block_id` via its `mechanism_id`, and
`capability_rows_map_to_expected_query_fact_mechanisms` (forward-declared on the
generated `U13.PROJECTION` / `U15.FINAL_LIFT` block-contract rows, not yet
implemented) is intended to assert the mapping is consistent with the capability. This
subplan's U2 blocks own only the
