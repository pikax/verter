...
- No `GraphTypeNode` arm is retired: `module_augmentation` (23) / `global_augmentation` (25) REMAIN the wire home (the `DeclarationAnalysisGraph` relocation was rejected). The §8 exporter `TypeNode::ModuleAugmentation` DTO and the `module_augmentation_is_public_graph_state` guard stay over the existing kinds 21–25.
...
Docs updated: keep the §8 exporter `TypeNode::ModuleAugmentation` DTO, the §3 `module_augmentation = 23` / `global_augmentation = 25` type-value arms, and the `module_augmentation_is_public_graph_state` guard over the EXISTING `GraphTypeNode` kinds 21–25 (the `DeclarationAnalysisGraph` relocation is rejected, not landed); add `SemanticQueryValue::DeclarationAnalysis` as the in-process value-domain counterpart; update the `/type-cache-architecture` skill's `augmentation_index` / `AugmentationTargetKey` notes and the `/type-resolution` merged/ambient-surface notes for the generalized `ResolveDeclarationAugmentation`.
