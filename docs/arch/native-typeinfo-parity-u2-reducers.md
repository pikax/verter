...
Context: The live manifest at `crates/verter_session/tests/typeinfo_ignored_test_manifest.rs` is the two-table 13-column ledger (`IgnoredTestRow { file, function, substrate, capability, organ, owning_u_block, block_id, semantic_queries, proof, status, mechanism_id, consumed_mechanisms, unblocker }` + the closed 7-row `AdditionalProofRow` table) with `EXPECTED_TOTAL_IGNORED_COUNT` DERIVED as `count(IgnoredTestRow where status == Ignored)` (= 362); the rows live in `manifest_data/typeinfo_ignored_test_manifest_rows.rs` (verified 362 rows) and the block contracts in `manifest_data/typeinfo_parity_blocks.rs`. The ledger carries the `block_id` / `status` / `proof` / `capability` / `organ` / `owning_u_block` / `semantic_queries` / `mechanism_id` / `consumed_mechanisms` columns, the `TYPEINFO_PARITY_BLOCKS` DAG (each `BlockContractRow` carrying `required_guards` + `verification_labels`), the block-DAG + bijection + per-key-mirror guards, and the §10.4.1 row→block_id partition table (the HAND-AUTHORED authoritative input the generator parses ONLY to assign each `IgnoredTestRow`'s `block_id`). LANDED state (U0-FINISH-A): the substrate extension above — that ledger + DAG + §10.4.1 partition + the `BlockContractRow` contract metadata (`required_guards` + `verification_labels`) + the `SemanticQueryName` mirror — is built on the tree, and `package.json` + `pnpm-lock.yaml` pin the `@typescript/native-preview` npm dependency to `7.0.0-dev.20260526.1` (the npm dependency pin only — DONE (U0-FINISH-A); CI prewarm + runtime tsgo-binary-version enforcement and the `tsgo`-forbidden / default-test guard remain U0-FINISH-B, so the oracle execution path does not yet honor the pin). REMAINING (U0-FINISH-B): the per-row executable `ProofRequirement` registry + row-test wrapper, the row-exact capability→mechanism→proof coverage GATE that DEFINES completeness (PART 2 §10.4), and the TS7 oracle harness — those three are NOT yet built on this tree; they are deferred to the oracle/proof gate. The FULL U0 block PLAN below (Deliverables / Exit / Verification) describes U0's overall scope across U0-FINISH-A + U0-FINISH-B — it is the block design, not a claim that every listed artifact is already landed. As that full plan, U0 builds all of it as one clean cutover and re-derives the binding total to EXACTLY 362 with the manifest parser (the authoritative count; 356 / 371 are stale, 384 is the raw `#[ignore]` line count including macro-body lines — Capability Map preamble). U0-FINISH-A has pinned the `@typescript/native-preview` npm dependency to `7.0.0-dev.20260526.1` (the npm dependency pin only; CI prewarm + runtime tsgo-binary-version enforcement and the `tsgo`-forbidden / default-test guard remain U0-FINISH-B — the oracle execution path does not yet honor the pin); U0-FINISH-B will land the two U0 generators (the proof registry + row-test wrapper, and the coverage GATE; the §10.4.1 partition table they consume is already built under U0-FINISH-A). The `SemanticQueryKeySpec` table generator is owned by `U2.QUERY_VALUE_DOMAIN` (which adds the keys it tabulates), NOT U0. Block LANDING is the git/CI protocol (PART 2 §§11–14): branch per block → green CI → three-reviewer LAND → squash-merge with the `Typeinfo-Block:` trailer — there is NO tracked `.cutover-state.typeinfo_parity` cursor, no namespaced xtask, and no crash-recovery substrate for U0 to build. This block exists now because no U2 (or U6) block's rows can flip `Lifted` until they are tracked in the extended ledger and gated by the coverage table.
...
- `package.json` + `pnpm-lock.yaml` — DONE (U0-FINISH-A): the `@typescript/native-preview` npm dependency is pinned to the exact oracle version `7.0.0-dev.20260526.1`; the floating `"latest"` range was removed and the lockfile resolves identically. REMAINING (U0-FINISH-B): CI prewarm + runtime tsgo-binary-version enforcement so the oracle execution path honors the pin (today CI still prewarms `@latest` and the runtime selects the newest npx-cache binary version-agnostically), land the oracle row generator (deterministic `OracleId` from `(fixture, query, compiler_options_hash, tsgo_version, oracle_schema_version)`, checked-in normalized snapshots, feature/env-gated regeneration), and the guard forbidding `tsgo` execution in runtime/default tests except the gated drift generator (PART 1 §4.4).
...
- The pinned `@typescript/native-preview` npm dependency (`7.0.0-dev.20260526.1`, in `package.json` + `pnpm-lock.yaml`) — DONE (U0-FINISH-A); CI prewarm + runtime tsgo-binary-version enforcement, the oracle row generator, and the `tsgo`-forbidden runtime guard — REMAINING (U0-FINISH-B) (the oracle execution path does not yet honor the pin).
...
- `package.json` + `pnpm-lock.yaml` pin the `@typescript/native-preview` npm dependency to the exact oracle version `7.0.0-dev.20260526.1` — DONE (U0-FINISH-A); CI prewarm + runtime tsgo-binary-version enforcement, the green `tsgo`-forbidden guard, and the oracle generator producing deterministic checked-in snapshots — REMAINING (U0-FINISH-B) (the oracle execution path does not yet honor the pin).
...
- The five landed U2 spine keys (the two augmentation spine keys `ResolveMergedDeclaration` / `ResolveDeclarationAugmentation` are forward-planned, owned by `U2.MODULE_AUGMENTATION` §746), the U2-landed added keys (`ResolveClassSurface`, `ApparentType`, `TemplateLiteralReduce`), the `Relate` full-identity upgrade, and every landed variant's explicit R21/R6-clean context (PART 1 §§2.1–2.8).
...
the not-yet-built U0-FINISH-B coverage table of PART 2 §10.4) is the authority: each row
maps to exactly one `block_id` via its `mechanism_id`, and
`capability_rows_map_to_expected_query_fact_mechanisms` (a U0-FINISH-B guard, not yet
