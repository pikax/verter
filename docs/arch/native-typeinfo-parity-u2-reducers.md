...
- The five landed U2 spine keys (the two augmentation spine keys `ResolveMergedDeclaration` / `ResolveDeclarationAugmentation` are forward-planned, owned by `U2.MODULE_AUGMENTATION` §746), the U2-landed added keys (`ResolveClassSurface`, `ApparentType`, `TemplateLiteralReduce`, the U2 overload key), the `Relate` full-identity upgrade, and every landed variant's explicit R21/R6-clean context (PART 1 §§2.1–2.8).
