...
1. **U0 foundations** — the manifest re-derivation to exactly 363
   ...
   (the parent §10.4.1 partition), the `.cutover-state.typeinfo_parity`
   namespace, and the crash-safe two-phase cutover/ledger transaction substrate.
...
```
U0.MANIFEST_SUBSTRATE  (no U-block prereqs; gates every other block's Verifying)
        │
        ▼
U2.QUERY_VALUE_DOMAIN  (typed value domain + seven U2 keys + three U2-landed added keys + finalized shape for U6-landed FlowReturn/ResolveCall + per-block SemanticQueryKeySpec)
        │
...
- `crates/verter_session/tests/g_misc3/cutover_state_arch_guard.rs` — extend with the `.cutover-state.typeinfo_parity` namespace guards (PART 2 §§11.8, 11.11, 11.12, 13.4): namespace isolation, locked+CAS writes, parallel-token preservation, stale-lease rejection, journal reconciliation, two-phase locked-transaction shape, the input-hash-PAIR gate receipt, the four-part done predicate, the no-vacuous-parent rule, the one-land-commit history guard `typeinfo_block_lands_as_single_squashed_commit` (PART 2 §11.11), and the three-reviewer LAND gate `typeinfo_block_accept_requires_review_land_verdict` as the SECOND `accept` precondition (PART 2 §11.12). The byte-level lock / atomic-rename / CAS / WAL-journal / receipt implementation is realized in this block under those named guards (PART 2 §11.8).
- `xtask` cutover-state surface — add the namespaced `xtask cutover-state typeinfo {dispatch,heartbeat,adopt,prepare-verify,gate-receipt,review-receipt,accept,abort}` subcommands (PART 2 §§11.12, 13.2), reading/writing ONLY the `[typeinfo_parity]` section (the CAS `revision` counter / `active_blocks` map / `landed_blocks` / `gate_receipts` / `review_receipts` / `land_records` / `pending_transactions`) plus the block's manifest row status + expected count; every write re-serializes the ENTIRE file so the legacy top-level namespace is preserved byte-faithfully (PART 2 §13.3). `review-receipt` persists the durable three-reviewer LAND / NITs-only receipt (1 Claude Code + 2 codex) bound to the gated input-hash pair, the SECOND `accept` precondition alongside `gate-receipt`. `accept` PERSISTS the `land_records` entry (trailer fields copied from the gate + review receipts, plus the content-only `post_accept_lifted_tree_hash` — the canonical projected content hash over the post-accept tracked tree MINUS the `.cutover-state*` cursor family, PART 2 §11.11) BEFORE the final done-edge (the `landed_blocks` append + lease release), so the single squashed land commit is content/identity bound to the accepted authored state (PART 2 §§11.11, 13.2). The legacy `land` / `dispatch` commands are untouched.
...
Deliverables:
- The two-table ledger (`IgnoredTestRow` × 363 + `AdditionalProofRow`) extended in place, with `EXPECTED_TOTAL_IGNORED_COUNT` defined as the live `count(IgnoredTestRow where status == Ignored)` (never a frozen constant — PART 2 §10.5).
...
- The `.cutover-state.typeinfo_parity` namespace, the two-namespace TOML schema (PART 2 §13.1), the namespaced two-phase xtask (including the `review-receipt` subcommand and the `review_receipts` state it persists, and the `land_records` state `accept` persists before the done-edge — its trailer fields copied from the gate + review receipts plus the content-only `post_accept_lifted_tree_hash` (the canonical projected content hash over the post-accept tracked tree MINUS the `.cutover-state*` cursor family) — PART 2 §§11.11, 11.12, 13.2), the crash-safe journaled transaction (snapshot isolation + WAL reconcile-on-read + input-hash-PAIR gate receipt + the three-reviewer review-LAND receipt + four-part done predicate), the one-land-commit history guard `typeinfo_block_lands_as_single_squashed_commit` (PART 2 §11.11) and the three-reviewer LAND gate `typeinfo_block_accept_requires_review_land_verdict` (PART 2 §11.12), and the legacy-deletion lifecycle (PART 2 §13.3).
...
- The frozen `const EXPECTED_TOTAL_IGNORED_COUNT: usize = 363` as a hand-maintained constant decoupled from row state — replaced by the derived live `count(status == Ignored)` (the literal `363` survives only as the U0 binding-total assertion, not as a lagging constant).
...
SemanticQueryKey/facts touched: none directly (U0 is the ledger/coverage/cutover substrate). U0 neither adds, changes, nor tabulates a `SemanticQueryKey` — the `SemanticQueryKeySpec` table generator and the U2 key changes both land in `U2.QUERY_VALUE_DOMAIN`. U0's coverage-table generator reads only the manifest (not the key enum).

Exact test rows lifted: none. U0 lifts no `#[ignore]` rows — it builds the substrate that tracks and gates the lifting of all 363 rows. Its own coverage is the META-coverage: every one of the 363 `IgnoredTestRow`s plus every `AdditionalProofRow` acquires a non-placeholder `mechanism_id`, an executable `ProofRequirement`, and a `semantic_queries`/facts mapping in the generated coverage table (PART 2 §10.4) before any block verifies.

Required new guards (PART 2 §§10.2–10.5, 11.8, 13.4):
- Proof + coverage: `every_oracle_id_resolves_to_checked_in_snapshot`, `every_guard_or_row_proof_resolves_to_default_suite_test`, `lifted_row_executes_declared_proof`, `every_manifest_row_has_non_placeholder_mechanism_and_executable_proof`, `capability_rows_map_to_expected_query_fact_mechanisms`, `ignored_test_row_table_holds_exactly_363_rows`, the source-`#[ignore]` ↔ `Ignored`-rows ↔ `EXPECTED_TOTAL_IGNORED_COUNT` bijection/count guards, `no_landed_typeinfo_block_has_live_ignored_rows`.
- Cutover / ledger transaction + namespace / parallel-safety: `typeinfo_state_snapshots_are_locked_and_precondition_checked`, `pending_typeinfo_transactions_reconciled_before_eligibility`, `block_cannot_enter_verifying_without_complete_coverage`, `typeinfo_block_prereqs_derive_from_manifest_status`, `typeinfo_block_prereqs_ignore_verifying_blocks`, `verifying_typeinfo_block_lease_blocks_dependents`, `workspace_gate_passes_before_typeinfo_block_acceptance`, `landed_typeinfo_blocks_have_required_guards_and_workspace_gate`, `cutover_state_landed_blocks_match_typeinfo_manifest`, `no_vacuous_parent_u_block_landing`, `zero_row_blocks_land_exactly_once`, `typeinfo_parity_block_dag_is_acyclic_and_consumed_keys_and_mechanisms_are_prereqs` (builds the block-dependency graph from `TYPEINFO_PARITY_BLOCKS` + the key-owning-block metadata + the block-mechanism metadata (`mechanism_id` + `consumed_mechanisms` on `IgnoredTestRow` / `AdditionalProofRow` / `BlockContractRow` + the `mechanism_owning_block` map) + each row's `semantic_queries` and `consumed_mechanisms` lists, and FAILS on (1) a cycle in the block prerequisite graph, (2) a row whose dominant `mechanism_id` owner ≠ its own `block_id`, (3) any row or block-level `consumed_mechanism` whose owning block is not itself or a transitive prerequisite, (4) any block or row consuming a key whose owning block is not itself or a transitive prerequisite OR a row whose `block_id` disagrees with its consumed-key prerequisites, and (5) prints the offending row/block + consumed key/mechanism + owning block + missing-prerequisite path — the guard that pins the `ResolveCall` U6-ownership so the preceding U2.CLASS_SURFACES / U2.JSX_FOUNDATIONS blocks consume it neither directly nor through an owned row, AND catches the fi08-class mechanism deadlock — parent §11.8), `typeinfo_block_lands_as_single_squashed_commit` (the WIP series squashes to one mainline land commit at `accept` — PART 2 §11.11), `typeinfo_block_accept_requires_review_land_verdict` (`accept` requires the bound three-reviewer LAND / NITs-only receipt — 1 Claude Code + 2 codex — PART 2 §11.12), `cutover_state_typeinfo_writes_are_locked_and_cas`, `parallel_typeinfo_block_landing_preserves_all_tokens`, `resume_rejects_stale_typeinfo_block_lease`, `cutover_state_typeinfo_namespace_isolated_from_legacy_cutover_tokens`, `legacy_cutover_completion_preserves_typeinfo_namespace_when_active`.
- The `tsgo`-execution-forbidden runtime/default-test guard (PART 1 §4.3).

Critical-rule guards: U0 introduces the ledger/cutover-substrate machinery but no NEW `(CRITICAL)` engine rule of its own — its guards (above) ARE the named guards the parent's PART 2 sections register. If U0 lands any new `(CRITICAL)` rule text in `CLAUDE.md`/skills (e.g. a "manifest is authoritative / 363 binding" rule), it registers the corresponding count/bijection guard in the same change per R6.

Proof requirement: U0 owns no `IgnoredTestRow`; its proof is the suite of structural guards above (each a `StructuralGuard`-class default-suite test), plus the generation-time failure of any generator whose row has no resolvable proof / placeholder mechanism. The coverage gate `block_cannot_enter_verifying_without_complete_coverage` is the precondition every later block's proof rides on.

Exit acceptance:
- The manifest holds EXACTLY 363 `IgnoredTestRow`s (count + bijection green), disjoint from `AdditionalProofRow`s, with `EXPECTED_TOTAL_IGNORED_COUNT == count(status == Ignored)`.
...
- The `.cutover-state.typeinfo_parity` namespace, xtask, and all transaction-contract + namespace guards are green; the legacy top-level namespace is preserved byte-faithfully across every write; a simulated mid-transaction crash is recoverable-or-rejected (WAL reconcile guards green).
...
- `cargo test --package verter_session --test typeinfo_ignored_test_manifest` (count/bijection/proof/coverage guards).
- `cargo test --package verter_session --test g_misc3 cutover_state_arch_guard` (namespace / transaction-contract guards — the `cutover_state_arch_guard` module is included via the `g_misc3` umbrella harness, so the target is `g_misc3` and `cutover_state_arch_guard` is the test-name filter).
...
Docs updated: keep the `/testing` skill's unignore-manifest + ledger/cutover notes current (the two-table ledger, `ProofRequirement`, the §10.4.1 coverage table, the `.cutover-state.typeinfo_parity` namespace, the two-phase prepare-verify → gate → accept lifecycle); record the oracle-version pin and the generated-artifact discipline (generators are `cargo run` targets, tests only diff) in `/build-and-profiling`.

Re-entry notes: U0 is idempotent — re-running re-derives 363 and regenerates its two artifacts (proof registry + row-test wrapper, and the §10.4.1 coverage table) plus the oracle-row snapshots deterministically. If a partial U0 left the cutover-state namespace half-written, the WAL reconcile-on-read path (PART 2 §11.6) rolls it back on the next under-lock observation. The manifest tells exactly which generated artifact is stale (its diff-test fails). Do not hand-edit any generated file; re-run its generator.
...
Critical-rule guards: this block implements the parent's `(CRITICAL)` typed-value-domain and per-key-identity rules; the value-domain + spec-table + per-key guards above ARE their R6 guards. Any new `(CRITICAL)` rule text added to docs in this change registers its guard here in the same change.
...
Deliverables:
- `Relate` as the sole assignability authority producing `RelationPayload`, under the full upgraded key, with relation-owned `InferBind` and the grown `InferTargetPattern` set.
- The coinductive-SCC discharge protocol with `CoinductiveCycle { keys }` proofs off the type-values surface.
- Conditional reduction consuming relation bindings (no parallel matcher).
...
- Any parallel inference matcher inside conditional reduction (folded into relation-owned `InferBind`).
...
Required new guards (parent §4.1, §6):
...
Docs updated: update the `/type-resolution` skill's relation / `InferBind` notes (the coinductive-SCC discharge protocol, the grown `InferTargetPattern` set, conditional reduction consuming relation bindings); keep the `/type-cache-architecture` `RelationBudget` full-identity-keyed non-admission notes current.
...
- `crates/verter_session/tests/typeinfo_ignored_test_manifest.rs` (+ `manifest_data/`) — register the NEW `mapped_modifiers.rs::mapped_modifier_minus_optional_preserves_explicit_undefined_on_required_property` `AdditionalProofRow` (coverage-only, block_id `U2.MAPPED_TEMPLATE`, `ProofRequirement::StructuralGuard(mapped_minus_optional_preserves_explicit_undefined_on_required_property)`); it is NOT one of the 363 `IgnoredTestRow`s and is excluded from the count + bijection (PART 2 §10.1).
...
Required new guards (parent §4.2):
...
Critical-rule guards: implements the parent's `(CRITICAL)` mapped-`-?` two-component rule; the two `mapped_minus_optional_*` guards are its R6 guards. The stripping fixture MUST use OPTIONAL properties (`a?: string; b?: number`) so it actually exercises optional-origin removal (parent §4.2).

Proof requirement: per-row — all mapped / template rows are TS7-oracle-pinned (`Ts7Oracle`); the `-?` row (`mapped_modifier_minus_optional_strips_optional_and_undefined`) is `OracleAndGuard` pairing the oracle with `mapped_minus_optional_strips_only_optional_origin_undefined`. The companion required-`| undefined`-preservation contract is a NEW `AdditionalProofRow` — none of the 363 `IgnoredTestRow`s covers it — registered as `mapped_modifiers.rs::mapped_modifier_minus_optional_preserves_explicit_undefined_on_required_property` (block_id `U2.MAPPED_TEMPLATE`, `ProofRequirement::StructuralGuard(mapped_minus_optional_preserves_explicit_undefined_on_required_property)`), coverage-only and excluded from the 363 count + bijection (PART 2 §10.1). It uses a REQUIRED `{ a: string | undefined }` property so it exercises explicit-`| undefined` preservation (distinct from the optional-origin stripping the ignored `-?` row exercises). Each row's / additional-row's declared proof is consumed by its generated wrapper. (See the JSX submatrix in `U2.JSX_FOUNDATIONS` for the other six `AdditionalProofRow`s; these seven are the complete `AdditionalProofRow` set.)
...
Required new guards (PART 1 §§2.2, 3):
...
Critical-rule guards: implements the parent's `(CRITICAL)` declaration-augmentation / value-domain / `GraphTypeNode`-purity rules; the augmentation-identity + value-domain + wire-relocation guards above are their R6 guards. The recovered-doc augmentation-placement amendment and the `module_augmentation_is_public_graph_state` retirement land in this change.
...
Additional coverage rows (`AdditionalProofRow`s — coverage-only, NOT in the 363 count/bijection — parent §8): the six no-new-key submatrix rows (`JSX.LibraryManagedAttributes<C,P>`, `JSX.ElementAttributesProperty`, `JSX.ElementChildrenAttribute`, `JSX.IntrinsicAttributes` / `IntrinsicClassAttributes<C>`, the class-component `JSX.ElementClass` check, `jsxImportSource` module-namespace resolution), each with its named guard and `ProofRequirement`.
...
Exit acceptance: all 9 `jsx.rs` rows lift and pass; the six submatrix `AdditionalProofRow`s pass; JSX resolves entirely through existing keys (no JSX key, no JSX `GraphTypeNode`) — all nine JSX guards green; the binding 363 `IgnoredTestRow` total is unchanged (the submatrix rows are coverage-only).
...
`row → block_id` partition over all 363 rows is §10.4.1 of the parent (the per-block
...
- **`TypeParameterFeatures` (17)** — the two const-type-param rows
  ...
  **U6.PREDICATE_ASSERTION**. The eleven flow-narrowing-of-generic rows
  ...
  access) lift in **U6.NARROWING**. §10.4.1 assigns each `substitution_types_*` /
...
- **`ApparentTypes` (20)** — the `apparent_types_ap01`–`ap15` (15) and `branded_*`
  ...
  **U6.NARROWING** — see the `TypeParameterFeatures` bullet.)
...
- **`ModernTsFeatures` (6)** — split by mechanism:
  ...
  (`satisfies` target-contextual validation + array-literal widening — parent §4.3);
...
the binding 363 total stays exact.
...
`pnpm install --frozen-lockfile`. A block reaches `Lifted` / `landed_blocks` only
after a green workspace gate over the EXACT accepted content AND the three-reviewer
LAND verdict (1 Claude Code + 2 codex; PART 2 §11.12), via the two-phase
prepare-verify → gate → accept lifecycle with the input-hash-PAIR gate receipt
(PART 2 §11.3); the block's WIP series squashes to ONE mainline land commit at
`accept` (PART 2 §11.11). The parent U2 token is the aggregate over every block above and is
done only when every row in the union of all U2-block row-sets is `Lifted` (PART 2
§11.9) — never vacuously. Downstream U3 / U8 / U10 / U11 / U13 stay blocked until
the whole U2 parent is done.
...
machinery; the U2 blocks lift their exact manifest rows through it.
