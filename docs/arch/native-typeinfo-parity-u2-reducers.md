...
- `package.json` — DONE (U0-FINISH-A): `"@typescript/native-preview"` is pinned to the exact oracle version `7.0.0-dev.20260526.1`; the floating `"latest"` range was removed and `pnpm-lock.yaml` resolves identically. REMAINING (U0-FINISH-B): land the oracle row generator (deterministic `OracleId` from `(fixture, query, compiler_options_hash, tsgo_version, oracle_schema_version)`, checked-in normalized snapshots, feature/env-gated regeneration) and the guard forbidding `tsgo` execution in runtime/default tests except the gated drift generator (PART 1 §4.4).
...
- The pinned oracle toolchain (`7.0.0-dev.20260526.1`) — DONE (U0-FINISH-A); the oracle row generator and the `tsgo`-forbidden runtime guard — REMAINING (U0-FINISH-B).
...
- `package.json` pins the exact oracle version — DONE (U0-FINISH-A); the `tsgo`-forbidden guard is green and the oracle generator produces deterministic checked-in snapshots — REMAINING (U0-FINISH-B).
