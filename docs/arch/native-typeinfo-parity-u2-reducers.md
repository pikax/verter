...
- Proof + coverage: `every_oracle_id_resolves_to_checked_in_snapshot`, `every_guard_or_row_proof_resolves_to_default_suite_test`, `lifted_row_executes_declared_proof`, `every_manifest_row_has_non_placeholder_mechanism_and_executable_proof`, `capability_rows_map_to_expected_query_fact_mechanisms`, `block_rows_cannot_lift_without_complete_coverage` (a branch flipping rows `Lifted` without complete coverage fails CI), `ignored_test_row_table_holds_exactly_362_rows`, `additional_proof_row_table_holds_exactly_7_rows`, `semantic_query_name_mirror_matches_live_tag_set` (the U0 `BlockContractRow.required_guards` carries these three schema guards directly), the source-`#[ignore]` ↔ `Ignored`-rows ↔ `EXPECTED_TOTAL_IGNORED_COUNT` bijection/count guards, `no_landed_typeinfo_block_has_live_ignored_rows`.
