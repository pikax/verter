...
5. **17-family `OracleId` enum** (`typeinfo_ignored_test_manifest.rs` `OracleId`):
   ...
   verified by `registry_entry_count_matches_declared` — is a SHIPPED (v3) 14th
   `IgnoredTestRow` field (generator-emitted: the registry-entry count for a `Lifted`
   row, `0` for every other row); the `migration_fingerprint` / `original_body_tokens`
   migration-fidelity provenance is retained SEPARATELY in `LIFTED_ROW_MIGRATIONS`.
   `status` is the separate lifecycle field the regenerator substitutes on lift.)
...
| `snapshot_id` | string | The deterministic id from Q4 (see derivation). Must equal the file stem. Derived from REGISTRY-ONLY, tsgo-free inputs (row-ref + query-helper payload + `host_project` + pinned env/algorithm versions, including the STABLE `env_corpus_id` pinned-env constant) — `oracle_env_hash` is NOT an input, so a coverage guard can compute the expected filename set from the registry ALONE. The full ≥256-bit BLAKE3 digest (not a 12-byte truncation). INCLUDES the row-ref. `oracle_schema_version` enters this id through the pinned env, so the v2→v3 bump changed every `snapshot_id` (hence every checked-in filename). |
| `migration_fingerprint_version` | integer (v3, REQUIRED) | The migration-fingerprint ALGORITHM version (`MIGRATION_FINGERPRINT_VERSION`, currently `1`) — bumped with the `verter.oracle.migration_fingerprint.vN` domain tag whenever the fidelity-tuple field set / shape changes. A MIRROR of the row's retained-lift metadata; NOT a `snapshot_id` input. |
| `migration_fingerprint` | string (`blake3:…`, v3, REQUIRED) | The MIRROR of the row's retained `migration_fingerprint` (§Q4) — computed ONCE at lift time by the closed `syn` extractor over the ORIGINAL pre-`#[oracle_row]` body (plus the captured-from-context `workspace_files`), recorded in the retained-lift metadata (`LIFTED_ROW_MIGRATIONS`), and copied here so each checked-in snapshot is bound to its row's migration authority. The fidelity tuple it hashes is: the per-query QUERY-IDENTITY (`helper_kind`, `primary_canonical`, `symbol_or_expression`, `type_arguments`, `projection_mode`, `host_setup_kind`, and the typed `source_locator` `{reference_canonical, reference_name, symbol_space}`) ∪ the row's `workspace_files` (each `{path, content_hash}`, sorted by path, the `sha256` `identity::content_hash` recipe) ∪ row identity ∪ declared query count ∪ the proof-shape discriminant. `source_locator` is body-derivable (the helper fixes its `symbol_space`); `workspace_files` is NOT body-token-extractable (the source routes through file-local consts / `upsert*` wrappers), so it is CAPTURED from the lift/registry context and RETAINED in `LiftMigrationProvenance.workspace_files` — the hermetic `original_extraction_input_auditable` re-derives the fingerprint from `original_body_tokens` + that retained axis alone. NOT a `snapshot_id` input. `snapshot_migration_fingerprint_matches_retained_lift_metadata` asserts this mirror equals the retained metadata; `snapshot_workspace_files_match_retained_provenance` pins the retained `workspace_files` to this snapshot's generator-written `identity.workspace_files`. |
...
- **Path-separator + leading-slash rule.** Per-row `workspace_files` /
  ...
  rooted at `oracle_env/<env_corpus_dir>/`). All path separators are normalized to `/`
...
- **On-disk corpus dir name (path boundary only).** The tracked corpus directory's
  name is the env corpus id with `:` → `-` applied at the path boundary via
  `oracle_core::identity::env_corpus_dir_name()` (`:` is NTFS-illegal in a path
  component); the LOGICAL `blake3:`/`sha256:` encoding is unchanged and still pinned —
  every id in snapshot JSON, `CURRENT_ENV_CORPUS_ID`, and `snapshot_id` derivation
  keeps the `:` separator. This doc writes the on-disk corpus root as
  `oracle_env/<env_corpus_dir>/`, with `<env_corpus_dir>` =
  `env_corpus_dir_name(env_corpus_id)`.
...
```json
{
  ...
  "migration_fingerprint": "blake3:80f71fbad19d…",
  "row_ref": {
...
- **The CANONICAL oracle tsconfig + VENDORED corpus root (the standalone-host
  ...
  `crates/verter_session/src/typeinfo/typeinfo_tests/oracle_env/<env_corpus_dir>/`
...
- **`compiler_options_hash` — a CLOSED, ENUMERATED recipe over the EFFECTIVE config,
  ...
  (`oracle_env/<env_corpus_dir>/oracle.tsconfig.json`, listed in `oracle_env_files` and
...
- **Corpus COMPLETENESS vs corpus STABILITY — two distinct properties, two distinct
  ...
  **COMPLETENESS**: that EVERY file tsgo actually consulted to produce the hover was
  ...
  - **The generator drives tsgo against ONLY the frozen vendored corpus root — there is
    ...
    `workspaceFolders` / module-resolution root at `oracle_env/<env_corpus_dir>/` and runs
  ...
  - **The CONCRETE vendored-lib mechanism (the named candidate, not an open question).**
    ...
    `oracle_env/<env_corpus_dir>/lib/` and referenced by a triple-slash
...
2. **The oracle-query-spec registry is a closed Rust table of PURE context-neutral
   ...
   **`oracle_query_ordinals: u16`** (the number of oracle queries the row declares it
   ...
   value are produced by `scripts/gen-typeinfo-ignore-manifest.mjs`
   ...
   **The count + contiguity verify CARDINALITY, not registry PAYLOAD correctness — the
   ...
   field. (SHIPPED in schema v3 as the `verter.oracle.migration_fingerprint.v1`
   layer — the closed `syn` extractor `oracle_migration_extract`, the retained
   `LIFTED_ROW_MIGRATIONS` metadata, and the v3 snapshot mirror. **v1 SCOPE:** the
   fingerprint hashes the per-query QUERY-IDENTITY axes — `helper_kind`,
   `primary_canonical` (path-const folded at capture), `symbol_or_expression`,
   `type_arguments`, `projection_mode`, `host_setup_kind`, and the typed
   `source_locator` `{reference_canonical, reference_name, symbol_space}` — ∪ the
   row's `workspace_files` (each `{path, content_hash}`, sorted by path) ∪ row
   identity ∪ declared query count ∪ the proof-shape discriminant. `source_locator`
   is body-derivable (the helper fixes its `symbol_space`); `workspace_files` is the
   one axis NOT body-token-extractable (the source routes through file-local consts
   / `upsert*` wrappers), so it is CAPTURED from the lift/registry context and
   RETAINED in `LiftMigrationProvenance.workspace_files`, then folded into the
   fingerprint and re-checked hermetically by `original_extraction_input_auditable`.
   GENUINELY DEFERRED to a future `.vN` / `MIGRATION_FINGERPRINT_VERSION` bump:
   non-empty `type_arguments` (the parameterized-printer spike) and proof-OBLIGATION
   modeling beyond the `Ts7Oracle` discriminant.) The fingerprint's full reach is
   EVERY value-affecting input AND the row's proof shape (`Ts7Oracle` vs
   `OracleAndGuard` + its `guard` id) the lift must preserve:
   ...
   - The lifting step, in the SAME block, mechanically extracts from the row's
     ...
     fidelity tuple ABOVE ∪ the row's `workspace_files` (each `{path, content_hash}`, sorted
     by path) ∪ the row's proof shape (`Ts7Oracle` vs `OracleAndGuard` + its `guard` id),
     recorded ONCE in the retained-lift metadata at lift time. Because `source_locator`,
     `workspace_files`, `host_setup_kind`, and the proof-shape discriminant are IN the
     fingerprint, a lift that drops or mis-records any of them — a `workspace_footprint` row
     migrated as `standalone`, a `source_locator` whose `symbol_space` flipped Type↔Value, a
     re-pointed/edited workspace file, or a bare `Ts7Oracle` seating for a row that asserted
     a footprint — FAILS the fidelity guard rather than passing. (SHIPPED in schema v3 as
     the `migration_fingerprint.v1` layer; the v1 fingerprint covers `helper_kind` /
     `primary_canonical` / `symbol_or_expression` / `type_arguments` / `projection_mode` /
     `host_setup_kind` / `source_locator` / `workspace_files` + the proof-shape
     discriminant. GENUINELY DEFERRED to a `MIGRATION_FINGERPRINT_VERSION` bump: non-empty
     `type_arguments` (the parameterized-printer spike) and the full proof-OBLIGATION reach
   ...
   - `registry_payload_matches_migration_fingerprint` asserts that the registry entries
     ...
     `type_arguments`, `projection_mode`, `host_setup_kind`, and `source_locator`) taken in
     `query_ordinal` order, ∪ the row's `workspace_files` ∪ the row's proof shape
     ...
     proof shape preserves its obligations, not merely a self-consistent target. A
     self-consistent wrong pair that ALSO re-points `workspace_files` or `source_locator`
     (and regenerates a matching snapshot — `snapshot_id` + `source_admission_digest_consistent`
     recompute over the wrong current files and self-agree) is caught HERE, since the
     immutable retained fingerprint anchors the original.
...
3. **The per-snapshot id (`snapshot_id`) is a deterministic, REGISTRY-DERIVABLE
   ...
   - **`env_corpus_id` has a CHECKED-IN, PINNED SOURCE-OF-TRUTH the registry reads — the
     ...
     id) so the on-disk corpus root `oracle_env/<env_corpus_dir>/` is locatable from the
   ...
   - **Exactly ONE current corpus directory; stale corpus dirs are GC'd or REJECTED.**
     ...
     corpus produces a NEW `oracle_env/<new_dir>/` directory (the new id's
     `env_corpus_dir_name` form) and orphans every snapshot
     ...
     directory, and it is the one named by `env_corpus_dir_name(CURRENT_ENV_CORPUS_ID)`** (plus the `CURRENT`
     pointer file). The regenerator DELETES the prior `oracle_env/<old_dir>/` directory
     ...
     one `<env_corpus_dir>/` directory, (b) its name equals
     `env_corpus_dir_name(CURRENT_ENV_CORPUS_ID)` (the `CURRENT` pointer's contents
     stay the logical id, equal to `CURRENT_ENV_CORPUS_ID`), and (c) every snapshot's stored `env_corpus_id` equals
     ...
     coexisting. That guard checks the four spellings (constant, pointer, dir name — the
     latter under the `env_corpus_dir_name` mapping — snapshot field) agree WITH EACH OTHER; it does NOT recompute the id from content. The
     ...
     recomputed id EQUALS `CURRENT_ENV_CORPUS_ID`, the `CURRENT` pointer, the dir name
     (under the `env_corpus_dir_name` mapping), AND every snapshot's `env_corpus_id` — so a corpus whose content drifted WITHOUT re-
   ...
   - **`env_corpus_id` is STABLE-ACROSS-ROWS, not IMMUTABLE — its regeneration
     ...
     `oracle_env/<new_dir>/`, rewrites `CURRENT_ENV_CORPUS_ID` in `oracle_query_specs.rs`
     and the `oracle_env/CURRENT` pointer file to the new id, DELETES the prior
     `oracle_env/<old_dir>/` directory, and rewrites every standalone snapshot under its
...
- Therefore FOUR coverage guards together close the registry⋈snapshot loop (and TWO
  ...
  - **`registry_covers_every_lifted_oracle_query` (forward — coverage by
    ...
    silently pass the count cross-check (the §Q4 `oracle_query_ordinals` count
    is `0` for an un-lifted row, so an entry with no snapshot would otherwise go
    undetected). The
  ...
  - **`oracle_env_corpus_is_closed` (the vendored-corpus membership + content gate)**:
    ...
    directory (`oracle_env/<env_corpus_dir>/`) and assert SET-EQUALITY between the
  ...
  - **`registry_entry_count_matches_declared` (SHIPPED v3 — the independent cross-check — ALL rows,
  ...
  - **`raw_capture_matches_oracle_value` (the captured-hover ↔ stored-value tie)**:
    ...
    for that path; a vendored corpus file's source is on-disk under `oracle_env/<env_corpus_dir>/`)
...
- **Implementation prerequisite (REALIZED) — the work the first `Ignored → Lifted` row
  ...
  1. **The regenerator source-model redesign (a `Lifted` row SURVIVES
     regeneration).** `scripts/gen-typeinfo-ignore-manifest.mjs` would, in the original
     ...
     - A **retained-lift metadata map** — a checked-in source-of-truth list
       ...
       a typed `non_typeexpr_obligations` set — was retired. The `migration_fingerprint`
       / `original_body_tokens` body-hash fidelity layer (§Q4) is SHIPPED (v3) and retained
       SEPARATELY in `LIFTED_ROW_MIGRATIONS` — a distinct retained-lift table from this
       `LIFTED_ROW_OVERRIDES` manifest-record map.)
     ...
     - **Realized first-lift state — the four lifts are bare `Ts7Oracle`, verified in
       ...
       `ProofRequirement::Ts7Oracle` (no `OracleAndGuard`, no obligation set). The
       cryptographic `migration_fingerprint` + `original_body_tokens` body-hash artifact
       (a `syn`-AST lift-time extractor) is SHIPPED (v3); the only remaining deferral is its
       v1 SUBSET scope — `workspace_files` / `source_locator` and the full proof-obligation
       reach await a `MIGRATION_FINGERPRINT_VERSION` bump (a `syn`-AST lift-time extractor that
...
5. **Snapshots are demand-driven, joined through the registry in `src`, and query
   ...
   `no_orphan_snapshot`, `registry_entry_count_matches_declared` (SHIPPED v3),
...
15. **Snapshot validity pins the CLOSED VENDORED corpus on the VALUE, not just compiler
    ...
    corpus (copies the bytes into a checked-in `oracle_env/<env_corpus_dir>/` dir) and
...
23. **The dominant host is `standalone`, driven under ONE canonical oracle config against
    ...
    (`oracle_env/<env_corpus_dir>/`) — tsgo's `--lsp` / root points at THAT frozen corpus,
...
29. **The oracle-env corpus is VENDORED + CLOSED.** The generator copies the BYTES of
    ...
    into a CHECKED-IN vendored directory `oracle_env/<env_corpus_dir>/` and drives tsgo
...
| `migration_fingerprint_extraction_is_static` (SHIPPED v3) | The `migration_fingerprint` is extracted by the one-time audited lift command via a STRUCTURED `syn::Block` parse of the original `#[test]` body (NOT a text/regex scan): it walks the body for every modeled query-helper call (`resolve_expr` / `resolve_with_mode` / `shallow_surface_expr` / `evaluate_expr`) + the modeled host-construction call (`make_host_with_footprint` → `standalone`), const-folds each query argument to a literal / closed enum path (path-consts + uniform file-local query wrappers like `resolve_alias` are folded/inlined at capture), and detects obligation-bearing assertions. A body whose query argument is NOT a const-foldable literal — a computed call, a loop-bound, a closure, an unfolded identifier — or whose host setup / query is hidden behind a non-modeled wrapper, or that carries an unmodeled obligation assertion, REJECTS LOUDLY (`ExtractError`): the row stays without provenance (`migration: None`), never an auto-lifted guessed/partial fingerprint. So the auto-liftable set is a SOUND SUBSET, never a guessed superset. Discriminating: a body computing its symbol through a call, or a loop-built query, FAILS extraction rather than producing an unverifiable fingerprint. |
| `original_extraction_input_auditable` (SHIPPED v3) | The retained-lift metadata (`LIFTED_ROW_MIGRATIONS`) STORES `original_body_tokens` — the canonicalized ORIGINAL `#[test]` body `syn` token stream (the EXACT extraction INPUT, `Span`-stripped, whitespace-insignificant, comment-free, path-const-folded), CHECKED IN at lift time alongside `migration_fingerprint`. This makes the INITIAL extraction input auditable, not merely self-compared: this guard RE-RUNS the closed extractor over the CHECKED-IN `original_body_tokens` and asserts the re-derived fingerprint EQUALS the recorded `migration_fingerprint` — HERMETICALLY, from the retained-lift artifact alone, with NO VCS archaeology. So a fingerprint that never matched its own claimed input is detectable. The token stream is an audit record, NOT a `snapshot_id` input. Discriminating: a retained-lift record whose `migration_fingerprint` is tampered while `original_body_tokens` is left intact FAILS. |
| `registry_entry_count_matches_declared` (SHIPPED v3) | For EVERY manifest oracle row, the registry entry count (`(row_file, row_function, *)`) EQUALS the row's declared `IgnoredTestRow.oracle_query_ordinals` AND the retained `LIFTED_ROW_MIGRATIONS.oracle_query_ordinals` (a three-way agreement), AND the registry's `query_ordinal`s are UNIQUE and CONTIGUOUS `0..count-1` (a `{0,1,3}` gap or `{0,0,1}` duplicate FAILS even at matching count). Includes the ZERO-COUNT rows: every `Ignored` / non-oracle / un-lifted row declares `oracle_query_ordinals == 0` and MUST have ZERO registry entries — a stray entry on a zero-count row FAILS. Catches a registry that under-counts (3 of 4), which the forward/reverse biconditional (both registry-derived) cannot see. Discriminating: a 3-entry registry for a row declaring 4 FAILS; ordinals `{0,1,3}` FAIL; any registry entry on a declared-`0` (Ignored) row FAILS. |
...
| `registry_payload_matches_migration_fingerprint` (SHIPPED v3/v1) | Cardinality (count + contiguity) does NOT prove the registry PAYLOAD matches the ORIGINAL hand-authored query. `migration_fingerprint` (on the retained-lift metadata) is THE migration-fidelity authority — NOT a hand-maintained registry field. At lift time the body (parsed as a `syn::Block`, see §Q4 "Extraction method") yields the ordered per-query FIDELITY tuple; the canonical-JSON over that ∪ row identity ∪ declared query count ∪ the proof-shape discriminant is recorded BEFORE the body is replaced by `#[oracle_row]`. This guard projects the row's registry entries into the SAME tuple, in `query_ordinal` order, and asserts they re-canonicalize to the SAME `migration_fingerprint`. **v1 tuple:** `helper_kind`, `primary_canonical`, `symbol_or_expression`, `type_arguments`, `projection_mode`, `host_setup_kind`, the typed `source_locator` `{reference_canonical, reference_name, symbol_space}`, and the row's `workspace_files` (each `{path, content_hash}`, sorted by path) — plus the proof-shape discriminant (`Ts7Oracle` vs `OracleAndGuard:<guard>`; not the specific `OracleId`, absent from the body). `source_locator` is body-derivable; `workspace_files` is CAPTURED from the lift/registry context (not the body token stream) and RETAINED in `LiftMigrationProvenance.workspace_files`, re-checked hermetically by `original_extraction_input_auditable` and pinned to the snapshot by `snapshot_workspace_files_match_retained_provenance`. Discriminating: a registry entry with the correct COUNT but a wrong `symbol_or_expression` / `primary_canonical` / `projection_mode` / `type_arguments` / `host_setup_kind` / `source_locator` / `workspace_files` FAILS — a fully self-consistent wrong `(spec ∧ snapshot)` pair (registry + snapshot both re-pointed at a different symbol, workspace setup, or source locator, with a regenerated matching snapshot; the FROZEN original-body fingerprint still anchors the original) FAILS here even though the symbol-binding + source-admission + env-pin rails pass it. (Tuple-widening to non-empty `type_arguments` / the full proof obligations bumps `MIGRATION_FINGERPRINT_VERSION`.) |
| `snapshot_migration_fingerprint_matches_retained_lift_metadata` (SHIPPED v3) | Every checked-in v3 snapshot's `migration_fingerprint` (+ `migration_fingerprint_version`) MIRROR equals its row's retained `LIFTED_ROW_MIGRATIONS` provenance — so a hand-edited snapshot mirror, or a snapshot whose row carries no retained provenance, FAILS. The mirror is NOT a `snapshot_id` input; it is validated as a VALUE on read against the retained-lift authority. Discriminating: a snapshot whose `migration_fingerprint` differs from its row's retained fingerprint FAILS; an orphan snapshot with no retained provenance FAILS. |
...
| `oracle_env_single_current_corpus` | `env_corpus_id` has a CHECKED-IN pinned source-of-truth: the `CURRENT_ENV_CORPUS_ID` constant in `oracle_query_specs.rs` (mirrored by the `oracle_env/CURRENT` pointer file), which the registry + every guard read to derive `snapshot_id` and locate the corpus dir WITHOUT opening a snapshot. Asserts (a) `oracle_env/` contains EXACTLY ONE `<env_corpus_dir>/` directory, (b) its name == `env_corpus_dir_name(CURRENT_ENV_CORPUS_ID)` (the `CURRENT` pointer's contents stay the logical id, == `CURRENT_ENV_CORPUS_ID`), (c) every snapshot's stored `env_corpus_id` == `CURRENT_ENV_CORPUS_ID`. This is a NAME/POINTER/CONSTANT/SNAPSHOT-FIELD EQUALITY check — it does NOT recompute the id from content; that loop is closed by `env_corpus_id_recomputes_from_corpus`. Regeneration re-vendors into `oracle_env/<new_dir>/`, rewrites the pin + pointer, DELETES the prior corpus dir, and rewrites every snapshot under its new id (clean cutover, no dual corpus). Discriminating: a leftover stale `oracle_env/<old_dir>/` dir, a `CURRENT`-pointer / constant mismatch, or a snapshot pinned to a retired corpus all FAIL. |
| `env_corpus_id_recomputes_from_corpus` | Closes the "the pinned id still NAMES the actual current corpus" loop offline: RECOMPUTES `env_corpus_id` from the canonical ON-DISK corpus under the EXACT pinned recipe (§Q1) — BLAKE3, domain-separated under the `env_corpus_id` tag, over the CORPUS LISTING: the canonical-path-sorted `[{ path, content_hash }]` pairs of the full `oracle_env/<env_corpus_dir>/` vendored corpus (the dir named `env_corpus_dir_name(CURRENT_ENV_CORPUS_ID)`) (each file's path + its per-file content hash under the pinned content normalization, NO file bytes hashed inline) — and asserts the recomputed id EQUALS `CURRENT_ENV_CORPUS_ID`, the `oracle_env/CURRENT` pointer's contents, the corpus DIRECTORY NAME (under the `env_corpus_dir_name` mapping), AND every snapshot's stored `env_corpus_id`. Where `oracle_env_single_current_corpus` checks the four spellings agree WITH EACH OTHER, this guard checks they all agree WITH THE ACTUAL CONTENT — so a corpus whose content was edited/added/removed WITHOUT re-pinning (the four spellings still mutually equal, but no longer naming what is on disk) FAILS. Runs OFFLINE (no tsgo). Discriminating: a one-byte edit to any vendored corpus file, or an added/removed corpus file, that leaves `CURRENT_ENV_CORPUS_ID` / `CURRENT` / dir-name / snapshot fields unchanged recomputes a DIFFERENT id and FAILS — catching a stale pin that mutual-equality alone cannot. |
...
5. **Env-pin completeness (`compiler_options_hash` + the closed vendored corpus).** Two
   ...
   (`oracle_env/<env_corpus_dir>/`) — the generator copies the BYTES of the full SHARED set
...
7. **The first lift's prerequisite set was the regenerator source-model + the 362-table
   ...
   all-`Ignored` model, `scripts/gen-typeinfo-ignore-manifest.mjs` built the table ONLY from
