...
5. **17-family `OracleId` enum** (`typeinfo_ignored_test_manifest.rs:332-350`):
...
10. **§6.3 today** (`native-typeinfo-parity.md:2257-2301`) describes the
   differential oracle as comparing **STRUCTURED** results — "Compare the projected
   `TypeInfoGraphPayload` / `RelationPayload` / `TypeDescriptor` structurally
   against tsgo's answer … NOT a text compare of display output" — under a
   **per-family divergence budget**. §4.3 (`:1701-1712`) says oracle rows are
   GENERATED, each id "deterministic from `(fixture, query,
...
3. **Its assertion does NOT additionally prove non-`TypeExpr` behavior — and the
   ...
   **The obligation set is the SOLE PROPERTY of the row's `LiftedRowRecord` ledger entry —
   ...
   obligations is admissible ONLY under `OracleAndGuard`; the set and the proof are
   cross-checked (a non-empty set under a bare `Ts7Oracle` proof FAILS; an obligation
   whose `proving_guard` has no `OBLIGATION_GUARD_REGISTRY` entry FAILS).
...
| `oracle_value` | object | The captured, NORMALIZED oracle answer — a `TypeExpr::to_json_value()` document (the internally-tagged `"kind"`/`"properties"`/`"memberKind"` codec, fact 2). |
...
- **The LSP `initialize` payload + declared CLIENT CAPABILITIES** — the exact
  ...
  generator declares (in particular `textDocument.hover.contentFormat` — pinned to the
  `markdown` form the §Q2 extraction grammar parses — and any capability that changes
  tsgo's response shape). A capability change that alters the hover content form is a
  `probe_synthesis_version` bump.
...
the `type __oracle_probe__N = <T>;` header inside tsgo's MARKDOWN hover. tsgo's hover is
a Markdown document, not bare TS, so the extraction is a small fixed GRAMMAR — not an
ad-hoc regex over the whole blob — versioned with the rest of probe synthesis:
- **Fenced code block is authoritative; inline/prose is ignored.** tsgo renders the
  type in a fenced \`\`\`typescript (or \`\`\`ts) code block (the example
  `"```typescript\ntype __oracle_probe__0 = {…}\n```"`). The extractor locates the
  code block(s), and ONLY a fenced \`\`\`typescript / \`\`\`ts block is parsed; any
  surrounding Markdown prose, inline `code` spans, or non-TS fences are ignored.
- **Multiple code blocks / JSDoc / comments.** A hover may carry several blocks (a
  type block plus a JSDoc/documentation block) and the type block may carry leading
  `/** … */` or `//` comments. The extractor selects the FIRST \`\`\`typescript block
  whose content parses as a `type __oracle_probe__N = …;` declaration naming the
  expected probe (the probe-header fence), strips leading/trailing comments and
  doc-blocks, and rejects a hover with NO such block (the probe-header fence FAILS).
- **Header recovery (the `type __oracle_probe__N =` prefix and the `;` terminator).**
  The extractor matches the exact `type __oracle_probe__<N> =` prefix (anchored on the
  probe name, the wrong-position fence) and the trailing `;`. The RHS between `=` and
  the terminating `;` is `<T>`. The terminator is found by scanning for the top-level
  `;` that closes the alias — NOT the first `;` — so a `;` nested inside the type body
  (e.g. an object type-literal `{ a: number; b: string }`) does NOT prematurely
  terminate; the extractor tracks BRACE/BRACKET/PAREN/ANGLE nesting depth and accepts
  only a depth-0 `;`. Nested braces are balanced before the terminator is accepted; an
  unbalanced/truncated block (the `…` truncation marker, an unclosed brace) FAILS the
  backstop.
...
matching the probe-driven generation in §Q2. Its only output is hover TEXT (fact 3),
which feeds the §Q2 pre-lowering admission + generation-time lowering, and it discards
the hover range (fact 3, `ipc.rs:1689`) — handled by the probe-header fence.
   ...
   **`(row_file, row_function, query_ordinal)`** — where `row_file` is EXACTLY
   ...
   - **`non_typeexpr_obligations` is NOT a per-entry registry field — it is a
     ...
     **The `expected` payload is what lets the per-obligation guard re-PROVE the original
     ...
     cannot prove — i.e. exactly when the row's `proof` is `OracleAndGuard{..}` (a bare
     `Ts7Oracle(_)` row's set is EMPTY).
     ...
     **Why a PER-OBLIGATION guard, not the single `OracleAndGuard.guard`.**
     ...
     (`typeinfo_ignored_test_manifest.rs:426/430`). A row that dropped MULTIPLE distinct
     ...
     - a row with obligations is admissible ONLY under `OracleAndGuard`;
     ...
     **Per-kind targeting rule.** A row may issue N oracle queries keyed by `query_ordinal`
     ...
     enum gains the five `*Obligation` variants above, the closed `ObligationExpectation`
...
- Therefore FOUR coverage guards together close the registry⋈snapshot loop (and TWO
  ...
  - **`registry_covers_every_lifted_oracle_query` (forward — coverage by
    ...
    `status = Lifted` AND `proof = Ts7Oracle(_) | OracleAndGuard{..}` has ≥1 registry
    entry (by `(row_file, row_function, *)`), and EVERY such registry entry has an
    existing snapshot. Re-derives each `snapshot_id` from the spec + pinned env and
  ...
  - **`registry_family_matches_manifest_oracle_id` (family agreement)**: for every
    ...
    `OracleAndGuard { oracle, .. }` (`typeinfo_ignored_test_manifest.rs:332` `OracleId`,
    `:426` `ProofRequirement`). A registry entry that names a different family than the
    row's proof would read/write the wrong `oracle_snapshots/<family>/` sub-directory;
    this guard pins entry-family ≡ proof-family.
...
- The footprint/audit exclusion (§Scope) means a `Lifted` row whose proof is bare
  ...
  the named guard proves the footprint. The forward guard treats both
  `Ts7Oracle(_)` and `OracleAndGuard{..}` proofs as oracle-bearing.
...
- **Implementation PREREQUISITE — the FULL accurate prerequisite set for the first
  ...
  1. **The regenerator source-model redesign (a `Lifted` row must SURVIVE
     ...
     - Introduce a **retained `Lifted`-row ledger** — a checked-in source-of-truth list
       ...
       ledger row schema is the FULL 13-column `IgnoredTestRow` record PLUS the two
       lift-time-derived fields (`non_typeexpr_obligations`, `migration_fingerprint`) that
       live discovery cannot supply because they are extracted from the original BODY, not
       the `#[ignore]` site:
       ...
       LiftedRowRecord {
           ...
           proof: ProofRequirement,            // Ts7Oracle(_) | OracleAndGuard{..}
     ...
     - `status` AND the retained columns are sourced FROM the ledger record, never from
       ...
       being `Ts7Oracle(_) | OracleAndGuard{..}`) PLUS the three lift-time fields — a
       non-empty `migration_fingerprint`, a non-empty `original_body_tokens` (the pinned
       extraction input artifact, re-extracted + fingerprint-compared hermetically by
       `original_extraction_input_auditable`), and a `non_typeexpr_obligations` set that is
       NON-EMPTY exactly when the row's `proof` is `OracleAndGuard{..}` (a bare
       `Ts7Oracle(_)` row's obligation set must be EMPTY; an `OracleAndGuard` row's must be
       non-empty and every member's `proving_guard` must resolve to an
       `OBLIGATION_GUARD_REGISTRY` entry) — so a
       ledger entry cannot drop a column the all-rows guards read NOR silently drop the
       row's non-`TypeExpr` obligations.
...
7. **Eligibility is structural, scoped to `TypeExpr` projections, and the
   ...
   obligations). It is admissible only under `ProofRequirement::OracleAndGuard`, and the
...
| `registry_covers_every_lifted_oracle_query` | Every `Lifted` oracle row (proof `Ts7Oracle(_)` or `OracleAndGuard{..}`) has ≥1 registry entry (by `(row_file, row_function, *)`), every entry joins to an existing manifest row, and every entry has an existing snapshot (re-derived `snapshot_id` resolves to a file). |
...
| `hover_extraction_grammar_is_versioned` | The hover-extraction grammar (§Q2) extracts `<T>` from the `type __oracle_probe__N = <T>;` header inside tsgo's MARKDOWN hover via a FIXED grammar — fenced \`\`\`typescript block only (prose/inline ignored), first probe-naming block among multiple, leading JSDoc/comment stripping, the alias terminator found at depth-0 `;` (nested object `;` does NOT prematurely terminate), and the RHS handed to the SAME OXC type parser (qualified/imported display forms parsed, not text-interpreted). Versioned by `probe_synthesis_version`; re-runnable OFFLINE from `raw_capture`. Discriminating: a hover with a nested-`;` object body extracts the WHOLE balanced RHS (not the first `;`); a truncated/unclosed block FAILS the backstop; a hover with no probe-naming \`\`\`typescript block FAILS the header fence. |
| `hover_driver_config_pinned` | The tsgo LSP HOVER-DRIVER CONFIG is a value-affecting identity axis FOLDED INTO `probe_synthesis_version` (§Q2 "Hover-driver config"): the canonical `hover_driver_config` blob covers the LSP `initialize` payload + declared CLIENT CAPABILITIES (`textDocument.hover.contentFormat` pinned to the `markdown` form the extraction grammar parses, plus any response-shape-affecting capability), the `workspace/didChangeConfiguration` / `initializationOptions` delivery block, the `textDocument/hover` request payload, and the hover DISPLAY PREFERENCES (alias-expansion / truncation-length / quote-style / member-ordering print preferences). The guard asserts the generator drives tsgo under exactly this pinned config and that the config blob is hashed into `probe_synthesis_version` (a change bumps the version + forces regeneration). Discriminating: two sessions with identical query + env but a different declared `contentFormat`, a different display preference (e.g. an alias-expansion or truncation-length toggle that changes the printed hover), or a different hover-request payload MUST resolve to a different `probe_synthesis_version` (and thus a different `snapshot_id`) — they can NEVER produce different captures under the SAME `snapshot_id`; a driver whose config is not folded into `probe_synthesis_version` FAILS. |
...
| Per-row lift tests (added per block) | Each lifted oracle query: normalized Verter `TypeExpr` == normalized snapshot `oracle_value`, with explicit negative assertions (no `Unknown`, no `any`/`never` where a concrete type is expected). |
