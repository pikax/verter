...
**Substituted locator lowering is FAIL-CLOSED until B2.** The substitution axis is the existing content-free `SubstitutionCanonicalHash` (inner private, constructed only via `empty()` plus a typed test constructor — a raw file content/whole hash cannot be wrapped as a substitution identity). B1 wires no production caller and no real canonical-substitution producer: substituted locator lowering routes through `Instantiate { args, .. }` (or a real canonical-substitution producer) in B2. Until then the axis carries only `empty()` in production — the substituted route is fail-closed, not silently mis-keyed.
...
- **Item.** Full typed-newtype restructuring of `ResolvedDeclSlotIdentity`'s raw env fields (`type_env_hash: HashValue`, `lib_env_hash: HashValue`, `project_identity: u32`) into sealed R6 key-dimension types — and, more broadly, retiring any remaining raw `[u8;16]`/`u32` env-hash key fields on widely-used query identities.
- **Why not now.** `ResolvedDeclSlotIdentity` is a widely-used existing query identity (family memos, `Instantiate`/`ResolveMacroPayload`, materialize/ref-cycle keys). Restructuring its fields requires a consumer flip across every construction/read site — out of the ADDITIVE B1 scope (B1 adds trait impls / witnesses / new types only; it flips no consumer and removes no field).
- **Owning future block.** The slot-identity migration (the block that already owns `ResolvedDeclSlotIdentity` restructuring); not B1, not B2.
- **Temporary behavior.** B1 lands an EXHAUSTIVE legacy R6 witness for the slot: it destructures ALL six fields (no `..`) and classifies the three raw env fields through the NAMED `LegacyEnvDim` path (never an unchecked `_`). The env-dimension newtypes and `SubstitutionCanonicalHash` have private inner fields with typed constructors.
- **Fail-closed guard.** The exhaustive legacy witness prevents silent new-field drift on the slot (a new field fails compilation until classified), and `LocatorLoweringKey` has NO production caller until B2 — so the interim untyped env fields key nothing live. Substituted locator lowering is fail-closed (§6.4).
- **Closure condition.** The slot-identity migration types the three fields as sealed R6 dimensions (or an equivalent typed identity), at which point the `LegacyEnvDim` classifier and its named path are deleted and the slot's witness routes them through `key_safe` like every other dimension.
