...
**Substituted locator lowering is FAIL-CLOSED until B2.** The substitution axis is the existing content-free `SubstitutionCanonicalHash` (inner private; production constructs it only via `empty()`, and the test helper `distinct_for_test(seed: u32)` takes a `u32` seed — NOT raw bytes — so no raw file content/whole hash can be poured into a substitution identity). It is content-free BY TYPE (a distinct nominal type at the key position); the `HashValue` bytes themselves remain provenance-ambiguous, a disclosed carried obligation (§9.1). B1 wires no production caller and no real canonical-substitution producer: substituted locator lowering routes through `Instantiate { args, .. }` (or a real canonical-substitution producer) in B2. Until then the axis carries only `empty()` in production — the substituted route is fail-closed, not silently mis-keyed.
