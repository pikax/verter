...
- The residual inventory (`residual_type_expr_body_reader_inventory.rs`) and the two remaining deferral docs (`authored-shape-graph-native-migration-deferral.md`, `hot-materialize-tripwire-residual-deferral.md`) are **deleted at landing**; the now-moot `q2-structural-body-cache-deferral.md` deletes EARLIER, with B2's incrementally-landable deletion slice (§5.5).
...
- **Delete** `structural_body_cache`, `PreparedStructuralBodyCache`, `StructuralBodyRegistry`, `StructuralBodyMemo`, and their instrumentation — never populated, key unsound for resolving-lowered bodies; confirmed dead in production (the `prepared_decl.rs` import + field/comment/accessor + empty construction, the `resolver_core/mod.rs` exports, `structural_body_memo.rs` + `structural_body_memo_instrumentation.rs` + their test files; the only guard lives inside the deleted module itself, so no external scanner needs preserving). The deletion slice also removes `docs/arch/q2-structural-body-cache-deferral.md` and updates the stale references in `docs/arch/parselower-design.md` and `docs/arch/semantic-db-overhaul-unified-remaining-plan.md`. The correctness-required lowered-body memo is the first-class `LowerLocator` query (`LocatorLoweringKey`, §6.4) on the existing `SemanticGraphStore` multi-candidate substrate. Any future perf cache is Stage 12. This deletion is the ONE incrementally-landable slice (dead code, no dual path).
...
**Parent transitive contributor rule (RULED — per-contributor queries alone are NOT sufficient on a WARM parent).** A warm parent `Instantiate` returns already-folded contributor nodes without re-running the child `LowerLocator`, and `MemoEntry::validate` checks only the stored `ReadSetSignature` facts — the augmentation facts recorded today (`ModuleAugmentationIndexShape` + per-augmenter `FileWholeHash`) fold `parse_stable_hash`, not `parse_env_hash`, so a parent built when a contributor had parse env P0 would warm-hit after that contributor moves to P1 with unchanged content. Therefore: when an `Instantiate` result folds ANY cross-file real-file contributor body into the parent value, the parent `ReadSetSignature` MUST record one typed source-env observation per folded contributor — `FileSourceEnv { canonical, parse_env_hash, parser_version, file_language_id }`, taken from the EXACT contributor artifact / `LowerLocator` source used (never a stale index entry) — validated STRICTLY against the live `StoreView` at warm-hit time: a warm parent hit REJECTS if any recorded contributor source-env identity differs or is untracked, even when `FileWholeHash` and `ModuleAugmentationIndexShape` still validate. Contributor source-env identity that cannot be observed coherently routes the parent result through `ReturnOnly`. `self_root_canonicals` stays `{base} ∪ {contributors}`; the observation is value-side validity only — `query_key_spec` gains NO contributor digest axis (`body_source` remains the base source-kind axis), and warm parent reads never re-run child queries.
...
**The B1-landed substitution + caller-projection axes DELETE in B2.** The landed `LocatorLoweringKey.substitution: SubstitutionCanonicalHash` (inner private; production constructs only `empty()`; the test helper `distinct_for_test(seed: u32)` takes a `u32` seed — NOT raw bytes) and the landed `projection: ProjectionReductionContext` field were fail-closed scaffolding: B1 wired no production caller, so neither axis keys anything live. B2 — the first production wiring — REMOVES both fields from `LocatorLoweringKey` / `FamilyKey::LowerLocator` (the R6 witness re-proves over the narrowed key): `LowerLocator` is strictly unsubstituted BY TYPE and lowers under the fixed `LocatorShape` context; substituted demands route through `Instantiate { args }` (above). The `SubstitutionCanonicalHash` NEWTYPE itself SURVIVES — the relation/flow/call key context holds it today; only the `LowerLocator` axis (this field) deletes. A session-owned canonical substitution table (lifetime/validation/self-roots/param→node access) was considered and REJECTED — it would split reuse that `Instantiate { args }` already models.
...
- **Item.** ONE deferred typed-newtype restructure covering THREE grandfathered raw-byte key surfaces:
  ...
  2. `file_artifact_store::ProjectIdentity`'s PUBLIC `Hash16` field (`pub struct ProjectIdentity(pub Hash16)`), reused directly as an R6 dimension in `LocatorLoweringKey` — its public inner still admits a raw content hash by construction (`ProjectIdentity(content_hash_bytes)`);
...
- **Owning future block.** The slot-identity / env-hash typed-newtype migration (the block that already owns `ResolvedDeclSlotIdentity` restructuring and the shared env-dimension typing); not B1, not B2.
...
| **B8** | Landing — delete residual inventory + guard + the remaining deferral docs (`q2-structural-body-cache-deferral.md` deletes with B2's deletion slice); correct §3.5 stale prose; remove WIP parity rail; land the §9 structural guards; squash B1–B8 into one atomic clean cutover; full gate. | S (A/C for mechanical import/serde/doc cleanup inside, only after the owning S review passes) |
