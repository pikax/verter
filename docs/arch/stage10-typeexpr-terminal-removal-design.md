...
- Augmentation stitch: `stitch_module_augmentations` demands each augmenter's body through the augmenter's OWN `LowerLocator` (keyed by the augmenter's locator + `ParseEnvHash`) and composes the returned carrier-preserving nodes — never a direct `lower_decl_body_with_provenance` call on another file's prepared body (§6.4 contributor rule).
