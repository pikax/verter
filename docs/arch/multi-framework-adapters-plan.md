...
**Svelte feature-completion (owner scope expansion, D-bi)**: the Svelte vertical does NOT stop at the
v1 scope matrix — every row tagged OUT-OF-SCOPE v1 (D-ad + D-bg) is PROMOTED to SUPPORTED by the
feature-completion blocks B8d–B8j (D-bl), which land on the critical path after B8c and before B12.
Each REUSES the B8a parse + B8c IDE substrate, replacing each typed-unsupported diagnostic with a
real projection/typeinfo that type-checks clean through the TSGO gate. Verter ships Svelte
feature-complete but marks it "experimental — not yet validated in real-world use"; the owner flips
it to production-ready after their own validation. Three blocks are OWNER-GATED (B8h precise legacy
events, B8i await-expressions reversing the D-bg await ruling, B8j `.svelte.ts`/`.svelte.js`
rune-module carriers reversing the D-bg ".svelte.ts is not a carrier" ruling) — see the §7
owner-decision list.
...
| **D-bg** | **Svelte feature-surface scope refresh (B8) — 2026-06-13 pre-execution re-audit; async `await` SYNTAX is OUT-OF-SCOPE v1 while the async-FAMILY runtime imports are transparent.** Re-audited against live `svelte.dev/docs/svelte/*` + `github.com/sveltejs/svelte/releases` + `packages/svelte/CHANGELOG.md` immediately before B8, per the owner RE-SCOPE directive. Audited latest stable `svelte@5.56.10` (UNCHANGED from D-ad's 2026-06-10 audit); the D-ad matrix stands materially correct. Codex-adjudicated (xhigh, `/tmp/verter-mf/svelte-scope-codex-consult.txt`): (1) `{@attach}` (5.29) IN-SCOPE — element `Attachment<E>` void-check vs component Symbol-keyed prop, the component attachment prop is NOT published PROPS metadata, carrier-parse + IDE-projection only (no script-fact/synth/PlannedDemand/api-projector contact); (2) await-expressions (5.36+, experimental, opt-in `experimental.async`, flag removed in Svelte 6) OUT-OF-SCOPE v1 in all 3 positions — parse-without-crash + void-checked + per-position `svelte-await-experimental` typed-unsupported diagnostic (not a known-bug row); (3) the async-FAMILY runtime APIs `fork()` (5.42) + `getAbortSignal()` (5.35) + `createContext()` (5.40/5.50) are transparent `svelte` imports with ZERO adapter contact (covered by the existing lifecycle/context transparent row) — the async SYNTAX stays out-of-scope but the imports never imply adapter work, blocking async-family scope creep into B8b/B8c; (4) `.svelte.ts`/`.svelte.js` rune modules OUT-OF-SCOPE v1 as carriers — they classify as plain `Script` STRUCTURALLY (the `.svelte` carrier-row suffix does not match a `.svelte.ts` path, which ends `.ts`/`.js`), B4b's threaded dialect is the substrate, and a `.svelte` component importing from them resolves as a normal script dependency. ADDITIVE B8 fixtures (no new test family, no wire/`FrameworkSurfaceKind` change): B8a — comments-in-tags (5.53) parse-without-crash + a discriminating `.svelte.ts`-is-NOT-a-carrier classification guard; B8c — `{@html}` accepting `TrustedHTML` (5.52) + customizable `<select>` (5.47) intrinsic projection. Function-valued `css`/`runes`/`customElement` compiler options (5.54) OUT (compiler/config domain — existing `<svelte:options>` parse suffices). The B8a §8 matrix carries the full refresh table; no OWNER decision required (every owner-named surface is explicitly dispositioned). **SUPERSEDED for the END-STATE (D-bi/D-bj/D-bk/D-bl, owner scope expansion 2026-06-14): the await SYNTAX (item 2) is PROMOTED to SUPPORTED with real await projection + an informational diagnostic (B8i, D-bj, owner-gated), and the `.svelte.ts`/`.svelte.js`-is-NOT-a-carrier ruling (item 4) is REVERSED — they become first-class non-component rune-module carriers (B8j, D-bk, owner-gated). The async-FAMILY runtime imports (item 3) stay transparent (unchanged).** | Owner RE-SCOPE directive (2026-06-10, STATE.md): "Svelte scope refresh REQUIRED before B8 execution: current Svelte docs surfaces (`{@attach …}`, newer await-expression behavior, any other post-runes additions) must be explicitly supported or explicitly out-of-scope." codex consult (xhigh, 2026-06-13): "Current refresh confirms the existing scope is materially right … the matrix is complete for B8 v1" after the four additive fixtures; one decision row added to prevent async-family scope creep. |
| **D-bh** | **The Svelte surfaces resolve through a NEW closed `PlannedDemand::SvelteSurface { owner, source: SvelteSurfaceSource }` variant — NOT the Vue-coupled `MacroPayload` arm and NOT a second resolver (B8b resolve-routing; codex-ratified STEP-0).** `PlannedDemand` grows from 4 to 5 closed variants; `ResolvedDemand` mirrors it with `SvelteSurface(ResolvedMacroPayload)`; the closed-vocabulary + exhaustive-match guards move 4→5. The adapter PLANS one `SvelteSurface` demand per requested source family (SLOTS plans TWO — `SnippetProps` + `LegacySlotInventory` — and `normalize` merges them, first-writer-wins on a duplicate slot name). The executor's private `ExecutorResolveCtx` gains ONE `SvelteSurface` arm that READS the registration's typed Svelte facts (`SvelteScriptFacts`, extended to carry the source-family inventory: `props_type`, `bindable_members`, `validated_snippet_members`, `legacy_props`, `dispatcher_events`, `instance_exports`) / candidates and dispatches the captured `TypeExpr`(s) through the SHARED resolver (`Instantiate`/decl-lowering in `Navigate` + `project_shallow_surface_from_base(… published(Shallow))`, or the synth-backed `default`/`$props` path) against the executor's ONE captured request view — never a second `current_store_view_for_query`. EMITS is provenance-validated: `dispatcher_events` contributes ONLY when `createEventDispatcher` resolved to the `svelte` package (not a bare-name match), with a userland-look-alike negative test. The Svelte descriptor's `supported_surfaces` narrows to `{PROPS, EMITS, SLOTS, EXPOSE, MODEL}` at the arm flip (OPTIONS unsupported per §9), so the executor fills OPTIONS structurally UNSUPPORTED while every other kind is supported-empty-or-resolved. The B8a P2 typed-resolved-package-identity (a typed `package` column on `ResolvedImportTarget`, session-computed where `workspace_is_package_backed` lives) replaces the `node_modules/svelte` path substring; the provider tests `target.package == "svelte"` structurally for BOTH snippet and dispatcher provenance. | codex STEP-0 consult (xhigh, 2026-06-14, `/tmp/verter-mf/b8b-step0-codex.txt`): VERDICT CHANGES — (a) mapping ratified + EMITS provenance-validation wording/test; (b) "choose the new demand variant — P0 if implemented via `MacroPayload` or a second resolver"; (c) one-bundle-per-`SvelteSurfaceSource` cache discipline ratified, add a deterministic duplicate-slot-name merge test. |
...
| **D-bi** | **Svelte feature-completion scope expansion (owner, 2026-06-14): every OUT-OF-SCOPE-v1 Svelte matrix row (D-ad + D-bg) is PROMOTED to SUPPORTED in new feature-completion blocks (B8d–B8j); Verter ships Svelte feature-complete but marks it "experimental — not yet validated in real-world use".** The v1 typed-unsupported diagnostics (B8c) are the correct INTERMEDIATE state — they are NOT the permanent contract; each new block flips its matrix rows OUT-OF-SCOPE→SUPPORTED, replacing the diagnostic with a real projection/typeinfo that REUSES the existing B8a parse (the parser already round-trips every matrix row without crash) and TYPE-CHECKS CLEAN through the TSGO/`@verter/svelte-jsx` gate with `@ts-expect-error` anti-`any`/`never` guards. "SUPPORTED" means EXACTLY: the projected TSX (or rune-module TS) type-checks cleanly through TSGO/tsserver, every prelude helper is exercised by negative `@ts-expect-error` fixtures, and NO projection calls the Rust typeinfo resolver — IDE precision comes from TypeScript seeing the API virtual files + shim types, never from the Svelte projector dispatching semantic queries (codex Overall Verdict). The new blocks land on the critical path AFTER B8c, BEFORE B12; existing `FrameworkSurfaceKind`/wire schema is sufficient (codex Q8 — no new variant; any `$events`/`$slots` IDE shape is virtual-file/shim surface, not wire). **DISCLAIMER POLICY (owner correction, supersedes the B8c/B12 "scoped v1 — many surfaces OUT-OF-SCOPE" wording for the FEATURE-COMPLETE end-state):** the experimental disclaimer means "not yet validated in real-world use" — NOT incomplete/scoped. Once B8d–B8j land, the Svelte matrix carries no permanent OUT-OF-SCOPE feature rows (only CSS-domain non-surfaces + SvelteKit-layer + compiler-config-domain items, which are correctly non-features). The B12 user-facing wording across every Svelte surface (`verter.frameworks` setting, `@verter/svelte-jsx` README, `/framework-adapters` skill, docs/README, marketplace) is "**experimental — not yet validated in real-world use**". Verter NEVER flips Svelte to production-ready; the OWNER does, after their own real-world validation. Vue stays the production reference adapter. | Owner SCOPE EXPANSION (2026-06-14, STATE.md): "Svelte must be FULLY FUNCTIONAL by the END of the plan." codex consult (xhigh, 2026-06-14, `/tmp/verter-mf/svelte-completion-codex-consult.txt`): Q9 confirmed — "Verter may ship Svelte feature-complete as experimental / not real-world validated; it must not claim production readiness; owner flips that after validation." |
| **D-bj** | **Await-expressions (5.36+) are PROMOTED to SUPPORTED with real await projection + an INFORMATIONAL experimental diagnostic (option (b); supersedes D-ad/D-bg's OUT-OF-SCOPE await rows; OWNER GO/NO-GO).** Block B8i. The projector emits the REAL await in all three positions — markup `{await e}` projects to a checkable `await (e)` against `Awaited<typeof e>`; `$derived(await …)` keeps the verbatim rune call site with the inner await void-checked; top-level `<script>` await projects as module-level top-level `await` (the projected `.svelte.tsx` is an ES module; TS allows it under `module: esnext` / `target: es2022`). The per-position `svelte-await-experimental` TYPED-UNSUPPORTED diagnostic is REPLACED by an INFORMATIONAL diagnostic noting experimental status; it is suppressed when the project targets Svelte 6 or an approved `experimental.async` config-detection surface exists (config-detection is a named follow-up, not a B8i blocker). The async-FAMILY runtime imports (`fork`, `getAbortSignal`, `settled`, `$effect.pending` decl-only) STAY transparent `svelte` imports (D-bg item 3 unchanged — only the await SYNTAX flips). Rationale: the IDE projection is a type-check surface, not a runtime guarantee; type-checking real await makes Svelte feature-complete for the surface while the informational diagnostic honors the experimental disclaimer (D-bi) until the flag stabilizes/removes in Svelte 6. | codex Q1/Q2 (2026-06-14): "REFINE / OWNER GO-NO-GO — choose option (b): always type-check the real awaited expression plus informational experimental diagnostic until Svelte 6/config stabilization. Marking it fully silent now is product-risky. This reversal of D-bg needs owner approval." OWNER-GATED. |
| **D-bk** | **`.svelte.ts` / `.svelte.js` rune modules are PROMOTED to first-class NON-COMPONENT rune-module CARRIERS — REVERSES the `.svelte.ts`/`.svelte.js`-is-NOT-a-carrier ruling of D-ad and D-bg (item 4); OWNER GO/NO-GO.** Block B8j. The reversal is SCOPED: a rune module is NOT a component carrier — it gets a NEW classification row in `verter_language` (a rune-module `FileLanguage`/carrier row matched BEFORE the plain `.ts`/`.js` rows, since a `.svelte.ts` path ends `.ts` and otherwise falls through to plain-script structurally, D-bg item 4's exact reason), NO PROPS/EMITS/SLOTS/EXPOSE/MODEL surface, NO `ComponentDefaultSynth`, NO `PlannedDemand`, NO component `ComponentApiProjector` (it has no `$props`/component API — it is a module of reactive values). Its ONLY adapter work is: (i) correct classification under the right dialect (the B4b plain-script substrate already threads `.ts`/`.js` dialect; the rune-module row carries the SAME `ScriptSourceType`); (ii) the rune ambient prelude prepended (unmapped, offset 0) so module-scope rune calls type-check. CRUX (codex Q3): the rune module's IMPORT-RESOLUTION virtual file must resolve to **prelude + real module bytes** so a consuming module sees correctly-inferred exported rune-derived types (`export const s = $state(0)` exports `number` only with the rune prelude in scope) — its `VirtualFileNaming` is NOT the component dual-file model; ide-surface and import-surface are the SAME prelude+content TS module. `.svelte.js` additionally needs a SOURCE-TYPE-SPECIFIC projection story: a TS `declare`-form prelude is NOT valid JS, so the `.svelte.js` rune prelude is rendered as a JS-valid ambient form (a JSDoc/`.d.ts`-sidecar declaration the JS file references, or the prelude injected via the host-owned typing channel) rather than inline `declare function`. NO new `FrameworkSurfaceKind`/wire change. The D-bg ".svelte.ts is not a carrier" disposition (item 4) is RETIRED by this row; D-bg's other items (await-family transparency, comments-in-tags, `{@attach}`) stand. | codex Q3 (2026-06-14): "Do not reuse the component carrier model. The right shape is a new rune-module language/carrier row before plain `.ts/.js`, no PROPS/EMITS/SLOTS/EXPOSE/MODEL, no component synth, no component API projector. Its TS import surface must resolve to prelude + real module bytes so exported `$state`/`$derived` values infer correctly. This is an owner decision because it reverses D-bg and extends routing/virtual-file semantics." OWNER-GATED. |
| **D-bl** | **Per-feature SUPPORTED projection designs + the B8d–B8j block grouping (codex-ratified, 2026-06-14).** Every promoted matrix row resolves through the EXISTING B8a/B8b/B8c substrate — the rune prelude, the void-check pattern, the prelude-checker pattern (`__verter_attach`/`__verter_snippet`/`ClassValue`), the `@verter/svelte-jsx` shim (extended per row), the `SvelteSurface` typeinfo arm, and CodeTransform-only mutation — and adds NO second resolver. Ratified designs: **F1 `style:`** (RATIFY) — strip the directive from the JSX attribute position (a `style:`-prefixed name is not a valid JSX identifier; mirrors the custom-property pass-through), void-check the value; shorthand `style:color` projects the implied identifier only when the name is a valid binding identifier. **F2 `transition:`/`in:`/`out:`** (REFINE) — NEW prelude checker `__verter_transition` over the `svelte/transition` `TransitionConfig` signature; the node param is the HOST ELEMENT instance type (`E extends Element`, fallback `Element` only for unknown/dynamic hosts); required/omitted-param overloads so a missing required `params` FAILS; directive stripped + checker spread-merged; `|local`/`|global` presentational (no type effect). **F3 `animate:`** (RATIFY) — NEW prelude checker `__verter_animate` over the `svelte/animate` `AnimationConfig` signature (`(node, { from: DOMRect, to: DOMRect }, params) => AnimationConfig`); the "must be keyed-each child" structural rule is at most a cheap syntax diagnostic — it introduces NO resolver work. **F4 wide `bind:` family** (REFINE) — a GENERATED CLOSED bind-contract table keyed by directive name (+ applicable element/tag constraints) is the SOURCE OF TRUTH, computing value type + direction (read/write/read-write); the one generic checker is an implementation helper only; `bind:this` projects as an ASSIGNMENT-COMPATIBILITY check (intrinsic tags via DOM element-instance maps, components via `InstanceType<typeof C>`, dynamic/unknown fallback `Element`/component-base) — NOT a `JSX.IntrinsicElements` attribute; `bind:group` inspects checkbox-vs-radio + value type (the loose `T | T[]` is too weak); media/dimension/contenteditable/`<details>` bindings each carry their typed contract + read/write direction. **F5 function bindings `bind:x={get, set}`** (REFINE) — mutual get/set consistency is NECESSARY but INSUFFICIENT; for ELEMENT binds instantiate `V` from the generated bind table; for COMPONENT binds derive `V` in TS from `InstanceType<typeof Child>["$props"][K]` (or a bindable-prop marker on the API file) — NO Rust resolver call; `{null, set}` is the write-only form. **F8 `<svelte:component>`/`<svelte:self>`** (RATIFY/REFINE) — TS helper checks over class-shaped component values + `InstanceType<C>["$props"]`; `<svelte:self>` checks against a LOCAL self-component contract generated from the current component's props; no component-metadata resolution in the projector. **F9 `<svelte:fragment>`** (RATIFY as projection support) — unwrap children, preserve/void-check the `slot` literal; slot-NAME precision is NOT claimed unless an API `$slots` contract is added (owner-gated extension). **F10 `<svelte:options namespace="svg|mathml">`** (RATIFY/REFINE) — SEPARATE shim entrypoints `@verter/svelte-jsx/svg` (`IntrinsicElements extends SVGElements`) + `/mathml` (`MathMLElements`), selected by the per-file pragma; NO per-element namespace switching inside one JSX namespace. **F11 stores `$x` auto-subscription** (REFINE) — a SYNTACTIC rewrite via a `Readable<T>`/`Writable<T>` store-get helper, but NOT "every `$x` blindly": EXCLUDE the prelude runes (`$props`/`$state`/…) and the `$$` magic, respect lexical declarations (the typed-IR knows which `$x` are store-subs), and for sourcemap accuracy rewrite ONLY the `$` byte / operator spans via `CodeTransform::overwrite` while preserving the original identifier/RHS bytes. **F12 `$$props`/`$$restProps`/`$$slots`** (RATIFY, legacy-mode only) — prelude declarations (`$$props`/`$$restProps: Record<string, any>`, `$$slots: Record<string, boolean>`); the loose `any` is an EXPLICIT, documented anti-`any`-gate exception ONLY for the legacy magic object itself (Svelte's legacy magic is intrinsically loose). **F13 legacy component `on:` payload** (REJECT-loose / OWNER GO-NO-GO) — `CustomEvent<any>` is NOT payload checking and must NOT be promoted as "fully functional"; real support needs an API-VISIBLE event map (e.g. a `$events` contract on the component API file) + a TS helper projection against `EventsOf<typeof Child>[K]`, sourced WITHOUT projector-time resolver dispatch — owner-gated (B8h). **Block grouping (codex Q6 restructure):** B8d directives/styling (F1–F3); B8e bindings (F4–F5); B8f special elements/namespaces (F8–F10); B8g legacy identifiers/runtime (F11–F12); B8h precise legacy component events (F13, owner-gated `$events` extension); B8i await-expressions (F6, owner-gated, D-bj); B8j rune modules (F7, owner-gated substrate extension, D-bk). Logically parallel-safe after B8c, but SERIALIZED at integration — every block touches the Svelte IDE projection module, the prelude/shim assets, and the TSGO fixtures. | codex consult (xhigh, 2026-06-14, `/tmp/verter-mf/svelte-completion-codex-consult.txt`): Q1–Q8 ratify the designs above; Q6 restructures the grouping (await + rune-modules split into their own owner-gated blocks; legacy events split from F11/F12); Q7 flags F7/F13-loose/F5-mutual-only/F4-loose-group as unsound-as-originally-proposed (all refined above); Q8 no wire change. |
...
| `svelte_feature_rows_supported_no_diagnostic` | B8d–B8j | test | every Svelte matrix row PROMOTED by its block (D-bl) emits NO typed-unsupported diagnostic and projects a real checkable form — re-run per block as it lands (B8d `style:`/`transition:`/`animate:`; B8e binds; B8f special elements/namespace; B8g stores/`$$`-magic; B8h legacy `on:`; B8i await; B8j rune modules); the B8c out-of-scope-row assertions for the flipped rows are retired in the same change (no dual diagnostic path) |
| `svelte_projection_no_resolver_dispatch` | B8d–B8j | static-grep | the Svelte `ide/` projection + every new prelude checker / bind-contract table / store-rewrite path reference NO semantic dispatch (`ProjectSemanticDispatch`, `SemanticQueryKey`, `execute_cooperative`), NO OXC (`oxc_parser::`, `lower_ts_type`, `parse_type_annotation`), and NO query-time resolution — IDE precision comes from TypeScript seeing the shim + API virtual files, never a projector-time resolver call (D-bi/D-bl); the generated bind-contract table is a closed `Eq + Hash` data table (whole-struct destructure unit pin, no `..`) |
| `svelte_feature_codegen_uses_code_transform` | B8d–B8j | static-grep | all new Svelte projection mutation (directive strip, checker spread-merge, store `$`-span rewrite, await projection, prelude extension) goes through `CodeTransform` ops — NO `String::replace`/manual splicing on `build_string()` output; the store-rewrite touches ONLY the `$` byte / operator spans (sourcemap-preserving overwrite, D-bl F11) |
| `svelte_jsx_shim_entrypoints_byte_pinned` | B8d (transition/animate), B8f (svg/mathml) | test | the `@verter/svelte-jsx` entrypoints the feature blocks add (the `svelte/transition`/`svelte/animate` checker references; the `svg`/`mathml` `IntrinsicElements` entrypoints) carry in-crate mirrors + byte-pin freshness (the `svelte_jsx_shim_freshness.rs` pattern, D-av) — drift fails the gate |
| `svelte_rune_module_not_component_carrier` | B8j | test | a `.svelte.ts`/`.svelte.js` path classifies as the NON-COMPONENT rune-module carrier (NOT plain `Script`, NOT the component carrier) and exposes NO component-API surface (no PROPS/EMITS/SLOTS/EXPOSE/MODEL demand, no synth, no component api-projector); a plain `.ts`/`.js` still classifies plain-script and a `.svelte` still classifies the component carrier (negative) — the B8a `.svelte.ts`-is-NOT-a-carrier guard fixture (D-bg) is RETIRED in the same change (D-bk) |
...
| B8d | Svelte directives + styling completion (F1–F3, D-bl) | B8c | (serialized after B8c) |
| B8e | Svelte bindings completion (F4–F5, D-bl) | B8c | (serialized after B8d) |
| B8f | Svelte special elements + namespaces (F8–F10, D-bl) | B8c | (serialized after B8e) |
| B8g | Svelte legacy identifiers + runtime (F11–F12, D-bl) | B8c | (serialized after B8f) |
| B8h | Svelte precise legacy component events (F13, OWNER-GATED, D-bl) | B8c, B8b | (serialized after B8g) |
| B8i | Svelte await-expressions (F6, OWNER-GATED, D-bj) | B8c | (serialized after B8h) |
| B8j | Svelte rune modules `.svelte.ts`/`.svelte.js` (F7, OWNER-GATED, D-bk) | B8c, B4b, B5 | (serialized; independent of B8h/B8i) |
...
**Critical path**: B2 → B3 → B4 → B4b → **{B5, B6}** → B8a → B8b/B8c → **{B8d → B8e → B8f → B8g →
B8h → B8i → B8j}** → B12 (the whole program — with React/Astro deferred and Angular extracted there
are no parallel vertical lanes; B8b ∥ B8c is the only post-substrate parallelism, and the Svelte
feature-completion blocks B8d–B8j are logically parallel-safe after B8c but SERIALIZED at integration
because each touches the Svelte IDE projection module + prelude/shim assets + TSGO fixtures, D-bl).
The feature-completion blocks PROMOTE every OUT-OF-SCOPE-v1 Svelte matrix row to SUPPORTED (D-bi);
B8h/B8i/B8j are OWNER-GATED (the owner-decision list below). B8a requires BOTH B5 (registry rows, synth seam,
...
**Owner decisions required before the OWNER-GATED feature-completion blocks dispatch (D-bi codex
consult, `/tmp/verter-mf/svelte-completion-codex-consult.txt`)**:
- **B8i await (D-bj)** — approve reversing the D-ad/D-bg await OUT-OF-SCOPE ruling and option (b)
  (real await projection + an INFORMATIONAL experimental diagnostic until Svelte 6 / config-detect).
- **B8j rune modules (D-bk)** — approve reversing the D-bg ".svelte.ts/.svelte.js is not a carrier"
  ruling and funding the NON-COMPONENT rune-module carrier substrate (new classification row +
  prelude+real-bytes import-resolution virtual file + the `.svelte.js` JS-valid prelude story).
- **B8h legacy `on:` payload (F13)** — decide whether legacy component-event support must be
  PAYLOAD-PRECISE (funds the API-visible `$events` event-map extension) or ships projection-only; the
  loose `CustomEvent<any>` is rejected either way.
- **B8f F9 `<svelte:fragment>` slot-name** — decide whether slot-NAME exactness is required (funds an
  API `$slots` contract) or projection-only fragment support is sufficient (the matrix default).
- **B8g F12** — approve the deliberate `any` anti-`any`-gate exception for the legacy
  `$$props`/`$$restProps`/`$$slots` magic objects (Svelte's legacy magic is intrinsically loose).
...
- **Symbol relocation table** (resolves the move/stay/delete ambiguity — `resolve_type/` is
  ...
  | `analyze_external_type_program`, `analyze_external_type_source`, `extract_export_surface`, `ExtractedExportSurface`, `extract_imported_type_bindings`, `hash_resolved_type`, `collect_required_import_names_for_external_type`, `imported_member_name_for_required_alias`, `required_import_alias_names_for_binding`, `resolve_external_type_with_companion{,_and_canonical}`, `resolve_external_type_in_program_with_analyzed_symbol_companion{,_and_canonical}`, `resolve_external_type_in_context_with_analyzed_symbol_companion{,_and_canonical}` (amended in B3's PR — discovered: the rest of the external-analysis public surface) | `script/type_surface/external.rs` (consumed by non-Vue script analysis; moved with the file) |
  | `merge_overload_groups`, `PARSER_SYNTACTIC_DEPTH_LIMIT`, `ResolutionDiagnostic{,Kind}` siblings' private machinery, `ShallowResolvedElements` (internal; its `emits` field renames to `call_signatures` alongside the public field) (amended in B3's PR — discovered: remaining module-internal surface) | `script/type_surface/` (default move — every unlisted `resolve_type/` symbol followed its file) |
  | NEW neutral inventory API from the `bindings.rs` split: `collect_pattern_binding_spans`, `collect_import_binding_spans`, `declaration_binding_span`, `callee_identifier_name` (amended in B3's PR — the generic import/decl/pattern inventory the thinned Vue classifier delegates to) | `script/bindings.rs` |
...
**Changes.**
- NEW `crates/verter_language/src/parse_artifact.rs` (D-g):
  `FrameworkParseArtifact { adapter_id, language_id, parser_version, common: FrameworkParseCommon, carrier: Arc<dyn CarrierParse> /* private */ }`;
  `FrameworkParseCommon { script_regions: Vec<ScriptRegion>, template_regions, style_regions, external_links, diagnostics: Vec<LanguageDiagnostic> }` (NO
...
- **Carrier privacy (D-m — compiles across crates, unlike `pub(crate)`)**: `verter_language`
  exposes a PUBLIC
  `#[doc(hidden)] __carrier_downcast_ref::<T>(artifact, token: &CarrierAccessToken) -> Option<&T>`
  (+ `Arc` form) that checks `artifact.adapter_id ==
...
- The Vue artifact is populated at the existing parse point
  ...
  `ParsedSfc` in `VueParseCarrier`; a typed accessor
  `vue_parse(&FrameworkParseArtifact) -> Option<&Arc<ParsedSfc>>` in the live session Vue adapter module
...
- The `crates/verter_session/src/framework/` substrate (D-q; joins B2's
  ...
  `FrameworkAdapterDescriptor { id: FrameworkAdapterId, tag: FrameworkTag, supported_surfaces: &'static [FrameworkSurfaceKind], carrier_language: Option<LanguageId>, virtual_file_naming: Option<VirtualFileNaming> (D-x) }`;
  `registry.rs` — `FrameworkAdapterRegistry` built once at
...
- **TWO-STAGE `ScriptFactProvider` seam (D-o)**: STAGE 1 — `ScriptFactProvider` trait
  ...
  `by_carrier_language: Map<LanguageId, ProviderList>` and
  `by_import_specifier: Map<InternedSpecifier, ProviderList>`. The per-file active set is COMPUTED EMPTY before any
...
- Component-default synthesis seam (D-n, parse-domain per D-au): NEW `framework/synth.rs` — trait
  `ComponentDefaultSynth { synthesise(cx: ComponentDefaultSynthCtx<'_>) -> Option<ShallowValueSymbol> }`
  with
  `ComponentDefaultSynthCtx { canonical_id, language, script_analysis: &ScriptAnalysisSnapshot, script_candidates: &FrameworkScriptCandidateSet, framework_parse: Option<&FrameworkParseArtifact> }`
  — every field PARSE-DOMAIN (D-au: synth
...
- **Svelte v1 scope matrix (current-docs audit — D-ad; extends and supersedes the D-u
  ...
  (`svelte.dev/docs/svelte/*`) on **2026-06-10**; audited Svelte version: **5.56.10** (latest
  ...
  **Event handling + attributes** (docs: `/basic-markup`, `/class`):
  ...
  | `style:` directive (+ `\|important` modifier) | SUPPORTED (B8d, F1, D-bl) | directive recorded + stripped from the JSX attribute position (a `style:`-prefixed name is not a valid JSX identifier — mirrors the custom-property pass-through); value expression void-checked (hover/diagnostics survive); shorthand `style:color` projects the implied identifier only when the name is a valid binding identifier; `\|important` presentational (no type effect) — NO diagnostic. (v1 OUT-OF-SCOPE was parse-without-crash + typed-unsupported diagnostic, superseded) |
  ...
  **Bindings** (docs: `/bind`):
  ...
  | `bind:group`, `bind:files`, `bind:this`, contenteditable/details/media/dimension/readonly bindings; `defaultValue`/`defaultChecked` interplay (5.6) | SUPPORTED (B8e, F4, D-bl) | resolved through a GENERATED CLOSED bind-contract table keyed by directive name (+ element/tag constraint) — the source of truth computing value type + direction (read/write/read-write); `bind:this` projects as an assignment-compatibility check (intrinsic tags via DOM element-instance maps, components via `InstanceType<typeof C>`, dynamic/unknown fallback `Element`); `bind:group` inspects checkbox-vs-radio + value type; media/dimension/contenteditable/`<details>` bindings each carry their typed read/write contract; `defaultValue`/`defaultChecked` are plain attributes typed by `SvelteHTMLElements`. (v1 OUT-OF-SCOPE was parse + void-check + diagnostic, superseded) |
  | Function bindings `bind:x={get, set}` (5.9; readonly `{null, set}`) | SUPPORTED (B8e, F5, D-bl) | NEW prelude checker checks get/set mutual consistency PLUS the actual bind-target type: element binds instantiate `V` from the generated bind table; component binds derive `V` in TS from `InstanceType<typeof Child>["$props"][K]` (or a bindable-prop marker on the API file) — NO Rust resolver call; `{null, set}` is the write-only form. (v1 OUT-OF-SCOPE was parse + void-check + diagnostic, superseded) |
  ...
  **Directives** (docs: `/use`, `/transition`, `/animate`):
  ...
  | `transition:` / `in:` / `out:` (+ params + `\|local`/`\|global` modifiers) | SUPPORTED (B8d, F2, D-bl) | NEW prelude checker `__verter_transition` over the `svelte/transition` `TransitionConfig` signature; node param = the HOST ELEMENT instance type (`E extends Element`, fallback `Element` for unknown/dynamic hosts); required/omitted-param overloads so a missing required `params` FAILS; directive stripped + checker spread-merged; `\|local`/`\|global` presentational. (v1 OUT-OF-SCOPE was parse + strip + diagnostic, superseded) |
  | `animate:` (+ params) | SUPPORTED (B8d, F3, D-bl) | NEW prelude checker `__verter_animate` over the `svelte/animate` `AnimationConfig` signature (`(node, { from: DOMRect, to: DOMRect }, params) => AnimationConfig`); directive stripped + checker appended; the "must be keyed-each child" structural rule is at most a cheap syntax diagnostic (NO resolver work). (v1 OUT-OF-SCOPE superseded) |
  ...
  **Special elements** (docs: `/svelte-window`, `/svelte-document`, `/svelte-body`,
  ...
  | `<svelte:options>` — `runes`, `namespace`, `customElement`, `css` (legacy `immutable`/`accessors` deprecated) | SUPPORTED (parse + flags; namespace via B8f, F10, D-bl) | parsed + recorded; the `runes` flag drives runes-vs-legacy classification; `customElement`/`css` recorded with NO v1 typing effect; a non-html `namespace` (svg/mathml) SELECTS a separate shim entrypoint — `@verter/svelte-jsx/svg` (`IntrinsicElements extends SVGElements`) / `/mathml` (`MathMLElements`) — via the per-file pragma (NO per-element switching inside one JSX namespace; the v1 typed-unsupported diagnostic is superseded) |
  | `<svelte:component>` / `<svelte:self>` (deprecated in runes mode) | SUPPORTED (B8f, F8, D-bl) | TS helper checks over class-shaped component values + `InstanceType<C>["$props"]`; `<svelte:self>` checks against a LOCAL self-component contract generated from the current component's props; NO component-metadata resolution in the projector. (v1 OUT-OF-SCOPE superseded) |
  | `<svelte:fragment>` (legacy slot construct) | SUPPORTED (B8f, F9, D-bl) | children projected unwrapped; the `slot` literal preserved/void-checked; slot-NAME precision is NOT claimed unless an API `$slots` contract is added (owner-gated extension). (v1 OUT-OF-SCOPE superseded) |
  ...
  **Await expressions** (docs: `/await-expressions` — EXPERIMENTAL, 5.36+, opt-in via
  ...
  | Surface | Status | Design / exact behavior |
  |---|---|---|
  | `await` at instance-script top level / inside `$derived(...)` / in markup expressions | SUPPORTED (B8i, F6, D-bj; OWNER-GATED) | REAL await projection in all three positions: markup `{await e}` → checkable `await (e)` against `Awaited<typeof e>`; `$derived(await …)` keeps the verbatim rune call site with the inner await void-checked; top-level `<script>` await → module-level top-level `await` (the projected `.svelte.tsx` is an ES module — TS allows it under `module: esnext` / `target: es2022`). An INFORMATIONAL experimental diagnostic replaces the v1 typed-unsupported `svelte-await-experimental` diagnostic; it is suppressed when the project targets Svelte 6 or an approved `experimental.async` config-detection surface exists (config-detection = named follow-up). Companions still type-check via rows above: `<svelte:boundary pending>`, `$effect.pending`, `settled()`. (v1 OUT-OF-SCOPE superseded by D-bj — owner go/no-go required; the async-FAMILY imports `fork`/`getAbortSignal`/`settled` stay transparent `svelte` imports, D-bg item 3 unchanged) |
  ...
  **Modules + runtime surfaces** (docs: `/svelte-js-files`, `/stores`, `/context`,
  ...
  | Surface | Status | Design / exact behavior |
  |---|---|---|
  | `.svelte.ts` / `.svelte.js` rune modules | SUPPORTED (B8j, F7, D-bk; OWNER-GATED) | first-class NON-COMPONENT rune-module carrier — a NEW `verter_language` classification row matched BEFORE the plain `.ts`/`.js` rows; NO PROPS/EMITS/SLOTS/EXPOSE/MODEL surface, NO synth, NO `PlannedDemand`, NO component api-projector. The rune ambient prelude is prepended (unmapped, offset 0) so module-scope rune calls type-check; the module's IMPORT-RESOLUTION virtual file resolves to PRELUDE + REAL module bytes so exported rune-derived types (`export const s = $state(0)` → `number`) infer correctly. `.svelte.js` renders the prelude in a JS-valid ambient form (not inline `declare`). REVERSES the D-ad/D-bg ".svelte.ts is not a carrier" ruling (D-bk — owner go/no-go required). |
  | Stores + `$x` auto-subscription | SUPPORTED (B8g, F11, D-bl) | a SYNTACTIC rewrite via a `Readable<T>`/`Writable<T>` store-get helper — but NOT every `$x` blindly: EXCLUDES the prelude runes (`$props`/`$state`/…) and the `$$` magic, respects lexical declarations (the typed-IR knows which `$x` are store-subs), and for sourcemap accuracy rewrites ONLY the `$` byte / operator spans via `CodeTransform::overwrite` while preserving the original identifier/RHS bytes. (v1 OUT-OF-SCOPE superseded) |
  ...
  | `$$props` / `$$restProps` / `$$slots` | SUPPORTED (B8g, F12, D-bl; legacy-mode only) | prelude declarations (`$$props`/`$$restProps: Record<string, any>`, `$$slots: Record<string, boolean>`); the loose `any` is an EXPLICIT documented anti-`any`-gate exception ONLY for the legacy magic object itself (Svelte's legacy magic is intrinsically loose). (v1 typed-unsupported diagnostic superseded) |
  ...
  | Legacy component `on:` payload checking | SUPPORTED (B8h, F13, D-bl; OWNER-GATED — payload-precise) | payload-precise checking requires an API-VISIBLE event map (a `$events` contract on the component API file) + a TS helper projection against `EventsOf<typeof Child>[K]`, sourced WITHOUT projector-time resolver dispatch. A loose `CustomEvent<any>` projection is REJECTED (codex Q5/Q7 — `any` defeats payload checking, must not ship as "fully functional"). Owner go/no-go on funding the `$events` API-surface extension. (v1 typed-unsupported diagnostic superseded by the precise design) |
...
**Svelte feature-surface scope refresh (B8) — pre-execution re-audit (D-bg).** Re-audited against
the LIVE official documentation (`svelte.dev/docs/svelte/*` + `github.com/sveltejs/svelte/releases`
+ `packages/svelte/CHANGELOG.md`) on **2026-06-13**, immediately before B8 execution, per the owner
RE-SCOPE directive (current Svelte docs surfaces — `{@attach …}`, await-expression behavior, any
post-runes additions — must be EXPLICITLY supported or EXPLICITLY out-of-scope). **Audited latest
stable: `svelte@5.56.10`** (UNCHANGED from the D-ad 2026-06-10 audit; declaration tags landed
5.56.0, and `5.56.x` remains the head of the published changelog). The D-ad matrix above stands
materially correct; the refresh confirms every owner-named surface and records a small set of
ADDITIVE coverage items + one decision row. A future Svelte version bump re-runs BOTH this audit and
the D-ad matrix before any scope claim changes.

> **END-STATE SUPERSESSION (D-bi/D-bj/D-bk/D-bl, owner scope expansion 2026-06-14).** The
> OUT-OF-SCOPE-v1 verdicts in BOTH the binding matrix above and the refresh table below record the
> correct INTERMEDIATE (B8a/B8b/B8c) state. They are PROMOTED to SUPPORTED by the feature-completion
> blocks B8d–B8j (D-bl). In particular: await-expressions (refresh-table item 2 / matrix await row) →
> SUPPORTED with real await projection + an informational diagnostic (B8i, D-bj, owner-gated); the
> `.svelte.ts`/`.svelte.js`-is-NOT-a-carrier ruling (refresh-table item 4 / matrix modules row) →
> REVERSED to first-class non-component rune-module carriers (B8j, D-bk, owner-gated). The
> async-FAMILY runtime imports (refresh-table item 3: `fork`/`getAbortSignal`/`createContext`) stay
> transparent `svelte` imports — only the await SYNTAX flips.

Codex architect adjudication (xhigh, read-only;
`/tmp/verter-mf/svelte-scope-codex-consult.txt`) — boundary calls:

| Feature (Svelte version) | Doc citation | In/Out v1 | Adjudicated boundary + substrate mapping |
|---|---|---|---|
| `{@attach expr}` (5.29, stable) | `/docs/svelte/@attach` | **IN-SCOPE** (CONFIRM) | Element vs component split is the correct projection boundary. ELEMENT: IDE projection void-checks the expression against `Attachment<E>` for the target intrinsic element. COMPONENT: Svelte's Symbol-keyed attachment prop — projected/parsed as a pass-through, **NOT** published as named PROPS metadata (it is a call-site mechanism, not a component API member). Substrate: carrier parse + IDE projection ONLY — no `ScriptFactProvider` candidate, no synth, no `PlannedDemand`, no api-projector, no surface-kind demand. Keep both element + component fixtures. |
| await expressions — top-level `<script>`, `$derived(await …)`, markup `{await …}` (5.36+, experimental, opt-in `experimental.async`, flag removed in Svelte 6) | `/docs/svelte/await-expressions` | **OUT-OF-SCOPE v1** (CONFIRM) | Experimental + opt-in + breaking-change-reserved (docs: details "subject to breaking changes outside of a semver major release"). Landed behavior EXACTLY: parse-without-crash in all 3 positions; project the awaited expression void-checked (inner hover/diagnostics survive); emit a per-await-position typed-unsupported diagnostic (slug `svelte-await-experimental`). NOT a known-bug ledger row. Revisit gate: surface stabilisation (flag removal / Svelte 6). |
| `fork(...)` async-family API (5.42) | `/docs/svelte/await-expressions` (Forking section) | **OUT-OF-SCOPE v1** (async semantics) / transparent import | Same experimental-async family as await. An ordinary `svelte` import (`.commit()`/`.discard()`); NO template/component-typing syntax. No adapter candidate, synth, surface kind, or projector special-case. Covered by the transparent-`svelte`-import disposition; the async SYNTAX it powers stays out-of-scope. → decision row D-bg. |
| `getAbortSignal()` (5.35) | `/docs/svelte/lifecycle-hooks` / svelte API | **IN-SCOPE** (transparent) | Ordinary `svelte` import typed by the package; zero adapter work — already covered by the "Lifecycle / context / imperative APIs (transparent)" row above. |
| `createContext()` (5.40; programmatic-instantiation extension 5.50) | `/docs/svelte/context` | **IN-SCOPE** (transparent) | Ordinary `svelte` context import typed by the package; covered by the existing transparent context/runtime row. No matrix change. |
| `{@html expr}` accepting `TrustedHTML` (5.52) | `/docs/svelte/basic-markup` (`@html`) | **IN-SCOPE** (CONFIRM; typing widening) | A widening of the already-SUPPORTED `{@html}` row — the expression is already projected into a string-accepting checkable position, which a `TrustedHTML` value satisfies. ADDITIVE: a B8c IDE fixture asserting a `TrustedHTML`-typed argument is accepted (no regression of the string case). |
| comments in tags (5.53) | `/docs/svelte/basic-markup` ; changelog 5.53.0 | **IN-SCOPE** (parser tolerance) | Source-grammar tolerance — comments inside element open tags. Part of the carrier parse-without-crash contract. ADDITIVE: an explicit B8a parser parse-without-crash fixture row. |
| `.svelte.ts` / `.svelte.js` rune modules | `/docs/svelte/svelte-js-files` | **OUT-OF-SCOPE v1** (CONFIRM) | Classify as plain `FileLanguage::Script`, serve the REAL file, no carrier participation; rune identifiers resolve through the user's own ambient setup. The classifier matches the carrier row by the `.svelte` suffix, so a `.svelte.ts` path ends with `.ts` and falls through to the plain-script `.ts` row STRUCTURALLY (`.svelte.ts → Script(Ts)`, `.svelte.js → Script(Js)`); B4b's threaded script dialect is the right substrate. No v1 metadata reason requires carrier participation — a `.svelte` component importing a type/value from `.svelte.ts` for `$props<T>()` resolves it as a normal script dependency. ADDITIVE: a B8a discriminating guard fixture asserting `.svelte.ts` is NOT classified as a carrier (none exists yet). |
| function-valued `css` / `runes` / `customElement` compiler options (5.54) | changelog 5.54.0 ; `/docs/svelte/svelte-options` | **OUT-OF-SCOPE** (compiler/config domain) | Compiler-option config, not a component-template typing surface. The existing `<svelte:options>` parse-and-record row is sufficient; no B8 work. |
| customizable `<select>` elements (5.47) | `/docs/svelte/basic-markup` ; changelog 5.47.0 | **IN-SCOPE** (intrinsic typing nuance) | An intrinsic-element typing nuance against the `SvelteHTMLElements` JSX namespace. ADDITIVE: a B8c IDE/intrinsic fixture if not already covered by the generic intrinsic-element projection. |

**Refresh dispositions (B8 sub-tasks + the decision row):**
- B8a ADDITIVE fixtures: (1) comments-in-tags parse-without-crash; (2) a discriminating
  `.svelte.ts`-is-NOT-a-carrier classification guard. Both fold into B8a Test 1 (parser
  round-trips) / Test 6 (out-of-scope-row behavior) — no new test family.
- B8c ADDITIVE fixtures: (3) `{@html}` accepting a `TrustedHTML`-typed argument; (4) customizable
  `<select>` intrinsic projection — both fold into the B8c IDE TSX projection tests.
- **Decision row D-bg** (recorded in §3): async `await` SYNTAX is OUT-OF-SCOPE v1 (parse-without-
  crash + `svelte-await-experimental` typed-unsupported diagnostic per await-bearing position); the
  async-FAMILY runtime imports (`fork`, `getAbortSignal`, `settled`, `$effect.pending` decl-only)
  are transparent `svelte` imports with no adapter contact. Prevents async-family scope creep into
  B8b/B8c.
- No OWNER decision required: every owner-named surface (`{@attach}`, await-expression behavior,
  post-runes additions) is now EXPLICITLY dispositioned. The matrix is COMPLETE for B8 v1 after the
  four additive fixtures above; no `FrameworkSurfaceKind` change and no wire-schema change is
  implied by the refresh.
...
- NEW `crates/verter_session/src/typeinfo/adapters/svelte/{mod.rs, adapter.rs}`:
  ...
  snippet-typed props + legacy `<slot>` inventory; EMITS = legacy `createEventDispatcher<E>`
  event map ONLY, provenance-validated from the `svelte` package (NOT a bare-name match;
  ...
  `SvelteScriptFacts`/`SvelteScriptCandidates` (the SAME parse-domain facts the synth consumed)
  and dispatches the captured `TypeExpr`(s) through the SHARED resolver — `Instantiate`/
  decl-lowering in `Navigate` + `project_shallow_surface_from_base(… published(Shallow))`, or
...
2. Negative: runes-mode `onClose` callback in PROPS, absent from EMITS. Negative: a userland
   `createEventDispatcher` look-alike does NOT contribute EMITS (provenance-validated from
   `svelte`). Negative: a userland `Snippet` look-alike does NOT contribute SLOTS (the B8a P2
   typed-resolved-package-identity fix).
3. Known-bug rows for surfaced pre-existing typeinfo gaps.
4. Corpus rows under `crates/verter_session/tests/framework_corpus/svelte/` — HERMETIC, locally-
   authored `.svelte` fixtures (runes props incl. the runes-callback-stays-PROPS row, legacy
   `export let`, `$bindable()` MODEL, instance EXPOSE, pure markup) resolved end-to-end through the
   registered adapter by `framework_corpus_svelte.rs`. (The `framework_corpus/` shared generator the
   React/Astro/Angular deferred blocks reference does not yet exist; B8b vendors the fixture set +
   a per-corpus round-trip test directly, the hermetic substance of the requirement, and the shared
   generator is built when the first deferred vertical that needs it lands.)
5. Deterministic duplicate-slot-name merge across the two SLOTS source rows (first-writer-wins).

Note (D-bh): `SvelteScriptFacts` (the resolved-validation payload) is extended to carry the
source-family data the executor's `SvelteSurface` arm needs — `props_type`, `bindable_members`,
`validated_snippet_members`, `legacy_props`, `dispatcher_events`, `instance_exports` — provenance-
validated where applicable (`dispatcher_events` only when `createEventDispatcher` resolved to the
`svelte` package; `validated_snippet_members` already so). The B8a P2 typed-resolved-package
identity (a typed `package` column on `ResolvedImportTarget`, session-computed) replaces the
`node_modules/svelte` path substring so the provider tests `target.package == "svelte"`
structurally.
...
- NEW `crates/verter_compiler/src/svelte/ide/`: `ParsedSvelte` → TSX via `CodeTransform`:
  ...
  **runes typing via the per-file ambient prelude (D-u, completed per D-ad)** — the same prelude
  ...
  the snippet-brand declarator
  `__verter_snippet<Params extends unknown[]>(render: (...args: Params) => unknown): Snippet<Params>`
  (D-ae(c));
  ...
  `__verter_attach` checker argument (spread-merged); object/array-valued `class` attributes → `ClassValue`
...
- **The `@verter/svelte-jsx` shim asset (D-ae, home/distribution per D-av)**: NEW
  ...
  with a byte-pin freshness test (`crates/verter_session/tests/cases/svelte_jsx_shim_freshness.rs`, the
...
- LSP: watcher/selector/virtual-file wiring for `.svelte` from the D-x naming column — IDE file
  ...
  `crates/verter_semantic/src/resolver_core/provider_projection_resolution.rs` (`provider_id_for_source` /
...
3. **Type-check validity gate (D-u — OXC parse-only is NOT sufficient)**: GATE PRECONDITION
   ...
   object-valued `class` fixture with an `@ts-expect-error` on a non-`ClassValue` payload; D-ae
...
6. D-x/D-al naming characterization against the PRODUCTION derivations: the Vue
   `VirtualFileNaming`
   row reproduces `crates/verter_semantic/src/resolver_core/provider_projection_resolution.rs` (`provider_id_for_source` —
   `{canonical}.ts` api) and (`provider_ide_id_for_source` — `{canonical}.tsx`/`.jsx`
...
## Svelte Feature-Completion Blocks (B8d–B8j, D-bi/D-bl)

Per the owner scope expansion (D-bi), every Svelte matrix row tagged OUT-OF-SCOPE v1 (D-ad + D-bg)
is PROMOTED to SUPPORTED by the blocks below. Each REUSES the existing B8a parse (the parser already
round-trips every row without crash) and the B8c IDE substrate (prelude, void-check pattern,
prelude-checker pattern, `@verter/svelte-jsx` shim, CodeTransform-only mutation), replacing the
typed-unsupported diagnostic with a real projection/typeinfo that TYPE-CHECKS CLEAN through the TSGO
gate. NO block adds a second resolver, no projector dispatches a semantic query, no
`FrameworkSurfaceKind`/wire change is implied (D-bi Q8). They land AFTER B8c, BEFORE B12; serialized
at integration (every block touches the Svelte IDE projection module + prelude/shim assets + TSGO
fixtures). B8h/B8i/B8j are OWNER-GATED (see each block's Dependencies + the owner-decision list in
§7).

### B8d — Svelte directives + styling completion (F1–F3)

**Context.** Flip the `style:`, `transition:`/`in:`/`out:`, and `animate:` matrix rows
OUT-OF-SCOPE→SUPPORTED (D-bl). All three are IDE-projection-only — no typeinfo/component-API surface.

**Changes.**
- `crates/verter_compiler/src/svelte/ide/`: F1 `style:color={c}` / `style:color|important` — directive
  recorded + STRIPPED from the JSX attribute position (a `style:`-prefixed name is not a valid JSX
...
- `@verter/svelte-jsx` shim (D-av) EXTENDED: ensure `svelte/transition` (`TransitionConfig`) +
  ...
  imports use; no new asset, only the new checker declarations referencing them.

**Legacy Deletions.** The B8c `style:`/`transition:`/`animate:` typed-unsupported diagnostic emission
+ their out-of-scope-row behavior assertions (superseded by the positive projection tests).
...
2. Type-check validity (TSGO gate, hermetic vendored `svelte` — `svelte/transition` +
   `svelte/animate` d.ts with NAMED param interfaces): a `transition:fly` fixture with
   a `{ delay: 200 }` value
...
**Dependencies.** B8c. Serialized after B8c.

### B8e — Svelte bindings completion (F4–F5)

**Context.** Flip the wide `bind:` family + function-bindings rows OUT-OF-SCOPE→SUPPORTED (D-bl).
IDE-projection-primarily; F5 touches MODEL only at the consumption site (no metadata change).

**Changes.**
- A GENERATED CLOSED bind-contract table (the closed Svelte-documented binding-name vocabulary) keyed
  by directive name (+ applicable element/tag constraint) is the SOURCE OF TRUTH, computing per
  binding: value type + direction (read / write / read-write). The one generic checker is an
  implementation helper, not the authority.
- `crates/verter_compiler/src/svelte/ide/`: F4 — `bind:this={el}` projects as an ASSIGNMENT-
  COMPATIBILITY check (intrinsic tags via DOM element-instance maps, components via
  `InstanceType<typeof C>`, dynamic/unknown fallback `Element`/component-base) — NOT a
  `JSX.IntrinsicElements` attribute; `bind:group` inspects checkbox-vs-radio + value type (the loose
  `T | T[]` is rejected); `bind:files` (`FileList | null`); media bindings
  (`bind:currentTime`/`duration`/`paused`/… against `HTMLMediaElement[prop]`, readonly ones
  read-direction); dimension bindings (`bind:clientWidth`/… readonly `number`); contenteditable
  bindings (`bind:innerHTML`/`innerText`/`textContent` → `string`); `<details bind:open>` (`boolean`);
  `defaultValue`/`defaultChecked` as plain attributes typed by `SvelteHTMLElements`.
- F5 — function bindings `bind:x={get, set}` (and write-only `{null, set}`) → a prelude checker that
  checks get/set mutual consistency AND the actual bind-target type: element binds instantiate `V`
  from the generated bind table; component binds derive `V` in TS from
  `InstanceType<typeof Child>["$props"][K]` (or a bindable-prop marker on the API file) — NO Rust
  resolver call.

**Legacy Deletions.** The B8c wide-`bind:`/function-binding typed-unsupported diagnostics +
out-of-scope-row assertions.

**Tests (failing first).**
1. A whole-table destructure unit test (no `..`) over the generated bind-contract table forces a
   conscious decision on any added binding name.
2. TSGO validity: `bind:this` on an `<input>` binds an `HTMLInputElement` var (`@ts-expect-error` on
   a wrong element type); `bind:this` on a component binds its `InstanceType`; `bind:group` on a
   checkbox vs radio checks the correct value/array shape; `bind:currentTime` binds `number`; a
   read-only `bind:duration` assignment-target FAILS to be written; F5 get/set mismatch FAILS; F5
   component bind against `InstanceType<typeof Child>["$props"][K]` checks the bindable prop type.
3. Sourcemap e2e for a bound expression.

**Verification.** The gate (TS touched, TSGO).

**Dependencies.** B8c. Serialized after B8d.

### B8f — Svelte special elements + namespaces (F8–F10)

**Context.** Flip `<svelte:component>`/`<svelte:self>`, `<svelte:fragment>`, and the
`<svelte:options namespace="svg|mathml">` rows OUT-OF-SCOPE→SUPPORTED (D-bl).

**Changes.**
- F8 `<svelte:component this={C} {...props}>` → a TS helper check over class-shaped component values
  + `InstanceType<C>["$props"]` (a prelude checker `__verter_dynamic_component`); `<svelte:self>`
  checks against a LOCAL self-component contract generated from the current component's own props —
  NO component-metadata resolution in the projector.
...
**Tests (failing first).**
1. TSX projection: no `<svelte:component`/`<svelte:fragment` residue (negative); fragment children
   unwrapped.
2. TSGO validity: a dynamic `<svelte:component this={C} prop={x}>` with a wrong `prop` FAILS; a
   non-component `this` FAILS; `<svelte:self>` with a wrong self-prop FAILS; an svg-namespace
   component type-checks its svg intrinsics (`@ts-expect-error` on an html-only attribute proving
   the svg table is in effect); `<svelte:fragment slot="x">` children type-check.
3. The `@verter/svelte-jsx/svg` + `/mathml` byte-pin freshness tests.

**Verification.** The gate (TS touched, TSGO).

**Dependencies.** B8c. Serialized after B8e.

### B8g — Svelte legacy identifiers + runtime (F11–F12)

**Context.** Flip the stores `$x` auto-subscription row + the `$$props`/`$$restProps`/`$$slots` row
OUT-OF-SCOPE→SUPPORTED (D-bl). Legacy-mode-primarily.

**Changes.**
- F11 stores `$x` → a SYNTACTIC rewrite via a `Readable<T>`/`Writable<T>` store-get prelude helper —
  NOT every `$x` blindly: EXCLUDE the prelude runes (`$props`/`$state`/`$derived`/`$effect`/`$inspect`/
  `$host`/`$bindable`) and the `$$` magic (F12); respect lexical declarations (the parsed typed-IR
  classifies which `$x` are store-subs vs runes vs magic — structural, not nominal). For sourcemap
  accuracy, rewrite ONLY the `$` byte / operator spans via `CodeTransform::overwrite`, preserving the
  original identifier/RHS bytes; a `$store` reference → store-get, a `$store = v` assignment →
  `store.set(v)` checked.
- F12 `$$props`/`$$restProps`/`$$slots` → prelude declarations
  (`$$props`/`$$restProps: Record<string, any>`, `$$slots: Record<string, boolean>`), legacy-mode only;
  the loose `any` is an
  EXPLICIT, documented anti-`any`-gate exception ONLY for the legacy magic object itself.

**Legacy Deletions.** The B8c stores-`$x`/`$$props` typed-unsupported diagnostics + out-of-scope
assertions.

**Tests (failing first).**
1. TSGO validity: a `$count` store-sub fixture reads `number` from `Writable<number>`; `$count = 5`
   checks the writable contract; a store-sub against a non-store FAILS; a `$$props` legacy fixture
   type-checks (the documented `any` exception — NO `@ts-expect-error` anti-`any` guard on the magic
   object); `$$slots.foo` checks `boolean`.
2. Sourcemap e2e: hover on the identifier of a rewritten `$store` lands on the original identifier
   bytes (the rewrite touched only the `$` span).
3. A discriminating test that the rune `$state` / `$$props` magic are NOT rewritten as store-subs
   (negative — the classifier excludes them).
...
**Context.** Flip the await-expressions row OUT-OF-SCOPE→SUPPORTED with REAL await projection +
an INFORMATIONAL experimental diagnostic (option (b), D-bj). OWNER-GATED: it reverses the D-ad/D-bg
await ruling.

**Changes.**
- `crates/verter_compiler/src/svelte/ide/`: REAL await in all three positions — markup `{await e}` →
  checkable `await (e)` against `Awaited<typeof e>`; `$derived(await …)` keeps the verbatim rune call
  site with the inner await void-checked; top-level `<script>` await → module-level top-level `await`
  (the projected `.svelte.tsx` is an ES module; the projection ensures `module: esnext` /
  `target: es2022` is in effect for the projected file so top-level await is valid).
- Replace the per-position `svelte-await-experimental` TYPED-UNSUPPORTED diagnostic with an
  INFORMATIONAL diagnostic; suppress it when the project targets Svelte 6 or an approved
  `experimental.async` config-detection surface exists (config-detection = named follow-up, not a
  blocker).
- The async-FAMILY runtime imports (`fork`/`getAbortSignal`/`settled`/`$effect.pending` decl-only)
  STAY transparent `svelte` imports (D-bg item 3 unchanged).

**Legacy Deletions.** The B8c `svelte-await-experimental` typed-unsupported diagnostic emission (all
3 positions) + the out-of-scope-row assertion.

**Tests (failing first).**
1. TSGO validity: a markup `{await fetchUser()}` projects real await + type-checks `Awaited<…>`
   (`@ts-expect-error` on a non-promise where awaited type is asserted); `$derived(await …)`
   type-checks; top-level `<script>` await type-checks under the module/target config.
2. The diagnostic is INFORMATIONAL (not error-severity) and is suppressed under a Svelte-6-target /
   config-detection fixture.
3. Negative: the async-FAMILY imports still type-check transparently (no adapter contact).

**Verification.** The gate (TS touched, TSGO).

**Dependencies.** B8c. OWNER GO/NO-GO (D-bj). Serialized after B8h.

### B8j — Svelte rune modules `.svelte.ts`/`.svelte.js` (F7, OWNER-GATED, D-bk)

**Context.** Promote `.svelte.ts`/`.svelte.js` rune modules to first-class NON-COMPONENT carriers —
REVERSES the D-ad/D-bg ".svelte.ts is not a carrier" ruling (D-bk). OWNER-GATED: it reverses D-bg and
extends routing/virtual-file semantics.

**Changes.**
- `crates/verter_language`: a NEW rune-module classification row matched BEFORE the plain `.ts`/`.js`
  rows (a `.svelte.ts` path ends `.ts` and otherwise falls through to plain-script structurally —
  D-bg item 4's exact reason). The row carries the SAME `ScriptSourceType` (`.svelte.ts → Ts`,
  `.svelte.js → Js` per the B4b dialect substrate). It is NOT a component carrier: NO PROPS/EMITS/
  SLOTS/EXPOSE/MODEL surface, NO `ComponentDefaultSynth`, NO `PlannedDemand`, NO component
  `ComponentApiProjector`.
- IDE/import projection: the rune ambient prelude prepended (unmapped, offset 0) so module-scope rune
  calls type-check; the module's IMPORT-RESOLUTION virtual file resolves to PRELUDE + REAL module
  bytes so exported rune-derived types (`export const s = $state(0)` → `number`) infer correctly in a
  consuming module. The rune module's `VirtualFileNaming` is NOT the component dual-file model — its
  ide-surface and import-surface are the SAME prelude+content TS module (a NEW `VirtualFileNaming`
  shape / classifier disposition, recorded in the registry).
- `.svelte.js` source-type-specific story: a TS `declare`-form prelude is NOT valid JS, so the
  `.svelte.js` rune prelude is rendered in a JS-valid ambient form (a JSDoc / sidecar `.d.ts`
  declaration the JS file references, or injected via the host-owned typing channel) — not inline
  `declare function`.

**Legacy Deletions.** The B8a discriminating guard fixture asserting `.svelte.ts` is NOT classified
as a carrier (D-bg ADDITIVE fixture) — RETIRED; replaced by a positive rune-module-carrier
classification test. The D-bg ".svelte.ts is not a carrier" disposition (item 4) is retired (D-bk).

**Tests (failing first).**
1. Classification: a `.svelte.ts`/`.svelte.js` path classifies as the rune-module carrier (NOT plain
   `Script`, NOT a component carrier); a plain `.ts`/`.js` still classifies plain-script (negative);
   a `.svelte` component still classifies the component carrier (negative).
2. TSGO validity: a `.svelte.ts` using `$state`/`$derived` at module scope type-checks (the prelude is
   in scope); a consuming module importing `export const s = $state(0)` sees `number` (the
   import-resolution virtual file carried the prelude — `@ts-expect-error` on a wrong inferred type);
   a `.svelte.js` rune module type-checks under the JS-valid prelude form.
3. A negative test that the rune module exposes NO component API surface (no `$props`/PROPS demand).

**Verification.** The gate (TS touched, TSGO).

**Dependencies.** B8c, B4b (the plain-script dialect substrate), B5 (the `VirtualFileNaming` column /
registry classification). OWNER GO/NO-GO (D-bk). Serialized after B8i (or first among the owner-gated
trio, owner's choice — it is independent of B8h/B8i).
...
- MCP framework-aware tools (`crates/verter_mcp/src/` scanner + tools accept any REGISTERED
  ...
  per-framework switch); playground Monarch grammar + TypeExplorer for Svelte; meta/typeinfo-facing
  APIs accept any registered framework. The original unplugin `.vue`-only ruling is superseded by
  invariant 4 and the native Svelte compiler's integration block: legacy/default bundler entries
  remain Vue-pinned, while `VerterSvelte`, auto `Verter`, and the `/sveltejs` subpath expose the
  shared carrier transform without changing Vue compatibility behavior.
...
- **Disclaimer policy (D-bi)** — apply the wording "**experimental — not yet validated in
  real-world use**" (NOT "scoped / many surfaces OUT-OF-SCOPE": once B8d–B8j land, the Svelte matrix
  carries no permanent OUT-OF-SCOPE feature rows) to EVERY user-facing Svelte surface: the
  `verter.frameworks` VS Code setting description, the `@verter/svelte-jsx` README/`package.json`
  description, the `/framework-adapters` skill (Svelte section), every `docs/`/README mention, and
  the VS Code marketplace description; a top-level scope banner notes Vue is the production reference
  adapter and Svelte is feature-complete-but-experimental. Verter NEVER claims Svelte
  production-readiness — the owner flips that after their own real-world validation. (The B8c-era
  "scoped v1 — many surfaces OUT-OF-SCOPE" wording is superseded once the feature-completion blocks
  land; if any owner-gated block (B8h/B8i/B8j) is NOT funded, the banner names the still-diagnostic
  rows precisely.)
...
**Dependencies.** All in-scope blocks (B1, B1b, B2–B6, B8a/B8b/B8c, and the feature-completion blocks
B8d–B8j — B8h/B8i/B8j subject to their owner go/no-go, D-bi).
...
- NEW `crates/verter_compiler/src/astro/`: `parser/` producing the **Astro Template IR (D-v)** —
  node kinds `Fragment | Element | ComponentRef | Expression | AttrSpread | SlotOutlet | RawText | RawHtml | Comment`;
  `ComponentRef` stores tag span, resolved frontmatter import binding, dotted path, attrs, spreads,
  children, and island directives (`IslandDirective { kind, value_span, raw_span }`);
  uppercase/dotted tags resolve against frontmatter value imports/bindings,
  ...
  an async render function; **`Astro` typing via per-file prelude (D-v)** —
  `import type { AstroGlobal } from "astro"; declare const Astro: AstroGlobal<Props>` (no `Props` →
...
| EMITS | defineEmits | legacy `createEventDispatcher<E>` map ONLY, provenance-validated from `svelte` (runes callbacks stay PROPS) | unsupported (callback props remain PROPS) | unsupported | `@Output()` / `output()` (`EventEmitter<T>`/`OutputRef<T>` payload) |
