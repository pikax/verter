...
| `looksLikeStringCompatibleType(type)` | `type: string` | **DELETED (no replacement needed).** The function existed solely to re-quote string defaults the producer had stripped; the producer now publishes the verbatim source slice (`default_value_source_text` in `verter_semantic::analysis::macros`), so the string-ness inference branch (`normalizeDefaultForCompat`) is gone with it. | NO |
