...
- **FN5.2** (Unknown-fence typed-degradation end-state) is **Stage 10** — a downstream typed-state
  ...
  home.
- **FN6** (inert detector-name spellings in the gate ident lists) is **CLOSED** — the ident sweep
  landed (the inert spellings and the inert extracting-gate rail are deleted; the detector-spelling
  liveness guard `hot_detector_spellings_are_live_or_synthetic` fails on any future inert spelling)
  together with the both-rails proof that keeps the syntactic tripwire and the structural rail
  demonstrably load-bearing in the default gate (the in-memory scanner revert-probe
  `hot_materialize_scanner_flags_in_memory_injected_offender` + the always-on scoped structural
  smoke).
...
FN6, the inert detector-name spellings, is CLOSED — the ident sweep + both-rails proof landed; it
never gated this file's deletion.)
