...
|---|---|---|---|
| `FileArtifactStore` | Content-addressed | `(canonical, content_hash, parse_env_hash, parse_key, build_toolchain_fingerprint, file_language_id)` | `parse_key` canonically identifies exact bytes, language, compatibility domain/epoch, and syntax profile. The one session-owned build fingerprint invalidates private derived-artifact shape. Existing per-store lanes remain independent. |
...
| Typed-IR resolve | Content-addressed | `(canonical, content_hash, parse_env_hash, type_env_hash, lib_env_hash, build_toolchain_fingerprint)` | Content + type-env + lib addressed |
