...
- **RESOLVED.** Injected-path identity is unified behind one filesystem-identity key. `verter_span::path::InjectedPathKey` canonicalizes (slash-normalize, lowercase drive, strip extended/trailing) then, iff the host filesystem folds case (`fs_is_case_insensitive()`), ASCII-lowercases the whole path — matching `fs_paths_equal`'s case policy, with a case-parameterized core unit-tested on every platform. `verter_workspace` exposes an `InjectedPathSet` (keyed by that key); `strip_injected_root_diagnostics` takes the set instead of exact equality. `verter_tsc` builds ONE `InjectedPathSet` and feeds it to BOTH the upstream strip AND a fail-closed map-boundary guard: each diagnostic is tagged with its collection `DiagOrigin` (Semantic/Syntactic/Config), and the guard — keyed by the normalized path AND origin — suppresses only a `Config`-origin diagnostic pointing at an injected companion, before any carrier remap. A `Semantic`/`Syntactic` diagnostic on a generated carrier still remaps to `.vue` (no over-suppression / false negatives). Discriminating tests cover the case-divergent reproducer, the over-suppression invariant, and path-identity folding.
