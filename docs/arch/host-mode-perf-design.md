...
### PERF-3-D3 — `map_one`'s `(1,1)` disk-miss fallback is silently lossy — DEFERRED

- **Debt.** `crates/verter_tsc/src/api_check.rs::map_one` homes a non-root (non-overlay) diagnostic at its own disk-read position by reading `disk.read_file(file_name)` and running the shared UTF-16 offset walk; on a `read_file` MISS it falls back to `(1, 1)` — a well-defined but WRONG position that silently mis-homes the diagnostic to the file's first character.
- **Not a live regression (current facts).** The miss set is real on-disk files tsgo already read into the program, and verter-tsc runs with cwd at the project root, so `file_name` resolves and the fallback is not currently reached in the parity corpus. It is a latent correctness hole, not an active drift.
- **Durable fix (deferred).** Replace the silent `(1,1)` with an explicit, non-silent content lookup across the shared FS/path layer: resolve relative `file_name`s against the project root through the shared path primitive, and on a genuine miss surface a typed mapping error (or a diagnostic annotated as position-unresolved) rather than fabricating `(1,1)`. Owner layer is `verter_tsc` (in-scope for the `verter_tsc` block, but the resolution primitive is shared).

### PERF-3-D8 — injected-path identity uses exact string equality — DEFERRED

- **Debt.** `crates/verter_workspace/src/tsgo_virtual_config.rs::strip_injected_root_diagnostics` decides whether a diagnostic's `fileName` is an injected companion (a generated carrier / the synthetic tsconfig) via exact string equality (`p == name`), while `crates/verter_tsc/src/api_check.rs::map_one` homes diagnostics through a NORMALIZED key (`norm_key`: forward-slashed, Windows-case-folded). Two different path-identity notions drive the SAME ownership decision, so a companion echoed by the engine in a different separator / drive-letter case would slip past the strip filter on Windows.
- **Owner layer.** `verter_workspace` — OUTSIDE the `verter_tsc`-scoped block, so it is deferred rather than folded into this change.
- **Durable fix (deferred).** Unify injected-path identity with the SAME normalized path key used by diagnostic homing (route both `strip_injected_root_diagnostics` and `map_one` through one shared `norm_key`/`canonicalize_path` identity), so the strip decision and the homing decision cannot diverge.

### PERF-3-D9 — per-diagnostic O(n) offset walk + disk reread — DEFERRED

- **Debt.** `crates/verter_tsc/src/api_check.rs::map_one` performs, PER diagnostic, a `disk.read_file` of the target file plus a full UTF-16 scan-from-start to convert the offset. A whole-program error set over a large program re-reads and re-scans the same files repeatedly — O(diagnostics × file length) work that does not scale.
- **Durable fix (deferred).** Add a per-collection content cache (read each target file once per `collect_diagnostics` pass) plus a reusable UTF-16 / TS line index owned by `verter_span` (a `LineIndex`-style structure built once per file and queried per offset), keeping the existing scalar `utf16_offset_to_*` primitives as thin wrappers over it. This is the scaling substrate the per-diagnostic walk should route through.
