...
Cross-surface debt surfaced during PERF-3a review that is intentionally NOT fixed in that block (out of its verter_tsc-only scope). These rows are the DURABLE record (promoted out of the gitignored `.feedback/` notes) so the follow-up is not lost.

### PERF-3-offset — tsgo `--api` offset semantics are UTF-16, not bytes (owned `--api` provider)

- **Bug.** `crates/verter_type_runtime/src/tsgo/owned.rs::map_api_diagnostic` copies the tsgo `--api` diagnostic's `pos`/`end` STRAIGHT into `TypeDiagnostic.start`/`end`. But the rc engine's `--api` `pos`/`end` are **UTF-16 code-unit** offsets (TypeScript position semantics), while `TypeDiagnostic.start/end` is a **byte** contract. On any non-ASCII content before the diagnostic (e.g. an em-dash `—` — 3 UTF-8 bytes / 1 UTF-16 unit — in generated-carrier comments) the offsets diverge, so the owned `--api` provider drifts LSP diagnostic positions. The owned provider's doc comment even mislabels them "byte offsets".
- **Why deferred.** It is a PRE-EXISTING bug in `verter_type_runtime` (the owned dual-surface provider), a DIFFERENT surface from verter-tsc. verter-tsc's own `--api` path already converts UTF-16 → `(line,col)` correctly via `crates/verter_tsc/src/offset_map.rs::offset_to_line_col`; only the owned provider is affected. Fixing `owned.rs` is outside the verter_tsc-only scope of the PERF-3a fix block.
- **Correct fix (follow-up).** Normalize `--api` offset semantics ONCE at the shared `verter_tsgo_api` boundary (define the wire contract: `pos`/`end` are UTF-16 units, and expose a single conversion helper), then: (a) fix `owned.rs::map_api_diagnostic` to convert UTF-16 → the byte/position contract its consumer expects; (b) de-duplicate verter-tsc's `offset_map` against that shared helper so there is ONE offset-normalization implementation. Tracked in code by `// TODO(PERF-3-offset)` at `owned.rs::map_api_diagnostic`.

### PERF-3-rootscope — `--api` typecheck queries only `root_files` (non-root imported-file diagnostics dropped)

- **Divergence.** verter-tsc's in-memory `--api` typecheck (`crates/verter_tsc/src/api_check.rs::collect_diagnostics`) enumerates ONLY `project.root_files` (the generated carriers) and calls the per-file `getSemanticDiagnostics`/`getSyntacticDiagnostics` on each. A real type error in a NON-root imported file (an imported `.ts` the project pulls in, e.g. `src/types.ts`) is therefore never queried and never surfaced — whereas the old whole-program `tsgo --project --noEmit` type-checked the entire program and surfaced it (as `vue-tsc` does). The map layer's root-attribution invariant (`map_one` drops a present-but-unknown `file_name` rather than mis-homing it onto the queried root) is CORRECT and NOT the cause; the gap is upstream, in WHICH files are queried.
- **Not caught by the corpus.** The Rail B parity oracle stays green at `PARITY OK: 70` because its 70 pinned errors all live in root carriers (`.vue`-derived TSX + `.vue.ts` stubs) and its one imported `.ts` (`types.ts`) is intentionally CLEAN. Options/config/global diagnostics (also not covered by the two per-file getters — see §4 point 2) share this gap.
- **Correct fix (follow-up).** Collect diagnostics for the full program, not just `root_files` — enumerate every project source file (or use a whole-program/global diagnostic surface if `--api` exposes one) and add options/config/global-diagnostic collection — then extend the parity corpus with an intentional error in a non-root imported `.ts` so the gap is guarded. Until then, verter-tsc `--noEmit` under-reports vs `vue-tsc` for errors that live outside the generated carriers.
