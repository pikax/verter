...
3. Feed carriers (TSX + public-API stubs + ambient shims) as in-memory overlay snapshot entries — **no temp files**.
4. Project model: replace the synthetic `extends` tsconfig + explicit `files` with **real-tsconfig membership + deterministic in-project virtual carrier names**, modeled on the EXISTING LSP design `crates/verter_lsp/src/external_ts/tsgo_virtual_config.rs` (reuse, don't reinvent). Keep `get_public_api` cross-component stubs; **do NOT fall back to the wildcard shim (`checker.rs:320`) for in-project Vue imports**.
5. Map byte-offset diagnostics → generated (line,col) → original `.vue` via the existing inline-sourcemap decode (`error_map.rs`) + a new offset→line/col shim.

DELETE: `TempDir`, all `fs::write` carrier/shim sites (`checker.rs:111/176/225/304/323/336/354`), `write_temp_tsconfig`, `invoke_checker` subprocess + poll, `parse_tsc_output`. New deps: `verter_tsgo_api` (+ tokio `block_on`); drop `tempfile`. The owned path is tsgo-only/version-gated → `--use-tsc`/`ForceTsc` is removed.
**GATE: land Fix 1 only AFTER PERF-0 parity passes.** If `--api` parity cannot match `tsgo --project`, KEEP the current checker until the API gap is closed (do not ship a regressed diagnostic set).
...
- **PERF-3 — verter-tsc → in-memory tsgo `--api`, delete temp-file path.** `TsgoClient` overlay snapshots + real-tsconfig membership (reuse `tsgo_virtual_config.rs`) + offset→line/col remap; delete temp files/subprocess/`--use-tsc`. Depends PERF-0 (parity), PERF-2. Lands ONLY if parity holds; else hold.
