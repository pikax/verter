...
Admission freezes the train/block manifest and automatic per-role effort policy. The legacy `lease_id` field is only an opaque round handle retained for wire compatibility; it grants no ownership, exclusion, capacity, or scheduling right and never blocks another admission. Finalization freezes one exact review-target SHA/tree and worktree. That worktree remains byte-unchanged for the review round. Read-only reviewers may inspect it; a task with write capability uses its own disposable worktree from the frozen SHA. A review-side change transfers only after invalidating the round and freezing a successor target. Remove disposable task worktrees after their result is recorded, and remove the train worktree after landing or abort.

Preflight selects proof proportionately for each unchanged acceptance outcome. Existing behavioral coverage, type/compiler/capability enforcement, static validation, canonical gates, bounded inspection, and benchmarks are valid with a terse rationale. A proposed new test must name a plausible regression or contract boundary not already discriminated, and should extend or table-drive existing coverage where possible. Negative/mutation evidence is reserved for plausible critical fail-closed/correctness boundaries or reproduced defects. Incremental and performance evidence applies only when the node touches that authority or hot path. Behavioral code changes still use TDD; tests are evidence, not a per-charter quota.

Before admission, enumerate independently landable outcomes. Reject or split a block that combines multiple authority changes or independently rollbackable/reviewable concerns, including root cutover, orchestration policy, lifecycle tooling, and product implementation. Generated or mechanical file count is not the criterion; the boundary is one coherent atomic outcome. Combining concerns is allowed only when separation would violate a proven atomic invariant, with a terse rationale and a neutral Architect ruling for critical ambiguity. The parent may land ordered subblocks consecutively, but each remains its own frozen review unit.

Before finalization, squash the candidate branch to one conventional commit over the current integration tip. Freeze, review, verify, and confirm that squashed SHA/tree, then land it by exact fast-forward. Never squash, rebase, regenerate, or otherwise change the tree after review; any such change starts a successor round.
...
Two completed review/fix cycles are the soft maximum; reconciled history and global round labels do not count. Consolidate each cycle once before a comprehensive fix. Residual P0/P1 after the second completed cycle stops automatic cycling until the sole neutral Architect decides whether to continue and sets an exact additional-cycle cap. Before cycle six, the Architect must explicitly decide whether to split the work into smaller independently reviewable sub-subblocks, regardless of severity. Architect use is read-only Codex CLI, OpenAI `gpt-5.6-sol`, `xhigh`; only the maintainer may waive or substitute when that budget is unavailable. Status reports are honest and terse, and only durable gate-bearing transitions create receipts.
