...
This policy uses the honest-operator trusted-local assurance model authorized by the 2026-08-28 superseding directive. Each node requires the complete risk-scaled review profile frozen into its lease: one adversarial lens for low risk, one or two assigned lenses for medium risk, and three distinct adversarial, conformance, and specialist lenses for high risk. `programctl harness-record` imports each prompt and report by one exact-byte read and records distinct author/agent/task identities, provider, model, effort, lease, round, node, worktree mode, and candidate bindings. These are operator attestations and audit records, not cryptographic proof of harness authenticity or malicious-owner resistance. A PASS must contain no findings; every FAIL finding has an exact severity, fingerprint, owner, and OPEN disposition. Acceptance requires every assigned current-round lens to PASS, fresh verification, risk-scaled confirmation, and completed cleanup of every writable review worktree. A failed review/fix cycle is complete only after the full assigned lens set is recorded and the round closes `FIX_REQUIRED`. P0/P1 block acceptance; P2/P3 follow the owning disposition policy.

Review targets are immutable. Read-only reviewers inspect the frozen train worktree and record cleanup as `not-created`. A write-enabled reviewer uses a distinct clean disposable Git worktree at the exact frozen SHA/tree, and acceptance waits for a durable `removed` cleanup record. Task and agent identities are fresh across roles and rounds, and no harness agent may equal the implementation holder. After two completed review/fix cycles with residual P0/P1, the neutral Architect must decide whether to continue and set an exact cap. After five completed cycles, cycle six always waits for an explicit Architect decomposition ruling, regardless of severity.

The low-administration implementation evidence is the exact holder-authored candidate Git SHA/tree/ref/worktree frozen by finalization. The implementation brief defines its computed effort and scope, but there is no second self-attested implementation harness receipt. Review, verification, and confirmation remain separate fresh harness evidence and cannot reuse the holder identity.
