...
`B1`, `B2` are `ACCEPTED`; `CM1` is `REVIEW` with all three review mandates `BLOCKING`, not `ACCEPTED`
(`docs/arch/architecture-lock/ledger/program-state.toml`, CM1 row). C1's predecessor set is therefore not
yet fully satisfied.
...
C1's predecessor set is not yet clear: `A6`, `B1`, `B2` are `ACCEPTED`, but `CM1` is `REVIEW` with all
three review mandates `BLOCKING` (`program-state.toml`, CM1 row) — not `ACCEPTED`. C1's own ledger `status`
