<!-- unified-charter-v2
id=CLI1
name=Shared application services, selection, invocation, reporters
phase=expansion
train=expansion.cli
product=cli
kind=implementation
...
optional=false
release_gating=none
source_refs=source:successor-expansion.md:L1484
external_requirements=
activation_gate=ORC0
charter=charters/expansion-cli/CLI1.md
...
# CLI1 — Shared application services, selection, invocation, reporters

Authority state is derived at dispatch. The canonical CLI must validate the current phase, exact predecessor receipts, activation/release gates, external authorizations, and atomic admission before mutation.

## Independently acceptable outcome

Shared application services, selection, invocation, reporters. The current owner is **separate package launchers and command-local project logic**. The final and sole owner is **one `verter` application service with thin command adapters**. This charter accepts one authority/migration/cutover boundary; it contains no independently dispatchable subblocks.
...
- **CLI0:** exact current receipt ID and digest for “`verter` command/package and semantic lock”; Git ancestry, touched-blob integration equivalence, and receipt currency must validate at dispatch and acceptance.
...
- **CLI1-AC1 — sole-owner outcome:** the named final owner must be sole and every displaced route named below must be deleted or structurally rejected. Prefer existing type, capability, dependency, compiler, or static enforcement. Add a negative or mutation test only for a plausible critical fail-closed/correctness boundary or a reproduced defect that existing evidence does not discriminate.
- **CLI1-AC2 — positive contract:** the named API/data boundary must preserve exact identities, provenance, completeness, and deterministic ordering. Reuse existing coverage or extend/table-drive one test before creating a new test.
- **CLI1-AC3 — incremental equivalence:** when the changed scope owns or affects incremental, cache, cancellation, stale-publication, or partial-result authority, prove incremental equals fresh and degraded outcomes cannot warm. Otherwise record a terse not-applicable rationale tied to the untouched authority.
- **CLI1-AC4 — bounded work:** when the changed scope owns or affects a hot path, prove no hidden duplicate parse, resolve, plan, emit, copy, allocation, or retained candidate using applicable existing counters, inspection, or benchmarks. Otherwise record a terse not-applicable rationale; do not add counters or a soak by default.
...
- `source:successor-expansion.md:L1484`
...
The recovery candidate mechanically split this source-owned atomic node into the following labels: `CLI1-A`, `CLI1-B`, `CLI1-C`, `CLI1-D`, `CLI1-E`, `CLI1-F`, `CLI1-G`. They have no separate dispatch, lease, receipt, migration manifest, deletion ownership, or review standing. Their useful source-described concerns are internal RED/GREEN checklist items of **CLI1**; CLI1 alone owns the complete migration population, exactly one final deletion/cutover, and atomic acceptance. Any quoted “suggested subblock” wording in transferred source text is non-authoritative planning context.
...
- Kind: `context`
- Source: `successor-expansion.md:1484-1484`
- Applicability: `CLI1`
- Exact text SHA-256: `84ef6be01e989164fdecbdd9b3ac8c5a269b004d3df7dae8d361b43c68144937`
...
- Kind: `forbidden`
- Source: `successor-expansion.md:1486-1491`
- Applicability: `CLI1`
- Exact text SHA-256: `1dd61fb91a5da26a81ac6e8e6f3aeebcce9db786b8700d063f3ca48643a47ced`
