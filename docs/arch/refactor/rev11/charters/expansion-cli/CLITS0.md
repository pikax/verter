...
- **CLITS0-AC1 — sole-owner proof:** add `clits0_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **CLITS0-AC2 — positive contract:** add `clits0_publishes_exact_applicationservices`; assert exact identities, provenance, completeness, and deterministic ordering.
- **CLITS0-AC3 — incremental equivalence:** add `clits0_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **CLITS0-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
