...
- **FMK0-AC1 — sole-owner proof:** add `fmk0_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **FMK0-AC2 — positive contract:** add `fmk0_publishes_exact_carrierprofileid`; assert exact identities, provenance, completeness, and deterministic ordering.
- **FMK0-AC3 — incremental equivalence:** add `fmk0_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **FMK0-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
