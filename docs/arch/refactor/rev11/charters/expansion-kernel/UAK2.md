...
- **UAK2-AC1 — sole-owner proof:** add `uak2_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **UAK2-AC2 — positive contract:** add `uak2_publishes_exact_carrierprofileid`; assert exact identities, provenance, completeness, and deterministic ordering.
- **UAK2-AC3 — incremental equivalence:** add `uak2_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **UAK2-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
