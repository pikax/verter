<!-- unified-charter-v2
id=TCM1
name=Compact mapping products inside CodeTransform
phase=rev11
...
optional=false
release_gating=none
source_refs=live:docs/arch/refactor/rev11/charters/TCM1.md
external_requirements=
activation_gate=ORC0
charter=charters/rev11-typescript-mapper/TCM1.md
...
Compact mapping products inside CodeTransform. The current owner is **rejected TCM0 closure package and string mapper plane**. The final and sole owner is **ratified dual-plane mapper/snapshot/oracle identity contract**. This charter accepts one authority/migration/cutover boundary; it contains no independently dispatchable subblocks.
...
- Deliver exactly “Compact mapping products inside CodeTransform” as the independently acceptable boundary; no neighboring authority is included.

## Acceptance IDs and discriminating proof

Preflight evidence selection: preserve all four acceptance outcomes below, then select the smallest evidence set that actually discriminates the touched contract. Existing behavioral coverage, compiler/type/capability enforcement, static validation, canonical gates, bounded inspection, and benchmarks are valid when accompanied by a terse rationale.

- **TCM1-AC1 — sole-owner outcome:** the named final owner must be sole and every displaced route named below must be deleted or structurally rejected. Prefer existing type, capability, dependency, compiler, or static enforcement. Add a negative or mutation test only for a plausible critical fail-closed/correctness boundary or a reproduced defect that existing evidence does not discriminate.
- **TCM1-AC2 — positive contract:** the named API/data boundary must preserve exact identities, provenance, completeness, and deterministic ordering. Reuse existing coverage or extend/table-drive one test before creating a new test.
- **TCM1-AC3 — incremental equivalence:** when the changed scope owns or affects incremental, cache, cancellation, stale-publication, or partial-result authority, prove incremental equals fresh and degraded outcomes cannot warm. Otherwise record a terse not-applicable rationale tied to the untouched authority.
- **TCM1-AC4 — bounded work:** when the changed scope owns or affects a hot path, prove no hidden duplicate parse, resolve, plan, emit, copy, allocation, or retained candidate using applicable existing counters, inspection, or benchmarks. Otherwise record a terse not-applicable rationale; do not add counters or a soak by default.
...
- `live:docs/arch/refactor/rev11/charters/TCM1.md`

## TCM0 remediation receiving criteria

- `TCM1-RC-IMPLEMENTATION-BASELINE`: before changing the mapper path, capture the current-path comparison reference with TCM0's locked workloads, metrics, package identity, method, and comparison rules. Bind the receipt to `TCM0-R-IMPLEMENTATION-BASELINE`; do not invent an absolute-machine threshold.
- Record every type/codec introduced or orphaned by TCM1 in the accumulated deletion/survival manifest. Do not rediscover or rewrite TCM0's current-tree inventory.
- Acceptance evidence must directly cite the stable residue and criterion IDs plus charter/authority digests; prose or proxy string matching is invalid.
...
- `live:docs/arch/refactor/rev11/charters/TCM1.md` — 25715 bytes, SHA-256 `36ce3e91698f630e1744ee5964ce4241ed46561478b57402e13994aeb5f57b46`
