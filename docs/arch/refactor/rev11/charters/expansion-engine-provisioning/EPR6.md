...
- Production surfaces: `crates/verter_language/src`, `crates/verter_lsp`, `crates/verter_lsp/src`, `crates/verter_session`, `crates/verter_tsgo_api`, `crates/verter_tsgo_api/src/actor`, `crates/verter_type_runtime`, `crates/verter_type_runtime/src/tsserver`, `docs/arch/refactor/rev11`, `packages/typescript-plugin`, `crates/verter_audit/src`, `crates/verter_bench`, `crates/verter_compiler/src`, `crates/verter_napi/src`, `crates/verter_semantic/src`, `crates/verter_session/src`, `crates/verter_wasm/src`, `docs/arch/refactor/rev11/evidence`, `packages/benchmark`, `scripts`, `.claude/skills`, `AGENTS.md`, `crates/verter_identity`, `crates/verter_identity/src`, `crates/verter_protocol/src`.
...
- **EPR5:** exact current receipt ID and digest for “Engine activation epochs, health, and truthful capability publication”; Git ancestry, touched-blob integration equivalence, and receipt currency must validate at dispatch and acceptance.
