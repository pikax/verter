...
const finding = (severity = "P1", fingerprint = "a".repeat(64)) => ({
  severity,
  fingerprint,
  owner: "trusted-local lifecycle",
  status: "OPEN",
});

function harnessFiles(root, name, report) {
  const promptFile = path.join(root, `${name}.prompt`);
...
function payloadArtifact(body) {
  const digest = crypto.createHash("sha256").update(body).digest("hex");
  return { bytes: Buffer.from(`${body}payload_sha256 = "${digest}"\n`), digest };
}
...
function recordHighReviewSet({ lifecycle, runtimeA, root, admitted, suffix, severity = "P1" }) {
  for (const [index, lens] of admitted.review_lenses.entries()) {
    const files = harnessFiles(root, `${suffix}-${lens}`, {
      verdict: "FAIL",
      findings: [finding(severity, `${index + 1}`.repeat(64))],
    });
    lifecycle.recordReview({
      runtimeRoot: runtimeA,
      roundId: admitted.round_id,
      leaseId: admitted.lease_id,
      holder: "author",
      lens,
      taskIdentity: `/root/${suffix}-${lens}`,
      agentIdentity: `${suffix}-${lens}-reviewer`,
      provider: "openai",
      model: "gpt-5.6-sol",
      effort: "high",
      ...files,
...
function acceptHighRound({ lifecycle, runtimeRoot, root, admitted }) {
  lifecycle.finalize({ runtimeRoot, leaseId: admitted.lease_id, holder: "author", candidate });
  for (const [index, lens] of admitted.review_lenses.entries()) {
    const files = harnessFiles(root, `activation-${lens}`, { verdict: "PASS", findings: [] });
    lifecycle.recordReview({
      runtimeRoot,
      roundId: admitted.round_id,
      leaseId: admitted.lease_id,
      holder: "author",
      lens,
      taskIdentity: `/root/activation-${lens}`,
      agentIdentity: `activation-reviewer-${index}`,
      provider: "openai",
      model: "gpt-5.6-sol",
      effort: "high",
      ...files,
  ...
  for (const role of ["verification", "confirmation"]) {
    const files = harnessFiles(root, `activation-${role}`, { verdict: "PASS", findings: [] });
    lifecycle.recordRole({
      runtimeRoot,
      roundId: admitted.round_id,
      leaseId: admitted.lease_id,
      holder: "author",
      role,
      taskIdentity: `/root/activation-${role}`,
      agentIdentity: `activation-${role}-agent`,
      provider: "openai",
      model: "gpt-5.6-sol",
      effort: "high",
      ...files,
  ...
  lifecycle.accept({ runtimeRoot, roundId: admitted.round_id, holder: "author" });
  lifecycle.recordLanding({
    runtimeRoot,
    roundId: admitted.round_id,
    holder: "author",
    canonicalRef: "refs/heads/program/architecture-lock",
    canonicalSha: candidate.sha,
    canonicalTree: candidate.tree,
  });
}
...
test("Architect prompts are neutral, non-exhaustive, exact-profile, and carry the durable-design mandate", () => {
  const roundTwo = architectPromptFor({
    type: "round-two-cap",
    nodeId: "ORC0",
    roundId: "ORC0-R2",
    roundOrdinal: 2,
  ...
  const overFive = architectPromptFor({
    type: "over-five-decomposition",
    nodeId: "ORC0",
    roundId: "ORC0-R6",
    roundOrdinal: 6,
  });
...
test("PASS reports with findings and malformed FAIL dispositions refuse before evidence mutation", () => {
  fixture(({ runtimeA, lifecycle, root }) => {
    ...
    const passWithFinding = harnessFiles(root, "bad-pass", {
      verdict: "PASS",
      findings: [finding()],
    });
    ...
    assert.throws(
      () =>
        lifecycle.recordReview({
          ...
          holder: "author",
          lens: "adversarial",
          taskIdentity: "/root/bad-pass",
          agentIdentity: "reviewer-a",
          provider: "openai",
          model: "gpt-5.6-sol",
          effort: "high",
          ...passWithFinding,
        }),
      /clean PASS|findings/i,
    );
    assert.deepEqual(fs.readFileSync(lifecycle.anchorPath), before);
    const malformed = harnessFiles(root, "bad-fail", {
      verdict: "FAIL",
      findings: [{ severity: "SEVERE", status: "CLOSED" }],
    });
    assert.throws(
      () =>
        lifecycle.recordReview({
          ...
          holder: "author",
          lens: "adversarial",
          taskIdentity: "/root/bad-fail",
          agentIdentity: "reviewer-b",
          provider: "openai",
          model: "gpt-5.6-sol",
          effort: "high",
          ...malformed,
        }),
      /severity|fingerprint|OPEN/i,
    );
    assert.deepEqual(fs.readFileSync(lifecycle.anchorPath), before);
  });
});
...
test("a failed review cycle completes only after every required lens and FIX_REQUIRED disposition", () => {
  fixture(({ runtimeA, lifecycle, root }) => {
    ...
    for (let index = 0; index < lenses.length - 1; index += 1) {
      const files = harnessFiles(root, `partial-${index}`, {
        verdict: "FAIL",
        findings: [finding("P1", String(index + 1).repeat(64))],
      });
      lifecycle.recordReview({
        runtimeRoot: runtimeA,
        roundId: admitted.round_id,
        ...
        holder: "author",
        lens: lenses[index],
        taskIdentity: `/root/partial-${index}`,
        agentIdentity: `reviewer-${index}`,
        provider: "openai",
        model: "gpt-5.6-sol",
        effort: "high",
        ...files,
    ...
    assert.throws(
      ...
      /complete.*review.*profile|all required lenses/i,
    );
    const finalFiles = harnessFiles(root, "partial-final", {
      verdict: "FAIL",
      findings: [finding("P1", "3".repeat(64))],
    });
    lifecycle.recordReview({
      runtimeRoot: runtimeA,
      roundId: admitted.round_id,
      ...
      holder: "author",
      lens: lenses[2],
      taskIdentity: "/root/partial-final",
      agentIdentity: "reviewer-2",
      provider: "openai",
      model: "gpt-5.6-sol",
      effort: "high",
      ...finalFiles,
    ...
    assert.throws(
      () =>
        lifecycle.recordRole({
          ...
          role: "verification",
          taskIdentity: "/root/shared-task",
          agentIdentity: "verifier-a",
          provider: "openai",
          model: "gpt-5.6-sol",
          effort: "high",
          ...files,
        }),
      /fresh.*task|already used/i,
    );
  });
});
...
test("cycle six requires an over-five Architect decomposition ruling regardless of severity", () => {
  ...
  fixture(({ runtimeA, lifecycle, root }) => {
    for (let cycle = 1; cycle <= 5; cycle += 1) {
      ...
      const files = harnessFiles(root, `cycle-${cycle}`, {
        verdict: "FAIL",
        findings: [finding("P3", String(cycle).repeat(64))],
      });
      lifecycle.recordReview({
        runtimeRoot: runtimeA,
        roundId: admitted.round_id,
        ...
        holder: "author",
        lens: "adversarial",
        taskIdentity: `/root/cycle-${cycle}`,
        agentIdentity: `reviewer-${cycle}`,
        provider: "openai",
        model: "gpt-5.6-sol",
        effort: "low",
        ...files,
    ...
    assert.throws(
      () => lifecycle.admit({ runtimeRoot: runtimeA, node: lowNode, candidate, holder: "author" }),
      /Architect.*decomposition|over-five/i,
    );
  });
});
...
test("reactivation publication extends one exact activation head and refuses forks or forged identity", () => {
  fixture(({ runtimeA, lifecycle, root }) => {
    const admitted = lifecycle.admit({
      runtimeRoot: runtimeA,
      node: highNode,
    ...
    acceptHighRound({ lifecycle, runtimeRoot: runtimeA, root, admitted });
    const rootArtifact = payloadArtifact('schema = 2\ntype = "activation-transition"\n');
    lifecycle.publishActivation({
      runtimeRoot: runtimeA,
      roundId: admitted.round_id,
      holder: "author",
      transitionId: "ACT-ROOT",
      transitionBytes: rootArtifact.bytes,
    });
    const rootReference = `ACT-ROOT:${rootArtifact.digest}`;
    const amendmentReference = `AMD-SUCCESSOR:${"a".repeat(64)}`;
    const reactivationArtifact = payloadArtifact('schema = 3\ntype = "activation-transition"\n');
    const published = lifecycle.publishReactivation({
      runtimeRoot: runtimeA,
      previousTransition: rootReference,
      authorityAmendment: amendmentReference,
      holder: "author",
      transitionId: "ACT-REACTIVATED",
      transitionBytes: reactivationArtifact.bytes,
    });
    assert.equal(published.previous_activation_transition, rootReference);
    assert.equal(published.authority_amendment, amendmentReference);
    assert.deepEqual(
      fs.readFileSync(path.join(runtimeA, "activations/ACT-REACTIVATED.toml")),
      reactivationArtifact.bytes,
    );
    const exactAnchor = fs.readFileSync(lifecycle.anchorPath);
    assert.throws(
      () =>
        lifecycle.publishReactivation({
          runtimeRoot: runtimeA,
          previousTransition: rootReference,
          authorityAmendment: amendmentReference,
          holder: "author",
          transitionId: "ACT-FORK",
          transitionBytes: payloadArtifact("fork = true\n").bytes,
        }),
      /fork|duplicate/i,
    );
    assert.deepEqual(fs.readFileSync(lifecycle.anchorPath), exactAnchor);
    assert.throws(
      () =>
        lifecycle.publishReactivation({
          runtimeRoot: runtimeA,
          previousTransition: `ACT-REACTIVATED:${"0".repeat(64)}`,
          authorityAmendment: amendmentReference,
          holder: "author",
          transitionId: "ACT-FORGED",
          transitionBytes: payloadArtifact("forged = true\n").bytes,
        }),
      /exact current activation head/i,
    );
    assert.deepEqual(fs.readFileSync(lifecycle.anchorPath), exactAnchor);
  });
});
...
test("round ordinals are global and an old closed round cannot regain acceptance", () => {
  fixture(({ runtimeA, runtimeB, lifecycle }) => {
    ...
    const before = fs.readFileSync(lifecycle.anchorPath);
    assert.throws(
      () => lifecycle.accept({ runtimeRoot: runtimeA, roundId: "ORC0-R1", holder: "author" }),
      /current round/i,
    );
    assert.deepEqual(fs.readFileSync(lifecycle.anchorPath), before);
  });
...
test("anchor loss blocks mutation until explicit reinitialization exposes lost continuity", () => {
  fixture(({ controlRoot, runtimeA, lifecycle }) => {
    ...
    assert.throws(
      () => lifecycle.admit({ runtimeRoot: runtimeA, node: highNode, candidate, holder: "author" }),
      /anchor.*lost|reinitial/i,
    );
    const reset = reinitializeLocalLifecycle({
      controlRoot,
      operator: "maintainer",
      reason: "local disk restored without control anchor",
    });
    assert.equal(reset.continuity, "unknown/lost");
    assert.equal(readLocalAnchor({ controlRoot }).lineage_generation, 2);
    const after = createLocalLifecycle({ controlRoot }).admit({
      runtimeRoot: runtimeA,
      node: highNode,
      candidate,
      holder: "author",
...
test("transaction-boundary crashes recover exactly one admission", () => {
  for (const failpoint of ["after-marker", "after-write-1", "before-anchor", "after-anchor"])
    fixture(({ runtimeA, runtimeB, lifecycle }) => {
      ...
      try {
        ...
        delete process.env.VERTER_TRUSTED_LOCAL_FAILPOINT;
      }
      const next = lifecycle.admit({
        runtimeRoot: runtimeB,
        node: highNode,
        candidate,
        holder: "other",
      ...
      const anchor = readLocalAnchor({ controlRoot: path.dirname(lifecycle.anchorPath) });
      assert.equal(
        Object.values(anchor.leases).filter((lease) => lease.node_id === "ORC0").length,
        2,
        failpoint,
      );
      assert.equal(
        fs.existsSync(path.join(path.dirname(lifecycle.anchorPath), "transaction.json")),
        false,
        failpoint,
      );
    });
...
test("second completed P1 review/fix cycle requires the fixed neutral Architect and an explicit additional-round cap", () => {
  fixture(({ runtimeA, lifecycle, root }) => {
    ...
    const escalation = JSON.parse(
      fs.readFileSync(
        path.join(runtimeA, "trusted-local", "architect-prompts", `${r2.round_id}.json`),
        "utf8",
      ),
    );
    ...
    const decision = path.join(root, "architect.json");
    fs.writeFileSync(
      decision,
      '{"provider":"openai","tool":"codex","model":"gpt-5.6-sol","reasoning_effort":"xhigh","decision":"CONTINUE","additional_round_cap":1}\n',
    );
    lifecycle.recordArchitectDecision({
      runtimeRoot: runtimeA,
      roundId: r2.round_id,
      operator: "maintainer",
      reportFile: decision,
    });
    const r3 = lifecycle.admit({
      runtimeRoot: runtimeA,
      node: highNode,
      candidate,
      holder: "author",
    });
    assert.equal(r3.round_id, "ORC0-R3");
    lifecycle.finalize({
      runtimeRoot: runtimeA,
      leaseId: r3.lease_id,
      holder: "author",
    ...
    assert.equal(
      fs.existsSync(
        path.join(runtimeA, "trusted-local", "architect-prompts", `${r3.round_id}.json`),
      ),
      true,
      "a critical round that exhausts the prior cap must become eligible for a fresh ruling",
    );
    assert.throws(
      () =>
        lifecycle.recordArchitectDecision({
    ...
    const legacyAnchor = JSON.parse(fs.readFileSync(lifecycle.anchorPath));
    delete legacyAnchor.rounds[r3.round_id].architect_escalation_required;
...
test("an exact neutral Architect STOP decision remains terminal", () => {
  fixture(({ runtimeA, lifecycle, root }) => {
    ...
    const stop = path.join(root, "architect-stop.json");
    fs.writeFileSync(
      stop,
      '{"provider":"openai","tool":"codex","model":"gpt-5.6-sol","reasoning_effort":"xhigh","decision":"STOP","additional_round_cap":0}\n',
    );
    lifecycle.recordArchitectDecision({
      runtimeRoot: runtimeA,
      roundId: "ORC0-R2",
    ...
    assert.throws(
      () => lifecycle.admit({ runtimeRoot: runtimeA, node: highNode, candidate, holder: "author" }),
      /stopped by the neutral Architect/i,
    );
  });
});
...
test("reconciled ORC0 R1 does not count as a review/fix cycle, but the second live failed cycle escalates", () => {
  const history = {
    type: "trusted-local-preactivation-history",
    acceptance_eligible: false,
    disposition: "REJECTED_AUDIT_ONLY",
    node_id: "ORC0",
    round_id: "ORC0-R1",
    lease_id: "historic-r1",
    minimum_successor_round_ordinal: 2,
  };
  fixture(
    ({ runtimeA, lifecycle, root }) => {
      ...
      assert.equal(first.round_id, "ORC0-R2");
      assert.equal(first.review_cycle_ordinal, 1);
      fail(first, "first-live");
      assert.equal(
        fs.existsSync(
          path.join(runtimeA, "trusted-local", "architect-prompts", `${first.round_id}.json`),
        ),
        false,
      );
      lifecycle.close({
      ...
      assert.equal(second.round_id, "ORC0-R3");
      assert.equal(second.review_cycle_ordinal, 2);
      fail(second, "second-live");
      lifecycle.close({
        runtimeRoot: runtimeA,
      ...
      assert.equal(
        fs.existsSync(
          path.join(runtimeA, "trusted-local", "architect-prompts", `${second.round_id}.json`),
        ),
        true,
      );
    },
...
test("finalization freezes an immutable review manifest repeated by review evidence", () => {
  fixture(({ runtimeA, lifecycle, root }) => {
    ...
    assert.match(finalized.review_target_sha256, /^[0-9a-f]{64}$/);
    const prompt = path.join(root, "frozen.prompt");
    const report = path.join(root, "frozen.json");
    fs.writeFileSync(prompt, "review frozen target\n");
    fs.writeFileSync(report, '{"verdict":"PASS","findings":[]}\n');
    const evidence = lifecycle.recordReview({
      runtimeRoot: runtimeA,
      roundId: admitted.round_id,
      leaseId: admitted.lease_id,
      holder: "author",
      lens: "adversarial",
      taskIdentity: "/root/frozen-adversarial",
      agentIdentity: "frozen-reviewer",
      provider: "openai",
      model: "gpt-5.6-sol",
      effort: "high",
      promptFile: prompt,
