...
function safeRelative(relative, label) {
  if (typeof relative !== "string" || relative.length === 0 || /[\0-\x1f\x7f\\]/u.test(relative))
    throw new Error(`${label}: invalid relative path`);
  if (
    path.posix.isAbsolute(relative) ||
    relative.split("/").some((part) => part === "" || part === "." || part === "..")
  )
    throw new Error(`${label}: path escapes its authority root`);
  return relative;
}
...
function ensureDirectory(packageRoot, relative) {
  ...
  for (const segment of safeRelative(relative, relative).split("/")) {
    ...
    if (stat.isSymbolicLink() || !stat.isDirectory())
      throw new Error(`${relative}: directory chain must contain only real directories`);
    if (!isInside(root, fs.realpathSync(current)))
      throw new Error(`${relative}: directory resolves outside the authority package`);
  }
  return current;
}
...
function parseValue(raw, location) {
  ...
  if (value.startsWith('"') || value.startsWith("[")) {
    try {
      ...
      throw new Error(`${location}: malformed string or array: ${error.message}`);
    }
  }
  if (/^-?(?:0|[1-9][0-9]*)$/u.test(value)) {
    const number = Number(value);
    if (!Number.isSafeInteger(number))
      throw new Error(`${location}: integer is outside the safe range`);
    return number;
  }
...
function parseFlatToml(text, location) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (/^[#[]/u.test(line))
      throw new Error(`${location}:${index + 1}: comments and tables are forbidden`);
    const match = /^([A-Za-z][A-Za-z0-9_]*) = (.+)$/u.exec(line);
    if (!match) throw new Error(`${location}:${index + 1}: malformed field`);
    const key = match[1];
    if (["__proto__", "prototype", "constructor"].includes(key))
      throw new Error(`${location}:${index + 1}: unsafe key`);
    if (Object.hasOwn(result, key))
      throw new Error(`${location}:${index + 1}: duplicate field ${key}`);
...
function parseTrustedRatifications(text, location) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (line === "") continue;
    if (line === "[[slot]]") {
      if (Object.hasOwn(root, "slot"))
        throw new Error(`${location}:${index + 1}: slot cannot use both inline and table forms`);
      current = Object.create(null);
      slots.push(current);
      continue;
    }
    if (/^[#[]/u.test(line))
      throw new Error(`${location}:${index + 1}: comments and unknown tables are forbidden`);
    const match = /^([A-Za-z][A-Za-z0-9_]*) = (.+)$/u.exec(line);
    if (!match) throw new Error(`${location}:${index + 1}: malformed field`);
    const key = match[1];
    if (["__proto__", "prototype", "constructor"].includes(key))
...
function schemaErrors(value, schema, location) {
  ...
  if (schema.enum && !schema.enum.includes(value))
    errors.push(`${location}: value is not in the permitted enum`);
  if (
    schema.type &&
    valueType(value) !== schema.type &&
...
function loadSchema(packageRoot, name) {
  ...
  try {
    ...
    throw new Error(`schemas/${name}: ${error.message}`);
  }
}
...
function validateObject(value, schema, location, validateSchemaObject) {
  ...
  if (validateSchemaObject) {
    for (const error of validateSchemaObject(value, schema, location))
...
function payload(text, location) {
  ...
  if (markers.length !== 1)
    return { errors: [`${location}: payload_sha256 must be the single final field`] };
...
function parseAmendmentText(text, packageRoot, location, dependencies) {
  ...
  let row = null;
  try {
    row = parseFlatToml(text, location);
  } catch (error) {
  ...
  if (row) {
    try {
      errors.push(
        ...validateObject(
          row,
          loadSchema(packageRoot, "amendment.schema.json"),
          location,
          dependencies.validateSchemaObject,
        ),
      );
    } catch (error) {
      errors.push(error.message);
    }
    if (framed.digest && row.payload_sha256 !== framed.digest)
      errors.push(`${location}: parsed payload digest differs from the final field`);
  }
...
function parseLockFile(packageRoot, dependencies) {
  ...
  let lock = null;
  try {
    lock = parseFlatToml(fs.readFileSync(lockFile, "utf8"), LOCK_RELATIVE);
  } catch (error) {
    errors.push(error.message);
  }
  if (lock) {
    try {
      errors.push(
        ...validateObject(
          lock,
          loadSchema(packageRoot, "authority-lock.schema.json"),
          "authority-lock",
          dependencies.validateSchemaObject,
        ),
      );
...
function loadTrustedRatifications(packageRoot, dependencies) {
  ...
  try {
    ...
    errors.push(
      ...validateObject(
        ledger,
        loadSchema(packageRoot, "trusted-ratifications.schema.json"),
        "trusted-ratifications",
        dependencies.validateSchemaObject,
      ),
    );
  } catch (error) {
  ...
  for (const [index, row] of ledger.slot.entries()) {
    ...
    if (
      ...
      row.ratified_by.length === 0 ||
      row.ratified_by !== row.ratified_by.trim() ||
      /[\0-\x1f\x7f]/u.test(row.ratified_by)
    )
      errors.push(`${rowLocation}: ratified_by must be a normalized identity`);
    if (!SHA256.test(row.receipt_sha256 || "") || /^0{64}$/u.test(row.receipt_sha256))
      errors.push(`${rowLocation}: receipt_sha256 must be a non-placeholder SHA-256`);
    if (usedPaths.has(row.receipt_path))
      errors.push(`${rowLocation}: receipt_path is used by more than one trusted slot`);
    ...
    try {
      ...
      if (actual !== row.receipt_sha256)
        errors.push(
          `${rowLocation}: trusted ratification receipt digest mismatch for ${row.receipt_path}`,
        );
    } catch (error) {
      errors.push(`${rowLocation}: ${error.message}`);
    }
    byKey.set(key, row);
  }
  return { ledger, byKey, errors };
...
function amendmentFiles(packageRoot, { ignoreWriteLock = false } = {}) {
  const directory = safeEntry(packageRoot, AMENDMENTS_RELATIVE, {
    type: "directory",
    optional: true,
  });
  ...
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.name === WRITE_LOCK_NAME)
      errors.push(`${AMENDMENTS_RELATIVE}: amendment write is incomplete or in progress`);
    else if (entry.isSymbolicLink() || !entry.isFile() || !entry.name.endsWith(".toml"))
      errors.push(`${AMENDMENTS_RELATIVE}: unsupported entry ${entry.name}`);
...
function graph(authority, label) {
  if (!authority || !Array.isArray(authority.nodes))
    throw new Error(`${label}: authority.nodes must be an array`);
  const byId = new Map();
  const children = new Map();
  const moduleById = new Map();
  const charterById = new Map();
  for (const rawNode of authority.nodes) {
    if (!rawNode || typeof rawNode !== "object" || !/^[A-Z][A-Z0-9-]*$/u.test(rawNode.id || ""))
      throw new Error(`${label}: invalid node id`);
    if (byId.has(rawNode.id)) throw new Error(`${label}: duplicate node ${rawNode.id}`);
    if (
      !Array.isArray(rawNode.predecessors) ||
      rawNode.predecessors.some((id) => typeof id !== "string")
    )
      throw new Error(`${label}: node ${rawNode.id} has invalid predecessors`);
    const node = Object.fromEntries(
      Object.entries(rawNode).filter(([key]) => !key.startsWith("_")),
    );
  ...
  for (const node of byId.values()) {
    for (const predecessor of node.predecessors) {
      if (!byId.has(predecessor))
        throw new Error(`${label}: node ${node.id} has missing predecessor ${predecessor}`);
...
function changedNodes(beforeGraph, afterGraph) {
  ...
  return [...ids]
    .filter((id) => canonical(beforeGraph.byId.get(id)) !== canonical(afterGraph.byId.get(id)))
    .sort();
}

const GLOBAL_CHANGE_ROOTS = new Set([
  "catalogs",
  "contracts",
  "provenance",
  "schemas",
...
function moduleMetadata(authority) {
  ...
  return new Map(
    authority.moduleModels.map(({ relative, model }) => [
      relative,
      Object.fromEntries(Object.entries(model).filter(([key]) => key !== "node")),
    ]),
  );
}
...
function impactSeeds(beforeAuthority, afterAuthority, beforeGraph, afterGraph, paths) {
  ...
  for (const relative of paths) {
    ...
    if (relative.startsWith("authority/dag/")) {
      ...
      for (const model of [beforeGraph, afterGraph])
        for (const [id, owner] of model.moduleById) if (owner === module) idsInModule.add(id);
    ...
    if (
      GLOBAL_CHANGE_ROOTS.has(root) ||
      relative === "authority/root.toml" ||
      relative.startsWith("authority/state/") ||
...
function authorityManifest(packageRoot) {
  ...
  for (const relative of AUTHORITY_DIGEST_ROOTS) {
    ...
    if (stat.isSymbolicLink() || !stat.isDirectory())
      throw new Error(`authority snapshot root must be a real directory: ${relative}`);
...
function changedPaths(beforeManifest, afterManifest) {
  ...
  return [...paths]
    .filter((relative) => beforeManifest.get(relative) !== afterManifest.get(relative))
    .sort();
}
...
function normalizedDerivedReceipts(values, label, impactClosure) {
  if (!Array.isArray(values))
    throw new Error(`${label}: deriveInvalidatedReceipts must return an array`);
  if (
    values.some(
      (value) => typeof value !== "string" || !/^[A-Z][A-Z0-9-]*:[0-9a-f]{64}$/u.test(value),
    )
  )
    throw new Error(`${label}: derived receipt references must use node-id:sha256`);
  if (new Set(values).size !== values.length)
    throw new Error(`${label}: derived receipt references must be unique`);
  const impacted = new Set(impactClosure);
  for (const value of values) {
    const nodeId = value.slice(0, value.indexOf(":"));
    if (!impacted.has(nodeId))
      throw new Error(`${label}: derived receipt node ${nodeId} is outside the impact closure`);
  }
  return [...values].sort();
}
...
function validateChangedPath(relative) {
  try {
    ...
    if (!AUTHORITY_DIGEST_ROOTS.includes(relative.split("/")[0]))
      return `changed path is outside the authority digest roots: ${relative}`;
    if (relative === LOCK_RELATIVE || relative.startsWith(`${AMENDMENTS_RELATIVE}/`))
      return `changed path names amendment-chain state: ${relative}`;
    return null;
  } catch (error) {
...
function validateAmendmentsInternal(authority, dependencies, { ignoreWriteLock = false } = {}) {
  const errors = [];
  if (typeof dependencies.computeAuthorityDigest !== "function")
    return ["amendments: computeAuthorityDigest dependency is required"];
  let trusted;
  try {
    trusted = loadTrustedRatifications(authority.packageRoot, dependencies);
  } catch (error) {
    trusted = { byKey: new Map(), errors: [error.message] };
  }
  errors.push(...trusted.errors);
  let currentDigest;
  let currentGraph;
  try {
    currentDigest = dependencies.computeAuthorityDigest(authority.packageRoot);
  } catch (error) {
    errors.push(`authority digest failed: ${error.message}`);
  }
  try {
    currentGraph = graph(authority, "current authority");
  } catch (error) {
    errors.push(error.message);
  }
  ...
  let lockResult;
  try {
    lockResult = parseLockFile(authority.packageRoot, dependencies);
  } catch (error) {
    lockResult = { lock: null, errors: [error.message] };
  }
  errors.push(...lockResult.errors);
  let inventory;
  try {
    inventory = amendmentFiles(authority.packageRoot, { ignoreWriteLock });
  } catch (error) {
    inventory = { files: [], errors: [error.message] };
  }
  ...
  if (currentDigest && lock.current_authority_sha256 !== currentDigest)
    errors.push(
      `authority lock current digest ${lock.current_authority_sha256} does not match ${currentDigest}`,
    );
  ...
  for (const file of inventory.files) {
    ...
    try {
      ...
      parsed = { row: null, digest: null, errors: [error.message] };
    }
    errors.push(...parsed.errors);
    if (!parsed.row || !parsed.digest) continue;
    const row = parsed.row;
    if (`${row.amendment_id}.toml` !== path.basename(file))
      errors.push(`${location}: filename must equal amendment_id`);
    if (ids.has(row.amendment_id))
      errors.push(`${location}: duplicate amendment id ${row.amendment_id}`);
    ...
    for (const key of [
      ...
      if (!sortedUniqueStrings(row[key]))
        errors.push(`${location}: ${key} must be strictly sorted and unique`);
    }
    ...
    if (typeof dependencies.deriveInvalidatedReceipts === "function") {
      try {
        ...
        if (missing.length || extra.length)
          errors.push(
            `${location}: invalidated_receipts must exactly equal mechanically derived receipts; missing=${JSON.stringify(missing)} extra=${JSON.stringify(extra)}`,
          );
      } catch (error) {
        errors.push(`${location}: invalidation derivation failed: ${error.message}`);
      }
    }
    if (
      !row.ratified_by ||
      ...
      errors.push(`${location}: ratified_by must be a non-empty normalized identity`);
    if (
      !SHA256.test(row.ratification_receipt_sha256 || "") ||
      /^0{64}$/u.test(row.ratification_receipt_sha256)
    )
      errors.push(`${location}: ratification receipt must be a non-placeholder SHA-256`);
    if (
      !SHA256.test(row.activation_predecessor_authority_sha256 || "") ||
      /^0{64}$/u.test(row.activation_predecessor_authority_sha256)
    )
      errors.push(
        `${location}: activation predecessor authority must be a non-placeholder SHA-256`,
      );
    const trustedKey = trustedSlotKey(
      "authority-amendment",
      row.ratified_by,
      row.ratification_receipt_sha256,
    );
    if (!trusted.byKey.has(trustedKey))
      errors.push(
        `${location}: amendment is not authorized by a trusted authority-amendment ratification slot`,
      );
    if (row.before_authority_sha256 === row.after_authority_sha256)
      errors.push(`${location}: before and after authority digests must differ`);
    if (!Number.isSafeInteger(row.before_generation) || row.before_generation < 0)
      errors.push(`${location}: before_generation must be a non-negative safe integer`);
    if (
      !Number.isSafeInteger(row.after_generation) ||
      row.after_generation !== row.before_generation + 1
    )
      errors.push(`${location}: generation must advance by exactly one`);
  }

  if (!Number.isSafeInteger(lock.generation) || lock.generation < 0)
    errors.push("authority lock generation must be a non-negative safe integer");

  if (records.length === 0) {
    if (lock.last_amendment_id !== "" || lock.last_amendment_sha256 !== "")
      errors.push("authority lock names an amendment but the chain is empty");
    if (lock.baseline_authority_sha256 !== lock.current_authority_sha256)
      errors.push("empty amendment chain requires identical baseline and current digests");
    if (lock.generation !== 0) errors.push("empty amendment chain requires generation 0");
    return errors;
  }
  if (!lock.last_amendment_id || !lock.last_amendment_sha256)
    errors.push("non-empty amendment chain requires a complete lock head");
  const reverse = [];
  ...
  if (visited.size !== records.length)
    errors.push("authority amendment chain does not contain every amendment file");
  const chain = reverse.reverse();
  if (chain.length) {
    const first = chain[0];
    const last = chain[chain.length - 1];
    if (first.row.previous_amendment_sha256 !== "")
      errors.push("first amendment must have an empty previous amendment digest");
    if (first.row.before_authority_sha256 !== lock.baseline_authority_sha256)
      errors.push("first amendment does not start at the locked baseline digest");
    if (first.row.before_generation !== 0)
      errors.push("first amendment must start at generation 0");
    for (let index = 1; index < chain.length; index += 1) {
      if (chain[index].row.before_authority_sha256 !== chain[index - 1].row.after_authority_sha256)
        errors.push(
          `${chain[index].row.amendment_id}: before digest does not equal the prior after digest`,
        );
      if (chain[index].row.previous_amendment_sha256 !== chain[index - 1].digest)
        errors.push(`${chain[index].row.amendment_id}: previous amendment digest mismatch`);
      if (chain[index].row.before_generation !== chain[index - 1].row.after_generation)
        errors.push(
          `${chain[index].row.amendment_id}: before generation does not equal the prior after generation`,
        );
    }
    if (last.row.after_authority_sha256 !== lock.current_authority_sha256)
      errors.push("last amendment after digest does not equal the locked current digest");
    if (
      last.row.amendment_id !== lock.last_amendment_id ||
      last.digest !== lock.last_amendment_sha256
    )
      errors.push("authority lock head does not name the last amendment");
    if (last.row.after_generation !== lock.generation)
      errors.push("authority lock generation does not equal the last amendment after generation");
    if (currentGraph && Array.isArray(last.row.changed_nodes)) {
      ...
      if (canonical(last.row.impact_closure) !== canonical(expected))
        errors.push(
          `${last.row.amendment_id}: impact_closure must exactly equal the changed-node descendant closure`,
        );
    }
  }
  return errors;
...
export function validateAmendments(authority, dependencies = {}) {
  try {
    return validateAmendmentsInternal(authority, dependencies);
  } catch (error) {
    return [`amendments: ${error.message}`];
  }
}
...
export function buildActiveAmendmentReactivation(authority, options, dependencies = {}) {
  if (!authority?.packageRoot)
    throw new Error("buildActiveAmendmentReactivation requires an authority packageRoot");
  const allowed = new Set([
    "previousPackageState",
    "previousActivationTransition",
    "previousAuthoritySha256",
    "previousAuthorityGeneration",
  ]);
  for (const key of Object.keys(options || {}))
    if (!allowed.has(key))
      throw new Error(`ACTIVE amendment reactivation option ${key} is not caller-controlled`);
  if (options?.previousPackageState !== "ACTIVE")
    throw new Error("amendment reactivation requires a prior ACTIVE package state");
  ...
  if (!transitionMatch || /^0{64}$/u.test(transitionMatch[1]))
    throw new Error(
      "amendment reactivation requires a non-placeholder prior activation transition",
    );
  if (
    !SHA256.test(options?.previousAuthoritySha256 || "") ||
    /^0{64}$/u.test(options.previousAuthoritySha256)
  )
    throw new Error(
      "amendment reactivation requires a non-placeholder prior ACTIVE authority digest",
    );
  if (
    !Number.isSafeInteger(options?.previousAuthorityGeneration) ||
    options.previousAuthorityGeneration < 0
  )
    throw new Error(
      "amendment reactivation requires a non-negative prior ACTIVE authority generation",
    );

  const chainErrors = validateAmendmentsInternal(authority, dependencies);
  if (chainErrors.length)
    throw new Error(
      `amendment reactivation requires a valid amendment chain: ${chainErrors.join("; ")}`,
    );
  const { lock, errors: lockErrors } = parseLockFile(authority.packageRoot, dependencies);
  if (lockErrors.length || !lock?.last_amendment_id || !lock.last_amendment_sha256)
    throw new Error(
      `amendment reactivation requires a non-empty authority amendment head${lockErrors.length ? `: ${lockErrors.join("; ")}` : ""}`,
    );
  const inventory = amendmentFiles(authority.packageRoot);
  if (inventory.errors.length)
    throw new Error(`amendment reactivation inventory is invalid: ${inventory.errors.join("; ")}`);
  const headFile = inventory.files.find(
    (file) => path.basename(file) === `${lock.last_amendment_id}.toml`,
  );
  ...
  const parsed = parseAmendmentText(
    fs.readFileSync(headFile, "utf8"),
    authority.packageRoot,
    `${AMENDMENTS_RELATIVE}/${path.basename(headFile)}`,
    dependencies,
  );
  if (parsed.errors.length || !parsed.row || parsed.digest !== lock.last_amendment_sha256)
    throw new Error(
      `amendment reactivation head is invalid: ${parsed.errors.join("; ") || "locked digest mismatch"}`,
    );
  const head = parsed.row;
  if (head.activation_predecessor_authority_sha256 !== options.previousAuthoritySha256)
    throw new Error(
      "amendment does not name the prior ACTIVE authority digest as its activation predecessor",
    );
  if (head.before_generation !== options.previousAuthorityGeneration)
    throw new Error("amendment does not start from the prior ACTIVE authority generation");
  if (
    head.after_authority_sha256 !== lock.current_authority_sha256 ||
    head.after_generation !== lock.generation
  )
    throw new Error("amendment does not end at the current locked authority identity");
  ...
  return Object.freeze({
    ...
    activation_predecessor_authority_sha256: head.activation_predecessor_authority_sha256,
...
function renderAmendment(row) {
  const prefix = [
    ...
    `activation_predecessor_authority_sha256 = ${quote(row.activation_predecessor_authority_sha256)}`,
...
function historySnapshot(packageRoot) {
  ...
  for (const file of inventory.files)
    result.set(`${AMENDMENTS_RELATIVE}/${path.basename(file)}`, fs.readFileSync(file));
  return result;
}
...
function equalHistory(left, right) {
  ...
  for (const [relative, bytes] of left)
    if (!right.has(relative) || !bytes.equals(right.get(relative))) return false;
  return true;
}
...
function restoreLock(lockFile, previousBytes, amendmentsDirectory) {
  ...
  const temporary = path.join(
    amendmentsDirectory,
    `.restore-lock-${crypto.randomBytes(8).toString("hex")}.tmp`,
  );
...
export function createAmendment(authority, options, dependencies = {}) {
  ...
  if (typeof dependencies.computeAuthorityDigest !== "function")
    throw new Error("createAmendment requires computeAuthorityDigest");
  if (typeof dependencies.loadAuthority !== "function")
    throw new Error("createAmendment requires loadAuthority");
  if (typeof dependencies.deriveInvalidatedReceipts !== "function")
    throw new Error("createAmendment deriveInvalidatedReceipts dependency is required");
  if (options && Object.hasOwn(options, "invalidatedReceipts"))
    throw new Error("invalidatedReceipts is derived and cannot be supplied by the caller");
  const id = options?.id;
  const beforeRoot = options?.beforeRoot;
  const ratifiedBy = options?.ratifiedBy;
  const receipt = options?.ratificationReceiptSha256;
  const requestedActivationPredecessor = options?.activationPredecessorAuthoritySha256;
  if (!AMENDMENT_ID.test(id || "")) throw new Error("amendment id must match AMD-[A-Z0-9-]+");
  if (typeof beforeRoot !== "string" || !fs.existsSync(beforeRoot))
    throw new Error("beforeRoot must name an existing authority package");
  if (
    typeof ratifiedBy !== "string" ||
    ratifiedBy.length === 0 ||
    ratifiedBy !== ratifiedBy.trim() ||
    /[\0-\x1f\x7f]/u.test(ratifiedBy)
  )
    throw new Error("ratifiedBy must be a non-empty normalized identity");
  if (!SHA256.test(receipt || "") || /^0{64}$/u.test(receipt))
    throw new Error("ratificationReceiptSha256 must be a non-placeholder SHA-256");
  ...
  try {
    fs.mkdirSync(writeLock, { mode: 0o700 });
  } catch (error) {
    if (error.code === "EEXIST")
      throw new Error("authority amendment write is already in progress");
    throw error;
  }
  ...
  try {
    ...
    if (!equalHistory(beforeHistory, currentHistory))
      throw new Error(
        "beforeRoot and current authority do not carry the same immutable amendment history",
      );
    ...
    if (beforeTrusted.errors.length)
      throw new Error(
        `beforeRoot trusted ratifications are invalid: ${beforeTrusted.errors.join("; ")}`,
      );
    if (afterTrusted.errors.length)
      throw new Error(
        `current trusted ratifications are invalid: ${afterTrusted.errors.join("; ")}`,
      );
    ...
    if (!beforeSlot || !afterSlot || canonical(beforeSlot) !== canonical(afterSlot))
      throw new Error(
        "a matching trusted authority-amendment ratification slot must exist unchanged in both before and after authority",
      );
    ...
    if (beforeLockResult.lock) {
      const beforeErrors = validateAmendmentsInternal(beforeAuthority, dependencies, {
        ignoreWriteLock: true,
      });
      if (beforeErrors.length)
        throw new Error(`beforeRoot amendment chain is invalid: ${beforeErrors.join("; ")}`);
    } else if (
      [...beforeHistory.keys()].some((relative) => relative.startsWith(`${AMENDMENTS_RELATIVE}/`))
    ) {
    ...
    if (!SHA256.test(beforeDigest) || !SHA256.test(afterDigest))
      throw new Error("authority digest callback must return lowercase SHA-256 values");
    if (beforeLockResult.lock && beforeLockResult.lock.current_authority_sha256 !== beforeDigest)
      throw new Error("beforeRoot digest does not match its authority lock");
    const paths = changedPaths(authorityManifest(beforeRoot), authorityManifest(packageRoot));
    if (paths.length === 0 || beforeDigest === afterDigest)
      throw new Error("an authority amendment must describe a non-empty authority change");
    ...
    const activationPredecessor = requestedActivationPredecessor || beforeDigest;
    if (!SHA256.test(activationPredecessor) || /^0{64}$/u.test(activationPredecessor))
      throw new Error("activationPredecessorAuthoritySha256 must be a non-placeholder SHA-256");
    const amendmentWithoutInvalidation = {
      ...
      activation_predecessor_authority_sha256: activationPredecessor,
    ...
    const invalidatedReceipts = normalizedDerivedReceipts(
      dependencies.deriveInvalidatedReceipts(
        invalidationContext("create", authority, amendmentWithoutInvalidation, beforeAuthority),
      ),
      `amendment ${id}`,
      closure,
    );
    const amendment = {
      ...amendmentWithoutInvalidation,
      invalidated_receipts: invalidatedReceipts,
    };
    const rendered = renderAmendment(amendment);
    amendment.payload_sha256 = rendered.digest;
    const parsedGenerated = parseAmendmentText(
      rendered.text,
      packageRoot,
      `${AMENDMENTS_RELATIVE}/${id}.toml`,
      dependencies,
    );
    if (parsedGenerated.errors.length)
      throw new Error(`generated amendment is invalid: ${parsedGenerated.errors.join("; ")}`);
    const lock = {
      schema: 2,
    ...
    const lockErrors = validateObject(
      lock,
      loadSchema(packageRoot, "authority-lock.schema.json"),
      "authority-lock",
      dependencies.validateSchemaObject,
    ...
    if (lockErrors.length)
      throw new Error(`generated authority lock is invalid: ${lockErrors.join("; ")}`);
    if (dependencies.computeAuthorityDigest(packageRoot) !== afterDigest)
      throw new Error("authority changed while the amendment was being prepared");
    ...
    const postErrors = validateAmendmentsInternal(authority, dependencies, {
      ignoreWriteLock: true,
    });
    if (postErrors.length)
      throw new Error(`installed amendment chain is invalid: ${postErrors.join("; ")}`);
    return { amendment, amendmentFile, amendmentSha256: rendered.digest, lock, lockFile };
  } catch (error) {
    if (lockInstalled) {
      try {
        restoreLock(path.join(packageRoot, LOCK_RELATIVE), previousLockBytes, amendmentsDirectory);
      } catch (restoreError) {
        throw new AggregateError(
          [error, restoreError],
          "amendment failed and the authority lock could not be restored",
        );
      }
    }
    if (amendmentInstalled && amendmentFile && fs.existsSync(amendmentFile))
      fs.unlinkSync(amendmentFile);
    throw error;
  } finally {
    for (const temporary of [amendmentTemporary, lockTemporary])
