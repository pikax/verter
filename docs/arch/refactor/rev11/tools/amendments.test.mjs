...
function loadFixtureAuthority(packageRoot) {
  return {
    packageRoot,
    nodes: JSON.parse(
      fs.readFileSync(path.join(packageRoot, "authority/dag/fixture.json"), "utf8"),
...
function dependencies(overrides = {}) {
  return {
    ...
    deriveInvalidatedReceipts: ({ impactClosure }) =>
      impactClosure.map((id) => `${id}:${sha256(`accepted:${id}`)}`).sort(),
    ...overrides,
  };
}
...
function makePackage(nodes) {
  ...
  fs.writeFileSync(
    path.join(root, "authority/dag/fixture.json"),
  ...
  fs.writeFileSync(
    path.join(root, "authority/state/ratification-receipts/test-maintainer.txt"),
    TRUSTED_RECEIPT_BYTES,
  );
  fs.writeFileSync(
    path.join(root, "authority/state/trusted-ratifications.toml"),
    [
      "schema = 2",
      "",
      "[[slot]]",
...
test("an empty baseline lock validates only while its exact authority digest remains current", (t) => {
  ...
  fs.writeFileSync(
    path.join(root, "authority/state/authority-lock.toml"),
    [
      "schema = 2",
  ...
  fs.writeFileSync(
    path.join(root, "authority/dag/fixture.json"),
    `${JSON.stringify([{ ...BASE_NODES[0], name: "unamended" }, BASE_NODES[1], BASE_NODES[2]], null, 2)}\n`,
  );
  assert.match(
    validateAmendments(authority(root), dependencies()).join("\n"),
...
test("creation rejects a forged ratifier, an unknown receipt, and a self-installed trust slot", async (t) => {
    ...
    await t.test(row.name, () => {
      ...
      try {
        assert.throws(
          ...
          /trusted authority-amendment ratification slot.*before and after authority/,
        );
        assert.equal(
          fs.existsSync(path.join(afterRoot, "authority/state/authority-lock.toml")),
          false,
        );
      } finally {
  ...
  const beforeRoot = makePackage(BASE_NODES);
  const afterRoot = makePackage([
    { ...BASE_NODES[0], name: "self-authorized" },
    BASE_NODES[1],
    BASE_NODES[2],
  ]);
  t.after(() => fs.rmSync(beforeRoot, { recursive: true, force: true }));
  t.after(() => fs.rmSync(afterRoot, { recursive: true, force: true }));
  fs.writeFileSync(
    path.join(beforeRoot, "authority/state/trusted-ratifications.toml"),
    "schema = 2\nslot = []\n",
  );
...
test("validation hashes the exact confined trusted receipt bytes and rejects a re-signed forged ratifier", (t) => {
  ...
  const forged = resignAmendment(
    original.replace(`ratified_by = "${TRUSTED_RATIFIER}"`, 'ratified_by = "forged-maintainer"'),
  );
  fs.writeFileSync(result.amendmentFile, forged);
  const lockFile = path.join(afterRoot, "authority/state/authority-lock.toml");
  fs.writeFileSync(
    lockFile,
    replaceLockHead(fs.readFileSync(lockFile, "utf8"), "AMD-TRUST-CHECK", headDigest(forged)),
  );
  assert.match(
    validateAmendments(authority(afterRoot), dependencies()).join("\n"),
    /not authorized by a trusted authority-amendment ratification slot/,
  );

  fs.writeFileSync(
    path.join(afterRoot, "authority/state/ratification-receipts/test-maintainer.txt"),
...
test("createAmendment records the exact path, changed-node, and before+after descendant closure", (t) => {
  ...
  assert.deepEqual(result.amendment.changed_paths, [
    "authority/dag/fixture.json",
    "charters/fixture/D.md",
  ]);
  assert.deepEqual(result.amendment.changed_nodes, ["A", "D"]);
  assert.deepEqual(result.amendment.impact_closure, ["A", "B", "C", "D"]);
  assert.deepEqual(
    result.amendment.invalidated_receipts,
    ["A", "B", "C", "D"].map((id) => `${id}:${sha256(`accepted:${id}`)}`),
  );
...
test("charter-only changes seed their node while global authority changes seed every node", async (t) => {
  await t.test("charter-only", () => {
    ...
    try {
      ...
      assert.deepEqual(
        result.amendment.invalidated_receipts,
        ["B", "C"].map((id) => `${id}:${sha256(`accepted:${id}`)}`),
      );
    } finally {
      fs.rmSync(beforeRoot, { recursive: true, force: true });
      fs.rmSync(afterRoot, { recursive: true, force: true });
  ...
  await t.test("global schema", () => {
    ...
    try {
      fs.writeFileSync(
        path.join(afterRoot, "schemas/global-policy.txt"),
        "global validation changed\n",
      );
      const result = createAmendment(
        authority(afterRoot),
        {
      ...
      assert.throws(
        () =>
          createAmendment(
            ...
            dependencies({ deriveInvalidatedReceipts: undefined }),
          ),
        /deriveInvalidatedReceipts dependency is required/,
      );
    } finally {
      fs.rmSync(beforeRoot, { recursive: true, force: true });
      fs.rmSync(afterRoot, { recursive: true, force: true });
  ...
  await t.test("caller override is forbidden", () => {
    ...
    try {
      assert.throws(
        ...
        /invalidatedReceipts is derived and cannot be supplied/,
      );
    } finally {
      fs.rmSync(beforeRoot, { recursive: true, force: true });
      fs.rmSync(afterRoot, { recursive: true, force: true });
...
test("validation rejects omitted and extra mechanically derived invalidated receipts", async (t) => {
  for (const mutation of ["omitted", "extra"])
    await t.test(mutation, () => {
      ...
      try {
        ...
        const forgedValues =
          mutation === "omitted"
            ? expected.slice(0, -1)
            : [...expected, `Z:${sha256("accepted:Z")}`];
        const forged = resignAmendment(
          original.replace(
            /^invalidated_receipts = .*$/m,
            `invalidated_receipts = ${JSON.stringify(forgedValues)}`,
          ),
        );
        fs.writeFileSync(result.amendmentFile, forged);
        const lockFile = path.join(afterRoot, "authority/state/authority-lock.toml");
...
test("validation rejects a schema-validly re-signed unknown amendment field", (t) => {
  ...
  const original = fs.readFileSync(result.amendmentFile, "utf8");
  const forged = resignAmendment(
    original.replace(/^payload_sha256/m, 'forged = "yes"\npayload_sha256'),
  );
  fs.writeFileSync(result.amendmentFile, forged);
  const lockFile = path.join(afterRoot, "authority/state/authority-lock.toml");
  fs.writeFileSync(
    lockFile,
    replaceLockHead(fs.readFileSync(lockFile, "utf8"), "AMD-TEST-3", headDigest(forged)),
  );
...
test("validation recomputes and rejects an incomplete impact closure even after re-signing", (t) => {
  ...
  const original = fs.readFileSync(result.amendmentFile, "utf8");
  const forged = resignAmendment(
    original.replace(/^impact_closure = .*$/m, 'impact_closure = ["A"]'),
  );
  fs.writeFileSync(result.amendmentFile, forged);
  const lockFile = path.join(afterRoot, "authority/state/authority-lock.toml");
  fs.writeFileSync(
    lockFile,
    replaceLockHead(fs.readFileSync(lockFile, "utf8"), "AMD-TEST-4", headDigest(forged)),
  );
...
test("the chain is append-only and duplicate amendment creation leaves the lock unchanged", (t) => {
  ...
  fs.writeFileSync(
    path.join(currentRoot, "authority/dag/fixture.json"),
    `${JSON.stringify(
      [{ ...BASE_NODES[0], name: "first" }, BASE_NODES[1], { ...BASE_NODES[2], name: "second" }],
      null,
      2,
    )}\n`,
  );
  ...
  assert.match(
    validateAmendments(authority(currentRoot), dependencies()).join("\n"),
...
test("amendment generations are monotonic, explicit, and portable across copied roots", (t) => {
  ...
  fs.writeFileSync(
    path.join(currentRoot, "authority/dag/fixture.json"),
    `${JSON.stringify(
      [{ ...BASE_NODES[0], name: "first" }, { ...BASE_NODES[1], name: "second" }, BASE_NODES[2]],
      null,
      2,
    )}\n`,
  );
  const second = createAmendment(
    authority(currentRoot),
    {
      id: "AMD-GENERATION-2",
      beforeRoot: secondBefore,
...
test("validation rejects non-monotonic amendment generations and a mismatched lock generation", (t) => {
  ...
  const original = fs.readFileSync(result.amendmentFile, "utf8");
  const forged = resignAmendment(
    original.replace(/^after_generation = 1$/m, "after_generation = 7"),
  );
  fs.writeFileSync(result.amendmentFile, forged);
  const lockFile = path.join(currentRoot, "authority/state/authority-lock.toml");
  fs.writeFileSync(
  ...
  fs.writeFileSync(
    ...
    fs
      ...
      .replace(
        /^last_amendment_sha256 = ".*"$/m,
        `last_amendment_sha256 = "${result.amendmentSha256}"`,
      ),
  );
  assert.match(
    validateAmendments(authority(currentRoot), dependencies()).join("\n"),
...
test("ACTIVE amendment reactivation is derived from the validated head and prior ACTIVE identity", (t) => {
  ...
  const amendment = createAmendment(
    authority(currentRoot),
    {
      ...
      ratifiedBy: TRUSTED_RATIFIER,
      ratificationReceiptSha256: TRUSTED_RECEIPT_SHA256,
      activationPredecessorAuthoritySha256: "d".repeat(64),
    },
    dependencies(),
  );
  ...
  const binding = buildActiveAmendmentReactivation(
    authority(currentRoot),
    {
      previousPackageState: "ACTIVE",
      previousActivationTransition: priorTransition,
      previousAuthoritySha256: amendment.amendment.activation_predecessor_authority_sha256,
  ...
  assert.deepEqual(binding, {
    ...
    activation_predecessor_authority_sha256:
      amendment.amendment.activation_predecessor_authority_sha256,
  ...
  assert.throws(
    ...
    /prior ACTIVE authority generation/,
  );
  assert.throws(
    () =>
      buildActiveAmendmentReactivation(
    ...
    /non-placeholder prior activation transition/,
  );
  assert.throws(
    () =>
      buildActiveAmendmentReactivation(
  ...
  assert.throws(
    ...
    /reactivatedBy is not caller-controlled/,
  );
});
