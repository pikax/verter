...
function validateReceiptContext(receipt, node, context, file) {
  ...
  if (node.id === "ORC0") {
    ...
    expectedActivation = orc ? reference("ORC0", orc) : "ORC0:MISSING";
...
export function deriveState(authority, { runtimeRoot = defaultRuntimeRoot(authority.packageRoot), openedOptional = [], now = Date.now() } = {}) {
  ...
  const resourceProfiles = catalogMap(authority.packageRoot, "resource-profiles.toml", "profile");
  for (const node of topological(authority.nodes).order) {
    ...
    if (node.id === "ORC0" && trustedOrc) states.set(node.id, { status: "ACCEPTED", blockers, trusted_local_acceptance: trustedOrc });
        ...
        for (const lease of leaseState.active) {
          const overlap = conflictReasons(node.conflict_domains, lease.conflict_domains, context.domains);
          if (overlap.length) blockers.push(`active lease conflict with ${lease.node_id}: ${overlap.join(",")}`);
        }
        const capacity = resourceProfiles.get(node.resource_class)?.capacity_hint || 0;
        const used = leaseState.active.filter((lease) => lease.resource_class === node.resource_class).length;
        if (used >= capacity) blockers.push(`resource capacity unavailable ${node.resource_class}: ${used}/${capacity}`);
