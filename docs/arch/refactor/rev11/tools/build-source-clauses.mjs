...
for (let block of blocks) {
  ...
  if (
    [
      "legacy-arch-reconciliation.md",
      "legacy-architecture-transfers.md",
      "github-control-plane-program.md",
      "successor-dag-amendment.md",
      "existing-node-amendments.md",
    ].includes(source)
  )
    applicable = scalar(block, "applicable_nodes");
  else if (target.startsWith("node:")) applicable = [target.slice("node:".length)];
  else {
    ...
    if (!candidates.length) applicable = ["ORC0"];
      ...
      applicable = [
        ...new Set(
          candidates
            .filter((candidate) => Math.abs(candidate.line - line) === distance)
  ...
  if (/^applicable_nodes = /m.test(block))
    block = block.replace(/^applicable_nodes = .*$/m, `applicable_nodes = ${serialized}`);
  else
    block = block.replace(/^disposition = /m, `applicable_nodes = ${serialized}\ndisposition = `);
  renderedBlocks.push(block.trimEnd());
  const relative = target.startsWith("contract:")
    ? target.slice("contract:".length)
    : nodesById.get(target.slice("node:".length))?.charter;
  if (relative) {
    const row = {
      id: scalar(block, "id"),
      ...
      source,
      from: line,
      to: scalar(block, "to_line"),
      target,
      digest: scalar(block, "text_sha256"),
...
for (const node of authority.nodes)
  if (!targetRows.has(node.charter)) targetRows.set(node.charter, []);
...
for (const [relative, rows] of targetRows) {
  ...
  const operative = pieces[0]
    .replace(/\n## Live authority inputs\n[\s\S]*?(?=\n## |\s*$)/g, "")
    .trimEnd();
  const node = nodesByCharter.get(relative);
  const boundLive = (node?.source_refs || [])
    .filter((ref) => ref.startsWith("live:"))
    .map((ref) => {
      const row = liveRows.get(ref);
      if (!row) throw new Error(`${node.id}: missing live lock ${ref}`);
      return `- \`${ref}\` — ${row.bytes} bytes, SHA-256 \`${row.sha256}\``;
    });
  const suffix = boundLive.length
    ? `## Live authority inputs\n\n${boundLive.sort().join("\n")}`
    : "";
...
for (const [file, output] of targetOutputs)
  if (fs.readFileSync(file, "utf8") !== output) stale.push(path.relative(PACKAGE_ROOT, file));
const packetRoot = path.join(PACKAGE_ROOT, "provenance/packet-source-clauses");
const packetOutputs = new Map();
for (const node of authority.nodes) {
  const rows = [...(packetRows.get(node.id) || [])].sort((left, right) =>
  ...
  const clauses = rows.length
    ...
    : "- none";
  packetOutputs.set(
    path.join(packetRoot, `${node.id}.md`),
    `# Exact operative source-clause attachment — ${node.id}\n\nSchema: 1. Node: \`${node.id}\`. Clause count: ${rows.length}. Generated from \`provenance/source-coverage.toml\`; every clause below is exact, operative, and applicable to this node.\n\n${clauses}\n`,
  );
}
const actualPacketFiles = fs.existsSync(packetRoot)
  ? fs
      .readdirSync(packetRoot)
      .filter((name) => name.endsWith(".md"))
      .map((name) => path.join(packetRoot, name))
  : [];
for (const file of actualPacketFiles)
  if (!packetOutputs.has(file)) stale.push(path.relative(PACKAGE_ROOT, file));
for (const [file, output] of packetOutputs)
  if (!fs.existsSync(file) || fs.readFileSync(file, "utf8") !== output)
    stale.push(path.relative(PACKAGE_ROOT, file));
if (check) {
  if (stale.length) {
    console.error(`STALE source clauses: ${stale.join(", ")}`);
    process.exit(1);
  }
  console.log(
    `build-source-clauses: PASS (${blocks.length} clauses; ${targetOutputs.size} operative targets)`,
  );
} else {
  ...
  console.log(
    `build-source-clauses: wrote ${blocks.length} clauses across ${targetOutputs.size} targets`,
  );
}
