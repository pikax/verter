...
const reviewProfiles = new Map(readToml(path.join(PACKAGE_ROOT, "catalogs/review-profiles.toml")).profile.map((profile) => [profile.id, profile]));
...
function replaceSection(text, heading, body) {
  return text.replace(new RegExp(`^## ${heading}\\n[\\s\\S]*?(?=^## )`, "m"), `## ${heading}\n\n${body}\n\n`);
}

function proofSection(node, text) {
  const testHomes = /^- Test homes:.*$/m.exec(section(text, "Acceptance IDs and discriminating proof"))?.[0];
...
function budgetSection(node, text) {
  ...
  if (preserved.length !== current.length - 1) throw new Error(`${node.id}: expected one performance budget`);
  preserved.push("- Performance budget: when preflight identifies touched authority or a hot path, equivalent-work counters may increase by 0 and wall/allocation/RSS regression allowance remains 0.0% unless an owning-authority amendment supplies exact replacement thresholds. Otherwise performance evidence is not applicable; do not create counters or a 100-request retention soak solely to satisfy this charter.");
...
for (const node of authority.nodes) {
  ...
  if (node.review_profile !== "history") {
    ...
    text = text.replace(/^## Review and lower-severity findings\n[\s\S]*?(?=^## )/m, `${reviewSection(node)}\n\n`);
    text = text.replace(/^## Dispatch-time immutable bindings\n[\s\S]*?(?=^## )/m, `${dispatchSection(node)}\n\n`);
    ...
    text = replaceSection(text, "Acceptance IDs and discriminating proof", proofSection(node, text));
    ...
    text = text.replace("required report-back schema (candidate SHA/tree, changed paths, migration/deletion counts, RED/GREEN commands and outputs, gate receipt digests, review report digests, residual findings, and abort/rescope decisions)", "required report-back schema (candidate SHA/tree, changed paths, migration/deletion counts, preflight proof selection and rationale, applicable behavioral TDD RED/GREEN evidence, selected existing/type/compiler/static/gate/inspection/benchmark evidence, gate receipt digests, review report digests, residual findings, and abort/rescope decisions)");
  }
