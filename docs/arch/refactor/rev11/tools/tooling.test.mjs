...
function receiptText(node, fields = {}) {
  ...
  const body = [
    ...
    'changed_paths = ["docs/arch/refactor/rev11/fixtures/receipt-candidate.txt"]',
...
test("receipt validation rejects digest-valid nonexistent Git identities", () => {
  ...
  withTempDir("rev11-receipt-git-", (directory) => {
    ...
    assert.match(
      validateReceiptFile(file, node, PACKAGE_ROOT).errors.join("\n"),
...
test("schema-incomplete routed receipts fail closed without typed-field crashes", () => {
  ...
  withTempDir("rev11-receipt-incomplete-", (directory) => {
    ...
    let checked;
    assert.doesNotThrow(() => {
      checked = validateReceiptFile(file, node, PACKAGE_ROOT);
    });
...
test("strict authority validation applies the activation schema", () => {
  withTempDir("rev11-schema-", (directory) => {
    ...
    const errors = validateAuthority(loadAuthority(directory), {
      strict: true,
...
test("activation transition schema separates initial activation from amendment reactivation", () => {
  const schema = JSON.parse(
    fs.readFileSync(path.join(PACKAGE_ROOT, "schemas/activation-transition.schema.json"), "utf8"),
  );
  const initial = {
    schema: 2,
    type: "activation-transition",
    transition_id: "ACT-INITIAL",
    from_state: "DORMANT",
    to_state: "ACTIVE",
    j1_receipt: `J1-LANDED-GRANDFATHERED:${"1".repeat(64)}`,
    orc0_receipt: `ORC0:${"2".repeat(64)}`,
    activation_authorization: `maintainer_unified_v2_activation:${"3".repeat(64)}`,
    authority_sha256: "4".repeat(64),
    activated_by: "maintainer",
    activated_at: "2026-08-28T12:00:00.000Z",
    payload_sha256: "5".repeat(64),
  };
  const reactivation = {
    schema: 3,
    type: "activation-transition",
    transition_id: "ACT-REACTIVATED",
    from_state: "ACTIVE",
    to_state: "ACTIVE",
    previous_activation_transition: `ACT-INITIAL:${"5".repeat(64)}`,
    authority_amendment: `AMD-SUCCESSOR:${"6".repeat(64)}`,
    activation_predecessor_authority_sha256: "4".repeat(64),
    before_authority_sha256: "4".repeat(64),
    after_authority_sha256: "7".repeat(64),
    before_generation: 0,
    after_generation: 1,
    reactivated_by: "pikax",
    ratification_receipt_sha256: "8".repeat(64),
    reactivated_at: "2026-08-28T13:00:00.000Z",
    payload_sha256: "9".repeat(64),
  };
  assert.deepEqual(lib.validateSchemaObject(initial, schema, "initial"), []);
  assert.deepEqual(lib.validateSchemaObject(reactivation, schema, "reactivation"), []);
  assert.match(
    lib
      .validateSchemaObject(
        { ...reactivation, authority_sha256: initial.authority_sha256 },
        schema,
        "hybrid",
      )
      .join("\n"),
    /exactly one schema variant, matched 0/i,
  );
});
...
test("charter validation rejects traversal before reading a file", () => {
  ...
  assert.match(
    validateCharters([node], PACKAGE_ROOT).join("\n"),
...
test("receipt validation requires exact predecessor and evidence context even for real Git objects", () => {
  ...
  const head = childProcess
    ...
    .trim();
  const tree = childProcess
    .execFileSync("git", ["show", "-s", "--format=%T", head], {
      cwd: PACKAGE_ROOT,
      encoding: "utf8",
      timeout: 30_000,
      killSignal: "SIGKILL",
...
test("external authorization is rejected unless its digest is immutably allowlisted", () => {
  ...
  withTempDir("rev11-external-manifest-", (runtimeRoot) => {
    ...
    fs.writeFileSync(
      path.join(external, "ORC0.toml"),
      `${body}payload_sha256 = "${digestPayload(body)}"\n`,
    );
    assert.match(
      lib.deriveState(authority, { runtimeRoot }).errors.join("\n"),
...
test("the closed legacy set is manifest-bound, not filename-derived", () => {
  withTempDir("rev11-legacy-manifest-", (directory) => {
    ...
    fs.writeFileSync(
      path.join(directory, "state/legacy-receipts/ORC0.toml"),
      `${body}payload_sha256 = "${digestPayload(body)}"\n`,
    );
    assert.match(
      lib
        .deriveState(authority, { runtimeRoot: path.join(directory, ".runtime-test") })
        .errors.join("\n"),
...
test("atomic admission and amendment APIs are part of the executable boundary", () => {
  ...
  assert.equal(typeof lib.reactivateTrustedProgram, "function");
...
test("gate execution custody runs real direct subprocesses and fails closed on exit and timeout", () => {
  withTempDir("rev11-gate-custody-", (directory) => {
    ...
    const [result] = lib.executeCommandPlan([`${process.execPath} ${pass}`], {
      cwd: directory,
      timeoutMs: 10_000,
    });
    ...
    assert.throws(
      () =>
        lib.executeCommandPlan([`${process.execPath} ${fail}`], {
          cwd: directory,
          timeoutMs: 10_000,
        }),
      /status=7/,
    ...
    assert.throws(
      () =>
        lib.executeCommandPlan([`${process.execPath} ${hang}`], { cwd: directory, timeoutMs: 50 }),
      /ETIMEDOUT|timed out|gate command failed/i,
    );
  });
});
...
test("review custody refuses a locally claimed capability absent immutable trusted ratification", () => {
  ...
  const checked = lib.validateReviewCapability(authority, {
    evidence_id: "FORGED-REVIEW",
    custody_binding: `review-capability:${"1".repeat(64)}`,
    reviewer_executable_sha256: "2".repeat(64),
...
test("the docs-domain final proof is bounded and cannot invoke the retired exhaustive docs gate", () => {
  const profiles = parseToml(
    fs.readFileSync(path.join(PACKAGE_ROOT, "catalogs/gate-profiles.toml"), "utf8"),
  ).profile;
  const docsDomain = profiles.find((profile) => profile.id === "docs-domain");
  assert.deepEqual(docsDomain.final, [
    "node docs/arch/refactor/rev11/tools/validate-program-dag.mjs --strict",
    "pnpm docs:build",
  ]);
  assert.equal(fs.existsSync(path.join(PACKAGE_ROOT, "tools/run-docs-gate.mjs")), false);
  const repository = path.resolve(PACKAGE_ROOT, "../../../..");
  const rootPackage = JSON.parse(fs.readFileSync(path.join(repository, "package.json"), "utf8"));
  const docsPackage = JSON.parse(
    fs.readFileSync(path.join(repository, "docs/package.json"), "utf8"),
  );
  assert.equal(rootPackage.scripts["docs:build"], "pnpm --filter docs build");
  assert.equal(docsPackage.scripts.build, "vitepress build");
  assert.doesNotMatch(
    `${rootPackage.scripts["docs:build"]}\n${docsPackage.scripts.build}`,
    /run-docs-gate|rev11\/tools/u,
  );
});
...
test("cold packets embed the exact node-scoped source clauses for small and large applicability sets", () => {
  ...
  for (const id of ["CCA1", "ORC0"]) {
    ...
    const attachment = fs
      .readFileSync(path.join(PACKAGE_ROOT, `provenance/packet-source-clauses/${id}.md`), "utf8")
      .trimEnd();
    const section = lib.packetSourceBindings(authority, node);
    assert.equal(
      section.includes(attachment),
      true,
      `${id} exact clause bytes must be in the persisted packet body`,
    );
...
test("review findings require candidate-bound one-time P2 disposition and authenticated next-cycle closure", () => {
  withTempDir("rev11-finding-disposition-", (directory) => {
    ...
    const completedAt = "2026-08-27T20:00:00.000Z";
    const closedAt = "2026-08-27T20:30:00.000Z";
    const expiresAt = "2099-08-27T21:00:00.000Z";
    const finding = {
      severity: "P2",
      fingerprint: "f".repeat(64),
      owner: "maintainer",
      class_wide_sweep: ["all matching call sites"],
      next_cycle_obligation: "close every matching call site in the ratified next cycle",
      next_cycle_receipt: "",
      authorization_binding: "",
      status: "AUTHORIZED_DEFERRED",
    };
    const closure = {
      schema: 1,
      type: "next-cycle-obligation-closure",
      receipt_id: "NEXT-REVIEW",
      node_id: "ORC0",
      ...
      owner: finding.owner,
      obligation: finding.next_cycle_obligation,
      status: "CLOSED",
      closed_at: closedAt,
      closure_evidence: ["class-wide sweep receipt SHA-256:abc"],
    };
    ...
    const disposition = {
      schema: 1,
      type: "one-time-finding-disposition",
      one_time: true,
      node_id: "ORC0",
      candidate_sha: candidateSha,
      candidate_tree: candidateTree,
      ...
      fingerprint: finding.fingerprint,
      owner: finding.owner,
      class_wide_sweep: finding.class_wide_sweep,
      expires_at: expiresAt,
      next_cycle_obligation: finding.next_cycle_obligation,
      next_cycle_receipt: finding.next_cycle_receipt,
    };
    ...
    const trustedLedger = [
      ...
      'receipt_path = "authority/state/ratification-receipts/p2-review.txt"',
      `receipt_sha256 = "${receiptDigest}"`,
      "",
      "[[slot]]",
      'purpose = "next-cycle-closure"',
      'ratified_by = "maintainer"',
      'receipt_path = "authority/state/ratification-receipts/p2-next-cycle.txt"',
      `receipt_sha256 = "${closureDigest}"`,
      "",
    ].join("\n");
    ...
    assert.match(
      lib
        .validateReviewFindingDispositions(
          authority,
          {
            ...row,
            findings: [
              { ...finding, authorization_binding: `finding-disposition:${"0".repeat(64)}` },
            ],
          },
          { now },
        )
    ...
    assert.match(
      lib
        .validateReviewFindingDispositions(
          authority,
          { ...row, candidate_tree: "3".repeat(40) },
          { now },
        )
        .join("\n"),
    ...
    assert.match(
      lib
        .validateReviewFindingDispositions(
          authority,
          { ...row, findings: [{ ...finding, severity: "P1" }] },
          { now },
        )
        .join("\n"),
      /blocking P1/i,
    ...
    assert.deepEqual(
      lib.validateReviewFindingDispositions(
        authority,
        {
          ...row,
          findings: [
    ...
    fs.writeFileSync(
      path.join(receiptDirectory, "p2-next-cycle.txt"),
      `${JSON.stringify(tooLateClosure)}\n`,
    );
    assert.match(
      lib
        .validateReviewFindingDispositions(authority, row, {
          now: Date.parse("2100-01-01T00:01:00.000Z"),
        })
        .join("\n"),
    ...
    fs.writeFileSync(
      ledgerFile,
      trustedLedger.split("[[slot]]")[0] + "[[slot]]" + trustedLedger.split("[[slot]]")[1],
    );
    assert.match(
      lib.validateReviewFindingDispositions(authority, row, { now }).join("\n"),
...
test("the exact committed custody boundary projects ORC0 only after finalization and keeps BR0, TCM0R, and amendments fail-closed", () => {
  ...
  withTempDir("rev11-custody-refusal-", (runtimeRoot) => {
    ...
    assert.equal(
      state.receipts.has("C1"),
      false,
      "an explicitly invalidated legacy receipt remains historical and is not projected as accepted",
    );
    assert.match(
      state.states.get("BR0").blockers.join("; "),
      /missing immutable static directive slot maintainer_rev11_repair_freeze_lift|missing immutable static directive slot maintainer_successor_genesis/,
    );
    assert.match(
      state.states.get("TCM0R").blockers.join("; "),
      /maintainer_tcm0_rescope_ratification|ORC0 activation/i,
    );
    const head = childProcess
      .execFileSync("git", ["rev-parse", "HEAD"], {
        cwd: PACKAGE_ROOT,
    ...
    const tree = childProcess
      ...
      .trim();
    const directive = lib.readToml(
      path.join(PACKAGE_ROOT, "authority/state/external-authorizations.toml"),
    ).authorization[0];
    ...
    assert.throws(
      () =>
        lib.importRuntimeArtifact(authority, {
          runtimeRoot,
          kind: "authorization",
          file: artifactFile,
        }),
      /directive-mode authorization-import is forbidden/i,
    );
    assert.throws(
      () =>
        lib.createDirectiveAuthorization(authority, {
          runtimeRoot,
          id: "ORC0",
          holder: "holder",
          leaseId: "missing",
        }),
      /exact ORC0 lease|candidate-finalize/i,
    );
    assert.throws(
      () =>
        lib.createAmendment(authority, {
          id: "AMD-LOCAL-BOOTSTRAP",
          beforeRoot: PACKAGE_ROOT,
          ratifiedBy: "local-self-bootstrap",
          ratificationReceiptSha256: "1".repeat(64),
          runtimeRoot,
        }),
      /trusted authority-amendment ratification slot/i,
    );
  });
});
