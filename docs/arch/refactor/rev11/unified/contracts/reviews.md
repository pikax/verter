...
This v2 policy is dormant and cannot supersede the currently binding review skill until ORC0 is ratified and accepted. After activation, every substantive node and every system, product, convergence, cutover, and terminal node requires three independent reviewers on one exact finalized candidate tree using the assigned lenses, model, and effort. Each digest-bound evidence object must be created by `programctl review-run`, which executes the exact executable named by a separately ratified immutable reviewer capability and reconciles its schema-valid canonical JSON report. Direct review evidence import is forbidden. The committed trusted-capability ledger is intentionally empty, so ORC0 activation remains review-custody-required until authentic external ratification exists; a repository script or locally written PASS report is not represented as independent GPT review. P0/P1 block final acceptance. A P2 may be accepted only when the currently binding owning policy explicitly authorizes that disposition and the evidence binds its stable fingerprint, named owner, class-wide bounded sweep, and a real next-cycle obligation/closure; otherwise it blocks. The immutable authorization receipt must be schema-valid, one-time, unexpired, and bound to the same node, candidate SHA/tree, review profile, lens, P2 severity, fingerprint, owner, sweep, obligation, and authenticated closure receipt. That closure must itself be schema-valid, trusted, digest-bound, `CLOSED`, time-consistent, and exact for the same identity; neither artifact can be replayed for another lens or finding. P3 follows the currently binding owning policy and does not consume a P2 disposition. There is no post-review content exemption: any content change invalidates the gate and all three verdicts. Final acceptance is exact-candidate clean 3/3 only: all three independent verdicts must be PASS, and “clean” means no P0/P1 and no unauthorized or undispositioned P2.

Every substantive delivery, convergence, product, system/foundational, public/wire, concurrency/lifetime, architecture, cutover, and terminal node requires its binding three-reviewer profile on `gpt-5.6-sol` at `ultra` effort. The recovery's mechanically generated proposal and split leaves are non-authoritative dispositions, not a lower-effort review class. No node may weaken final review because its implementation appears mechanical or small.
