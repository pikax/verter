...
- **CMP6-AC1 — sole-owner proof:** add `cmp6_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **CMP6-AC2 — positive contract:** add `cmp6_publishes_exact_compilerequest`; assert exact identities, provenance, completeness, and deterministic ordering.
- **CMP6-AC3 — incremental equivalence:** add `cmp6_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **CMP6-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
