...
- **TCM1-AC1 — sole-owner proof:** add `tcm1_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **TCM1-AC2 — positive contract:** add `tcm1_publishes_exact_certifiedtypeenginebinding`; assert exact identities, provenance, completeness, and deterministic ordering.
- **TCM1-AC3 — incremental equivalence:** add `tcm1_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **TCM1-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
