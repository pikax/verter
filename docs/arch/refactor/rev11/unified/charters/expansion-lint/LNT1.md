...
- **LNT1-AC1 — sole-owner proof:** add `lnt1_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **LNT1-AC2 — positive contract:** add `lnt1_publishes_exact_ruleid`; assert exact identities, provenance, completeness, and deterministic ordering.
- **LNT1-AC3 — incremental equivalence:** add `lnt1_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **LNT1-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
