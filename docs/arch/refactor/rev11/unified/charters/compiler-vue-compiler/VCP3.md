...
- **VCP3-AC1 — sole-owner proof:** add `vcp3_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **VCP3-AC2 — positive contract:** add `vcp3_publishes_exact_vuesemanticsnapshot`; assert exact identities, provenance, completeness, and deterministic ordering.
- **VCP3-AC3 — incremental equivalence:** add `vcp3_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **VCP3-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
