...
- **BR0-AC1 — sole-owner proof:** add `br0_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **BR0-AC2 — positive contract:** add `br0_publishes_exact_activation_receipt`; assert exact identities, provenance, completeness, and deterministic ordering.
- **BR0-AC3 — incremental equivalence:** add `br0_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **BR0-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
