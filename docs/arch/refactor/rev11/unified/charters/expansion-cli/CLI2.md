...
- **CLI2-AC1 — sole-owner proof:** add `cli2_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **CLI2-AC2 — positive contract:** add `cli2_publishes_exact_applicationservices`; assert exact identities, provenance, completeness, and deterministic ordering.
- **CLI2-AC3 — incremental equivalence:** add `cli2_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **CLI2-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
