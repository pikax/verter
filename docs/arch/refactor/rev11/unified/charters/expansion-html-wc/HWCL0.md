...
- **HWCL0-AC1 — sole-owner proof:** add `hwcl0_rejects_displaced_authority`; planting any deleted route must make the targeted gate fail.
- **HWCL0-AC2 — positive contract:** add `hwcl0_publishes_exact_htmlfacts`; assert exact identities, provenance, completeness, and deterministic ordering.
- **HWCL0-AC3 — incremental equivalence:** add `hwcl0_incremental_equals_fresh`; cancellation/stale/partial outcomes must be refused from warm publication.
- **HWCL0-AC4 — bounded work:** capture equivalent-work counters for the named surfaces; no extra parse, resolve, plan, emit, copy, allocation, or retained candidate is hidden by wall time.
