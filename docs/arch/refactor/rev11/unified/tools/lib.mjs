...
function validateSourceRefs(authority) {
  ...
  for (const node of authority.nodes) for (const ref of node.source_refs) {
    if (ref.startsWith("source:")) {
      ...
      try { confinedFile(repoRoot, relative, `${node.id} live reference`); }
  ...
  for (const file of files) {
    ...
    try {
      ...
      if (row.gate_profile === "docs-domain") {
        const docsExecution = result.results.find((entry) => entry.command === "node docs/arch/refactor/rev11/unified/tools/run-docs-gate.mjs");
      ...
      const scripts = fs.readdirSync(path.join(authority.packageRoot, "tools"), { withFileTypes: true })
        ...
        .map((entry) => path.posix.join("docs/arch/refactor/rev11/unified/tools", entry.name)).sort();
      ...
      ].map((entry) => `node docs/arch/refactor/rev11/unified/tools/${entry}`);
      const plan = { schema: 1, type: "unified-docs-gate-plan", package_root: "docs/arch/refactor/rev11/unified", syntax_inputs: scripts, test_inputs: tests, validators };
