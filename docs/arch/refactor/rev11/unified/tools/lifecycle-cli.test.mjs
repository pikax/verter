...
function prepare(temp) {
  ...
  const packageRoot = path.join(repo, "docs/arch/refactor/rev11/unified"); fs.rmSync(packageRoot, { recursive: true, force: true }); fs.cpSync(lib.PACKAGE_ROOT, packageRoot, { recursive: true });
  git(repo, "add", "-A", "docs/arch/refactor/rev11/unified"); git(repo, "commit", "-m", "test: install exact authority package");
...
function createCandidate(env) {
  ...
  const file = path.join(worktree, "docs/arch/refactor/rev11/unified/fixtures/orc0-trusted-local.txt"); fs.writeFileSync(file, "trusted-local candidate\n");
  git(worktree, "add", "docs/arch/refactor/rev11/unified/fixtures/orc0-trusted-local.txt"); git(worktree, "commit", "-m", "test: trusted-local ORC0 candidate");
...
test("programctl drives J1 grandfathering through trusted-local ORC0 activation", { timeout: 180_000 }, () => {
  ...
  try {
    ...
    git(env.repo, "add", "docs/arch/refactor/rev11/unified/authority/state/activation.toml"); git(env.repo, "commit", "-m", "test: bind J1 grandfathered landing");
