# A0 Preflight Record — Pre-Candidate Entry Inspection (HISTORICAL)

> **Scope of this record.** This is the pre-candidate entry inspection of the repository
> at base `9af553dd2`, written BEFORE any candidate existed and BEFORE the maintainer
> decisions recorded in [`maintainer-rulings.md`](maintainer-rulings.md) were made. It is
> retained as historical evidence of the entry state and of the blocking conditions found
> at that time. It is **NOT the current A0 state** — see
> [`A0-summary.md`](A0-summary.md) for the committed description of the A0 landing;
> the live A0 state and the exact-candidate record live in the external ledger and
> evidence root. Machine-absolute paths in the original capture have been replaced with stable
> placeholders, defined once here: `<REPO_ROOT>` = the local checkout of `pikax/verter`;
> `<DOWNLOADS>` = the operator's local downloads directory; `<EXTERNAL_EVIDENCE_ROOT>` =
> the external, non-committed program-evidence directory; `<SESSION_SCRATCHPAD>` = the
> orchestrator session's temporary scratchpad directory. Two capture-time factual slips
> are corrected inline with bracketed notes.
...
Only three loose artifacts exist in `<DOWNLOADS>` \[correction: four loose artifacts
existed — the original count omitted the start-here document (landed as
`release/opus-start-here-v11.md`, SHA-256
`d1186cd49c7d7368d9ec97a998516dfba806cc0075977c1ba169cbb0d9884f28`)\].
...
`<SESSION_SCRATCHPAD>/v11/`. Aggregate reconstruction digest
...
- root `<REPO_ROOT>`; remote `origin` = `https://github.com/pikax/verter` (fetch and push)
...
- dirty: **false**; untracked (`--untracked-files=all`): **0**; submodules: **none**; worktrees: **1**
  \[capture-time value only: a second worktree was later created for the Revision 11
...
- `gh` authenticated as `pikax`; the token's scopes were inspected and recorded in the
  external evidence root (enumeration withheld from this committed record)
...
`<EXTERNAL_EVIDENCE_ROOT>/program-state.toml` — template `schema = 1`, `revision = 11`,
...
- `<EXTERNAL_EVIDENCE_ROOT>/program-state.toml` (unvalidated)
- `<EXTERNAL_EVIDENCE_ROOT>/A0/facts.md`
- `<EXTERNAL_EVIDENCE_ROOT>/A0/A0-record.md` (the external original of this record;
  the committed copy at `evidence/A0-preflight-blocked.md` is sanitised — machine
  paths replaced with the placeholders defined above — and annotated with the
  bracketed corrections)
- reconstructed authority package (unattested):
  `<SESSION_SCRATCHPAD>/v11/`
...
performance-proven; A0 establishes an entry state only, and that entry state was unattested at the
