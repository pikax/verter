#!/bin/bash
# Reopen-3 reattestation session for the two locked cells
# BF2_VUE_ORACLE_MANIFEST_GENERATE / BF2_SVELTE_ORACLE_MANIFEST_GENERATE.
# Runs the frozen, unmodified generate-official-case-manifests.mjs under the
# committed deny-network sandbox profile, N times, capturing wall time
# (real, /usr/bin/time -l) and peak RSS per run, and verifying the
# deterministic correctness oracle (exact row counts + byte-identical TSVs
...
VUE_MODULES="/tmp/bv0-oracle-installs/vue"
