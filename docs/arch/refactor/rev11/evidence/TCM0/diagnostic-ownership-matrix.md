...
## Open item — required convergence, no valid later-block owner yet
...
not a decision it can execute itself (no production code changes in this block). The later-block
owner is **not** TCM1/TCM2: `TCM1.md` forbids semantic-API clients, and `TCM2.md` assigns
semantic-session ownership to TCM3. See `OPEN-GAPS.md`'s `G-DIAGNOSTIC-CONVERGENCE` row for the
named owner (TCM0) and gate (TCM0's own acceptance). Until that gate closes, this item stays OPEN
