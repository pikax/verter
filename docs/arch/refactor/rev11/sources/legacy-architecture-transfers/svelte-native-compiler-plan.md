...
`svelte@5.56.10` plus helper-topology goldens (structure + helper-call sequence, NOT bytes).
...
2. **Official Svelte 5 compiler output is the conformance target**, pinned to the EXACT version
   **`svelte@5.56.10`** (the current lockfile version; verified `compiler.VERSION === '5.56.10'`).
...
### 3.2 Official-output `$.` helper-call matrix (empirically derived from `svelte@5.56.10`)

This matrix is DERIVED from real `svelte@5.56.10` compiler output (`compile(src, {generate:'client'})`
...
pinned `svelte@5.56.10` compiler — it is not, and is not intended to be, a hand-maintained exhaustive
...
**The full empirical `$state` classification rule (verified against `svelte@5.56.10`).** A `$state`
...
| `{#each}` keyed | `$.each(node, FLAG, () => items, (item) => key, ($$anchor, item) => {…})` — the `item` binding is a SIGNAL: reads inside the block are `$.get(item)` (e.g. `$.set_text(text, $.get(item).x)`, `() => ($.get(item).x++)`), NOT inert plain locals (§3.3). The SIGNAL marking is owned by 5e; discharged D-35 supplies the member interpolation carrier. | `<!--[-->` + `const each_array = $.ensure_array_like(items)` + `for` loop pushing items + `<!--]-->` | 5e | in-scope |
...
- When the `runes` option is UNSET, mode is INFERRED from rune presence — a component with NO runes
  ...
  legacy flag.
- When the `runes` option is SET, it OVERRIDES inference. Verified empirically against `5.56.10`:
...
`import 'svelte/internal/flags/async';`. Confirmed empirically: at `5.56.10` the official compiler did
...
- **Each/await-introduced bindings are SIGNAL reads, not inert plain locals (verified).** A keyed
  ...
  is NOT this signal and must not be rewritten). See the 5e rows in §3.2. The 5e-local responsibility is marking each/await-introduced bindings as `$.get`/`$.set` signals. The global member/optional-member/call/binary/conditional/static text carrier is owned by Block 4 and is now discharged as **D-35**; it consumes the 5e binding fact so `{item.x}` emits `$.get(item).x` identically at top level and inside blocks. A naked component-root text-node region remains separately refused as `RootTextRegion`.
...
### 3.7 `<style>` compilation + CSS scoping (first-class, empirically derived from `svelte@5.56.10`)
...
- **CSS hash.** The `svelte-<hash>` class is derived from the component CSS via the official compiler's
  ...
  block's STEP-0 pins the hash algorithm against `svelte@5.56.10` goldens). The scoped selector is emitted
...
post-release completeness).** `svelte@5.56.10` REFUSES a value `enum` / `namespace` (value-`TSModuleDeclaration`)
...
### 3.8 `<svelte:options>` + compile-option axis (first-class, empirically derived from `svelte@5.56.10`)
...
`customElement`. Verified at `5.56.10`: `compile('<svelte:options tag="my-el"/><h1>hi</h1>')` throws
...
`compile(src, { generate, ...option })` at `5.56.10`); the block's STEP-0 pins the exhaustive per-option
goldens (the §3.2 matrix-scope note: exhaustive goldens defer to block-start).

**Output-affecting options (each verified against `5.56.10`):**
...
**Exhaustive field accounting (axis closed at `5.56.10`).** Every field of the three authoritative interfaces in `svelte/types/index.d.ts` is classified above — none remain unaccounted:
...
- `ModuleCompileOptions` (~line 1159) — 6 own fields, ALL classified: output-affecting → `dev` (5k), `generate` (4/8), `experimental.async` (5j) (3); inert → `filename`, `rootDir`, `warningFilter` (3). `experimental` has exactly one member at `5.56.10` (`async`); no other experimental sub-field exists.
...
The only output-affecting field previously missing from this table was `name`; it is now listed. No other unlisted field changes the emitted JS/CSS topology at `5.56.10`.
...
The oracle and the runtime harness are anchored to ONE pinned version, **`svelte@5.56.10`**:

- **Pin constant.** A single source-of-truth constant `SVELTE_ORACLE_VERSION = "5.56.10"` (the oracle
...
- **jsdom harness uses the SAME pin.** The behavioral harness imports `svelte/internal/client` from
  the identical `svelte@5.56.10` resolved in the lockfile (no second floating dependency) so emitted
...
Verter and official `svelte@5.56.10` compile the exact same sixteen-revision authored-source cycle in
...
| D-11 | **Near drop-in feature parity with the official Svelte 5 compiler** — every real official-compiler feature is in-scope with a block (§9.1 in-scope matrix). The ONLY justified exclusions are deeper-than-official optimizations (D-9) and surfaces official `5.56.10` does not emit; experimental async stays gated, mirroring official's own gate (§3.2.2). No silent feature deferrals. |
...
- **Any helper the official `5.56.10` compiler does not emit** — by definition not a parity surface;
...
- **Playground Svelte preview.** Verified Vue-hardwired: `packages/playground/src/core/importMap.ts::
  ...
  and `svelte/internal/server` for SSR) entries to the import map (CDN ESM at the pinned `5.56.10`),
...
- **Comparison and equivalence.** Verter-Svelte and exact pinned `svelte@5.56.10` compile the same ten
...
- **Isolation and provenance.** Wall measurements use a bounded process per fixture/backend. Every
  ...
  `5.56.10` pin. The unmeasured parent attests the clean full Git SHA and exact native artifact name
...
- Svelte runtime typed-unsupported (`CompileUnsupported`) diagnostics — removed for each surface once
  ...
  `CompileUnsupported` path for an official `5.56.10` feature remains.
...
- Svelte golden oracle (feature-gated; client + SSR), pinned to `svelte@5.56.10`.
...
- jsdom behavioral runtime harness (client) against the pinned `svelte/internal/client@5.56.10`
  runtime; SSR string-render harness (Block 8) against the pinned `svelte/internal/server@5.56.10`
...
- Numeric perf gate (§7), run incrementally per feature-family sub-block landing (not only at Block
  12): every oracle-backed manifest fixture stays ≤ 1.10× pinned official `svelte@5.56.10` on median
