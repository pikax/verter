...
guard, admission/budget)`. The U2-landed rows are the FIVE landed spine keys + three U2 added keys +
the `Relate` upgrade + the migrated `Instantiate`/`ResolveMacroPayload` (the two augmentation spine
keys `ResolveMergedDeclaration` / `ResolveDeclarationAugmentation` are **forward-planned** — see
§2.2's landed-vs-forward-planned note — and carry NO spec row); the parent §2.9 table is the
