...
| **LSP diagnostics** | No — the authoritative resolved-TYPE TEXT rests on hover, not diagnostics | `Correct` | Verter diagnostics are native lint verdicts (unknown-prop, missing-slot) plus native operational warnings, not resolved-type strings competing with tsc's resolved type — so there is no "type X vs type Y" contradiction here. Projection work and connected-query-depth exhaustion surface as the stable `verter/type-expansion-budget` and `verter/type-query-depth-limit` codes respectively; these warnings survive when lint configuration is absent because they report an incomplete semantic demand, not an optional lint rule. (The suppression on the `sync_coordinator` publish path is narrow and partial: `publish_merged_diagnostics` drops only `verter/unknown-prop` / `verter/unknown-model` when a TypeProvider is active, `crates/verter_lsp/src/sync_coordinator.rs:381-388`; the background-publish path does not apply that `retain`, `crates/verter_lsp/src/background_init.rs:404-444`.) |
...
1. **`resolver_is_single_spec`** — a **CLOSED absence invariant** enforced by **two
   ...
   **(b) Structural field-inventory assertion over an EXPLICIT, CLOSED target list.** A
   ...
   `ComponentMetaResultKey`, `MaterializeRuntimeKey` / `MaterializationCacheKey`, `ShapeCacheKey`, the named
...
| **reuse (unchanged):** `tsgo_not_reachable_from_resolver`, `oracle_consumption_path_has_no_tsgo_spawn`, `cache_key_axes_are_minimal_and_normalized`, `cache_satisfaction_is_materialized_point_not_nominal_demand` + `backfill_writes_only_recorded_materialized_points` | The existing U0 / parity-engine guards: tsgo never on the query path; the static `DemandAxis` minimality mask; the landed materialized-point warm-hit/backfill rule (which superseded the retired planned `cache_satisfaction_is_demand_lattice_not_enum_order`). | Unchanged and untouched by this model — there is no spec dimension interacting with them. |
...
| reuse (unchanged): `tsgo_not_reachable_from_resolver`, `oracle_consumption_path_has_no_tsgo_spawn`, `cache_key_axes_are_minimal_and_normalized`, `cache_satisfaction_is_materialized_point_not_nominal_demand` + `backfill_writes_only_recorded_materialized_points` | parity engine / U0 | tsgo never on the query path (§7.4); the static `DemandAxis` minimality mask and the landed materialized-point warm-hit/backfill rule (superseding the retired planned `cache_satisfaction_is_demand_lattice_not_enum_order`) — UNCHANGED and untouched by this model (there is no spec dimension to interact with them) |
