...
| SDL-5 | Construct-signature intersection instance side (`SV03_construct_intersection`) | 7.0.2 reduces the instance type of a construct-signature intersection to `B`; the consumer-expanded answer is the FIRST arm's instance (measured `DeclRef(A)`). | Owed by the `SignaturesOfType` set authority over `ReduceIntersection`. `independent difference/incompleteness` — a typed gap, not an order effect. |
