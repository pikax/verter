...
direct compiler-core closure charter in the program DAG.
...
- **Svelte (experimental):** native **client** compilation is experimental and
  ...
  Svelte compiler product charter in the program DAG.
...
The detailed type-resolution and editor-facing diagnostic rules live in the
owning
[type-resolution skill](https://github.com/pikax/verter/blob/main/.claude/skills/type-resolution/SKILL.md).
...
The full cache contract is documented in the owning
...
the external LSP branch**: LSP-owned architecture-guard cleanup,
...
language-service convergence charter,
engine-activation charter (both in the program DAG),
...
owned by the Rev11
native-checker constitution in the program DAG,
the [testing skill](https://github.com/pikax/verter/blob/main/.claude/skills/testing/SKILL.md),
and the
[type-cache architecture skill](https://github.com/pikax/verter/blob/main/.claude/skills/type-cache-architecture/SKILL.md).
Those authorities describe planned work, not shipped capability; this document
remains the authority for what is shipped.
