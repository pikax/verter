...
pub fn extract_navigation_links(
    components: &[crate::template::TemplateComponentUsage],
    file_path: &str,
) -> Vec<NavigationLink> {
...
fn extract_nav_target_from_props(props: &[crate::template::TemplatePropUsage]) -> NavigationTarget {
    for prop in props {
        if prop.name == "to" {
            if !prop.is_bound {
                // Static: to="/about"
...
pub fn build_route_analysis(
    project_root: &std::path::Path,
    template_components: &[(String, Vec<crate::template::TemplateComponentUsage>)],
