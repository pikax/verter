...
impl NapiVerterHost {
    ...
    /// node kind. The compile shape is the host default; a caller that needs
    /// to state production mode, SSR, source maps, or a target explicitly
    /// uses the typed `compileRequest` route instead.
    ///
    ...
    #[napi(js_name = "getIde")]
    pub fn get_ide(&self, canonical_id: String) -> Result<Option<NapiIdeResponse>> {
        let host_profile = verter_session::CompileProfile::default();
        let result = catch_panic(std::panic::AssertUnwindSafe(|| {
    ...
    ///
    /// The default compile shape is normalized to an IDE/TSX-bearing target
    /// INTERNALLY, so the bundler default (no TSX bit) still produces the IDE
    /// surface. Returns `true` when the carrier HAS an IDE surface, and
    /// `false` ONLY for a genuine no-IDE-surface file (a non-carrier / plain
    /// script). A real failure (missing source / compile error) rejects.
    #[napi(js_name = "ensureIdeCompiled")]
    pub fn ensure_ide_compiled(&self, canonical_id: String) -> Result<bool> {
        let host_profile = verter_session::CompileProfile::default();
        catch_panic(std::panic::AssertUnwindSafe(|| {
...
/// the full output-affecting projection a bundler render states: Vue's
/// private render worker reproduces the host-backed runtime output
/// byte-for-byte, while Svelte keeps the effective host-backed path.
#[napi(object)]
pub struct NapiCompileBatchRenderProfile {
    ...
    /// default (`!isProduction` — dev preserves, prod strips). Do NOT
