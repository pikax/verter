...
pub mod __private {
    ...
    /// shadow / custom `Arc` that could OWN a `TypeExpr` FAILS to compile (closing
    /// the bare/shadowed-`Arc` hole a spelling check leaves open). The bound
    /// resolves NON-recursively — it does NOT require `Owner: NoTypeExprWitness` —
    /// so it proves the shape WITHOUT reintroducing the `Arc<[Self]>: NoTypeExpr`
