...
use oxc_semantic::{
    AstNodes, NodeId, ReferenceId, ScopeId, Scoping, Semantic, SemanticBuilder, SymbolFlags,
    SymbolId,
};
...
pub fn canonicalize_module(code: &str, authored: &BTreeSet<String>) -> Result<CanonModule, String> {
    ...
    let comments = anchor_comments(&semantic, code);
...
impl Classifier {
    fn build(semantic: &Semantic, authored: &BTreeSet<String>) -> Classifier {
        let scoping = semantic.scoping();
        let nodes = semantic.nodes();
    ...
    fn classify_binding(&self, ident: &BindingIdentifier) -> Canon {
    ...
    fn classify_reference(&self, scoping: &Scoping, ident: &IdentifierReference) -> Canon {
...
fn anchor_comments(semantic: &Semantic, source: &str) -> HashMap<NodeId, Vec<Canon>> {
...
fn refused(group: &str, variant: &str) -> ! {
    panic!("refused AST variant {group}::{variant} (out of contract: TS/JSX/intrinsic)")
}

fn kind_name_of_declaration(declaration: &Declaration) -> &'static str {
    match declaration {
...
struct Canonizer<'a, 'b> {
    scoping: &'b Scoping,
    #[allow(dead_code)]
    nodes: &'b AstNodes<'a>,
    classifier: &'b Classifier,
    comments: std::cell::RefCell<HashMap<NodeId, Vec<Canon>>>,
    specifier_import_sources_seen: std::cell::RefCell<BTreeSet<String>>,
}
...
impl<'a, 'b> Canonizer<'a, 'b> {
    ...
    fn canon_program(&self, program: &Program) -> Canon {
    ...
    fn extract_imports(&self, program: &Program) -> Vec<ImportEntry> {
