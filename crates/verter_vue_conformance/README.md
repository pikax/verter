...
source map maps its own compiler's output, so Verter-map vs official-map is
meaningless for golden comparison. Verter's source-map correctness is
verified separately by the position-encoding tests. Line numbers and
generated positions generally are waived (the two compilers structure output
differently, so positions are cosmetic).
...
## Structural comparator (`src/canon.rs`, `src/compare.rs`)
...
parens, ordinary comments, quote delimiters, empty statements, **line
numbers / generated positions generally**, and the spelling of
private/compiler-generated local bindings — via scope-aware alpha
equivalence (`oxc_semantic` is fail-closed; `BindingKey = (structural scope
identity, declaration ordinal, binding pattern slot, binding kind)`,
...
anchored to their AST occurrence node, and the ordered
`(severity, code, message)` diagnostic sequence (never positions). Source
maps are NOT compared (see "Oracle topology" above). Every JS AST variant is
represented; TS/JSX/intrinsic variants are explicitly refused (no catch-all).
...
`$evtclick` ABI rename), and common (source-authored rename, exported-default
rename, diagnostics reorder, PURE-comment drop/move, import
source/imported-helper change). Cosmetic controls include whitespace reformat
AND a line-number shift — both still PASS.
...
golden (code + diagnostics; source maps are NOT compared — see above).
Every cell Verter currently fails is tracked in
