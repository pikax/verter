...
/// query. Membership structs therefore store `CompiledGlob`s, compiled once
/// at snapshot/membership construction time. [`NormalizedGlob`] keeps its
/// plain-string value semantics (`Clone`/`PartialEq`/`Hash`).
///
