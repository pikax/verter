...
use crate::analysis::type_eval::{evaluate_with_lookup, EvalEnv, EvalLookup, NoopEvalLookup};
use crate::analysis::type_expr::{LiteralValue, ObjectExpr, ObjectMember, TypeExpr};
    ...
    match expr {
        ...
        TypeExpr::Ref {
            ...
            if let Some(decl) = decl {
                ...
                let saved = crate::analysis::type_eval::bind_type_parameters_with_lookup(
                    &decl,
                    type_arguments,
                    env,
                    lookup,
                ...
                crate::analysis::type_eval::restore_type_parameters(saved, env);
        ...
        TypeExpr::Conditional {
            ...
            if crate::analysis::type_eval::is_assignable_to(&check_eval, &extends_eval) {
            ...
            } else if crate::analysis::type_eval::is_definitely_not_assignable(
                &check_eval,
                &extends_eval,
            ) {
...
fn function_expr_to_call_sig(
    sig: &crate::analysis::type_expr::FunctionExpr,
    env: &mut EvalEnv,
    diagnostics: &mut Vec<ExpansionDiagnostic>,
    normalize_members: bool,
    lookup: &mut dyn EvalLookup,
...
fn is_infinite_source(source: &TypeExpr) -> bool {
    matches!(
        ...
        TypeExpr::Primitive(crate::analysis::type_expr::PrimitiveName::String)
            ...
            | TypeExpr::Primitive(crate::analysis::type_expr::PrimitiveName::Symbol)
    )
}
