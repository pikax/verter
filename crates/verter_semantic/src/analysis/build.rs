...
use crate::analysis::exports::extract_export_signatures_from_program;
            ...
            Statement::ExportDefaultDeclaration(export) => {
                if let Some(expr) = export.declaration.as_expression() {
                    let source_map: FxHashMap<String, String> = import_map
                        ...
                        .collect();
                    options_api = crate::analysis::options::try_extract_options_from_expression(
                        expr,
                        content,
                        &source_map,
                        &program.comments,
...
fn try_extract_dom_query(expr: &Expression<'_>, dom_query_calls: &mut Vec<DomQueryCallSite>) {
    ...
    let parsed = match kind {
        DomQueryKind::QuerySelector | DomQueryKind::QuerySelectorAll => {
            crate::analysis::style::parse_selector(&selector_text)
        }
        DomQueryKind::GetElementById => {
            // Synthesize a selector: #id
            crate::analysis::style::parse_selector(&format!("#{selector_text}"))
        }
