                ...
                for member in &obj.properties {
                    if let ObjectMember::Property(p) = member {
                        if let Some(existing_index) = merged.iter().position(|existing| {
                            ...
                            {
                                existing_prop.ty = match (
                                    extract_object_shape(&existing_prop.ty),
                                    extract_object_shape(&p.ty),
                                ) {
                                    (Some(existing_shape), Some(next_shape)) => merge_intersection(vec![
                                        TypeExpr::Object(Arc::new(existing_shape)),
                                        TypeExpr::Object(Arc::new(next_shape)),
                                    ]),
                                    _ => p.ty.clone(),
                                };
