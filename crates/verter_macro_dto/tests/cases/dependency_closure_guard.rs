//! STRUCTURAL dependency-closure firewall for the neutral macro-DTO crate.
//!
//! `verter_macro_dto` is the dependency-neutral hand-off vocabulary between
//! the shared resolution engine and the compiler's macro codegen — BOTH a
//! parser/compiler/oxc-free resolution-side producer AND `verter_compiler`
//! must be able to depend on it, so its own production closure must reach
//! NEITHER side's machinery.
//!
...
//! `verter_macro_dto` never reaches:
//!
//! * `verter_parser`, `verter_compiler`, `verter_session`,
//!   `verter_semantic`, `verter_protocol`, `verter_type_expr`,
//!   `verter_type_expr_oxc` — forbidden outright (unlike the query-boundary
//!   crate, this crate must not even reach the typed IR: it is `NoTypeExpr`
//!   all the way down);
//! * every `oxc`-prefixed crate — forbidden, with ONE pinned exception:
//!   the span-primitive subtree. `verter_no_typeexpr` (a sanctioned
//!   production dependency — it owns the `Span`-is-a-witness leaf fact)
//!   depends on `verter_span`, which depends unconditionally on `oxc_span`,
//!   which carries its own small oxc-prefixed tail (the miette diagnostics
//!   fork, the arena allocator, string/estree primitives). The sanctioned
//!   set is computed STRUCTURALLY as `oxc_span`'s own production closure
//!   and then equality-pinned: its `oxc`-prefixed membership must EQUAL the
//!   audited allowlist ([`OXC_SANCTIONED_ALLOWLIST`]), so an `oxc_span`
//!   release that grows (or shrinks) the tail is a re-audit gate, never a
//!   silent grant. The subtree is additionally canaried: it must contain no
...
//!   grow to admit a parser). Its sole entry edge from this crate's closure
//!   must be `verter_span → oxc_span`: a direct dependency on `oxc_span`
//!   (or any member of the tail) from any other closure crate fails.
//!
...
//! library, and this guard's own tooling dev-deps (`serde_json`, the
//! negative-witness `verter_span` / `verter_type_expr`) must not self-trip
//! it.
...
/// Crates whose presence anywhere in the neutral DTO crate's production
/// closure is a firewall breach, regardless of route.
const FORBIDDEN_EVERYWHERE: [&str; 7] = [
    "verter_parser",
    "verter_compiler",
    "verter_session",
    "verter_semantic",
...
fn workspace_manifest() -> PathBuf {
    // tests/ lives at crates/verter_macro_dto/, two levels below the
    // workspace root.
...
/// closure. `required_reachable` is the non-vacuity canary: crates the root
/// GENUINELY reaches through real (code-used) dependencies, so a broken
/// walk that reached nothing cannot pass — it must name the root's actual
/// reach, never a generic list.
    ...
    for forbidden in FORBIDDEN_EVERYWHERE {
        assert!(
            ...
            "{root}: FIREWALL BREACH — `{forbidden}` is reachable in the production \
             dependency closure; the neutral macro-DTO vocabulary must reach neither the \
             parser/compiler front-end nor the session/semantic/protocol/typed-IR side"
        );
    }
...
#[test]
fn macro_dto_closure_excludes_parser_compiler_session_semantic_and_typed_ir() {
    let metadata = workspace_metadata();
    let graph = ResolveGraph::from_metadata(&metadata);

    // The non-vacuity canary names the root's GENUINE reach:
    // `verter_macro_dto` code-uses both marker crates (every public
    // aggregate derives `NoTypeExpr` + `NoStoredSpan`) and reaches
    // `verter_span` transitively through `verter_no_typeexpr` (which owns
    // the `Span`-is-a-witness leaf fact).
    assert_firewalled_closure(
        &graph,
        "verter_macro_dto",
        &["verter_no_typeexpr", "verter_no_storedspan", "verter_span"],
    );
}
