...
fn faithful_no_import_fixture() -> (&'static str, &'static str, ProviderPositionMapper) {
    ...
    let mapper = ProviderPositionMapper::source_map(
        PositionMapper::from_json(&json).expect("valid source map"),
    );
...
/// `(vue_source, tsx, mapper)`.
fn faithful_no_mapped_run_fixture() -> (&'static str, &'static str, ProviderPositionMapper) {
    let vue_source = concat!(
        "<script setup lang=\"ts\">\n", // line 0
...
fn strict_mapper_does_not_resolve_synthetic_preamble_offset() {
    ...
    assert!(
        tsx_range_to_carrier_range(user_code_off, user_code_off + 5, &tsx_li, &mapper, &vue_li)
            .is_some(),
    ...
    assert!(
        tsx_range_to_carrier_range(0, 0, &tsx_li, &mapper, &vue_li).is_none(),
