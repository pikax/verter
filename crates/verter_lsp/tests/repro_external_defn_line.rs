...
/// A minimal valid `PositionMapper`. For an EXTERNAL (non-`.vue`) target the mapper is
/// never consulted by `merge_definitions` — it only matters for `.vue.tsx`/`.vue.jsx`
/// targets — so a one-token map is sufficient to satisfy the signature.
fn trivial_mapper() -> PositionMapper {
...
fn external_dts_definition_keeps_the_real_line_not_zero() {
    ...
    let vue_source_exists = |_p: &str| false;
