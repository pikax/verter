...
fn broken_member_access_completion_mapping_operates_over_valid_virtual_file() {
    ...
    let strict =
        merge::vue_position_to_tsx_offset_validated(&boundary, &o.vue_li, &mapper, &o.tsx_li);
    ...
    if let Some(gen) = mapper.vue_to_tsx(verter_span::LspPosition::new(recv.line, recv.character)) {
