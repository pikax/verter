...
/// carrier-source [`TextEdit`]s, or fail with a human-readable reason.
///
/// Called only once the provider has returned a NON-EMPTY auto-import edit set, so every failure
/// here is a hard rejection: missing IDE context / carrier URI / open document, or an edit that cannot
...
pub(super) fn resolve_tsgo_auto_import_edits(
    ...
    let carrier_uri = server
        .carrier_uri_from_ide_path(tsx_path)
        .ok_or_else(|| format!("no carrier URI for {tsx_path}"))?;
    let doc = server
        .documents
        .get(&carrier_uri)
