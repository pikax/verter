...
/// [`vue_position_to_tsx_offset_validated`] has returned `None`. The strict mappers
/// ([`vue_position_to_tsx_offset`], [`vue_position_to_tsx_offset_validated`],
/// [`PositionMapper::vue_to_tsx`], [`PositionMapper::tsx_to_vue`]) keep their strict in-run
/// semantics; this helper never feeds any other feature path.
///
...
/// 3. **At-cursor anchor** — [`PositionMapper::mapped_run_ending_at_src`] at the cursor column
...
///    operator).
/// 4. **Before-operator anchor** — otherwise, [`PositionMapper::mapped_run_ending_at_src`] at
...
pub(crate) fn vue_completion_member_boundary_offset(
    position: &Position,
    vue_line_index: &LineIndex,
    mapper: &PositionMapper,
    tsx_line_index: &LineIndex,
    ...
    let cursor_byte = vue_line_index.position_to_offset(position)? as usize;
    let before = vue_source.get(..cursor_byte)?;
    ...
    let line_start = vue_line_index.line_start(position.line as usize)? as usize;
    let cursor_col_utf16: u32 = vue_source
