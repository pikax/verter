...
/// is inherently asynchronous, while the on-disk store dispatches its synchronous
/// fsync'd swap to a blocking worker. The reconciler awaits either implementation,
/// preserving fail-closed errors without blocking an async runtime worker.
///
...
pub enum ReconcileReason {
    ...
    SourceSynced,
    /// A whole-workspace scan publication. Store membership and local provider
    /// routing are committed per source; plugin graph invalidation is deferred to
    /// one batch after the carrier pass.
    WorkspaceScan,
...
impl MembershipReconciler {
    ...
    /// Promote the IDE projection from an existing current-session
    /// advertisement into the provider's interactive working set. This is a
    /// control-plane transition only: the workspace scan already committed the
    /// generated bytes, project owner, and routing metadata, so editor open must
    /// not wait for a fresh semantic/typeinfo compile.
    ///
    /// Returns `false` when the source has no current advertised membership; the
    /// ordinary sync path will publish and activate it later.
    pub async fn activate_published_source(
        &self,
    ...
    async fn apply_owned(
        ...
        // 2. Provider routing/bootstrap transition through the resilient actor
        // command API. The plugin/store owns membership; registration hydrates
        // local routing and shares at most one cold-project bootstrap. NO ledger
        // lock is held across these awaits.
        //
        ...
        if let Some(provider) = &self.provider {
            ...
            for companion in &companions {
                // Publication is deliberately broader than the provider's active
                // working set. Both projections remain addressable metadata; editor
                // open promotes only the IDE projection through
                // `activate_published_source`. A publication trigger (source sync,
                // import warmup, workspace scan, or config refresh) is not evidence
                // that the carrier belongs in TypeScript's root set.
                provider
