...
impl TestSessionBuilder {
    ...
    pub(crate) async fn build(self) -> Option<RealProviderTestSession> {
        ...
        materialize_real_provider_framework_types(fixture_name);
        ...
        if matches!(self.kind, TestProviderKind::Tsserver) {
            session.server().test_settle_workspace_carriers().await;
        }
...
/// Materialize the type-only dependencies that the parity fixtures declare in
/// their package manifests but intentionally do not install in-place.
///
/// Real-provider tests run TypeScript with the fixture itself as the configured
/// project root. Relying on an ancestor workspace's incidental package links
/// makes that project non-hermetic and, when a package is absent, silently turns
/// generated carrier helpers into `any`. Copy only declaration/package metadata
/// from the repository's lockfile-resolved packages. Each file is replaced
/// atomically, so separate nextest processes may prepare the same ignored
/// fixture `node_modules` tree without exposing a partial declaration to a
/// provider that has already started.
fn materialize_real_provider_framework_types(fixture: &str) {
...
fn copy_type_package_directory(
        ...
        .unwrap_or_else(|error| panic!("read fixture dependency {}: {error}", source.display()))
    {
        ...
        if file_type.is_dir() {
            // Package-local runtime dependencies are resolved from the repository's
            // lockfile-owned node_modules. Copying a nested node_modules would turn
            // this small type fixture into an installation clone.
    ...
    // The package manifest is the commit marker for this directory. In
    // particular, write the root manifest only after every declaration that its
    // `types`/`exports` fields may expose has landed.
...
fn write_react_jsx_ambient_atomically(package_root: &std::path::Path) {
    // Deliberately React-shaped: `div` accepts `className`, not `class`. The Vue
    // carrier's per-file `@jsxImportSource vue` must therefore win for valid
    // authored Vue markup to remain error-clean.
