...
fn changelog_is_token_clean_and_regeneration_stays_clean() {
    ...
    let pattern_lines_lower: Vec<String> = cliff
        .lines()
        .filter(|l| l.contains("pattern"))
        .map(|l| l.to_ascii_lowercase())
        .collect();

    // (a) TARGETING — proven on the ACTUAL pattern behavior, not just fragment
    // co-occurrence: extract each `pattern = "..."` value, strip the `(?i)` flag and
    // every optional separator class `[-_ .]?`, and assert the RECONSTRUCTED literal
    // equals the no-separator form of the real token (assembled from fragments). This
    // demonstrates the regex matches the token even with no internal separator, which
    // is exactly the form a commit subject would carry.
    let pattern_values: Vec<String> = cliff
        .lines()
        .filter_map(|l| {
            let l = l.trim();
            let key = "pattern = \"";
        ...
        .collect();
    // The real tokens in their NO-SEPARATOR form (assembled from fragments).
    let token_no_sep: [String; 3] = [
        format!("{}{}{}", "nex", "us", "ui"),
        format!("{}{}{}{}", "ac", "cio", "rese", "arch"),
        format!("{}{}{}", "jud", "is", "app"),
    ];
    for token in &token_no_sep {
        let matched = pattern_values.iter().any(|p| {
            // Reconstruct what the regex matches with EMPTY separators.
            let reconstructed = p
                .replace("(?i)", "")
                .replace("[-_ .]?", "")
                .to_ascii_lowercase();
        ...
        assert!(
            matched,
            "no cliff.toml pattern reconstructs (with empty separators) to a targeted \
             private token — the redactor would miss the no-separator commit-subject form"
        );
    }

    // (b) NO RECONSTRUCTABLE CONTIGUOUS FORM: for each token, neither its full form
    // nor either reconstructable HALF (the boundaries an un-fragmented regex would
    // expose) may appear contiguously in any pattern line. Forbidding the halves
    // catches BOTH a full-token revert AND a half-fragmented revert (where a regex
    // splits only at the half boundary). Every forbidden form below is ASSEMBLED FROM
    // RUNTIME FRAGMENTS, so this guard source itself spells no contiguous token/half.
    let forbidden_contiguous: Vec<String> = vec![
        // reconstructable halves
        format!("{}{}", "nex", "us"),
        format!("{}{}", "ac", "cio"),
        format!("{}{}", "rese", "arch"),
        format!("{}{}", "jud", "is"),
        // full tokens (no separator)
        format!("{}{}{}", "nex", "us", "ui"),
        format!("{}{}{}{}", "ac", "cio", "rese", "arch"),
        format!("{}{}{}", "jud", "is", "app"),
    ];
    for (idx, forbidden) in forbidden_contiguous.iter().enumerate() {
        let leaked = pattern_lines_lower
            .iter()
            .any(|l| l.contains(forbidden.as_str()));
        // The failure message reports only an ABSTRACT INDEX — it must NEVER echo
        // `forbidden` itself, or a regression would print the reconstructed private
        // token straight into the CI transcript (a leak-on-failure).
        assert!(
            !leaked,
            "cliff.toml pattern spells a reconstructable private token/half contiguously \
             (forbidden form #{idx}); split every fragment across the separator class. \
             (The offending form is intentionally NOT printed to avoid leaking it.)"
        );
    }
}
