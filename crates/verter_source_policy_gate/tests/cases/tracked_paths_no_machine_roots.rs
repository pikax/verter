...
//! reference the markers to check for them. Every other tracked file is read
//! and scanned without exceptions. The RUNTIME
...
fn scan_marker_violations(root: &Path, paths: &[String]) -> MarkerScanReport {
...
fn tracked_files_contain_no_machine_specific_path_markers() {
    ...
    // Every tracked file is read and scanned. Only this guard's exact source
    // path is intrinsically admitted because it must spell the markers it
    // enforces; there is no directory, suffix, glob, or "docs" bypass.
    let report = scan_marker_violations(&root, &paths);
