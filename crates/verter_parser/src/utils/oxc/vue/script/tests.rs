...
// F22 — props-vs-emits surface discrimination through the real macro path.
//
// A props member whose VALUE is an emit-shaped indexed access
// (`onClose: LayerEmits['close']`) must stay a prop on the `defineProps`
// surface, while the same shape on the `defineEmits` surface still becomes
// an emit (the reka-ui DismissableLayer forwarding pattern).

#[test]
fn define_props_keeps_indexed_access_tuple_member_as_prop() {
    let source =
        "interface LayerEmits { close: [] }\nconst props = defineProps<{ onClose: LayerEmits['close'] }>();";
    let allocator = Allocator::default();
    let ret = Parser::new(&allocator, source, SourceType::tsx()).parse();
    let result = parse_script(&ret.program, ScriptMode::Setup, 0, source);
    let type_params = result
        ...
        .expect("defineProps macro with type params");
    assert!(
        type_params
            .resolved
            .props
        ...
        "F22: `onClose: LayerEmits['close']` must stay a prop, props={:?} call_sigs={:?}",
        type_params
            .resolved
            .props
            .iter()
    ...
    assert!(
        type_params.resolved.call_signatures.is_empty(),
        "F22: props member must not be reclassified as an emit"
    );
}

#[test]
fn define_emits_reclassifies_indexed_access_tuple_member() {
    let source =
        "interface LayerEmits { close: [] }\nconst emit = defineEmits<{ onClose: LayerEmits['close'] }>();";
    let allocator = Allocator::default();
    let ret = Parser::new(&allocator, source, SourceType::tsx()).parse();
    let result = parse_script(&ret.program, ScriptMode::Setup, 0, source);
    let type_params = result
        ...
        .iter()
        .find_map(|i| match i {
            ScriptItem::Macro(ScriptMacro::DefineEmits {
                type_params: Some(tp),
                ..
            }) => Some(tp),
            _ => None,
        })
        .expect("defineEmits macro with type params");
    assert!(
        type_params
            .resolved
            .call_signatures
        ...
        "on the emits surface `onClose: LayerEmits['close']` must become an emit, call_sigs={:?}",
        type_params
            .resolved
            .call_signatures
            .iter()
