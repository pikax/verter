...
//!
//! oxc's parser, the passes that walk its syntax tree (the syntax-tree
//! clone, the semantic builder, codegen) and Verter's own lowering and
//! evaluation recurse once per level of the tree, and none of them refuses
//! a level: a source nested deeply enough overflows the thread's stack and
//! aborts the process. [`scan`] measures, in one pass over the source
//! text, an upper bound of that depth, so a parse can refuse a source
//! beyond [`super::SYNTAX_NESTING_LIMIT`] before anything recurses.
//!
