...
const totalPrice = computed(() => filteredItems.value.reduce((sum, i) => sum + i.price, 0).toFixed(2))
    ...
    <table class="data-table">
      ...
      <tbody>
        ...
        >
          ...
          <td><span class="badge" :class="'cat-' + item.category">{{ item.category }}</span></td>
...
<style scoped>
.large-component { max-width: 1200px; margin: 0 auto; padding: 20px; }
.toolbar { display: flex; gap: 10px; margin: 20px 0; align-items: center; }
.search { flex: 1; padding: 8px; border: 1px solid #ddd; border-radius: 4px; }
.category-select { padding: 8px; }
.stats { margin-left: auto; display: flex; gap: 15px; color: #666; }
.bulk-actions { padding: 10px; background: #fff3cd; border-radius: 4px; display: flex; align-items: center; gap: 10px; }
...
.data-table th.sorted { color: blue; }
.data-table tr.selected { background: #e3f2fd; }
.data-table tr.inactive { opacity: 0.6; }
.name-cell { display: flex; flex-direction: column; }
.description { font-size: 0.85em; color: #888; }
.badge { padding: 2px 8px; border-radius: 10px; font-size: 0.85em; }
.cat-A { background: #e8f5e9; }
.cat-B { background: #e3f2fd; }
.cat-C { background: #fff3e0; }
.price { font-family: monospace; }
.tag { margin: 0 2px; padding: 1px 6px; background: #f0f0f0; border-radius: 3px; font-size: 0.8em; }
...
.danger { color: white; background: #e53935; border: none; padding: 4px 12px; border-radius: 3px; }
