...
use crate::syntax::{
    plugin::{CompilerErrorCode, Diagnostic},
    plugins::oxc_parser::{
...
pub fn parse_element_props<'alloc>(
    ...
    for &idx in &sorted_indices {
        ...
        match prop.kind {
            PropKind::If => {
                if condition_if_scope.is_some() {
                    diagnostics.push(
                        Diagnostic::error("oxc_parser", CompilerErrorCode::XVIfNoExpression)
                            .with_message("Duplicate v-if directive on the same element."),
                    );
                }
                let scope =
            ...
            PropKind::ElseIf => {
                if condition_else_if_scope.is_some() {
                    diagnostics.push(
                        Diagnostic::error("oxc_parser", CompilerErrorCode::XVIfNoExpression)
                            .with_message("Duplicate v-else-if directive on the same element."),
                    );
                }
            ...
            PropKind::Else => {
                if condition_else_scope.is_some() {
                    diagnostics.push(
                        Diagnostic::error("oxc_parser", CompilerErrorCode::XVElseNoAdjacentIf)
                            .with_message("Duplicate v-else directive on the same element."),
                    );
                }
            ...
            PropKind::For => {
                if for_scope.is_some() {
                    diagnostics.push(
                        Diagnostic::error("oxc_parser", CompilerErrorCode::XVForNoExpression)
                            .with_message("Duplicate v-for directive on the same element."),
                    );
                }
            ...
            PropKind::Slot => {
                if template_scope.is_some() || slot_scope.is_some() {
                    diagnostics.push(
                        Diagnostic::error(
                            "oxc_parser",
                            CompilerErrorCode::XVSlotDuplicateSlotNames,
                        )
                        .with_message("Duplicate v-slot directive on the same element."),
                    );
                }
                if is_template {
