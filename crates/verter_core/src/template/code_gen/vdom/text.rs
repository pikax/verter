...
pub fn process_text<'alloc>(
    ...
    if kind == ChildKind::Text {
        let has_entities = has_html_entities(content);
        let need_condense = has_consecutive_ws(content);
        let need_escape = needs_js_escaping(content);

        if has_entities {
            // Decode HTML entities first, then condense/escape
            let mut decoded = String::with_capacity(content.len());
            decode_html_entities_into(&mut decoded, content);
            let result = if need_condense {
                condense_and_escape(&decoded)
            } else if needs_js_escaping(&decoded) {
                let mut buf = String::with_capacity(decoded.len() + 8);
                escape_js_string_into(&mut buf, &decoded);
                buf
            } else {
                decoded
            };
            out.overwrite(text.start, text.end, &result);
