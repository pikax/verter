...
use crate::new_impl::ast::types::{ChildrenMode, PropFlag, PropFlags};
use crate::new_impl::template::oxc::types::{ExpressionFlag, ExpressionFlags};
...
pub fn compute_patch_flags(
    ...
    // PROPS: other dynamic binds or events (beyond class/style which have own flags)
    if has_other_dynamic_binds || prop_flag.has(PropFlags::HasEventListener) {
...
mod tests {
    ...
    #[test]
    fn patch_flags_other_dynamic_binds() {
        let flag = compute_patch_flags(
            PropFlag::empty(),
            ExpressionFlag::empty(),
            ChildrenMode::Empty,
