//! Vapor mode template code generation backend.
//!
...
impl<'ast, 'alloc> VaporCodeGen<'ast, 'alloc> {
    ...
    fn handle_v_if_chain(
        ...
        match cond.kind {
            ElementNodeConditionKind::If => {
                ...
                let cond_expr =
                    if let (Some(vs), Some(ve)) = (cond.prop.value_start, cond.prop.value_end) {
                        out.alloc_str(&source[vs as usize..ve as usize])
                    } else {
                        "true"
                    };
            ...
            ElementNodeConditionKind::ElseIf => {
                ...
                // If no pending chain, drop silently (orphan v-else-if)
            }
    ...
    fn write_vif_branches(&self, branches: &[VIfBranch<'alloc>], idx: usize, stmt: &mut String) {
        ...
        if let Some(cond_expr) = branch.condition {
            // v-if or v-else-if: _createIf(() => (cond), () => { body })
            let resolved = self.resolver.resolve_simple_expr(cond_expr);
            ...
            // If there are more branches, add else argument
            if remaining > 0 {
                stmt.push_str(", () => ");
                ...
                if next.condition.is_some() {
                    ...
                    stmt.push('}');
                    // Close any remaining parens from parent _createIf
                }
                stmt.push(')');
            }
        } else {
    ...
    fn build_component_root(
        ...
        let props_str = self.build_component_props(el, source, out);
        ...
        for prop in &el.props {
            ...
            if prop.is_directive {
                ...
                if name.starts_with('@') || name == "v-on" {
                    ...
                    entry.push_str(": () => ");
                    entry.push_str(&self.resolver.resolve_simple_expr(value));
                    entries.push(entry);
                    continue;
                }
    ...
    ///    - Return statement
    fn assemble_output(&self, out: &mut CodeGenOutput<'alloc>) -> String {
        use super::shared::helpers::push_u32;
        ...
        // 2. Delegated events (between templates and render function)
...
impl<'ast, 'alloc> TemplateCodeGen<'alloc> for VaporCodeGen<'ast, 'alloc> {
    ...
    fn leave_element(
        ...
        if el.tag_type == TagType::Component {
            ...
            let root = self.build_component_root(el, tag_name, node_ref, source, state, out);
            if self.depth == 0 {
        ...
        if el.v_condition.is_some() && self.depth == 0 {
            self.handle_v_if_chain(el, source, state, has_dynamic_text, out);
            return;
        }
