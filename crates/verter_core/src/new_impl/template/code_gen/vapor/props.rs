...
pub fn process_dynamic_props<'a>(
    element: &ElementNode,
    source: &'a str,
    ...
    delegated_events: &mut Vec<&'a str>,
    delegated_events_set: &mut rustc_hash::FxHashSet<&'a str>,
) {
    // Process template ref (ref="name") — emits _setTemplateRef
    if let Some(ref_prop) = &element.v_ref {
        process_template_ref(ref_prop, source, state, counters, out);
    }
    ...
    for prop in &element.props {
        ...
        if name == "v-show" {
            ...
            let resolved = resolver.resolve_simple_expr(value);
            ...
            out.add_vapor_import(VaporHelper::ApplyVShow);
            continue;
        }

        // ===== v-model =====
        if name == "v-model" {
            process_v_model(element, prop, value, source, resolver, state, counters, out);
            continue;
        }
        ...
        if name == "v-html" {
            let node_ref = state.ensure_node_ref(counters);
            let resolved = resolver.resolve_simple_expr(value);
            state.own_effects.push(VaporEffect::SetHtml {
                node_ref,
                expr: out.alloc_str(&resolved),
            });
        ...
        if name == "v-bind" && arg.is_none() {
            let node_ref = state.ensure_node_ref(counters);
            let resolved = resolver.resolve_simple_expr(value);
            state.own_effects.push(VaporEffect::SetDynamicProps {
                node_ref,
                expr: out.alloc_str(&resolved),
            });
        ...
        // Resolve the expression via binding resolver before storing in effects
        let resolved = resolver.resolve_simple_expr(value);
        let resolved_expr = out.alloc_str(&resolved);

        match classify_directive(arg, &element.prop_flag) {
            DirectiveKind::Class => {
                state.own_effects.push(VaporEffect::SetClass {
                    node_ref,
                    expr: resolved_expr,
                });
            ...
            DirectiveKind::Style => {
                state.own_effects.push(VaporEffect::SetStyle {
                    node_ref,
                    expr: resolved_expr,
                });
            ...
            DirectiveKind::Prop(attr) => {
                state.own_effects.push(VaporEffect::SetProp {
                    node_ref,
                    attr,
                    expr: resolved_expr,
                });
            ...
            DirectiveKind::Attr(attr) => {
                state.own_effects.push(VaporEffect::SetAttr {
                    node_ref,
                    attr,
                    expr: resolved_expr,
                });
                out.add_vapor_import(VaporHelper::SetAttr);
            }
...
fn process_event<'a>(
    ...
    if !non_delegatable && DELEGATABLE_EVENTS.contains(&event_name) {
        ...
        write_handler_expression(&mut line, handler, resolver, &runtime_mods, &key_mods);
        line.push(')');
        state.child_statements.push(out.alloc_str(&line));
...
fn write_handler_expression(
    ...
    let resolved = resolver.resolve_simple_expr(handler);
...
fn process_v_model<'a>(
    ...
    let helper = determine_model_helper(element, tag_name, source);
    ...
    match helper {
        "_applyTextModel" => out.add_vapor_import(VaporHelper::ApplyTextModel),
        "_applyCheckboxModel" => out.add_vapor_import(VaporHelper::ApplyCheckboxModel),
        "_applyRadioModel" => out.add_vapor_import(VaporHelper::ApplyRadioModel),
        "_applySelectModel" => out.add_vapor_import(VaporHelper::ApplySelectModel),
        _ => out.add_vapor_import(VaporHelper::ApplyTextModel),
    }
}
...
fn process_template_ref<'a>(
    ref_prop: &crate::new_impl::types::NodeProp,
    source: &'a str,
    state: &mut VaporElementState<'a>,
    counters: &mut VaporCounters,
    out: &mut CodeGenOutput<'a>,
...
fn classify_directive<'a>(
    arg: Option<&'a str>,
    prop_flag: &crate::new_impl::ast::types::PropFlag,
) -> DirectiveKind<'a> {
...
mod tests {
    ...
    fn make_resolver_with(
        entries: &[(
            &'static str,
            crate::new_impl::template::code_gen::binding::BindingType,
        )],
        is_inline: bool,
