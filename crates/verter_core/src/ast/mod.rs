...
use crate::ast::types::{AstNode, AstNodeKind, ElementContent, TemplateAst};
...
impl TemplateAst {
    ...
    pub fn attach_to_parent(&mut self, parent: NodeId, child: NodeId) {
        ...
        let idx = {
            ...
            let AstNodeKind::Element(el) = &mut p.kind else {
                unreachable!("attach_to_parent: parent is not an Element");
            };
    ...
    pub fn dfs(&self, start: NodeId, mut f: impl FnMut(NodeId, &AstNode)) {
        let mut stack: SmallVec<[NodeId; 16]> = smallvec::smallvec![start];
...
mod ast_tests {
    ...
    use crate::ast::types::{AstNodeKind, TextNode};
