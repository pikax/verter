//! Single-pass batch chunk-rebuild operations for [`CodeTransform`].
//!
//! A single `prepend_left` / `overwrite` inserts into the chunk Vec with
//! `Vec::insert`, which is O(n) per call and O(n*m) across `m` edits. The batch
//! operations here apply a whole sorted edit set in ONE O(n+m) forward pass that
//! rebuilds the chunk Vec, reusing the transform's `scratch` buffer so the second
//! and later batch calls allocate nothing. They are the bulk-apply path used after
//! template compilation has collected all binding-prefix prepends and all
//! attribute/close-tag overwrites.
//!
//! Each operation requires its input slice sorted by position (ascending) and
//! preserves the existing chunk ordering invariants: positioned `Original` /
//! `Overwritten` chunks keep monotonically increasing source positions, and at an
//! equal insertion anchor an unmapped prepend ranks ahead of a source-mapped one.
//! [`PrependMerge`] folds the unmapped and source-mapped prepend channels into one
//! rebuild without materializing a combined Vec.
...
#[allow(dead_code)] // Some batch API methods are only exercised by tests currently
