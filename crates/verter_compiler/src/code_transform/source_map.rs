...
impl<'a> CodeTransform<'a> {
    ...
    pub fn generate_map_with_preamble(
        ...
        // them once, then let each mapped chunk consume its own ordered slice.
    ...
    /// - Every explicitly requested authored boundary can emit at most one
    ///   extra token — `sourcemap_locations.len()`.
