//! Vue macro scope-reference validation.
//!
//! Official `@vue/compiler-sfc` `checkInvalidScopeReference`: runtime macro
//! arguments (`defineProps` / `defineEmits` / `defineOptions` / `defineModel`
//! options, `withDefaults` defaults, and `defineProps` reactive-destructure
//! defaults) are hoisted outside `setup()`, so a reference to a locally
