...
/// installing the package or relying on a provider virtual module.
///
/// Uses `import("vue").X` syntax because top-level imports are not allowed inside
/// `declare module` blocks.
///
/// See also [`VERTER_TYPES_STANDALONE_DTS`] for the unwrapped external-module
/// version.
...
/// `declare module "@verter/types" { ... }` wrapper, suitable for standalone or
/// virtual declaration consumers.
///
...
fn emit_helper_imports_inner(
    ...
    // Runtime imports from @verter/types. `globalComponentsNav` backs the
    // GlobalComponents fallback-const NAV PROBES: its return type references
    // the augmentation-merged `GlobalComponents` interface DIRECTLY from a real
    // on-disk declaration file, so member go-to-definition keeps its
    // declaration links (an alias-type hop drops them) without adding a
