...
pub fn compose_generated_chunk(
    ...
    // convention (e.g. `assert!(plain.main.source_map.is_empty(), "no
    // demand, no main map")` in the Svelte carrier tests) — a caller that
    // did not request source maps produces empty strings on both sides,
    // not malformed JSON. Only a NON-empty string is parsed; an empty side
    // contributes no tokens to the composed map instead of failing the
    // whole splice. Composing generated bytes must not depend on whether
    // the caller asked for a map.
