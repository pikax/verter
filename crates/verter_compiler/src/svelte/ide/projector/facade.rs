...
//! (`no_oversize_files`). The facade is the component's public type a consumer
//! sees from an in-project bare `import Comp from "./Comp.svelte"` (which the
//! engine resolves to the `Comp.svelte.tsx` IDE carrier — the bare-import-probe
//! identity, §2.2/§2.9).

/// Synthesise the Svelte component's PUBLIC-FACADE default export for the IDE
/// carrier (the in-project bare-import target). Mirrors the MINIMAL facade
...
/// `$events` / `$slots` — so the in-project import is a real component type
/// (the two crates cannot share code; `verter_compiler` is the lower crate).
///
