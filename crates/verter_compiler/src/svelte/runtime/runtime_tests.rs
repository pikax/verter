...
fn special_element_events_target_the_global() {
    ...
    let window_target = |src: &str, name: &str| -> EventTarget {
        ...
        let owner = match target {
            ...
            | EventTarget::Window(node)
            | EventTarget::Document(node)
            | EventTarget::Body(node) => node,
        };
        let expected_kind = match name {
            "window" => SpecialKind::Window,
            "document" => SpecialKind::Document,
            "body" => SpecialKind::Body,
            _ => unreachable!("the test passes only global-host names"),
        };
        assert!(
            matches!(ir.node(owner), IrNode::Special(s) if s.kind == expected_kind),
            "the global target must retain its exact authored {name} node"
        );
        target
    };
    ...
    // Two hosts of the same kind retain distinct owner identities. Host-kind-only
    // targeting cannot preserve their source-ordered init groups.
    let src = "<script>function a(){} function b(){}</script><svelte:window onresize={a}/><svelte:window onkeydown={b}/>";
    let parsed = parse_svelte(src);
    let ir = lower_parsed_svelte_to_ir(src, &parsed, &SvelteRuntimeOptions::default(), &alloc)
        .expect("lowering succeeds");
    let owners = ir
        ...
        .filter_map(|op| match op {
            RuntimeOp::Event {
                target: EventTarget::Window(owner),
                ..
            } => Some(*owner),
            _ => None,
        })
        .collect::<Vec<_>>();
    assert_eq!(owners.len(), 2, "both window events must be represented");
    assert_ne!(
        owners[0], owners[1],
        "each event must retain its own host node"
    );
...
fn torn_expression_facts_are_fail_closed_not_defaults() {
    ...
    let torn = super::expr::AnalyzedExpr::torn(
        "a ..b",
        verter_span::Span::new(0, 5),
        super::expr::ScopeId(0),
    );
    ...
    let interned = super::expr::AnalyzedExpr::interned(
        "obj.x",
        verter_span::Span::new(0, 5),
        super::expr::ScopeId(0),
        facts,
