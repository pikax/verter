...
pub(super) fn classify_interpolation_shape(
    ...
    let expression = if typescript {
        peel_typescript_runtime_expression(&stmt.expression)
    } else {
        &stmt.expression
    };
    if let Expression::StaticMemberExpression(member) = expression {
        ...
        loop {
            object = if typescript {
                peel_typescript_runtime_expression(object)
            } else {
                object
            };
            match object {
...
/// Peel exactly the TypeScript wrappers Svelte's script preprocessing erases.
/// Parentheses are transparent only while traversing a TS wrapper spine; a plain
/// JavaScript `{(x)}` retains the existing interpolation-breadth boundary.
...
pub(super) fn classify_bind_shape(
    ...
    if let Some(e) = expr {
        if e.bind_target.lvalue_contains_ts && !typescript {
            return Err(refuse());
        }
    }
    match target {
