...
//! stages `bundle.main` as the complete compile-artifact handoff (so
//! `has_runtime_surface()` becomes true and the host emits the `Main` virtual
//! node from its staged root artifact), and every unsupported surface FAILS
    ...
    pub(super) fn push_expr(&mut self, span: Span, scope: ScopeId) -> ExprId {
        ...
        match collect_expr_references_in(self.alloc, text) {
            ...
            Err(()) => {
                ...
                self.expressions.push(AnalyzedExpr::torn(text, span, scope))
            }
        }
    }
...
pub(super) fn lower_parsed_svelte_to_ir_resolved<'a>(
    ...
    let script_grammar =
        crate::svelte::parser::template_ast::script_body_grammar_for_source(source);
    // One dialect-faithful typed program per script slot. The script-item
    // classifier and emitter consume these carriers directly; a `lang="ts"`
    // component is parsed as TypeScript (not TSX), while a plain component stays
    // strict module JavaScript.
    let instance_program =
        instance_source.and_then(|text| expr::parse_script_module(alloc, text, script_grammar));
    let module_program =
        module_source.and_then(|text| expr::parse_script_module(alloc, text, script_grammar));
    ...
    let script_imports = client_surface_imports::classify_script_imports(
        module_program.as_ref(),
        instance_program.as_ref(),
        script_grammar,
    );
    let store_candidates =
        store_subscriptions::store_base_candidates(instance_program.as_ref(), &script_imports);
    let store_exempt = store_subscriptions::rune_root_accessor_exemptions(&store_candidates);
    let script_runes = instance_program
        .as_ref()
        .map(|program| script_uses_runes(program, &store_exempt))
        .unwrap_or(false)
        || module_program
            .as_ref()
            .map(|program| script_uses_runes(program, &store_exempt))
    ...
    if let (Some(_text), Some(span)) = (instance_source, parsed.instance_content()) {
        if instance_program.is_none() {
            errors.push(
    ...
    if let (Some(_text), Some(span)) = (module_source, parsed.module_content()) {
    ...
    let maybe_runes = !runes
        ...
        && !instance_program
            .as_ref()
            .map(|program| instance_forces_definite_legacy(program))
            .unwrap_or(false);
    ...
    let analysis = RuntimeAnalysis {
        scripts: ScriptAnalysis {
            ...
            instance_source,
            instance_span: parsed.instance_content(),
            instance_program,
            module_source,
            module_span: parsed.module_content(),
            module_program,
        },
        script_imports,
