...
pub struct AnalyzedExpr<'a> {
    ...
    pub source: &'a str,
    /// Absolute byte span of [`Self::source`] in the component source.
...
impl<'a> AnalyzedExpr<'a> {
    ...
    pub(crate) fn torn(source: &'a str, span: verter_span::Span, scope: ScopeId) -> Self {
        Self {
            source,
            span,
            scope,
...
pub struct ScriptAnalysis<'a> {
    /// The parser-wide script grammar selected by the component source. Svelte
    /// uses one grammar for both script slots; downstream classification and
    /// emission must therefore never infer TypeScript support per slot.
    pub grammar: crate::svelte::parser::ScriptBodyGrammar,
    /// The instance-script body source, if present.
    pub instance_source: Option<&'a str>,
    /// Absolute component-source span of [`Self::instance_source`].
    pub instance_span: Option<verter_span::Span>,
    /// The canonical instance-script program parsed under [`Self::grammar`].
    /// Runtime script completion consumes this typed carrier rather than
    /// reparsing statement slices during classification or emission.
    pub instance_program: Option<Program<'a>>,
    /// The module-script body source, if present.
    pub module_source: Option<&'a str>,
    /// Absolute component-source span of [`Self::module_source`].
    pub module_span: Option<verter_span::Span>,
    /// The canonical module-script program parsed under [`Self::grammar`].
    pub module_program: Option<Program<'a>>,
}

/// Parse one Svelte script body under the component's authoritative parser-wide
/// grammar. Unlike [`reparse_module`] (the legacy TSX-lenient analysis seam),
/// this is dialect-faithful: plain scripts are module JavaScript and `lang="ts"`
/// scripts are module TypeScript. Script lowering stores this program on the IR
/// and consumes it by typed statement identity.
pub fn parse_script_module<'a>(
    alloc: &'a Allocator,
    text: &str,
    grammar: crate::svelte::parser::ScriptBodyGrammar,
) -> Option<Program<'a>> {
    let source_type = match grammar {
        crate::svelte::parser::ScriptBodyGrammar::Js => SourceType::mjs(),
...
pub(crate) fn expr_is_proxiable(
    ...
    // Svelte removes these TypeScript-only value wrappers before running
    // `should_proxy`; proxy classification must observe the surviving runtime
    // expression, not the wrapper node (`0 as number` is non-proxiable, exactly
    // like `0`). This is safe for plain scripts because the official parse gate
    // rejects TS syntax before runtime lowering.
    loop {
        expr = match expr {
