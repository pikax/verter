...
/// The authored node target of a runtime op (for the dead-options-attr skip and
/// exact special-host ownership). A global-target event retains the special
/// node that authored it even though its runtime target is a global object.
pub(super) fn op_target_node(op: &RuntimeOp) -> Option<NodeId> {
    match op {
        ...
        RuntimeOp::Event { target, .. } => Some(match target {
