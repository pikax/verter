...
pub(super) struct EventEmit {
    ...
    pub handler: super::output::MappedCode,
}
...
/// A single emitted component-function-body statement, already lowered to its
/// final client JavaScript. Module-scope imports and statements use separate plan
/// fields; the emitter only sequences this closed body vocabulary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub(super) enum ClientScriptItem {
    ...
    BodyStatement {
        ...
        code: String,
        /// Mapping-preserving form for a source-rewritten canonical statement.
        /// Synthesized/specialized statements carry `None`.
        mapped: Option<super::output::MappedCode>,
    },
}

impl ClientScriptItem {
    pub(super) fn mapped_code(&self) -> super::output::MappedCode {
        match self {
            Self::BodyStatement {
                code: _,
                mapped: Some(mapped),
            } => mapped.clone(),
            Self::BodyStatement { code, mapped: None } => super::output::MappedCode::unmapped(code),
        }
    }
}
...
pub(super) enum ClientRuntimeOp {
    ...
    Event {
        ...
        /// The authored element or special-host node that owns this registration.
        /// Global runtime targets retain it so init emission can order distinct hosts.
        target: ClientNodeId,
        emit: EventEmit,
