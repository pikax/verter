        ...
        for attr in &s.attrs {
            match attr {
                ...
                AttrIr::Event {
                    ...
                    events.push(rendered.as_str().trim().to_string());
                }
...
impl<'a> ClientEmitter<'a> {
    ...
    pub(super) fn emit_svelte_element(
        &mut self,
        ...
        let mut setup = super::output::SvelteRuntimeOutput::default();
        ...
        if let Some(sc) = &el.set_class {
            ...
            if sc.reactive {
                ...
                emit_text_effect(
                    &mut setup,
                    &[EffectBody::Expr(super::output::MappedCode::unmapped(body))],
                    &deps,
                );
            } else {
                let body = self.assemble_set_class(
