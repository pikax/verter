...
fn render_callee_scan_peels_the_snippet_call_and_scans_only_the_callee() {
    ...
    assert!(
        !super::needs_context(&alloc, props, &[], &["children?.()"]),
    ...
    // scanned).
    assert!(
        super::needs_context(&alloc, None, &[], &["({ snip(){} }).snip()"]),
    ...
    assert!(
        !super::needs_context(&alloc, Some("let __r = $state(0);"), &[], &["localFn()()"]),
...
fn render_callee_arrow_param_shadow_inside_callee_stays_frame_free() {
    ...
    assert!(
        !super::needs_context(&alloc, instance, &[], &["((Snips) => Snips.row)()"]),
        "an arrow param shadowing the unsafe root inside the callee must NOT set needs_context"
    );
    assert!(
        super::needs_context(&alloc, instance, &[], &["(() => Snips.row)()"]),
