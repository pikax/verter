...
impl PreparedTemplateValue {
    ...
    pub(super) fn test_raw(surface: AuthoredValueSurface, text: &str) -> Self {
        Self {
            surface,
            expression: PreparedExpression::Raw(text.to_string()),
            facts: PreparedValueFacts {
                has_call: false,
                has_state: false,
    ...
    pub(super) fn inline_expression(&self) -> String {
        match &self.expression {
            ...
            PreparedExpression::LegacySequence(seq) => format!("({seq})"),
        }
    }
    ...
    pub(super) fn memo_input(&self) -> &str {
        match &self.expression {
            PreparedExpression::Raw(text) | PreparedExpression::LegacySequence(text) => text,
        }
    }
    ...
    pub(super) fn thunk(&self) -> String {
        match &self.expression {
            PreparedExpression::Raw(text) => js_thunk(self.unthunk_callee.as_deref(), text),
            PreparedExpression::LegacySequence(seq) => format!("() => ({seq})"),
        }
    }
    ...
    pub(super) fn arrow_body(&self) -> String {
        match &self.expression {
            PreparedExpression::Raw(text) => concise_arrow_expr_body(text),
            PreparedExpression::LegacySequence(seq) => format!("({seq})"),
        }
    }
}
        ...
        let expression = match policy(surface).legacy {
            LegacyPolicy::BuildExpression => {
                match self.legacy_wrap_rewritten(
                    ...
                    Some(seq) => PreparedExpression::LegacySequence(seq),
