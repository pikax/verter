...
use super::expr::{
    ...
    collect_var_hoists, for_left_names, function_scope_names, reparse_module, AnalyzedExpr,
    BindingInfo, BindingRuntimeKind, BindingTable, ScopeGraph, ScopeId,
};
...
pub(super) fn classify_store_facts(
    ...
    let candidates = store_base_candidates(&alloc, instance_source, &ir.analysis.script_imports);
    ...
    let (const_names, closure_fns, class_names) =
        store_dependency_closure(&alloc, instance_source, &subscribed_bases);
    let fn_decl_names = collect_top_level_function_names(&alloc, instance_source);
