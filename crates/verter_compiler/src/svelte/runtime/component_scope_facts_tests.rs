...
use crate::svelte::runtime::client_surface_imports::classify_script_imports;
...
fn facts_for(module: Option<&str>, instance: Option<&str>) -> ComponentScopeFacts {
    ...
    let imports = classify_script_imports(module, instance);
...
fn template_declarations_enter_conflicts_but_not_roots() {
    ...
    let alloc = Allocator::default();
    let imports = classify_script_imports(None, None);
...
fn refuses_a_torn_script_rather_than_fabricating_facts() {
    ...
    let imports = classify_script_imports(None, Some(torn));
    assert_eq!(
    ...
    let imports_mod = classify_script_imports(Some(torn), None);
    assert_eq!(
    ...
    let clean = "const a = 1;";
    let imports_ok = classify_script_imports(None, Some(clean));
    assert!(
        build_component_scope_facts(
            &alloc,
            None,
    ...
    let imports_none = classify_script_imports(None, None);
    assert!(
...
fn fail_closes_on_a_tsx_ambiguous_angle_bracket_type_assertion() {
    ...
    let imports = classify_script_imports(None, Some(src));
    let template_decls = FxHashSet::default();
    let exprs = ExprArena::new();
    assert!(
