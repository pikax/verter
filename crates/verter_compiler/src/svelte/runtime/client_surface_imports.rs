...
//! side-effect, and mixed — in BOTH the instance `<script>` and `<script module>`
//! slots, with `with { … }` import attributes preserved. The
...
pub(super) fn classify_script_imports(
    ...
    ClassifiedScriptImports {
        module: module_program.map_or_else(
            || Ok(Vec::new()),
            |program| classify_static_imports(program, UserImportSlot::Module, grammar),
        ),
        instance: instance_program.map_or_else(
            || Ok(Vec::new()),
            |program| classify_static_imports(program, UserImportSlot::Instance, grammar),
        ),
    }
}

/// Test-only source adapter for legacy unit fixtures that exercise the import
/// classifier in isolation. Production supplies canonical programs from
/// `ScriptAnalysis`; this adapter retains the old TSX-lenient parse so a
/// JS-grammar test can reach the classifier's defense-in-depth type-only gate.
#[cfg(test)]
pub(super) fn classify_script_imports_from_sources(
    module_source: Option<&str>,
    instance_source: Option<&str>,
) -> ClassifiedScriptImports {
    let alloc = oxc_allocator::Allocator::default();
    let module_program =
        module_source.and_then(|source| super::expr::reparse_module(&alloc, source));
    let instance_program =
        instance_source.and_then(|source| super::expr::reparse_module(&alloc, source));
    let grammar = crate::svelte::parser::ScriptBodyGrammar::Js;
    ClassifiedScriptImports {
        module: module_program.as_ref().map_or_else(
            || Ok(Vec::new()),
            |program| classify_static_imports(program, UserImportSlot::Module, grammar),
        ),
        instance: instance_program.as_ref().map_or_else(
            || Ok(Vec::new()),
            |program| classify_static_imports(program, UserImportSlot::Instance, grammar),
        ),
    }
...
fn classify_static_imports(
    ...
    for stmt in &program.body {
        ...
        if let Some(import) = classify_import_declaration(import, slot, grammar)? {
            imports.push(import);
        }
    }
    Ok(imports)
...
fn classify_import_declaration(
    ...
    slot: UserImportSlot,
    grammar: crate::svelte::parser::ScriptBodyGrammar,
) -> Result<Option<UserImport>, UnsupportedSvelteRuntimeSurface> {
    ...
    let typescript = grammar == crate::svelte::parser::ScriptBodyGrammar::Ts;
    if import.import_kind != oxc_ast::ast::ImportOrExportKind::Value {
        return if typescript {
            Ok(None)
        } else {
            Err(refuse("type-only import"))
        };
    }
    ...
    if let Some(with_clause) = &import.with_clause {
        if with_clause.keyword != oxc_ast::ast::WithClauseKeyword::With && !typescript {
    ...
    let mut elided_type_specifier = false;
    ...
    // A named clause containing only `type` members has no runtime side effect in
    // Svelte's TypeScript preprocessing and disappears entirely. Preserve an
    // authored `import {} from ...` (which is a side-effect import) by keying this
    // decision on whether a type member was actually elided.
    if elided_type_specifier && specifiers.is_empty() {
        return Ok(None);
    }
    Ok(Some(UserImport {
        slot,
