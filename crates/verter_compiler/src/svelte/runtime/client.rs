...
use super::client_module_frame::emit_imports;
...
pub use super::client_output::{ClientModule, ScopedCssArtifact};
...
pub(super) fn emit_client_module(
    ...
    options: ClientEmitOptions<'_>,
    allocator: &Allocator,
) -> Result<ClientModule, super::ClientCompileError> {
    let mut emitter = ClientEmitter::new(plan);
    emitter.injected_css = options.injected_css;
    emitter.emit(
        source,
        html_plan,
        topology,
        options.filename,
        options.want_source_map,
        allocator,
    )
}
...
impl<'a> ClientEmitter<'a> {
    ...
    fn emit(
        ...
        let mut out = super::output::SvelteRuntimeOutput::default();
        ...
        // (1a) Non-import `<script module>` statements, source order. Imports
        // were hoisted into their two prelude slots above; declarations and
        // exports remain in the module body before snippets/template factories.
        for statement in &self.plan.module_statements {
            out.push_mapped(statement);
            if !statement.as_str().ends_with('\n') {
                out.push('\n');
            }
        }
        if !self.plan.module_statements.is_empty() {
            out.push('\n');
        }
        ...
        let _ = html_plan;
        let mut hoists = super::output::SvelteRuntimeOutput::default();
    ...
    fn emit_body(&mut self, out: &mut super::output::SvelteRuntimeOutput) {
        ...
        for item in &self.plan.body_statements {
            out.push('\t');
            out.push_mapped(&item.mapped_code());
            out.push('\n');
        }
    ...
    pub(super) fn emit_walk(
        ...
        let ctx = super::html::region_ctx(self.ir());
        let items = clean_nodes(self.ir(), &scope.roots, ctx);
        let init_nodes = self.non_rendering_init_nodes(&scope.roots);
        ...
        if mounts_fragment {
            ...
            let [CleanItem::Node(only)] = items.as_slice() else {
                ...
                // (e.g. a `{@debug}`-only block body). Emit its init phase; there is
                // no DOM walk.
                self.emit_non_rendering_inits(out, &init_nodes);
                return;
            };
            let only = *only;
            ...
            // Fragment-owned non-rendering init nodes precede every DOM-node init,
            // independent of their position among rendered siblings.
            self.emit_non_rendering_inits(out, &init_nodes);
    ...
    ///
    /// `init_nodes` is the fragment-owned non-rendering init stream. It is emitted
    /// before the DOM walk while preserving mutual authored order.
