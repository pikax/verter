...
impl<'a> SupportedClientIr<'a> {
    ...
    /// reactive reads/writes share one transform per statement.
    pub(super) fn build_module_statements(
        &self,
        spans: &[Span],
    ) -> Result<Vec<super::output::MappedCode>, UnsupportedSvelteRuntimeSurface> {
        if spans.is_empty() {
            return Ok(Vec::new());
        }
        let (Some(source), Some(program)) = (
            self.ir.analysis.scripts.module_source,
            self.ir.analysis.scripts.module_program.as_ref(),
        ) else {
            return Err(UnsupportedSvelteRuntimeSurface::ModuleScriptItem {
                construct: "module statement",
                span: Span::new(0, 0),
            });
        };
        let root_scope = self.ir.root_scope().scope;
        let Some(module_scope) = self.ir.analysis.scopes.parent(root_scope) else {
            return Err(UnsupportedSvelteRuntimeSurface::ModuleScriptItem {
                construct: "module statement",
                span: Span::new(0, 0),
            });
        ...
        let mut lowered = Vec::with_capacity(spans.len());
        for span in spans {
            let Some(statement) = program.body.iter().find(|statement| {
                let candidate = statement.span();
            ...
            let mapped = super::expr_rewrite::rewrite_script_statement(
                ...
                source,
                module_scope,
                super::expr_rewrite::RewriteContext {
                    bindings: &self.ir.analysis.bindings,
                    scopes: &self.ir.analysis.scopes,
                    prop_reads: &self.prop_reads,
                    proxy_inits: &proxy_inits,
                    typescript: self.ir.analysis.scripts.grammar
            ...
            lowered.push(mapped.with_source_offset(source_offset));
        }
        Ok(lowered)
    }
    ...
    ///
    /// The classifier mints a closed
    /// [`SupportedInstanceScriptItem`](super::instance_items::SupportedInstanceScriptItem)
    /// vocabulary. Dedicated Svelte constructs retain their specialized lowering;
    /// ordinary statements are transformed from the retained canonical program.
    /// Imports and module-script statements are emitted by their module-scope lanes,
    /// so this stage produces only component-function-body statements.
    ///
    /// Specialized function carriers and general statements both use the fallible
    /// AST-backed rewriter, so signal reads and writes become `$.get`/`$.set` and
    /// TypeScript syntax is erased without emitting an untransformed statement.
        ...
        for item in &self.script_items {
            ...
            match expr_emit::lower_simple_instance_item(item) {
                ...
                SimpleItemLowering::NeedsRewriter => {
                    if let Item::GeneralStatement { span } = item {
                        let (Some(source), Some(program)) = (
                        ...
                        let mapped = super::expr_rewrite::rewrite_script_statement(
                            ...
                            source,
                            root_scope,
                            super::client_plan_rewrite::expression_context(self),
                        )?
                        .with_source_offset(source_offset);
                        let code = mapped.as_str().to_string();
                    ...
                    let code = match item {
                        Item::PropsIdDecl {
                            ...
                            if siblings.is_empty() {
                                ...
                                let declarations = siblings
                                    .iter()
                                    .map(|(name, init)| {
                                        let init = init
                                            .as_deref()
                                            .map(|source| self.rewrite_source(source, root_scope))
                                            .transpose()?;
                                        Ok::<_, UnsupportedSvelteRuntimeSurface>(match init {
    ...
    fn lower_props_default(
        ...
        if facts.ts_wrapped
            && self.ir.analysis.scripts.grammar != crate::svelte::parser::ScriptBodyGrammar::Ts
        {
