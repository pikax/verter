...
use super::client_plan_support::OpDedup;
...
pub(super) use super::client_plan_types::{
    ClientAttr, ClientBindTarget, ClientNode, ClientNodeId, ClientRuntimeOp, ClientScriptItem,
    ElementLifecycleOp, EventEmit, EventEmitTarget, EventMode, PlannedInterpolation, RegionOps,
};
...
pub(super) struct ClientModulePlan<'a> {
    ...
    /// Non-import `<script module>` statements after source-preserving reactive
    /// rewriting and TypeScript erasure, in authored order. Emitted after the
    /// import prelude and before snippets/template factories.
    pub(super) module_statements: Vec<super::output::MappedCode>,
...
pub(super) struct SupportedClientIr<'a> {
    ...
    /// The typed instance-script carriers consumed by `build_script_items`.
    /// Svelte-owned declarations use dedicated variants; ordinary statements
    /// retain canonical-program identity for source-preserving transformation.
...
impl<'a> SupportedClientIr<'a> {
    ...
    pub(super) fn build(
        ...
        let module_statements =
            projection.build_module_statements(&classified.module_statements)?;
