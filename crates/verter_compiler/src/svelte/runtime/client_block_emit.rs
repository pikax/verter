...
impl<'a> ClientEmitter<'a> {
    ...
    pub(super) fn emit_region_with_after_update(
        ...
        // already emitted in (a); emit the non-rendering init stream directly,
        // then the ops + the after_update; there is no clone/walk/mount.
        let roots = self.ir().template_scope(scope_id).roots.clone();
        let has_dom =
            !clean_nodes(self.ir(), &roots, super::html::region_ctx(self.ir())).is_empty();
        if !has_dom {
            let init_nodes = self.non_rendering_init_nodes(&roots);
            ...
            // their hoisted consts + non-rendering inits).
        ...
        match self.region_frame[&scope_id].clone() {
            ...
            RegionFrame::FromHtml {
                ...
                // (c) The DOM walk first emits the fragment's non-rendering init
                // stream, then populates the node/interp maps and interleaves nested
                // block calls at their `<!>` anchor positions.
    ...
    fn emit_after_update(&self, out: &mut super::output::SvelteRuntimeOutput, after_update: &str) {
    ...
    /// - the fragment's non-rendering init stream (before reactive ops, in mutual source order);
    ...
    fn emit_text_first_region(
        ...
        // The fragment's non-rendering init nodes emit before its reactive ops,
        // preserving their mutual source order.
        let roots = self.ir().template_scope(scope_id).roots.clone();
        let init_nodes = self.non_rendering_init_nodes(&roots);
        self.emit_non_rendering_inits(out, &init_nodes);
        // The reactive ops (`$.template_effect(() => $.set_text(text, …))`).
    ...
    /// The non-rendering init stream directly owned by `children`. The official
    /// fragment visitor emits debug/head/global-host init nodes before its DOM
    /// walk, preserving their mutual source order but ignoring rendered sibling
    /// positions for this phase.
    pub(super) fn non_rendering_init_nodes(&self, children: &[NodeId]) -> Vec<NodeId> {
        children
            .iter()
            .copied()
            .filter(|&child| {
                matches!(
                    ...
                    ClientNode::Debug { .. } | ClientNode::Head(_) | ClientNode::SpecialHost { .. }
                )
            })
            .collect()
    }

    /// Emit the typed non-rendering init nodes in their authored order.
    pub(super) fn emit_non_rendering_inits(
        &mut self,
        out: &mut super::output::SvelteRuntimeOutput,
        nodes: &[NodeId],
    ) {
        for &node in nodes {
            match self.client_node(node) {
                ...
                ClientNode::SpecialHost { .. } => self.emit_special_host_ops(out, node),
    ...
    fn emit_debug_effect(&self, out: &mut super::output::SvelteRuntimeOutput, node: NodeId) {
    ...
    fn emit_await_block(
        ...
        for (index, slot) in slots.iter().enumerate().take(last + 1) {
            ...
            match slot {
                Some(closure) => out.push_mapped(closure),
    ...
    fn branch_closure(
        ...
        let mut buf = super::output::SvelteRuntimeOutput::default();
        buf.push_str("($$anchor");
