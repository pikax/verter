...
impl<'a> SupportedClientIr<'a> {
    ...
    /// kind. Each global host event also retains its authored special-node identity,
    /// so shape lookup and init ordering remain exact across repeated host kinds.
    pub(super) fn project_event_op(
        ...
        // event (`<svelte:window|document|body>`) carries both the global kind and its
        // authored special node, so its accepted shape uses the same exact
        // (node, event type, handler) key. An event op with NO recorded shape is a
        // classifier/plan divergence
        // — fail closed defensively (never emit an unclassified, possibly-non-function
        // handler).
        let (owner, emit_target, shape) = match target {
            EventTarget::Node(node_id) => {
                ...
                (
                    node_id,
                    EventEmitTarget::Node(ClientNodeId(node_id.0)),
                    shape,
                )
            }
            EventTarget::Window(node_id)
            | EventTarget::Document(node_id)
            | EventTarget::Body(node_id) => {
                let shape = find_event_shape(
                    &self.event_shapes,
                    node_id,
                    &event.event_type,
                ...
                let host = match target {
                    EventTarget::Window(_) => EventEmitTarget::Window,
                    EventTarget::Document(_) => EventEmitTarget::Document,
