...
//! from the enclosing body skeleton (`is_non_body_special`) and emits its `$.head(...)` at its
//! source position (interleaved during the walk, like a `{@debug}`).
...
impl<'a> ClientEmitter<'a> {
    ...
    pub(super) fn emit_svelte_head(&mut self, out: &mut String, node: NodeId) {
