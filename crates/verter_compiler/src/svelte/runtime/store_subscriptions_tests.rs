...
use super::super::client_surface_imports::classify_script_imports_from_sources;
use super::super::expr::{
    collect_expr_references, reparse_module, AnalyzedExpr, BindingInfo, BindingRuntimeKind,
    BindingTable, ScopeGraph, ScopeId,
};
...
fn candidates(instance: &str, module: Option<&str>) -> FxHashSet<String> {
    ...
    let imports = classify_script_imports_from_sources(module, Some(instance));
    let program = reparse_module(&alloc, instance).expect("test source parses");
    store_base_candidates(Some(&program), &imports)
}
...
fn scan_template(
    ...
    collect_store_subscriptions(None, exprs, graph, bindings, root, Some(module), cands)
}
...
fn dependency_closure_is_seeded_by_subscriptions_only() {
    ...
    let (consts, fns, classes) =
        store_dependency_closure(Some(instance), Some(&program), &set(&["c"]));
    ...
    let (consts, fns, classes) =
        store_dependency_closure(Some(instance), Some(&program), &FxHashSet::default());
    assert!(
...
fn dependency_closure_admits_transitive_store_dependencies() {
    ...
    let program = reparse_module(&alloc, instance).expect("test source parses");
    let (consts, fns, classes) =
        store_dependency_closure(Some(instance), Some(&program), &set(&["doubled"]));
...
fn top_level_function_names_are_collected() {
    ...
    let program = reparse_module(&alloc, instance).expect("test source parses");
    let names = collect_top_level_function_names(Some(&program));
