...
pub enum EventTarget {
    ...
    Node(NodeId),
    /// The `window` global, retaining its authored `<svelte:window>` node.
    Window(NodeId),
    /// The `document` global, retaining its authored `<svelte:document>` node.
    Document(NodeId),
    /// The `body` element, retaining its authored `<svelte:body>` node.
    Body(NodeId),
}
