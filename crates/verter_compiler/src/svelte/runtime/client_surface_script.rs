...
pub(super) struct ClassifiedScriptItems {
    /// Non-import module-script statements in source order, identified against
    /// the canonical typed module program. The client plan rewrites/strips each
    /// statement and emits it in the module-body slot before template hoists.
...
/// Classify instance and module script items, returning module-scope user imports,
/// canonical module-statement spans, and the `$host`-usage fact. Ordinary module
/// statements and exports retain canonical identity; unsupported module runes and
/// TypeScript value constructs fail closed. On the instance side, Svelte-owned
/// forms retain their dedicated typed carriers while ordinary statements use the
/// canonical general carrier.
pub(super) fn classify_script_items(
    ...
    // Ordinary declarations, statements, and exports are carried by typed
    // statement identity. Svelte runes in module scope retain their distinct
    // fail-closed boundary; never pass an unlowered `$state`/`$props` call through.
    let module_statements = match (
        ir.analysis.scripts.module_source,
        ir.analysis.scripts.module_program.as_ref(),
    ) {
        (Some(module), Some(program)) => {
            let scan = scan_unsupported_rune_forms(
                ...
                module,
                false,
                ir.component.custom_element.is_some(),
                store_exempt,
            );
            if let Some(refusal) = scan.refusal {
                refuse_module_item_containing(program, refusal.span())?;
            }
            collect_module_statement_spans(program)?
        }
        _ => Vec::new(),
    };
...
/// Map a module-scope rune refusal to the canonical top-level statement that
/// owns it. Ordinary module statements are admitted, so a preceding declaration
/// must never steal the refusal's source span.
fn refuse_module_item_containing(
    ...
    for stmt in &program.body {
        ...
        let span = stmt.span();
        if span.start > refusal_span.start || span.end < refusal_span.end {
            continue;
        }
    ...
    Err(UnsupportedSvelteRuntimeSurface::ModuleScriptItem {
        construct: "module rune",
        span: refusal_span,
    })
}

/// Collect every non-import module statement in source order. TypeScript
/// runtime-erased declarations (`interface`, `type`) are retained here so the
/// shared TS stripper removes them on the same transform. TS constructs that
/// require a preprocessor (`enum`, value namespace, import-equals) remain
/// fail-closed; emitting the generic repository enum conversion would diverge
/// from Svelte's `typescript_invalid_feature` contract.
fn collect_module_statement_spans(
    program: &oxc_ast::ast::Program<'_>,
) -> Result<Vec<Span>, UnsupportedSvelteRuntimeSurface> {
    use oxc_span::GetSpan;
    let mut out = Vec::new();
    for stmt in &program.body {
        match stmt {
            ...
            _ => out.push(Span::new(stmt.span().start, stmt.span().end)),
        }
    }
    Ok(out)
}
