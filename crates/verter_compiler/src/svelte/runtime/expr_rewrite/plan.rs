...
use super::super::expr::{
    ...
    statement_position_user_effect_span, BindingRuntimeKind, EffectFamilyCallKind,
};
...
pub(super) use super::plan_types::{
    Edit, MappedToken, Occurrence, ReplacementMapping, RewriteResolveCtx,
};
use super::{ClientLvalue, PropRead, RewriteRole};
...
/// [`rewrite_expression_full`](super::rewrite_expression_full). A refusal (an
/// unsupported write target, `await`, grammar-invalid TypeScript target, or a
/// resolved occurrence left un-rewritten) returns the typed surface.
...
/// Plan reactive/runtime edits for one canonical script statement without
/// reparsing it. The statement belongs to a program retained on the runtime IR;
/// `source` and `comments` are that program's source/comment table, so every edit
/// remains in the original script coordinate space and composes with TypeScript
/// erasure on one `CodeTransform`.
pub(super) fn plan_script_statement_edits(
    ...
    let mut planner = RewritePlanner::new();
    planner.plan(&collector.occurrences);
    if let Some(surface) = planner.take_refusal() {
        return Err(surface);
    }
    debug_assert!(
...
/// unsupported expression form it hits (`await`, a destructuring write target,
/// grammar-invalid TypeScript target, a non-statement-position `$inspect`
...
impl BindingOccurrenceCollector<'_> {
    ...
    fn member_root_bindable_prop_source(&self, mut object: &Expression<'_>) -> Option<String> {
        loop {
            match object {
                ...
                Expression::TSAsExpression(e) => object = &e.expression,
                Expression::TSSatisfiesExpression(e) => object = &e.expression,
                Expression::TSNonNullExpression(e) => object = &e.expression,
                Expression::TSTypeAssertion(e) => object = &e.expression,
                Expression::TSInstantiationExpression(e) => object = &e.expression,
    ...
    ///   grammar-invalid TypeScript target or a destructuring write).
    ...
    fn record_assignment(&mut self, assign: &AssignmentExpression<'_>) {
        ...
        match target {
            ...
            ClientLvalue::UnsupportedReactiveTarget | ClientLvalue::UnsupportedTarget => {
                // A grammar-invalid TypeScript target or destructuring write fails closed.
