...
pub(super) fn expression_context<'client>(
    client: &'client SupportedClientIr<'_>,
) -> expr_rewrite::RewriteContext<'client> {
    expr_rewrite::RewriteContext {
        bindings: &client.ir.analysis.bindings,
        scopes: &client.ir.analysis.scopes,
        prop_reads: &client.prop_reads,
        proxy_inits: &client.proxy_inits,
        typescript: client.script_is_typescript(),
    }
}

impl<'a> SupportedClientIr<'a> {
    pub(super) fn script_is_typescript(&self) -> bool {
    ...
    pub(super) fn rewrite_source(
        ...
        expr_rewrite::rewrite_expression_full(source, scope, expression_context(self))
            .map(|r| r.text)
    }
    ...
    pub(super) fn rewrite_source_plain_js(
        ...
        expr_rewrite::rewrite_expression_plain_js(source, scope, expression_context(self))
            .map(|r| r.text)
    }
    ...
    pub(super) fn rewrite_value_preserving_source(
        ...
        .map(|r| r.mapped_code(analyzed.span.start))?;
