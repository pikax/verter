...
fn classify_target_expr(expr: &Expression) -> Option<BindTargetKind> {
    let mut expr = expr;
    // Unwrap parentheses + a non-null assertion (`name!`) to reach the lvalue core. A
    // SEQUENCE expression is reached THROUGH the synthetic wrap paren (`(get, set)`),
    // so the unwrap stops at a `SequenceExpression` to classify the function-pair form
    // BEFORE descending into its first element.
    loop {
        match expr {
