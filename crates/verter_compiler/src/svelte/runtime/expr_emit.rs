...
pub(super) fn state_primitive_decl(
    ...
    match lowering {
        Some(StateLowering::PlainLet) => format!("{DECL_LET_PREFIX}{name} = {arg};"),
        ...
        // authority).
        None => format!("{DECL_LET_PREFIX}{name} = $.state({arg});"),
    }
}

/// The declaration prefix every emitted instance-script `let <name> = …;`
/// declaration opens with.
///
/// The authored-provenance mapping for a declaration's NAME token is anchored
/// at `DECL_LET_PREFIX.len()`, so the offset is a property of THIS constant
/// rather than a number transcribed at the mapping site — a change to the
/// emitted keyword moves both together.
pub(super) const DECL_LET_PREFIX: &str = "let ";

/// One authored-name mapping over a generated fragment whose `name` token
/// starts at byte offset `prefix_len`: carries the authored name token's own
/// source position. Returns `None` when `name` is not actually at that
/// offset — the caller decides whether that is a bug (an assert) or an
/// expected miss (a silent unmapped fragment).
///
/// `name_span_start` is relative to the instance-script source; the caller
/// applies the script's own offset with
/// [`MappedCode::with_source_offset`](super::output::MappedCode::with_source_offset).
fn mapped_name_at(
    code: &str,
    prefix_len: usize,
    name: &str,
    name_span_start: u32,
) -> Option<super::output::SourceMapping> {
    if !code.get(prefix_len..)?.starts_with(name) {
        return None;
    }
    let start = prefix_len as u32;
    Some(super::output::SourceMapping {
        generated_start: start,
        generated_end: start + name.len() as u32,
        source_start: name_span_start,
    })
}

/// [`mapped_name_at`] anchored at the fixed [`DECL_LET_PREFIX`], for a `let
/// <name> = …` declaration fragment the emitter SYNTHESIZED (so the prefix is
/// a true compile-time constant — a mismatch here is an internal bug, not a
/// legitimate divergent input shape). A fragment whose name token is NOT at
/// the prefix offset carries NO mapping rather than a mapping pointing at the
/// wrong generated bytes: a missing segment is an honest gap the conformance
/// targets catch, while a misplaced one silently lands IDE navigation on the
/// wrong token.
pub(super) fn declaration_name_mapped_code(
    code: String,
    name: &str,
    name_span_start: u32,
    ...
    match mapped_name_at(&code, DECL_LET_PREFIX.len(), name, name_span_start) {
        ...
        None => {
            ...
            super::output::MappedCode::unmapped(code)
        }
    }
}

/// [`mapped_name_at`] for a `function <name>(...) { … }` declaration
/// fragment lowered VERBATIM from its authored source slice (an `async`
/// and/or generator `function*` declaration shares this carrier — there is no
/// fixed keyword literal the name sits after). `name_prefix_len` is the
/// caller-computed byte offset of the name token WITHIN the function's own
/// source slice (`id.span.start - func.span.start`, from the same parse that
/// admitted the declaration) — never a guessed/reconstructed literal. The
/// rewritten BODY may carry its own signal-read edits, but the declaration's
/// HEAD (keyword(s) through the name) is always untouched verbatim source, so
/// it stays at that same offset in the emitted fragment.
///
/// A mismatch (an admitted shape whose head the rewriter unexpectedly
/// touched) degrades to an UNMAPPED fragment — never a panic: unlike the
/// synthesized-fragment case above, the source shapes reaching this carrier
/// are real authored input, not an internal invariant this crate controls
/// end to end.
pub(super) fn function_decl_name_mapped_code(
    code: String,
    name: &str,
    name_prefix_len: u32,
    name_span_start: u32,
) -> super::output::MappedCode {
    match mapped_name_at(&code, name_prefix_len as usize, name, name_span_start) {
        Some(mapping) => super::output::MappedCode::from_parts(code, vec![mapping]),
        None => super::output::MappedCode::unmapped(code),
    }
}
