...
/// One authored-name mapping over a declaration fragment that opens with
/// [`DECL_LET_PREFIX`]: the generated `name` token starts immediately after the
/// prefix, and carries the authored name token's own source position.
///
...
/// A fragment whose name token is NOT at the prefix offset carries NO mapping
/// rather than a mapping pointing at the wrong generated bytes: a missing
/// segment is an honest gap the conformance targets catch, while a misplaced
/// one silently lands IDE navigation on the wrong token.
pub(super) fn declaration_name_mapped_code(
    code: String,
    name: &str,
    name_span_start: u32,
) -> super::output::MappedCode {
    let start = DECL_LET_PREFIX.len();
    let name_is_at_prefix_offset =
        code.starts_with(DECL_LET_PREFIX) && code[start..].starts_with(name);
    debug_assert!(
        name_is_at_prefix_offset,
        "a declaration fragment whose name token is not at the prefix offset cannot carry \
         its authored provenance: {code}"
    );
    if !name_is_at_prefix_offset {
        return super::output::MappedCode::unmapped(code);
    }
    let start = start as u32;
    let mappings = vec![super::output::SourceMapping {
        generated_start: start,
        generated_end: start + name.len() as u32,
        source_start: name_span_start,
    }];
    super::output::MappedCode::from_parts(code, mappings)
}
