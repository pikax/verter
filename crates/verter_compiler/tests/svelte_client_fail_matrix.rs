...
//! sub-shape; the gate asserts it FAILS CLOSED with the EXACT owning vertical (the
//! `5x` block) and emits NO `Main` module (never a silent empty module, never a
...
//! owner. Adding a row is trivial — append a `(name, source, block)` tuple.
//!
//! Most rows carry a `$state` so the component is in RUNES mode (a runeless
//! component fails closed at the legacy-mode gate `5i` first, before the surface
...
/// One fail-matrix row: a name, the component source, the expected owning vertical
/// (the `UnsupportedSvelteRuntimeSurface::owning_block`), and the expected EXACT
/// machine-stable diagnostic id (`UnsupportedSvelteRuntimeSurface::diagnostic_code`).
///
/// Pinning the exact `code` — not merely the owning block or the
/// `svelte-runtime-unsupported-` prefix — catches a WITHIN-OWNER drift: a row that
/// silently changes its refusal arm (e.g. a `bind:value` that starts refusing as a
/// `DynamicAttribute` instead of a `Binding`) keeps the same owner-block letter but
...
struct FailRow {
    ...
    source: &'static str,
    /// The expected owning vertical (`5a`..`5q` / `8`).
    block: &'static str,
...
const FAIL_MATRIX: &[FailRow] = &[
    ...
    FailRow {
        // A module script is demoted entirely (5s script-import) — refused before the
    ...
    FailRow {
        ...
        // silently dropped the 2nd arg (`$.state(0)`) — now it fails closed (5g).
    ...
    FailRow {
        ...
        // instance-script-item gate (5w, construct `function`) BEFORE its body's
        // destructuring write is ever lowered (the broad statement-rewrite path is
        // gone). The destructuring-write refusal (5p) now only fires for a write in a
    ...
    FailRow {
        // `$derived` is demoted entirely (5g) — refused before the async-rewrite gate.
        name: "derived_await",
        source: "<script>let c = $state(0); let d = $derived(await Promise.resolve(c));</script>\n<button onclick={() => c++}>{d}</button>\n",
        block: "5g",
        code: "svelte-runtime-unsupported-advanced-rune",
    },
    // ── non-`let` rune DECLARATION KIND (5g) ─────────────────────────────────
    ...
    FailRow {
        // `$effect` is demoted entirely (5g) — refused before the async-rewrite gate.
        name: "effect_async",
        source: "<script>let c = $state(0); $effect(async () => { await c; });</script>\n<button onclick={() => c++}>{c}</button>\n",
        block: "5g",
        code: "svelte-runtime-unsupported-advanced-rune",
    },
    // ── $props() advanced forms (5g) + bound (5c) ────────────────────────────
    FailRow {
    ...
    FailRow {
        ...
        // reads regardless — now it fails closed (5g).
    ...
    FailRow {
        // `$derived` is demoted entirely (5g) — refused before the member-bind gate.
    ...
    FailRow {
        // An instance import is demoted (5s) — refused before the member-bind gate.
    ...
    FailRow {
        ...
        // fires, not the state-shape gate) is the deferral-ledger member-bind form (5c).
    ...
    FailRow {
        // A component reference (a capitalized tag) is the 5f vertical; no import is
    ...
    FailRow {
        // A bare-identifier handler (not an inline arrow) is the wrapper form (5d); no
    ...
    FailRow {
        ...
        // `build_template_chunk` breadth (5r), refused before the async-rewrite gate.
        name: "text_await",
    ...
    FailRow {
        ...
        // gate fires the text-first 5q refusal). A non-reactive root read would fail at
        // the 5n static-interpolation gate first.
    ...
    FailRow {
        // An empty template (no rendered root) — the comment-anchor root shape (5q). A
        // top-level `$state` reassignment keeps it runes-mode without a `$effect` (which
        // is now demoted to 5g).
        name: "empty_root",
        source: "<script>let c = $state(0); c = 1;</script>\n",
        block: "5q",
        code: "svelte-runtime-unsupported-root-text-region",
    ...
    FailRow {
        ...
        // the spread gate is the surface, not the state-shape gate.
        name: "spread",
        source: "<script>let rest = {}; let c = $state(0);</script>\n<div {...rest}></div>\n<button onclick={() => c++}>{c}</button>\n",
        block: "5b",
        code: "svelte-runtime-unsupported-spread-or-html",
    },
    FailRow {
        name: "html_tag",
        source: "<script>let h = $state('<b>x</b>');</script>\n<div>{@html h}</div>\n",
        block: "5b",
        code: "svelte-runtime-unsupported-spread-or-html",
    },
    FailRow {
        // A HYPHENATED custom element is refused as a CUSTOM ELEMENT at the tag level
        // (5h) — the element classifier fails closed on the tag BEFORE the attr walk,
    ...
    // ── identifier-unsafe / reserved-word element tags (5v) ──────────────────
    ...
    // ── out-of-allowlist intrinsic elements (5a, the strict element allowlist) ───
    ...
    FailRow {
        ...
        // rejects them at the attr gate (5a).
    ...
    // ── duplicate attributes / directives (5a) ───────────────────────────────
    ...
    // ── instance-script-item allowlist (5w) — non-allowlist top-level items ──────
    FailRow {
    ...
    // refusal here (the magic-identifier 5w surface).
    // ── magic identifiers (5w) — auto-injected legacy objects ────────────────────
    FailRow {
        ...
        // as an unsupported FEATURE (5w), a deferrable refusal (official-accepted).
    ...
    FailRow {
        // An `{#each}` block (5e); `items` is a plain-local array (not array-state) and
    ...
    // ── Complex interpolations are fail-closed; the 5r vertical owns them ────
    FailRow {
        name: "interp_logical",
    ...
    // ── CONVERGENCE: $state primitive-only + $props default (5g) ─────────────────
    FailRow {
    ...
    // ── CONVERGENCE: demoted event-handler shapes (5d) ───────────────────────────
    FailRow {
    ...
    // ── CONVERGENCE: demoted bind shapes (5c) ────────────────────────────────────
    FailRow {
    ...
    // ── CONVERGENCE: script imports / module scripts (5s) ────────────────────────
    FailRow {
    ...
    // ── CONVERGENCE: complex text chunks (5u) ────────────────────────────────────
    FailRow {
    ...
    // ── free / undeclared bind:this target (5c) ──────────────────────────────────
    // A free `bind:this={button}` (no declared local) is official-accepted but outside
    // the §1.2 core (the shape-3 target is a DECLARED `let el;`); it fails closed (5c)
    ...
    // ── $state over a SHADOWED `undefined` (5g) ──────────────────────────────────
    ...
    // §1.2 core, so it fails closed as an unsupported FEATURE (5x) — never an emitted
...
#[test]
fn fail_matrix_every_row_fails_closed_with_exact_owner_and_no_main() {
    let mut wrong = Vec::new();
    for row in FAIL_MATRIX {
        match compile(row.source) {
            Err(ClientCompileError::Unsupported(surface)) => {
                // (1) The exact owning vertical.
                if surface.owning_block() != row.block {
                    wrong.push(format!(
                        "{}: expected owner {}, got {} ({:?})",
                        row.name,
                        row.block,
                ...
                // (2) The EXACT machine-stable diagnostic id — an EQUALITY check, not
                // merely the `svelte-runtime-unsupported-` prefix. This catches a
                // within-owner drift (a row that silently changes its refusal arm to
                // a sibling code while keeping the same owner-block letter).
            ...
            // (3) NO `Main` — an accepted module is a fail-matrix failure.
...
#[test]
fn fail_matrix_row_codes_are_consistent_with_owning_block() {
    // The expected `code` and `block` on each row must be a VALID
    // `UnsupportedSvelteRuntimeSurface` owning_block/diagnostic_code pairing. The
    // pairing is MOSTLY 1:1, but one owner (5a, the form / regular-element vertical)
    // owns THREE codes — `dynamic-attribute` (input cleanup / dynamic attrs / a forbidden
    // static attr), `duplicate-attribute` (a same-key duplicate attribute / directive),
    // and `element` (an out-of-allowlist intrinsic tag). The table
    // enumerates every VALID `(block, code)` pair and asserts each row's pair is a
    // member. This guards the TABLE itself (independently of the live compiler), so a
    // copy-paste that pairs the wrong code with a block fails here.
    let valid_pairs: &[(&str, &str)] = &[
        ("5a", "svelte-runtime-unsupported-dynamic-attribute"),
        ("5a", "svelte-runtime-unsupported-duplicate-attribute"),
        ("5a", "svelte-runtime-unsupported-element"),
        ("5b", "svelte-runtime-unsupported-spread-or-html"),
        ("5c", "svelte-runtime-unsupported-binding"),
        ("5d", "svelte-runtime-unsupported-non-delegated-event"),
        ("5e", "svelte-runtime-unsupported-block"),
        ("5f", "svelte-runtime-unsupported-component"),
        ("5g", "svelte-runtime-unsupported-advanced-rune"),
        ("5h", "svelte-runtime-unsupported-host-custom-element"),
        ("5j", "svelte-runtime-unsupported-experimental-async"),
        ("5n", "svelte-runtime-unsupported-static-interpolation"),
        ("5p", "svelte-runtime-unsupported-destructuring-write"),
        ("5q", "svelte-runtime-unsupported-root-text-region"),
        ("5r", "svelte-runtime-unsupported-complex-interpolation"),
        ("5s", "svelte-runtime-unsupported-script-import"),
        ("5t", "svelte-runtime-unsupported-typescript"),
        ("5u", "svelte-runtime-unsupported-complex-text"),
        ("5v", "svelte-runtime-unsupported-element-name"),
        // 5w (the script-completion vertical) owns TWO codes — a non-allowlist
        // instance-script item and a magic identifier (`$$slots`/`$$props`/`$$restProps`).
        ("5w", "svelte-runtime-unsupported-instance-script-item"),
        ("5w", "svelte-runtime-unsupported-magic-identifier"),
        // 5x (the close-tag-structure completion vertical) owns the implicit `<p>`
        // autoclose DOM re-parenting feature.
        ("5x", "svelte-runtime-unsupported-paragraph-autoclose"),
    ];
    for row in FAIL_MATRIX {
        assert!(
            valid_pairs.contains(&(row.block, row.code)),
            "{}: declared (block {}, code {}) is not a valid owning_block/code pair",
            row.name,
            row.block,
            row.code
        );
...
fn generated_event_handler_expression_kinds_land_on_boundary() {
    ...
    // statement-rewrite breadth and fails closed (5d). An arrow whose body is NOT a
    ...
    // closed at the instance-script-item gate (5w), masking the handler-shape boundary
    ...
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── demoted: non-arrow handler shapes (5d) ───────────────────────────────
        (
            "function_expr",
            "function () { c++; }",
            Expected::FailClosed,
        ...
        // ── demoted: arrow with a NON-$state-write body (5d) ─────────────────────
...
fn generated_effect_shapes_all_fail_closed() {
    ...
    // effect topology is a deferral-ledger follow-up, 5g). EVERY `$effect(arg)`
    ...
    // the 5j surface, but the `$effect` position refusal (5g) wins first.
    ...
    for (label, stmt) in variants {
        ...
        // reference is refused by the rune-form/position scan (5g) before the generic
...
fn generated_props_pattern_and_default_shapes_land_on_boundary() {
    ...
    // closed (5g). (Literal defaults were supported; demoted to the §1.2-class
    // no-default core.)
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── demoted: other out-of-boundary pattern shapes (5g) ───────────────────
        (
            "rest",
            "let { a, ...rest } = $props();",
            Expected::FailClosed,
...
fn generated_bind_target_shapes_land_on_boundary() {
    ...
    // `bind:this` to an identifier is supported; EVERYTHING else fails closed (5c).
    let variants: &[(&str, &str, &str, Expected)] = &[
        ...
        // ── demoted: a $state MEMBER target (was supported; now 5c) ───────────────
        (
            "value_state_member",
            "let o = $state({ x: '' });",
            "<input bind:value={o.x} />",
...
fn generated_lang_ts_components_all_fail_closed() {
    ...
    // script-completion follow-up (5t), so EVERY `lang="ts"` component fails closed
...
fn generated_static_attr_shapes_land_on_boundary() {
    ...
    // emission (5a); a customized built-in (`is=`) fails at the element gate (5h).
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── customized built-in (`is=`) fails at the element gate (5h) ───────────
        (
            "customized_builtin_is",
            "<button is=\"my-btn\">x</button>",
            Expected::FailClosed,
        ),
        // ── custom element (`<my-widget>`) fails at the element gate (5h) ────────
        (
            "custom_element",
            "<my-widget foo=\"bar\"></my-widget>",
            Expected::FailClosed,
    ...
    for (label, markup, expected) in variants {
        ...
        // than the 5n static-fold / 5q root-text path); the element under test
...
fn generated_rune_declaration_kinds_land_on_boundary() {
    ...
    // the instance-script-item gate, 5w).
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── $derived — demoted ENTIRELY (no supported kind, 5g) ──────────────────
        (
            "derived_let",
            "let d = $derived(c * 2);",
            Expected::FailClosed,
...
fn generated_interpolation_expression_kinds_land_on_boundary() {
    ...
    // evaluator and fails closed (5r). A bare identifier resolving to a non-reactive
    // binding is the static-fold deferral (5n).
    ...
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── demoted: complex expression shapes (5r build_template_chunk) ─────────
...
fn generated_state_init_shapes_land_on_boundary() {
    ...
    // (5g). (Object/array proxy state was supported; demoted.)
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── demoted: non-primitive (deep-reactive proxy) inits (5g) ──────────────
    ...
    for (label, decl, expected) in variants {
        ...
        // (5g) regardless of whether `make` is declared.
        let source = format!(
            "<script>let c = $state(0); {decl}</script>\n<button onclick={{() => c++}}>{{c}}</button>\n"
        );
...
fn generated_text_chunk_shapes_land_on_boundary() {
    ...
    // repeated-space run, or a backtick / `${` escaping need fails closed (5u). The
    ...
    let variants: &[(&str, &str, Expected)] = &[
        ...
        // ── demoted: complex literal chunks (5u) ─────────────────────────────────
...
fn fail_matrix_covers_every_documented_sub_shape() {
    ...
    // outside the §1.2 core: a free / undeclared `bind:this` target (×2, 5c), a `$state`
    // over a shadowed `undefined` + a `$state(NaN)` (×2, 5g), and the implicit `<p>`
    // autoclose (×2, 5x).
    assert_eq!(
        FAIL_MATRIX.len(),
        120,
