...
/// committed goldens passes `--check`, and a DRIFTED copy (the official-positive
/// semantic-comment oracle emptied) makes `--check` exit NON-ZERO. This proves the live
...
fn live_check_exits_nonzero_on_a_drifted_golden() {
    ...
    // Drift the checked official-positive fixture: erase the raw-derived
    // signature while leaving its normalized comment-stripped module intact.
    // A generator that compared only `clientModule` would false-pass this.
    let victim = temp_goldens.join("matrix/script_instance_general.client.json");
    ...
    let signature = value
        .get_mut("semanticCommentSignature")
        .and_then(|value| value.as_array_mut())
        .expect("victim has a `semanticCommentSignature` array");
    assert!(!signature.is_empty(), "victim is official-positive");
    signature.clear();
    std::fs::write(
        &victim,
    ...
    assert!(
        ...
        "the live --check MUST exit non-zero when a semantic-comment oracle drifts from the pinned \
         compiler; it returned success, which would let a stale field pass.\n{}{}",
