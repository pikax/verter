...
/// The exact sources executed by the pinned-runtime breadth smoke. Keeping
/// these in the official topology gate prevents the behavioral fixtures from
/// becoming projection-echoed tests of Verter's output alone.
const SUPPORTED_RUNTIME_BREADTH: &[&str] = &[
    "runtime/breadth_spread_html",
    "runtime/breadth_await",
    "runtime/breadth_snippet",
    "runtime/breadth_dynamic_child",
    "runtime/breadth_dynamic_parent",
    "runtime/breadth_special_head_window",
    "runtime/breadth_legacy_store",
    "runtime/breadth_typescript_script",
    "runtime/breadth_runes_effect",
    "runtime/breadth_scoped_css",
];
...
const SUPPORTED_MATRIX: &[&str] = &[
    // 5n script completion: module declarations/exports and broad instance
    // statements are preserved in source order while reactive references inside
    // instance functions/control-flow still lower through the client rewriter.
    "matrix/module_import_export",
    "matrix/script_instance_general",
    // 5t TypeScript completion: runtime-erased declarations/annotations and
    // type-only imports disappear, mixed imports retain only value specifiers,
    // and TS-wrapped bind lvalues canonicalize to their runtime target. The
    // module row independently pins TS erasure in the module-script slot.
    "matrix/typescript_script",
    "matrix/typescript_module_items",
    // Non-rendering init nodes preserve their mutual source order while
    // preceding the DOM walk. The runtime breadth fixture pins window -> head;
    // this reverse row prevents an unconditional host-before-head workaround.
    "matrix/special_init_order_reverse",
    "matrix/typescript_props",
    "matrix/typescript_prop_lvalues",
...
/// The golden's required semantic-comment oracle, derived from the raw pinned
/// Svelte output before `clientModule` drops cosmetic carrier comments.
fn golden_semantic_comment_signature(golden: &serde_json::Value) -> Vec<String> {
    golden["semanticCommentSignature"]
        .as_array()
        .expect("a client golden carries `semanticCommentSignature`")
        .iter()
        .map(|entry| {
            entry
                .as_str()
                .expect("semantic-comment signature entries are strings")
                .to_string()
        })
        .collect()
}
...
fn expr_sig(expr: &Expression) -> String {
    ...
    match expr {
        ...
        // The residual variants are TS wrappers, JSX, or V8-intrinsic forms, none of which can occur
        // after the comparator's successful `SourceType::mjs()` parse. Panic rather than producing a
        // lossy signature: a future ordinary-JS OXC variant must be encoded before the gate can pass.
        other => panic!(
            "Svelte conformance comparator reached an unencoded expression: {:?}",
...
fn stmt_sig(stmt: &Statement) -> String {
    match stmt {
        ...
        // TS/module-only statement forms cannot occur after a successful mjs parse. Keep this
        // residual fail-closed so a future ordinary-JS OXC variant cannot silently collapse.
        other => panic!(
            "Svelte conformance comparator reached an unencoded statement: {:?}",
            std::mem::discriminant(other)
        ),
    }
}
...
fn decl_sig(decl: &Declaration) -> String {
    match decl {
        ...
        // TS-only declarations cannot occur after a successful mjs parse. Keep this residual
        // fail-closed so a future ordinary-JS OXC variant cannot silently collapse.
        other => panic!(
            "Svelte conformance comparator reached an unencoded declaration: {:?}",
            std::mem::discriminant(other)
        ),
    }
}
...
fn conformance_sig(code: &str, side: &str) -> ModuleConformanceSig {
    ...
    ModuleConformanceSig {
        ...
        comment_sig:
            verter_compiler::svelte_semantic_comments::semantic_comment_signature_from_program(
                code,
                &ret.program,
            ),
    }
...
///
/// The normalized golden module remains comment-stripped, but
/// `official_comment_sig` is extracted from the RAW pinned Svelte output by the
/// same Rust/OXC function used for `emitted_sig`.
fn assert_modules_structurally_equal(
    slug: &str,
    emitted: &str,
    golden: &str,
    official_comment_sig: &[String],
) {
    ...
    assert_eq!(
        emitted_sig.comment_sig, official_comment_sig,
...
#[test]
fn expression_residual_fails_closed_instead_of_producing_a_lossy_signature() {
    ...
    assert!(
        outcome.is_err(),
        "a TS expression residual must fail closed"
    );
}

#[test]
fn statement_residual_fails_closed_instead_of_producing_a_lossy_signature() {
    let allocator = Allocator::default();
    let parsed = oxc_parser::Parser::new(
        &allocator,
        "type Alias = string;",
    ...
    assert!(outcome.is_err(), "a TS statement residual must fail closed");
}

#[test]
fn declaration_residual_fails_closed_instead_of_producing_a_lossy_signature() {
    let allocator = Allocator::default();
    let parsed = oxc_parser::Parser::new(
        &allocator,
        "type Alias = string;",
    ...
    let outcome = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| decl_sig(declaration)));
    assert!(
        outcome.is_err(),
        "a TS declaration residual must fail closed"
    );
}
...
fn first_semantic_comment_anchor(code: &str) -> String {
    let signature = verter_compiler::svelte_semantic_comments::semantic_comment_signature(code)
        .unwrap_or_else(|error| panic!("the module did not parse as JS: {error:?}\n{code}"));
    let first = signature
        .first()
        .unwrap_or_else(|| panic!("no semantic comment found in:\n{code}"));
    let anchor_start = first
        .find("anchor=\"")
        .map(|index| index + "anchor=\"".len())
        .expect("signature carries an anchor");
    let anchor_end = first[anchor_start..]
        .find("\",ord=")
        .map(|index| anchor_start + index)
        .expect("signature carries an ordinal after its anchor");
    first[anchor_start..anchor_end].to_string()
}
...
fn all_supported_slugs() -> Vec<&'static str> {
    SUPPORTED_FIXTURES
        .iter()
        .chain(SUPPORTED_RUNTIME_BREADTH.iter())
...
fn all_supported_slugs_wires_every_corpus_const() {
    let corpora: &[(&str, &[&str])] = &[
        ...
        ("SUPPORTED_RUNTIME_BREADTH", SUPPORTED_RUNTIME_BREADTH),
...
fn supported_matrix_enumerates_every_documented_sub_shape() {
    ...
    assert_eq!(
        ...
        58,
        "the supported matrix must enumerate all 58 documented supported sub-shapes \
         (the 6 script-completion rows: module items, broad instance items, TypeScript \
         instance lowering, TypeScript module lowering, typed props lowering, and \
         TypeScript prop-lvalue lowering + the special-init reverse-order row + \
         the 16 §1.2/rune/attr + \
         the 11 5c DOM-hosted bind-family rows incl. radio group + \
...
fn assert_client_topology_matches_official(slug: &str) {
    ...
    assert_modules_structurally_equal(
        slug,
        &code,
        &golden_client_module(&golden),
        &golden_semantic_comment_signature(&golden),
    );
}
...
fn emitted_client_topology_matches_official_goldens() {
    for &slug in &all_supported_slugs() {
        assert_client_topology_matches_official(slug);
    }
}

#[derive(serde::Deserialize)]
#[serde(rename_all = "camelCase")]
struct SveltePerfManifest {
    schema_version: u32,
    fixtures: Vec<SveltePerfManifestFixture>,
}

#[derive(serde::Deserialize)]
struct SveltePerfManifestFixture {
    name: String,
    slug: String,
    coverage: Vec<String>,
}
...
#[test]
fn svelte_performance_manifest_is_oracle_proven_and_representative() {
    let path = repo_root().join("packages/benchmark/src/svelte-perf-manifest.json");
    let text = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("read Svelte performance manifest: {error}"));
    let manifest: SveltePerfManifest = serde_json::from_str(&text)
        .unwrap_or_else(|error| panic!("parse Svelte performance manifest: {error}"));
    assert_eq!(
        manifest.schema_version, 1,
        "unsupported performance-manifest schema"
    );
    assert_eq!(
        manifest.fixtures.len(),
        10,
        "the benchmark denominator is an explicit ten-fixture contract"
    );
    ...
    for fixture in &manifest.fixtures {
        ...
        assert!(
            slugs.insert(&fixture.slug),
            "duplicate benchmark slug {}",
            fixture.slug
        );
        assert!(
            !fixture.coverage.is_empty(),
            "{} has no coverage labels",
            fixture.name
        );
        coverage.extend(fixture.coverage.iter().map(String::as_str));
        largest_source = largest_source.max(fixture_source(&fixture.slug).len());
        assert_client_topology_matches_official(&fixture.slug);
    }

    assert!(
        largest_source > 4_096,
        "the corpus must contain a genuinely large authored component, got {largest_source} bytes"
    );
    for required in [
        "typescript",
        "module-script",
        ...
        assert!(
            ...
            "the benchmark corpus is missing the `{required}` coverage class"
        );
    }
}
...
#[test]
fn bind_5c_emit_is_fully_encoded_by_the_fail_closed_comparator_d17() {
    ...
    // bind slug and computes the AST-structural conformance signature. Because every
    // unencoded residual now panics, successful construction is the behavioral proof
    // that every emitted node was encoded rather than collapsed.
    for &slug in BIND_5C_SLUGS {
        let code = emit(slug);
        let sig = conformance_sig(&code, "emitted");
        assert!(
            !sig.module_sig.is_empty(),
            "5c bind emit for {slug} must produce a structural signature"
        );
    }
}

#[test]
fn component_emit_is_fully_encoded_by_the_fail_closed_comparator() {
    ...
    for &slug in SUPPORTED_COMPONENTS {
        ...
        assert!(
            !sig.module_sig.is_empty(),
            "component emit for {slug} must produce a structural signature"
        );
    }
}
...
fn codegen_corpus_is_nonempty_and_every_cell_emits_valid_js() {
    ...
    for slug in &slugs {
        ...
        let _required_raw_oracle = golden_semantic_comment_signature(&golden);
    }
}
