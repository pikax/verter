...
const FAIL_MATRIX: &[FailRow] = &[
    ...
    FailRow {
        // A module-scope `$state` declarator requires dedicated Svelte lowering;
        // ordinary module statements are admitted, but the rune fails closed with
        // the precise module-item diagnostic.
        name: "state_module",
    ...
    FailRow {
        // The function statement is admitted, while the destructuring reactive
        // write in its body retains the precise `DestructuringWrite` refusal.
        name: "instance_top_level_function",
        source: "<script>let c = $state(0); function r() { ({ c } = { c: 1 }); }</script>\n<button onclick={() => c++}>{c}</button>\n",
        code: "svelte-runtime-unsupported-destructuring-write",
    },
    ...
    FailRow {
        ...
        name: "lang_ts_enum",
        source: "<script lang=\"ts\">enum Mode { A } let count = $state(0);</script>\n<button onclick={() => count++}>{count}</button>\n",
        code: "svelte-runtime-unsupported-instance-script-item",
    },
    // ── CONVERGENCE: <script lang=ts> ───────────────────────────────────────
    FailRow {
        name: "lang_ts_namespace",
        source: "<script lang=\"ts\">namespace N { export const x = 1 } let count = $state(0);</script>\n<button onclick={() => count++}>{count}</button>\n",
        code: "svelte-runtime-unsupported-instance-script-item",
    },
    ...
    FailRow {
        ...
        name: "lang_ts_import_equals",
        source: "<script lang=\"ts\">import fs = require('fs'); let c = $state(0);</script>\n<button onclick={() => c++}>{c}</button>\n",
        code: "svelte-runtime-unsupported-instance-script-item",
    },
...
#[test]
fn generated_lang_ts_components_emit_valid_js() {
    // Exact `lang="ts"` selects the parser-wide TypeScript grammar. Clean and
    // wrapped lvalues plus annotated script bodies must all erase to valid JS.
    ...
    for (label, decl, markup) in variants {
        ...
        assert_variant(&format!("lang_ts::{label}"), &source, Expected::Supported);
    }
}
...
fn generated_rune_declaration_kinds_land_on_boundary() {
    ...
    // Non-rune `var` and `const` declarations are ordinary statements and remain
    // supported; the declaration-kind restriction applies only to rune carriers.
    let variants: &[(&str, &str, Expected)] = &[
        ...
        ("nonrune_var", "var step = 2;", Expected::Supported),
        ("nonrune_const", "const step = 2;", Expected::Supported),
    ];
    for (label, decl, expected) in variants {
