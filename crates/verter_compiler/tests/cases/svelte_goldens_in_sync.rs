...
struct CommittedGolden {
    ...
    /// Required on every golden. Client entries are derived from the raw
    /// pinned compiler module by the shared Rust/OXC extractor; servers carry
    /// an empty list because there is no client module.
    #[serde(rename = "semanticCommentSignature")]
    semantic_comment_signature: Vec<String>,
    css: CssTopology,
}
...
#[test]
fn official_positive_semantic_comment_golden_is_non_empty() {
    let path = goldens_dir().join("matrix/script_instance_general.client.json");
    let raw = std::fs::read_to_string(&path)
        .unwrap_or_else(|error| panic!("read {}: {error}", path.display()));
    let golden: serde_json::Value = serde_json::from_str(&raw)
        .unwrap_or_else(|error| panic!("parse {}: {error}", path.display()));
    let signature = golden
        .get("semanticCommentSignature")
        .and_then(serde_json::Value::as_array)
        .unwrap_or_else(|| {
            panic!(
                "{} must carry the required official semantic-comment oracle",
                path.display()
            )
        });
    ...
    assert!(
        !signature.is_empty(),
        "the official-positive `/*! general-license */` fixture must produce a non-empty semantic-comment oracle"
    );
}
...
fn validate_committed_golden(path: &Path, raw: &str) -> Result<(), String> {
    ...
    if golden.backend == "server" && !golden.semantic_comment_signature.is_empty() {
        return Err(format!(
            "{}: a server golden must carry an empty `semanticCommentSignature`",
            path.display()
        ));
    }
    if golden
        .semantic_comment_signature
        .iter()
        .any(String::is_empty)
    {
        return Err(format!(
            "{}: `semanticCommentSignature` entries must be non-empty",
            path.display()
        ));
    }
    if !golden.semantic_comment_signature.is_sorted() {
        return Err(format!(
            "{}: `semanticCommentSignature` must use the extractor's stable sorted order",
            path.display()
        ));
    }
...
fn structural_guard_discriminates_a_corrupted_golden() {
    ...
    // The semantic-comment oracle is likewise required, never optional or
    // default-empty. Remove it from the official-positive fixture and prove
    // the same deny-unknown typed schema rejects the omission.
    let semantic_path = dir.join("matrix/script_instance_general.client.json");
    let semantic_raw = std::fs::read_to_string(&semantic_path)
        .unwrap_or_else(|error| panic!("read {}: {error}", semantic_path.display()));
    let mut missing_semantic: serde_json::Value =
        serde_json::from_str(&semantic_raw).expect("semantic golden parses as JSON");
    missing_semantic
        .as_object_mut()
        .expect("golden is a JSON object")
        .remove("semanticCommentSignature");
    let missing_semantic_src =
        serde_json::to_string(&missing_semantic).expect("serialize missing semantic field");
    assert!(
        validate_committed_golden(&semantic_path, &missing_semantic_src).is_err(),
        "a golden missing required `semanticCommentSignature` MUST fail the structural guard"
    );
