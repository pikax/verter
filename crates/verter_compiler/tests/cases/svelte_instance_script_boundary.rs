//! The completeness gate for instance-script statement classification and the
//! `<svelte:options>` boundary.
//!
...
//! ordinary JavaScript statements retain canonical program identity through the
//! typed `GeneralStatement` carrier, while Svelte-owned constructs use dedicated
//! carriers and unsupported constructs fail closed with a typed reason.
//!
//! Exact `lang="ts"` scripts additionally erase runtime-free TypeScript syntax on
//! the same `CodeTransform` as reactive rewrites. Plain scripts stay JavaScript
//! grammar and reject TypeScript syntax.
//!
//! ## The `<svelte:options>` acceptance
//! Supported axes are resolved by the shared compile-options layer; malformed or
//! unsupported forms retain their precise official or typed refusal.
//!
//! ## Matrices
//! - POSITIVE: dedicated Svelte carriers, ordinary statement families, TypeScript
//!   erasure, and supported options forms all emit parseable JavaScript.
//! - NEGATIVE: unsupported rune declaration kinds, destructuring shapes,
//!   TypeScript value constructs, magic identifiers, and invalid options forms.
//! - NEGATIVE (magic): `$$slots` / `$$props` / `$$restProps` / `let $$anchor`.
//! - STRUCTURAL GUARDS: one import authority, canonical statement identity, and no
//!   TypeScript erasure on a plain-script accept path.
...
// ─────────────────────────────────────────────────────────────────────────────
// POSITIVE matrix — dedicated Svelte carriers, ordinary statements, and options.
// ─────────────────────────────────────────────────────────────────────────────

#[test]
fn positive_supported_instance_script_shapes_emit_valid_main() {
    // Each row is a complete supported component using one or more dedicated
    // carriers; each must emit a valid-JS `Main`.
    let rows: &[(&str, &str)] = &[
...
// ─────────────────────────────────────────────────────────────────────────────
// POSITIVE matrix — ordinary OXC top-level statement families use the canonical carrier.
// ─────────────────────────────────────────────────────────────────────────────

#[test]
fn ordinary_top_level_statement_families_emit_valid_main() {
    // Rune-free ordinary statements emit through `GeneralStatement`. Dedicated
    // rune and magic scans still run before this carrier and retain their precise
    // refusal identities.
    let rows: &[(&str, &str)] = &[
        ...
        // Ordinary variable declarations are carried in source order.
        ("plain_let", "let x = 0;"),
    ...
    for (label, body) in rows {
        if !emits_valid_main(body) {
            leaks.push(format!(
                "{label}: expected a valid Main, got {:?}",
                compile(&with_supported_template(body))
            ));
        }
    }
    assert!(
        leaks.is_empty(),
        "ordinary top-level statement families must emit valid JavaScript:\n{}",
        leaks.join("\n")
    );
}
...
fn negative_variable_declaration_sub_shapes_fail_closed() {
    // These Svelte-owned or grammar-invalid declaration shapes cannot fall back to
    // the ordinary statement carrier and therefore fail closed precisely.
    let rows: &[(&str, &str)] = &[
        // Decl kind on a rune.
        ...
        // A multi-rune declarator requires dedicated lowering.
        ("multi_declarator", "let a = $state(0), b = $state(1);"),
        // TypeScript syntax in a plain script remains a parse rejection.
    ...
    assert!(
        leaks.is_empty(),
        "every unsupported Svelte-owned variable-declaration sub-shape must fail closed:\n{}",
        leaks.join("\n")
    );
}

#[test]
fn uninitialized_bare_let_emits_with_or_without_bind_this_use() {
    // A bare local emits both as an ordinary unused declaration and through the
    // dedicated `bind:this` carrier when used as a target.
    let unused =
        "<script>let v = $state(0); let el;</script>\n<button onclick={() => v++}>{v}</button>\n";
    assert!(
        emits_valid_main(unused),
        "an unused bare local must emit:\n{:?}",
        compile(unused)
    );
...
fn negative_shadowed_magic_name_is_not_a_false_refusal() {
    ...
    // component still reaches the handler classifier. The arrow-param form isolates
    // shadowing from the separate function-declaration statement lane.
    let src =
...
fn redeclaration_gate_covers_module_scope_bindings_with_official_parity() {
    ...
    // redeclaration gate must NOT fire, and the ordinary statements must emit.
    let var_rebind = compile("<script>var x = 1; var x = 2;</script>\n<button>x</button>\n")
        .expect("legal `var`/`var` re-binding must compile");
    assert!(
        parses_as_js(&var_rebind),
        "legal `var`/`var` re-binding must emit valid JavaScript:\n{var_rebind}"
    );
}

// ─────────────────────────────────────────────────────────────────────────────
// STRUCTURAL GUARDS — canonical identity and single classification authorities.
// ─────────────────────────────────────────────────────────────────────────────
...
#[test]
fn guard_client_script_lowering_uses_typed_canonical_carriers() {
    // The retired source-slice lowering must stay absent. Script lowering consumes
    // typed carriers; ordinary statements route through the canonical statement
    // identity retained on `GeneralStatement`.
    for file in [
        "client_plan.rs",
    ...
    // The consumer lowers dedicated items and the general canonical carrier.
    ...
    assert!(
        plan.contains("Item::GeneralStatement") && plan.contains("rewrite_script_statement("),
        "client_plan_script.rs must lower ordinary statements through canonical AST identity"
    );
}
...
#[test]
fn guard_general_statement_lane_does_not_restore_source_slice_reparsing() {
    // Ordinary statements must retain canonical program identity. The removed
    // source-slice helper and wildcard reparse arm must not return.
...
fn guard_supported_instance_script_item_enum_is_the_classified_surface_field() {
    // The typed carrier vocabulary is retained on the classified surface.
    // `ClassifiedClientSurface` carries `script_items:
    ...
    // The carrier enum + its classifier live in the dedicated `instance_items.rs`
    ...
    assert!(
        items.contains("GeneralStatement"),
        "instance_items.rs must retain a canonical general-statement carrier"
    );
}
