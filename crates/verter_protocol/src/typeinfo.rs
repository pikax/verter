...
//! dispatching.
//!
//! The graph contracts ([`graph`]) are protobuf-authoritative — the
//! Rust types are generated by `prost-build` and re-exported from the
//! [`graph`] submodule. The TypeScript surface is generated by the
//! existing protobuf path under `packages/proto/src/gen/verter/v1/`.
...
/// operation name; the wire type is unchanged.
pub use graph::TypeInfoGraphRequest as TypeInfoRequest;
