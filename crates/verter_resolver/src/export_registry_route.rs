...
fn resolve_type_via_registry_inner<R: RegistryRouteResolver>(
    ...
    for specifier in &registry.wildcard_edges {
        ...
        if !visited.contains(&visit_key) && current_level_seen.insert(child_canonical.clone()) {
    ...
    while !current_level.is_empty() {
        ...
        for child_canonical in &current_level {
            ...
            seeded_level.push((child_canonical.clone(), child_registry));
        }

        for (child_canonical, child_registry) in &seeded_level {
...
mod tests {
    ...
    #[test]
    fn resolve_type_via_registry_batches_full_level_before_returning_match() {
        let mut resolver = TestResolver::default();
        resolver.registries.insert(
            "/src/index.ts".to_string(),
            ExportRegistryView {
        ...
        assert_eq!(
            ...
            "BFS should seed and inspect the full first wildcard level before returning a match",
        );
        assert_eq!(
            route.tracked_deps,
            vec![
            ...
            "tracked deps should reflect the inspected winning level and exclude deeper queued descendants",
        );
        assert!(
