//! File path ↔ `file://` URI conversion helpers — the single shared owner for
//! the whole workspace.
//!
//! These are pure string transforms with no I/O and no engine dependency, so
//! they live in the leaf `verter_span` crate alongside the canonical-path
//! primitives ([`crate::path`]). Every consumer — the LSP transport, the
//! external-TypeScript session decision layer, the type-runtime backends —
//! routes `file://` parsing through this one module so the path derived from a
//! URI is identical everywhere (`verter_type_runtime::uri` re-exports these).
...
/// intentionally NOT routed through [`crate::path::canonicalize_path`] (which
...
/// shared [`crate::path::fs_is_case_insensitive`] policy to make both forms hit
/// the same cache bucket; a case-sensitive filesystem (Linux) preserves case.
/// Shared file identity still uses [`crate::path::canonicalize_path`].
pub fn normalize_file_uri_for_cache(uri: &str) -> String {
    let decoded = percent_decode(uri);
    if crate::path::fs_is_case_insensitive() {
...
mod tests {
    ...
    fn normalize_file_uri_for_cache_folds_case_per_fs_policy() {
        ...
        // `fs_is_case_insensitive` policy (Windows + default macOS fold; Linux
        // preserves). Platform-faithful + discriminating: on macOS the pre-fix
        // `cfg(windows)`-only fold left a mixed-case URI UNfolded, so the
        // case-variant form missed the bucket; routing through the shared policy
        // folds it there too.
