...
impl ObjectExpr {
    ...
    /// byte `delta` (see [`TypeExpr::shift_spans`]).
    fn shift_spans(&mut self, delta: i64) {
...
impl FunctionExpr {
    ...
    /// default by a signed byte `delta` (see [`TypeExpr::shift_spans`]).
    fn shift_spans(&mut self, delta: i64) {
...
impl TypeExpr {
    ...
    /// Recursively shift every embedded declaration-site [`Span`] by a signed
    /// byte `delta`, in place.
    ///
    /// Spans on a `TypeExpr` are byte offsets into the source buffer the type
    /// was lowered from. When a type is lowered from a *synthetic* buffer (e.g.
    /// a JSDoc `{Type}` payload wrapped in `type __T = <payload>` before the OXC
    /// parse), its spans are in that buffer's coordinates. Shifting by
