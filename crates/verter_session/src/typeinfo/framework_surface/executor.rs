    ...
    fn execute_framework_surface(
        ...
        if selector.has_export_name {
            ...
            if let ComponentExport::Named(name) = &resolved_selector.export {
                let is_export = self
                    .ensure_indexed_ready(resolved_selector.canonical.as_ref())
                    .is_some_and(|indexed| indexed.shallow_state.export_target(name).is_some());
                if !is_export {
                    let error = malformed(&format!(
...
impl ExecutorResolveCtx<'_> {
    ...
    fn resolve_macro_payload(
        ...
        let Some(indexed) = self.ctx.ensure_indexed_ready(owner) else {
