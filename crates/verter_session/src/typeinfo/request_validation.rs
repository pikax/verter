...
///
/// Bumped to 2 when the env-LESS decl-slot roots carrier was retired:
/// a v1 payload's roots rode the env-less slot (tag 2, now reserved),
/// which lacks `whole_hash`, so a v1 envelope decoded by a v2 server
/// would silently drop its roots. Unknown-field wire-compat is NOT
/// semantic compat — the closed-set validator rejects v1 rather than
/// admit a payload whose roots cannot be recovered.
