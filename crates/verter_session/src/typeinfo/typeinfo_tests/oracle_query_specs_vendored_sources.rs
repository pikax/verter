...
/// Vendored source bytes of `/fixtures/call_resolution.ts` (the registry is the
/// source-byte authority). Inlined verbatim (PURE owned `&'static str`); the
/// guard `inlined_registry_source_is_byte_identical_to_fixture_files` asserts
/// byte-identity with `fixtures/call_resolution.ts`.
#[allow(dead_code)]
pub(crate) const CALL_RESOLUTION_SOURCE: &str = r#"// @ai-generated - Synthetic call-site resolution fixture.
//
...
/// The workspace-file set every call-resolution oracle row upserts: the single
/// canonical fixture, shared by every row in the family.
#[allow(dead_code)]
const CALL_RESOLUTION_FILES: &[WorkspaceFileSpec] = &[WorkspaceFileSpec {
    path: "/fixtures/call_resolution.ts",
    source: CALL_RESOLUTION_SOURCE,
}];


/// Vendored source bytes of `/fixtures/const_type_param.ts` (the registry is the
/// source-byte authority). Inlined verbatim (PURE owned `&'static str`); the
/// guard `inlined_registry_source_is_byte_identical_to_fixture_files` asserts
/// byte-identity with `fixtures/const_type_param.ts`.
#[allow(dead_code)]
pub(crate) const CONST_TYPE_PARAM_SOURCE: &str = r#"// @ai-generated - Synthetic `const` type-parameter typeinfo fixture.
...
/// The workspace-file set the const-type-parameter oracle rows upsert: the
/// single canonical fixture, shared by both rows.
#[allow(dead_code)]
const CONST_TYPE_PARAM_FILES: &[WorkspaceFileSpec] = &[WorkspaceFileSpec {
    path: "/fixtures/const_type_param.ts",
    source: CONST_TYPE_PARAM_SOURCE,
}];
