...
///
/// `Shallow` / `Navigate` / `Expanded` are all in use (the lifted oracle rows
/// are captured in `Expanded`); `Skeleton` is carried for schema totality (the
/// BFS / generic-helper traversal mode, not yet a snapshot-row mode).
#[allow(dead_code)]
