...
/// The comparability overlap oracle answers under the CONFIGURED strict
/// family, and it proves disjointness only for pairs it can actually
/// prove.
///
/// Two boundaries, both about minting a FALSE proof into the shared
/// relation family:
///
/// 1. **Regime.** With `strictNullChecks` OFF, `null` / `undefined`
///    inhabit every remaining type, so a pair naming one of them is not
///    disjoint. Answering with the strict-regime verdict would warm a
///    disjointness proof the configured relation contradicts.
/// 2. **Weak surface.** A surface that constrains nothing is inhabited by
///    every non-nullish value, primitives included: `string & {}` is
///    `string`. The oracle must fall through to the structural answer
///    rather than declare the pair uninhabited. A CONSTRAINING surface
///    (`{ x: number }`) stays proven-disjoint from the primitive tags —
///    that boundary is pinned by the mixed-kind table in
///    `tests/cases/relation_nominal_authority.rs`.
#[test]
fn comparable_overlap_oracle_reads_the_strict_regime_and_proves_only_what_it_can() {
    use crate::semantic_query::{QueryResult, RelationKind, RelationOutcome, SemanticQueryValue};
    ...
    let template = graph.intern_node(SemanticNodeData::TemplateLiteral {
        quasis: Arc::from(vec![Arc::from("a"), Arc::from("")].into_boxed_slice()),
        expressions: Arc::from(Vec::new().into_boxed_slice()),
    });
    let string = graph.intern_node(SemanticNodeData::Primitive(PrimitiveKind::String));
    let open_surface = graph.intern_node(SemanticNodeData::Object(empty_surface(Vec::new())));

    let comparable = |host: &VerterHost, a, b| {
        let dispatch = ProjectSemanticDispatch::new(host);
        let key = dispatch.relate_key_for_kind(a, b, RelationKind::Comparable);
        match dispatch.execute(key.to_query_key()) {
            QueryResult::Value(output) => match &output.value {
                SemanticQueryValue::Relation(payload) => payload.outcome.clone(),
                other => panic!("a comparability ask answers a relation, got {other:?}"),
            },
            other => panic!("a comparability ask answers a value, got {other:?}"),
        }
    };

    // A template literal is a string, so it cannot inhabit `undefined` —
    // under the TS-strict regime the oracle proves it.
    assert_eq!(
        comparable(&host, template, undefined),
        RelationOutcome::NotAssignable,
        "strictNullChecks ON: a template literal and `undefined` are provably disjoint"
    );

    // With strictNullChecks OFF, `undefined` inhabits every remaining
    // type, so the SAME pair must not carry a disjointness proof.
    host.relation_knobs
        .strict_family_relax_bits
        .store(0b01, std::sync::atomic::Ordering::Relaxed);
    assert_ne!(
        comparable(&host, template, undefined),
        RelationOutcome::NotAssignable,
        "strictNullChecks OFF: `undefined` inhabits the template literal type, so the \
         oracle must not mint a strict-regime disjointness proof"
    );
    host.relation_knobs
        .strict_family_relax_bits
        .store(0, std::sync::atomic::Ordering::Relaxed);

    // A primitive versus an open structural surface is UNPROVEN in either
    // regime: `string & {}` is `string`, so the oracle must not answer
    // with a disjointness proof.
    assert_ne!(
        comparable(&host, string, open_surface),
        RelationOutcome::NotAssignable,
        "a primitive and an open object surface share inhabitants; the overlap oracle \
         must not declare them disjoint"
    );
}
