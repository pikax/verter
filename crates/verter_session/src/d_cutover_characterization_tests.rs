...
/// Plan §9 row 4: `engine.project_expr_surface_shape` migration.
/// Verifies the dispatch-first path is present and emits Shallow
/// projection requests.
#[test]
fn migrate_engine_project_expr_surface_shape_preserves_env() {
    let cmqe_src = include_str!("resolver_core/component_meta_query_engine.rs");
    // `project_expr_surface_shape` body routes a Shallow ProjectPath
    // before falling back to owner_engine.
    assert!(
        cmqe_src.contains("ProjectionMode::Shallow"),
        "project_expr_surface_shape must emit Shallow ProjectionMode post-migration"
    );
}
