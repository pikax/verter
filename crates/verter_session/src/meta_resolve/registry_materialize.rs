//! Registry structural-preference predicate + member-route preservers.
//!
//! Owns the graph-native structural-PREFERENCE predicate
//! (`component_meta_registry_prefers_structural_materialization_node`) plus the
//! seven registry-route preserver predicates
...
//!
//! The structure-preserving materialiser itself is node-domain and lives in the
//! query-engine owner
//! (`ComponentMetaQueryEngine::materialize_registry_structural_candidate`), where
//! reference / route leaves resolve through the node-domain Class-A /
//! whole-surface helpers and the result's object-surface fact is read off the
//! producing node.
//!
//! These free fns are `pub(crate)` so the host_methods.rs / macro_member_walk.rs
//! siblings and `meta_resolve_tests.rs` reach them via the shell's
...
/// [`TypeExpr::Function`]. Mirrors the wrap decision the node-domain registry
/// structural materialiser applies to function-like members.
