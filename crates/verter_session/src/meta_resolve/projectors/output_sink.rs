...
use crate::project_semantic_dispatch::output_materialization::{
    wrap_output_type_expr, MaterializedOutputTypeExpr, OutputProjector, OutputTypeExpr,
};
use crate::project_semantic_dispatch::ProjectSemanticDispatch;
use crate::resolver_core::ResolverContext;
use crate::semantic_query::{
    DeclIdentity, ProjectionMode, QueryResult, SemanticNodeData, SemanticNodeId, SemanticQueryKey,
    SurfaceMember,
};
...
/// Plain SHELL raise of a graph `node` to a bare [`TypeExpr`] through the
/// projectors output capability. Mints the cap over the supplied dispatch,
/// materializes the node into a sealed [`OutputTypeExpr`], and unwraps it.
/// `None` is the miss signal (the node — or a node required while raising it —
/// is unavailable from the live graph store).
///
/// MODULE-PRIVATE: the bare [`TypeExpr`] this hands back never leaves the
/// `output_sink` sink — only the sink's own boundary-consuming functions call
/// it, and they wrap the result back into a sealed carrier / published DTO
/// before returning.
fn shell_raise_to_type_expr(
...
/// Seal a freshly-computed raw [`TypeExpr`] into a sealed [`OutputTypeExpr`]
/// carrier through the projectors output capability. Mints the cap over the
/// supplied dispatch and seals via the capability-gated
/// [`wrap_output_type_expr`] minting helper. Used by the sink sites that
/// assemble a [`MaterializedOutputTypeExpr`] from a value they computed
/// directly (rather than from a boundary-method carrier).
///
/// MODULE-PRIVATE to the sink.
fn seal_type_expr(dispatch: &ProjectSemanticDispatch<'_>, type_expr: TypeExpr) -> OutputTypeExpr {
    let cap = MetaResolveProjectorsOutputCap::new(dispatch);
...
/// the sink's high-level APIs fold into an [`ExpandedField`] / published
/// surface.
///
/// MODULE-PRIVATE to the sink.
...
fn member_shape_peek_or_compute(
    ...
    let member_reduction_context =
        crate::meta_resolve::materialize::type_expr_materializer_context(mode);
    ...
    let reduction_context = crate::meta_resolve::materialize::type_expr_materializer_context(mode);
    let materialized =
...
fn admit_type_expr_shape_if_possible(
    ...
    use crate::project_semantic_dispatch::raise::MaterializedOutputTypeExpr;
    // Publication-admit: seal the freshly-computed `TypeExpr` into the
    // carrier via the module-private seal primitive (mint stays in this sink).
    let dispatch = ProjectSemanticDispatch::new(ctx);
    let value = MaterializedOutputTypeExpr::from_parts(
        None,
        seal_type_expr(&dispatch, materialized_type_expr),
        dep_signature,
        false,
    );
    ...
    if let Some(key) = crate::component_meta_caches::ShapeCacheKey::type_expr_whole_with_context(
        Arc::<str>::from(scope_canonical_id),
        Arc::new(expr.clone()),
        crate::meta_resolve::materialize::type_expr_materialize_reduction_context(expr, mode),
    ) {
...
/// `MaterializedOutputTypeExpr` it computed — admission is best-effort.
...
pub(crate) fn surface_member_to_expanded_field(
    ...
    // boundary primitive to obtain the published member `TypeExpr`; the bare
    // value is folded into the `ExpandedField` DTO below and never escapes.
    let r#type = {
        let dispatch = ProjectSemanticDispatch::new(ctx);
        unwrap_materialized(materialized, &dispatch)
    };
    debug_assert_eq!(
...
fn reduce_field_type_expr_with_mode(
    query_engine: &mut crate::resolver_core::ComponentMetaQueryEngine<'_>,
    scope_canonical_id: &str,
    expr: TypeExpr,
...
pub(crate) fn project_model(
    ...
    let (raised, exactness, raise_failed, contains_reducible) = {
        ...
        // the module-private `shell_raise_to_type_expr` primitive. The
        // capability mint + the carrier unwrap stay confined to this terminal
        // `output_sink` sink module — `project_model` lives HERE precisely so
        // the bare `TypeExpr` it raises never crosses out to a non-sink
        // projector child.
        let (raised, raise_failed) = match shell_raise_to_type_expr(&dispatch, payload_node) {
            Some(raised) => (raised, false),
