    ...
    pub fn from_object(view: &SurfaceView) -> Self {
        Self {
            members: view
                .positive_members()
                .iter()
                .map(shallow_member_from_surface)
                .collect(),
...
pub(crate) fn presence_intersection_members(
    ...
    let arm_surfaces: Vec<Option<ShallowSurface>> = arm_members
        ...
        .map(|members| {
            Some(ShallowSurface {
                members: members.iter().map(shallow_member_from_surface).collect(),
...
fn merge_declaration_surfaces(
    ...
    let members: Vec<ShallowSurfaceMember> = core
        ...
        .into_iter()
        .map(|merged| {
            if merged.member.method_kind == Some(verter_type_expr::ObjectMethodKind::Method)
                && merged.values.len() > 1
            {
...
fn merge_intersection_surfaces_with_graph(
    ...
    let members: Vec<ShallowSurfaceMember> = by_key
        .into_iter()
        .map(|(_, accum)| accum.finish(graph))
        .collect();
...
fn shallow_surface_from_projection_alternative(
    ...
    let index_signatures = alternative
        .indices()
        .iter()
        .filter_map(|index| {
