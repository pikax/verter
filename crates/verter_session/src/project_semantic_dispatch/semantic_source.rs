...
pub(crate) enum StrictSourceRaiseFailure {
    ...
    /// provenance yet (`docs/arch/stage10-b6-p4b-debt-rows.md` DEBT ROW
    /// #2), so a deref'd body's interior failure fails the raise. Proven
    /// schema absence never trips this: an ABSENT schema slot of a
