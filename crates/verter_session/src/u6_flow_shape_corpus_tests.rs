...
const CONFORMANCE: &[(Owner, usize, usize, usize)] = &[
    ...
    // class) and X10/X27 (union normal-form cross members). D13 moved X95
    // — the evolving-`let` both-branch join — to MatchesChecker with the
    // branch join over the shared product lattice.
    (Owner::U6ValueInference, 93, 69, 21),
