    ...
    fn solve_or_project_leaf_expr_until_stable(
        ...
        for _ in 0..3 {
            ...
            .or_else(|| {
                ...
                // the empty-path terminal (see the inline contract on
                // `lower_and_project_to_expanded_via_host_threaded`
                // for the documented constraint). Per the "Keep
                // Expanded" callsite contract, this callsite preserves
                // the legacy behaviour while the helper signature
                // itself becomes mode-explicit.
                crate::meta_resolve::project_expr_surface_expr_via_host_threaded(
                    self,
                    scope_canonical_id,
                    &current,
