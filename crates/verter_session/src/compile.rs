...
fn filter_setup_return(script: &mut String, template_code: Option<&str>) {
    ...
    //   "\nreturn { name1, name2, ... };\n"
    // followed by the wrapper end marker.

    // Find the closing marker for the setup wrapper. The runtime compiler
    // emits `\n}});\n` for the `_defineComponent` (TS) and `Object.assign`
    // (JS + options) shapes, and `\n}};\n` for the plain-object shape
    // (JS without options, official non-inline parity). Accept both.
    let wrapper_pos = ["\n}});\n", "\n}};\n"]
        .iter()
        .filter_map(|marker| script.find(marker))
        .min();
    let Some(wrapper_pos) = wrapper_pos else {
        return;
    };
...
mod tests {
  ...
  return (_openBlock(), _createElementVNode("div", { ref: "container" }, [_createElementVNode("span", null, $setup.msg)]))
    ...
    /// @ai-generated - filter_setup_return filters unused bindings on the
    /// plain-object wrapper shape (JS script setup ends with `\n}};\n`).
    #[test]
    fn filter_setup_return_plain_object_wrapper() {
        let mut script = concat!(
            "const __sfc__ = {\n",
            "  setup(__props) {\n",
            "    const a = 1;\n",
        ...
        .to_string();
        filter_setup_return(&mut script, Some("$setup.a"));
        assert!(
            script.contains("return { a };"),
            "unused binding must be filtered on the plain-object shape, got: {}",
            script
        );
    }
    ...
    fn filter_setup_return_markers_present_in_real_compile_output() {
        ...
        // JS script setup emits the plain-object wrapper (official parity),
        // which closes with `\n}};\n`.
        assert!(
            script.code.contains("\n}};\n"),
            "JS script output must contain plain wrapper end marker '\\n}}}};\\n', got:\n{}",
            script.code
        );
        assert!(
            script.code.contains("\nreturn "),
    ...
    /// @ai-generated - Canary: TS script setup keeps the `_defineComponent`
    /// wrapper, whose `\n}});\n` end marker filter_setup_return also accepts.
    #[test]
    fn filter_setup_return_define_component_marker_for_ts() {
        use oxc_allocator::Allocator;
        use verter_compiler::compile::CodegenOptions;
        use verter_compiler::compile::{compile as compile_sfc, VerterCompileOptions};
        ...
        assert!(
            script.code.contains("\n}});\n"),
            "TS script output must contain _defineComponent wrapper end marker '\\n}}}});\\n', got:\n{}",
            script.code
        );
        assert!(
            script.code.contains("\nreturn "),
