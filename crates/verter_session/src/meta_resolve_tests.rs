...
<template><div /></template>"#,
    ...
    fn type_node_needs_member_route_materialization_matches_type_expr_predicate() {
        ...
        use crate::semantic_query::{DepVersion, SemanticNodeData, ValueRootKey};
        ...
        // Bare DeclRef — local: `Ref<Local, []>` → no args, not pkg → true
        let local_ref = graph.intern_node(SemanticNodeData::DeclRef {
            identity: local.clone(),
        });
        let mut fence: Vec<(StdArc<str>, DepVersion)> = Vec::new();
        assert!(
            type_node_needs_member_route_materialization(host, local_ref, &mut fence, 0),
            "DeclRef Local (no args, not pkg) must need materialization"
        );

        // Bare DeclRef — pkg: `Ref<FromPkg, []>` → no args, IS pkg → false
        let pkg_ref = graph.intern_node(SemanticNodeData::DeclRef {
            identity: pkg.clone(),
        });
        fence.clear();
        assert!(
            !type_node_needs_member_route_materialization(host, pkg_ref, &mut fence, 0),
            "DeclRef pkg-backed must NOT need materialization"
        );

        // InstantiationRef (any base): args non-empty → false
        let inst_local = graph.intern_node(SemanticNodeData::InstantiationRef {
            base: local.clone(),
            args: StdArc::from(vec![local_ref].into_boxed_slice()),
        });
        fence.clear();
        assert!(
            !type_node_needs_member_route_materialization(host, inst_local, &mut fence, 0),
            "InstantiationRef (any base) must NOT need materialization \
             — mirrors TypeExpr 'type_arguments.is_empty() == false'"
        );

        // IndexedAccess { object: DeclRef(local), index: 'a' }
        // cycle on local-only IndexedAccess: false; package: walk down -> local -> false.
        // Net: !cycle (true) && !package (true) = true.
        let local_indexed = graph.intern_node(SemanticNodeData::IndexedAccess {
            object: local_ref,
            index: IndexKey::String(StdArc::from("a")),
        });
        fence.clear();
        assert!(
            type_node_needs_member_route_materialization(host, local_indexed, &mut fence, 0),
            "IndexedAccess on local DeclRef root must need materialization"
        );

        // IndexedAccess { object: DeclRef(pkg), index: 'a' }
        // package check follows object -> DeclRef pkg -> true. Net: false.
        let pkg_indexed = graph.intern_node(SemanticNodeData::IndexedAccess {
            object: pkg_ref,
            index: IndexKey::String(StdArc::from("a")),
        });
        fence.clear();
        assert!(
            !type_node_needs_member_route_materialization(host, pkg_indexed, &mut fence, 0),
            "IndexedAccess on package root must NOT need materialization"
        );

        // TypeOf — no DeclRef root extractable. cycle=false. package=false.
        // Net: true.
        ...
        fence.clear();
        assert!(
            type_node_needs_member_route_materialization(host, typeof_node, &mut fence, 0),
            "TypeOf at top must need materialization (no package root, no cycle root)"
        );

        // TypeParam — same as TypeOf.
        let type_param_node = graph.intern_node(SemanticNodeData::TypeParam {
            decl: local.clone(),
            param_index: 0,
            constraint: None,
            default: None,
        ...
        fence.clear();
        assert!(
            type_node_needs_member_route_materialization(host, type_param_node, &mut fence, 0),
            "TypeParam at top must need materialization"
        );

        // Array { element: DeclRef(local), .. } — recurse into element
        // matches TypeExpr::Array { element: TypeExpr::Ref(local), .. }
        // Net: same as the bare local DeclRef → true.
        let array_local = graph.intern_node(SemanticNodeData::Array {
            element: local_ref,
            readonly: false,
        });
        fence.clear();
        assert!(
            type_node_needs_member_route_materialization(host, array_local, &mut fence, 0),
            "Array<Local> must need materialization (recurse to element)"
        );
        let array_pkg = graph.intern_node(SemanticNodeData::Array {
            element: pkg_ref,
            readonly: false,
        });
        fence.clear();
        assert!(
            !type_node_needs_member_route_materialization(host, array_pkg, &mut fence, 0),
            "Array<Pkg> must NOT need materialization"
        );

        // KeyOf { base: DeclRef(local) } — recurse to base
        let keyof_local = graph.intern_node(SemanticNodeData::KeyOf { base: local_ref });
        fence.clear();
        assert!(
            type_node_needs_member_route_materialization(host, keyof_local, &mut fence, 0),
            "keyof Local must need materialization"
        );

        // Tuple — any element triggers
        let tuple_with_local = graph.intern_node(SemanticNodeData::Tuple {
            elements: StdArc::from(
                vec![crate::semantic_query::TupleElement {
                    label: None,
                    value: local_ref,
        ...
        fence.clear();
        assert!(
            type_node_needs_member_route_materialization(host, tuple_with_local, &mut fence, 0),
            "[Local] tuple must need materialization (element triggers)"
        );
        ...
        fence.clear();
        assert!(
            !type_node_needs_member_route_materialization(host, obj, &mut fence, 0),
            "Object body itself does NOT need member route materialization \
             at this predicate (falls through TypeExpr's `_ => false`)"
        );

        // Primitive — falls through.
        let prim = graph.intern_node(SemanticNodeData::Primitive(
            crate::semantic_query::PrimitiveKind::String,
        ));
        fence.clear();
        assert!(
            !type_node_needs_member_route_materialization(host, prim, &mut fence, 0),
            "Primitive falls through TypeExpr's `_ => false`"
        );
    }
    ...
    /// and respect its result; for now (matching the legacy adapter
    /// quirk) the cycle invocation does not need to apply at non-Ref
    /// tops, but the predicate must short-circuit to `false` when
    /// `ref_root_reaches_transitive_cycle_node` returns true.
    #[test]
    fn type_node_needs_member_route_materialization_short_circuits_on_pkg() {
        use crate::meta_resolve::type_node_needs_member_route_materialization;
        use crate::semantic_query::{DepVersion, SemanticNodeData};
        ...
        // KeyOf{ base: pkg_ref } — recurses into base, which is package-backed.
        let keyof_pkg = graph.intern_node(SemanticNodeData::KeyOf { base: pkg_ref });
        let mut fence: Vec<(StdArc<str>, DepVersion)> = Vec::new();
        assert!(
            !type_node_needs_member_route_materialization(host, keyof_pkg, &mut fence, 0),
            "keyof Pkg must NOT need materialization (package short-circuits)"
        );
    }
