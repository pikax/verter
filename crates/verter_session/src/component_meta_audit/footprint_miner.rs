        ...
        match edge.kind {
            ...
            | OriginEdgeKind::ProjectPath => {
                ...
                let path = match &edge.meta {
                    OriginEdgeMetaDto::ProjectMember { member_key, .. } => {
...
fn display_label_for(data: &SemanticNodeData) -> Arc<str> {
    match data {
        ...
        SemanticNodeData::ObjectSpreadProgram(program) => {
            Arc::from(format!("ObjectSpreadProgram[{}]", program.effects.len()))
        }
...
fn index_key_label(idx: &IndexKey) -> String {
    match idx {
        ...
        IndexKey::UniqueSymbol(identity) => {
            serde_json::to_string(identity).expect("unique-symbol identity is serializable")
        }
        IndexKey::Computed(_) => "<type>".to_string(),
    }
}
...
fn translate_meta(kind: CoreOriginEdgeKind, meta: &OriginMeta) -> OriginEdgeMetaDto {
    match kind {
        ...
        CoreOriginEdgeKind::ProjectMember => match meta {
            OriginMeta::ProjectedMember { key, provenance } => OriginEdgeMetaDto::ProjectMember {
                member_key: translate_property_key(key),
...
fn translate_path_segment(seg: &PathSegment) -> ProjectPathSegment {
    match seg {
        PathSegment::Member(key) => ProjectPathSegment::Member {
            key: translate_property_key(key),
        },
        PathSegment::Index(idx) => match idx {
            ...
            IndexKey::UniqueSymbol(identity) => ProjectPathSegment::Index {
                key: Arc::from(
                    serde_json::to_string(identity)
                        .expect("unique-symbol identity is serializable"),
                ),
            },
            IndexKey::Computed(_) => ProjectPathSegment::Index {
                key: Arc::from("<type>"),
            },
        },
    }
...
fn translate_property_key(key: &PropertyKey) -> verter_audit::AuditPropertyKey {
    match key {
        PropertyKey::String(value) => verter_audit::AuditPropertyKey::String {
            value: Arc::clone(value),
        },
        PropertyKey::Number(value) => verter_audit::AuditPropertyKey::Number { value: value.get() },
        PropertyKey::UniqueSymbol(identity) => {
            let owner_kind = match identity.owner.kind() {
                verter_type_expr::facts::TopLevelOwnerKind::Module => {
                    verter_audit::AuditTopLevelOwnerKind::Module
                }
                verter_type_expr::facts::TopLevelOwnerKind::Instance => {
                    verter_audit::AuditTopLevelOwnerKind::Instance
                }
                verter_type_expr::facts::TopLevelOwnerKind::Frontmatter => {
                    verter_audit::AuditTopLevelOwnerKind::Frontmatter
                }
            };
            verter_audit::AuditPropertyKey::UniqueSymbol {
                identity: verter_audit::AuditUniqueSymbolIdentity {
                    canonical_id: Arc::clone(&identity.canonical_id),
                    owner_kind,
                    owner_ordinal: identity.owner.ordinal(),
                    symbol: Arc::clone(&identity.symbol),
