...
/// Drive a cross-file Ref-carrier `defineProps<T>()` fixture: the
/// macro payload is an imported interface, so the lowered TypeArg is
/// a `TypeExpr::Ref { name, type_arguments }` carrier rather than an
/// inline `TypeExpr::Object`. On the resolved payload surface
/// `read_surface_members` hits the catch-all empty case (the surface
/// node's data is NOT `SemanticNodeData::Object` for the carrier
/// path), so the projector pipeline's per-member emit fires zero
/// times. The macro-shape orchestrator in
/// `meta_resolve::materialize::macro_shapes` is the sole producer of
/// the published surface on this code path, and it MUST emit
/// `PublishedField` origin edges for every admitted property name.
fn run_cross_file_ref_macro_probe_request() -> RequestAuditRecord {
    let host = crate::VerterHost::new_standalone(crate::types::HostConfig {
        analysis_level: crate::types::AnalysisLevel::Full,
        audit_enabled: true,
        footprint_capture: true,
    ...
    // Generic interface with an `extends Pick<...>` clause — the
    // same shape pattern nuxt-ui uses for its components
    // (`AccordionProps<T extends AccordionItem> extends Pick<...>`).
    // The macro payload lowers to a Ref carrier with non-empty
    // type_arguments, so `read_surface_members(surface_node)` returns
    // empty members and the projector pipeline's per-member emit is
    ...
    project
        .upsert_base(
            ...
            export interface AccordionProps<T extends string = string> extends Pick<BaseProps, 'collapsible' | 'disabled'> {\n    \
                title: T;\n    \
                expanded: boolean;\n    \
    ...
    project
        .upsert_base(
            "/Accordion.vue",
            "<script setup lang=\"ts\" generic=\"TItem extends string\">\n\
            import type { AccordionProps } from './types';\n\
            defineProps<AccordionProps<TItem>>();\n\
            </script>\n\
            <template><div /></template>\n",
        )
        .unwrap();
    ...
    let host = project.host();
    let (_analysis, resolution) = host
        .get_component_meta_with_resolution("/Accordion.vue")
        .expect("resolver must produce metadata for the cross-file Ref-carrier fixture");
    host.take_audit_record(resolution.request_id)
        .expect("audit record must publish for the cross-file Ref-carrier fixture")
}

/// PublishedField corpus-divergence gate (DISCRIMINATING).
///
/// The hermetic `audit_publishes_member_edge_with_published_field_provenance_at_macro_boundaries`
/// fixture uses inline `TypeExpr::Object` macro payloads; on that
/// path `resolve_payload_surface` returns a `SemanticNodeData::Object`
/// shell, so `read_surface_members` enumerates the inline members
/// and the projector pipeline's per-member emit fires. Real corpus
/// components (e.g. nuxt-ui Accordion / Table / ChatMessages) use
/// cross-file generic `defineProps<T>()` where the lowered TypeArg
/// is a `TypeExpr::Ref` carrier. The carrier path's surface node has
/// non-Object data, `read_surface_members` returns empty, and the
/// projector emit goes vacuous — leaving the Rule-5 validator's
/// `PublishedField` branch dead on real corpus traces.
///
/// This test pins the macro-shape orchestrator's PublishedField
/// wiring (the corpus-divergence fix landed in Round 19a Commit 1).
/// Pre-fix the test FAILS with `published_field_names` empty (the
/// projector pipeline's emit is dead on the Ref carrier path).
/// Post-fix it PASSES because
/// `produce_macro_object_shapes_for_purpose` emits one
/// `PublishedField` edge per property in every shape it admits to
/// `evaluated_types.define_props` / `define_emits` / `define_slots`.
#[test]
fn audit_publishes_member_edge_with_published_field_provenance_on_cross_file_ref_carrier_macro() {
    let record = run_cross_file_ref_macro_probe_request();
    let footprint = record
        .footprint
        .as_ref()
        .expect("footprint must publish when HostConfig::footprint_capture is enabled");
    ...
    let expected: &[&str] = &["title", "expanded", "onToggle", "collapsible", "disabled"];
    let missing: Vec<&&str> = expected
        .iter()
        .filter(|name| !published_field_names.iter().any(|seen| seen == **name))
        .collect();
    ...
    assert!(
        missing.is_empty(),
        "PublishedField corpus-divergence gate (DISCRIMINATING): a \
         cross-file Ref-carrier macro payload \
         (`defineProps<AccordionProps>()` where the type is imported \
         from a sibling module) MUST emit `PublishedField` origin \
         edges for every property admitted onto the published surface. \
         Pre-Round-19a the projector-side emit was vacuous because \
         `read_surface_members` on the carrier surface returns no \
         Object members; the macro-shape orchestrator in \
         `meta_resolve::materialize::macro_shapes` is the sole \
         producer on this path and MUST emit PublishedField edges \
         alongside every `evaluated_types.define_*.push(...)`. \
         Missing names: {missing:?}. Observed PublishedField edge \
         names: {published_field_names:?}.",
    );

    assert!(
        !published_field_names.is_empty(),
        "Discriminator: at least one PublishedField edge must be \
         observed on the cross-file Ref-carrier fixture. Zero \
         observed — the macro-shape orchestrator's PublishedField \
         wiring is dead code.",
    );
}
