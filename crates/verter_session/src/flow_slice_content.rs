...
pub struct SliceLoopElement {
    ...
    /// target, whose names the evaluator does not model.
...
impl<'a> Lowerer<'a> {
    ...
    fn lower_discarded_effects(
        ...
        match unwrap_parenthesized(expression) {
            ...
            {
                let modeled = self
                    .evolving_reset_statement(assignment)
            ...
            compound @ (Expression::AssignmentExpression(_) | Expression::UpdateExpression(_)) => {
                self.lower_discarded_compound_write(compound, context, out);
            }
            ...
            // `a && b` runs `b` where `a` is truthy, `a || b` where it is
            // falsy, and `a ?? b` where it is nullish — a test this lowering
            // expresses no guard for, so `a` runs first as its own
            // discarded value and `b` on an unguarded arm.
            Expression::LogicalExpression(logical) => {
                let branch = match logical.operator {
                    LogicalOperator::And => self.lower_discarded_branch(
                        Some(&logical.left),
                        Some(&logical.right),
                        None,
                        context,
                    ...
                    LogicalOperator::Or => self.lower_discarded_branch(
                        Some(&logical.left),
                        None,
                        Some(&logical.right),
                        context,
                    ...
                    LogicalOperator::Coalesce => {
                        ...
                        self.lower_discarded_branch(None, Some(&logical.right), None, context, out)
                    }
                };
                out.push(branch);
            }
