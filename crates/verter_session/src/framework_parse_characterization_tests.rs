...
fn route_owned_eval_state_carries_parse_payload_for_vue() {
    ...
    let (raw, parse_payload, whole_hash) = host
        .cached_route_owned_eval_state(vue_id)
        .expect("route-owned eval state for the imported Vue file");
    ...
    let (_, parse_payload, _) = host
        .cached_route_owned_eval_state(dts_id)
        .expect("route-owned eval state for the imported declaration file");
    assert!(
