...
const ENGINE_PROMOTION_LIB_TS: &str = r#"export interface ChildProps {
  alpha: string;
  beta: number;
}
"#;

const ENGINE_PROMOTION_UNRELATED_TS: &str = r#"export interface UnrelatedZ {
  qux: boolean;
}
"#;
...
#[test]
fn read_once_shallow_first_lazy_for_engine_state_promotion() {
    let host = build_hermetic_host(&[
        ("/A.vue", ENGINE_PROMOTION_OWNER_VUE),
        ("/lib.ts", ENGINE_PROMOTION_LIB_TS),
        ("/unrelated.ts", ENGINE_PROMOTION_UNRELATED_TS),
    ]);

    // First query — cold path: /A.vue + /lib.ts loaded; /unrelated.ts NOT loaded.
    let q1 = host.get_component_meta("/A.vue");
    assert!(
        q1.is_some(),
        "first get_component_meta on /A.vue (5m bridge migration window) must produce a result"
    );
    let after_first = host.audit().loaded_files();
    let after_first_set: std::collections::HashSet<&str> =
        after_first.iter().map(|s| s.as_ref()).collect();
    assert!(
        after_first_set.contains("/A.vue"),
        "owner /A.vue must be loaded after first 5m-bridge query (got {after_first:?})"
    );
    assert!(
        after_first_set.contains("/lib.ts"),
        "transitively-needed /lib.ts must be loaded after first 5m-bridge query (got {after_first:?})"
    );
    assert!(
        !after_first_set.contains("/unrelated.ts"),
        "unrelated /unrelated.ts MUST NOT be loaded after first 5m-bridge query \
         (lazy expansion contract — the bridge migration must not trigger spurious \
         cross-file walks via the engine fast-path; got {after_first:?})"
    );

    // Second query — warm path: NO additional reads / shallows / lowerings.
    let baseline_reads = host.audit().total_reads();
    let baseline_shallow = host.audit().total_shallow_processes();
    let baseline_lowerings = host.audit().total_lowerings();
    let q2 = host.get_component_meta("/A.vue");
    ...
    assert_eq!(
        read_delta, 0,
        "second 5m-bridge query must NOT trigger additional read \
         (the bridge helpers must not bypass the host-owned warm cache; got delta={read_delta})"
    );
    assert_eq!(
        shallow_delta, 0,
        "second 5m-bridge query must NOT trigger additional shallow process (got delta={shallow_delta})"
    );
    assert_eq!(
        lowering_delta, 0,
        "second 5m-bridge query must NOT trigger additional lowering (got delta={lowering_delta})"
    );
    assert_eq!(
        format!("{:?}", q1),
        format!("{:?}", q2),
        "first and second 5m-bridge component-meta results must be debug-equal — \
         the bridge migration must not cause result drift on warm-hit"
    );
}
