...
// them here would leave every item and test module in `types.rs` inside the
// wall. Only the re-exported type crosses the boundary; the fields, `new`,
// and the selectors stay in `types/tsc_response.rs`.
mod tsc_response;
...
mod tests {
    ...
    /// The destination-keyed channel selector: a JavaScript-labeled companion
    /// path takes the dialect rendering, everything else the TS-labeled one,
    /// and an unsplit response serves the same bytes from every selector.
    ///
    /// DISCRIMINATING: swapping the two arms of `code_for_companion_path`
    /// flips the first two assertions; dropping the `None` fallback of
    /// `ts_labeled_code` flips the unsplit half.
    #[test]
    fn tsc_response_selects_the_rendering_by_companion_path_extension() {
        let split = TscResponse::new(
            Arc::from("/** jsdoc rendering */\n"),
            None,
            verter_compiler::tsc::SfcScriptDialect::JavaScript,
            Some(Arc::from("declare const TS: 1\n")),
        );
        for js_path in [
            "/p/Comp_0.vue.js",
            "/p/Comp_0.vue.jsx",
            ...
            assert_eq!(
                &**split.code_for_companion_path(js_path),
                "/** jsdoc rendering */\n",
                "{js_path} is JavaScript-labeled and takes the dialect rendering"
            );
        }
        for ts_path in [
            "/p/Comp.vue.verter.ts",
            "/p/Comp.vue.ts",
            "/p/Comp_0.tsc.tsx",
            "/p/Comp.d.vue.ts",
            "/p/no-extension",
            ...
            assert_eq!(
                &**split.code_for_companion_path(ts_path),
                "declare const TS: 1\n",
                "{ts_path} is TypeScript-labeled (or unknown ⇒ conservative TS) \
                 and takes the TS-labeled rendering"
            );
        }
        assert_eq!(&**split.dialect_labeled_code(), "/** jsdoc rendering */\n");
        assert_eq!(&**split.ts_labeled_code(), "declare const TS: 1\n");

        let unsplit = TscResponse::new(
            Arc::from("declare const Only: 1\n"),
            None,
            verter_compiler::tsc::SfcScriptDialect::TypeScript,
            None,
        );
        for path in ["/p/Comp.vue.verter.ts", "/p/Comp_0.vue.js"] {
            assert_eq!(
                &**unsplit.code_for_companion_path(path),
                "declare const Only: 1\n",
                "an unsplit response serves the same bytes to every destination"
            );
        }
    }
