//! Lazy-invocation gating tests for the `expr_needs_projection_rescue`
//! probe.
//!
//! These tests assert that the projection-needs probe
...
//! and field-level probe calls are NOT wired into the same
...
/// times the projection-needs probe fired during this resolution.
///
...
/// gating contract.
fn projection_probe_count_for(host: &Arc<VerterHost>, canonical: &str) -> u64 {
...
/// computes the projection-needs var (the lazy-compute arm is
/// unreached). The walker stage observes a non-empty exact shape
/// with primitive members only (`shape_needs_member_rescue`
/// returns false), so the projection-needs probe at the walker
/// seam is gated off. Both seams skip the probe — counter must be 0.
#[test]
fn produce_macro_shapes_skips_projection_probe_when_known_define_props_surface_wins() {
    let host = build_hermetic_host(&[("/A.vue", KNOWN_DEFINE_PROPS_SURFACE_VUE)]);

    let calls = projection_probe_count_for(&host, "/A.vue");
    ...
    assert_eq!(
        ...
        "a known explicit-object defineProps surface (inline object literal with \
         ...
         fields fast path; the eager projection-needs probe and the walker's \
...
/// — the projection-needs var is never computed there. After
/// production the walker observes an exact-nonempty shape (two
/// properties); its gate `properties.is_empty() || (...)` evaluates
/// to FALSE, so the projection-needs probe at the walker's seam
/// stays gated off. Inner per-property probe calls (which iterate
/// `define_props.result.value.properties` looking for member-route
/// work) are NOT wired into this counter.
#[test]
fn macro_member_walk_skips_projection_probe_when_define_props_shape_already_needs_member_work() {
    ...
    assert_eq!(
        ...
        "member-rescue shapes (non-empty properties + at least one non-object \
         member like `keyof Inner`) must skip the projection-needs probe at both \
         the produce stage's lazy fallback (branch 4 wins, no probe computed) \
         and the walker seam (per-property loop covers member-route work); \
...
/// path (no projection-needs probe consulted), and the walker
/// observes a non-empty shape with no member-route needs
/// (`shape_needs_member_rescue` returns false because all members
/// are primitives). The walker gate skips the lowered probe.
#[test]
fn macro_member_walk_skips_projection_probe_for_exact_define_props_shape() {
    ...
    assert_eq!(
        ...
        "exact non-empty defineProps shapes (literal Object with primitive \
         members) must skip the projection-needs probe at both the produce and \
...
///
/// The macro-shape produce stage falls through to the lazy-probe
/// arm — the projection-needs probe MUST fire there. This
...
#[test]
fn macro_member_walk_fires_projection_probe_for_empty_define_props_shape() {
    ...
    assert!(
        ...
        "non-Object, non-Ref defineProps lowered (e.g. `keyof Inner`) MUST \
         fire the projection-needs probe at the macro-shape produce stage's \
         ...
         for this shape; over-skipping here would lose the projection / \
         fallback path; counter expected >= 1, got {calls}",
    );
}
