...
pub(crate) fn compute_changed_removed_nodes(
        ...
        || slice_changes.script_changed
    {
        ...
        if slice_changes.template_changed {
        ...
        if main_depends_on_styles && !slice_changes.style_indices_changed.is_empty() {
            changed_nodes.push(VirtualNodeKind::Main);
        }
    ...
    (sorted_nodes(changed_nodes), removed)
}
...
mod tests {
    ...
    fn compute_not_changed_returns_empty() {
        ...
        let (changed, removed) = compute_changed_removed_nodes(&sc, false, &prev, &new, false);
    ...
    fn compute_first_insert_all_new() {
        let sc = SliceChanges::default();
        let new = make_nodes(true, true, 1, 0);
        let (changed, removed) = compute_changed_removed_nodes(&sc, true, &[], &new, false);
    ...
    fn compute_structure_change_returns_all_new() {
        ...
        let (changed, _removed) = compute_changed_removed_nodes(&sc, true, &prev, &new, false);
        assert_eq!(changed.len(), new.len());
    }
    ...
    fn compute_template_only_change() {
        ...
        let prev = make_nodes(true, true, 1, 0);
        let new = make_nodes(true, true, 1, 0);
        let (changed, removed) = compute_changed_removed_nodes(&sc, true, &prev, &new, false);
    ...
    fn compute_node_removed() {
        ...
        let prev = make_nodes(true, true, 2, 0);
        let new = make_nodes(true, true, 1, 0);
        let (_changed, removed) = compute_changed_removed_nodes(&sc, true, &prev, &new, false);
