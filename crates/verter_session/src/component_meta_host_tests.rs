...
label: ''
    ...
    // The helpers are overlay-only, so the published source demands
    // through the SESSION view (a base-view probe cannot see them).
    let button_ty = session
        .inner
        .demand_semantic_source_type_expr(
            "/src/Button.vue",
            button_entry.type_source.present().expect("present source"),
        )
        .unwrap_or_else(|| panic!("Button's published registry source must demand-materialize"));
    let TypeExpr::Object(button_shape) = &button_ty else {
        panic!("expected Button helper to materialize as an object, got {button_ty:?}");
    };
    let variants_member = button_shape
        .properties
        .iter()
