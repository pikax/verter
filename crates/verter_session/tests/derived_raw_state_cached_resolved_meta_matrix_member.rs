...
//! (`ResolvedComponentMetaCacheEntry`) must carry `FactKey::Member`
//! facts in its `fact_versions: Arc<[FactVersionRef]>` signature.
//!
//! Pre-1A: wrapper holds `Vec<FactVersionRef>` and source-grep
//! FAILS. Post-1A: substrate is `Arc<[FactVersionRef]>` and a
//! `Member` fact validates through the per-domain fast-path.
...
fn cached_resolved_meta_signature_carries_member() {
    ...
    assert!(
        ...
        "permissive view must accept a Member fact in the Arc-stored signature"
    );
}
