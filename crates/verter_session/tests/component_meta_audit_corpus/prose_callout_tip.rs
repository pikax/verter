...
//! runtime component at `/prose/callout/Tip.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_callout_tip_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/callout/Tip.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/callout/Tip.vue", src)])
        .resolve("/prose/callout/Tip.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/callout/Tip.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_callout_tip: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_callout_tip: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
