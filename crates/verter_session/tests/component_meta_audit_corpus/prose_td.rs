...
//! runtime component at `/prose/Td.vue`. The assertion posture is
...
fn corpus_audit_prose_td_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Td.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Td.vue", src)])
        .resolve("/prose/Td.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_prose_td: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
