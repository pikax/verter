...
//! runtime component at `/BlogPosts.vue`. The assertion posture is
...
#[test]
fn corpus_audit_blog_posts_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/BlogPosts.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/BlogPosts.vue", src)])
        .resolve("/BlogPosts.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/BlogPosts.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_blog_posts: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_blog_posts: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
