...
//! runtime component at `/ChatReasoning.vue`. The assertion posture is
...
#[test]
fn corpus_audit_chat_reasoning_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ChatReasoning.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ChatReasoning.vue", src)])
        .resolve("/ChatReasoning.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ChatReasoning.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_chat_reasoning: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_chat_reasoning: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
