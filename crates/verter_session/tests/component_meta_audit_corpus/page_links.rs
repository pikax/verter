...
//! runtime component at `/PageLinks.vue`. The assertion posture is
...
#[test]
fn corpus_audit_page_links_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/PageLinks.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/PageLinks.vue", src)])
        .resolve("/PageLinks.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/PageLinks.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_page_links: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_page_links: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
