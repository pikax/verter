...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/Prompt.vue`.
...
#[test]
fn corpus_audit_prose_prompt_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "fixtures/prose/Prompt.vue"
    );
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Prompt.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_prompt: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_prompt: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
