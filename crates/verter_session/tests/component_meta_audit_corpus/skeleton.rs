...
//! runtime component at `/Skeleton.vue`. The assertion posture is
...
#[test]
fn corpus_audit_skeleton_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Skeleton.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/Skeleton.vue", src)])
        .resolve("/Skeleton.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Skeleton.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_skeleton: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_skeleton: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
