...
//! runtime component at `/InputDate.vue`. The assertion posture is
...
#[test]
fn corpus_audit_input_date_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/InputDate.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/InputDate.vue", src)])
        .resolve("/InputDate.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/InputDate.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_input_date: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_input_date: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
