...
//!
//! Corpus coverage for the vendored Vue fixture at `/Tooltip.vue`.
...
#[test]
fn corpus_audit_tooltip_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Tooltip.vue");
    let result = AuditedRequest::builder()
        .files([("/Tooltip.vue", src)])
        .resolve_component_meta("/Tooltip.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Tooltip.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_tooltip: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_tooltip: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
