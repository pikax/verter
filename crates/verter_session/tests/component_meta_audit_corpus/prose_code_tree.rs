...
//! runtime component at `/prose/CodeTree.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_code_tree_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/CodeTree.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/CodeTree.vue", src)])
        .resolve("/prose/CodeTree.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/CodeTree.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_code_tree: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_code_tree: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
