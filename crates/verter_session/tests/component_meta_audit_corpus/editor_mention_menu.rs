...
//! runtime component at `/EditorMentionMenu.vue`. The assertion posture is
...
fn corpus_audit_editor_mention_menu_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/EditorMentionMenu.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_editor_mention_menu: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
