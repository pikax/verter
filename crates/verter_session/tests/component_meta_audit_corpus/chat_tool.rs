...
//! runtime component at `/ChatTool.vue`. The assertion posture is
...
fn corpus_audit_chat_tool_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ChatTool.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ChatTool.vue", src)])
        .resolve("/ChatTool.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_chat_tool: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
