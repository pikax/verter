...
//! runtime component at `/Chip.vue`. The assertion posture is
...
fn corpus_audit_chip_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Chip.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_chip: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
