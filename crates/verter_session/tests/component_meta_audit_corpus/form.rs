...
//!
//! Corpus coverage for the vendored Vue fixture at `/Form.vue`.
...
#[test]
fn corpus_audit_form_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Form.vue");
    let result = AuditedRequest::builder()
        .files([("/Form.vue", src)])
        .resolve_component_meta("/Form.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Form.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_form: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_form: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
