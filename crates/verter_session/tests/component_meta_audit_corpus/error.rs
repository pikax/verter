...
//! runtime component at `/Error.vue`. The assertion posture is
...
fn corpus_audit_error_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Error.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_error: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
