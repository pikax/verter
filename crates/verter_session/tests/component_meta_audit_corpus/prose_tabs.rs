...
//! runtime component at `/prose/Tabs.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_tabs_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Tabs.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Tabs.vue", src)])
        .resolve("/prose/Tabs.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Tabs.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_tabs: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_tabs: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
