...
//! runtime component at `/PricingTable.vue`. The assertion posture is
...
fn corpus_audit_pricing_table_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/PricingTable.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/PricingTable.vue", src)])
        .resolve("/PricingTable.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_pricing_table: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
