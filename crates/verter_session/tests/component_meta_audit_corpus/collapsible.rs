...
//! runtime component at `/Collapsible.vue`. The assertion posture is
...
fn corpus_audit_collapsible_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Collapsible.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/Collapsible.vue", src)])
        .resolve("/Collapsible.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_collapsible: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
