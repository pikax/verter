...
//! runtime component at `/DashboardPanel.vue`. The assertion posture is
...
#[test]
fn corpus_audit_dashboard_panel_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/DashboardPanel.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/DashboardPanel.vue", src)])
        .resolve("/DashboardPanel.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/DashboardPanel.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_dashboard_panel: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_dashboard_panel: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
