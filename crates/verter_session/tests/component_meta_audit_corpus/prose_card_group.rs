...
//! runtime component at `/prose/CardGroup.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_card_group_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/CardGroup.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/CardGroup.vue", src)])
        .resolve("/prose/CardGroup.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/CardGroup.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_card_group: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_card_group: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
