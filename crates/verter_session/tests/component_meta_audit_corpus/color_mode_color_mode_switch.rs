...
//! runtime component at `/color-mode/ColorModeSwitch.vue`. The assertion posture is
...
#[test]
fn corpus_audit_color_mode_color_mode_switch_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/color-mode/ColorModeSwitch.vue"
    );
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/color-mode/ColorModeSwitch.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_color_mode_color_mode_switch: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_color_mode_color_mode_switch: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
