...
//! runtime component at `/prose/callout/Warning.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_callout_warning_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/callout/Warning.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/callout/Warning.vue", src)])
        .resolve("/prose/callout/Warning.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/callout/Warning.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_callout_warning: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_callout_warning: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
