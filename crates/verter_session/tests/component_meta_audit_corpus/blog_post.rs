...
//! runtime component at `/BlogPost.vue`. The assertion posture is
...
#[test]
fn corpus_audit_blog_post_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/BlogPost.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/BlogPost.vue", src)])
        .resolve("/BlogPost.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/BlogPost.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_blog_post: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_blog_post: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
