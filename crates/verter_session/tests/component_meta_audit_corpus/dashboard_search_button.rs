...
//! runtime component at `/DashboardSearchButton.vue`. The assertion posture is
...
#[test]
fn corpus_audit_dashboard_search_button_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/DashboardSearchButton.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/DashboardSearchButton.vue", src)])
        .resolve("/DashboardSearchButton.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/DashboardSearchButton.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_dashboard_search_button: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_dashboard_search_button: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
