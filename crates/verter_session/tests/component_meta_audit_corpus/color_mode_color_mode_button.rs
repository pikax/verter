...
//! runtime component at `/color-mode/ColorModeButton.vue`. The assertion posture is
...
#[test]
fn corpus_audit_color_mode_color_mode_button_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/color-mode/ColorModeButton.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/color-mode/ColorModeButton.vue", src)])
        .resolve("/color-mode/ColorModeButton.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/color-mode/ColorModeButton.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_color_mode_color_mode_button: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_color_mode_color_mode_button: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
