...
//! runtime component at `/prose/Kbd.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_kbd_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Kbd.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Kbd.vue", src)])
        .resolve("/prose/Kbd.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Kbd.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_kbd: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_kbd: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
