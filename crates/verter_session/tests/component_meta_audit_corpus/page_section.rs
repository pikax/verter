...
//! runtime component at `/PageSection.vue`. The assertion posture is
...
#[test]
fn corpus_audit_page_section_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/PageSection.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/PageSection.vue", src)])
        .resolve("/PageSection.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/PageSection.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_page_section: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_page_section: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
