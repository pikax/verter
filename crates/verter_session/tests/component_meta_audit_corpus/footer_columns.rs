...
//! runtime component at `/FooterColumns.vue`. The assertion posture is
...
#[test]
fn corpus_audit_footer_columns_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/FooterColumns.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/FooterColumns.vue", src)])
        .resolve("/FooterColumns.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/FooterColumns.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_footer_columns: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_footer_columns: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
