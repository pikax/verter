...
//!
//! Corpus coverage for the vendored Vue fixture at `/Icon.vue`.
...
#[test]
fn corpus_audit_icon_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Icon.vue");
    let result = AuditedRequest::builder()
        .files([("/Icon.vue", src)])
        .resolve_component_meta("/Icon.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Icon.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_icon: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_icon: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
