...
//! runtime component at `/content/ContentSearch.vue`. The assertion posture is
...
#[test]
fn corpus_audit_content_content_search_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/content/ContentSearch.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/content/ContentSearch.vue", src)])
        .resolve("/content/ContentSearch.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/content/ContentSearch.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_content_content_search: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_content_content_search: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
