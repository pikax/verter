...
//! runtime component at `/Toast.vue`. The assertion posture is
...
fn corpus_audit_toast_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Toast.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_toast: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
