...
//! runtime component at `/Container.vue`. The assertion posture is
...
#[test]
fn corpus_audit_container_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Container.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/Container.vue", src)])
        .resolve("/Container.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Container.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_container: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_container: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
