...
//! runtime component at `/Textarea.vue`. The assertion posture is
...
fn corpus_audit_textarea_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Textarea.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_textarea: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
