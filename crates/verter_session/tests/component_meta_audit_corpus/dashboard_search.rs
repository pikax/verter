...
//! runtime component at `/DashboardSearch.vue`. The assertion posture is
...
#[test]
fn corpus_audit_dashboard_search_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/DashboardSearch.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/DashboardSearch.vue", src)])
        .resolve("/DashboardSearch.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/DashboardSearch.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_dashboard_search: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_dashboard_search: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
