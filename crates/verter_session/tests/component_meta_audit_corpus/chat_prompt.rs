...
//! runtime component at `/ChatPrompt.vue`. The assertion posture is
...
#[test]
fn corpus_audit_chat_prompt_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ChatPrompt.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ChatPrompt.vue", src)])
        .resolve("/ChatPrompt.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ChatPrompt.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_chat_prompt: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_chat_prompt: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
