...
//! runtime component at `/PageCard.vue`. The assertion posture is
...
#[test]
fn corpus_audit_page_card_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/PageCard.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/PageCard.vue", src)])
        .resolve("/PageCard.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/PageCard.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_page_card: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_page_card: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
