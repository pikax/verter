...
//! runtime component at `/Main.vue`. The assertion posture is
...
fn corpus_audit_main_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Main.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_main: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
