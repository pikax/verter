...
//! runtime component at `/prose/Script.vue`. The assertion posture is
...
fn corpus_audit_prose_script_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Script.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Script.vue", src)])
        .resolve("/prose/Script.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_prose_script: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
