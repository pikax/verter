...
//! runtime component at `/ScrollArea.vue`. The assertion posture is
...
#[test]
fn corpus_audit_scroll_area_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ScrollArea.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ScrollArea.vue", src)])
        .resolve("/ScrollArea.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ScrollArea.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_scroll_area: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_scroll_area: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
