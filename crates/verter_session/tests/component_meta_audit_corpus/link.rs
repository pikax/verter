...
//!
//! Corpus coverage for the vendored Vue fixture at `/Link.vue`.
...
#[test]
fn corpus_audit_link_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Link.vue");
    let result = AuditedRequest::builder()
        .files([("/Link.vue", src)])
        .resolve_component_meta("/Link.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Link.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_link: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_link: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
