...
//! runtime component at `/EditorEmojiMenu.vue`. The assertion posture is
...
fn corpus_audit_editor_emoji_menu_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/EditorEmojiMenu.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_editor_emoji_menu: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
