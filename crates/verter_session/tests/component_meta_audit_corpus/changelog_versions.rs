...
//! runtime component at `/ChangelogVersions.vue`. The assertion posture is
...
#[test]
fn corpus_audit_changelog_versions_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ChangelogVersions.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ChangelogVersions.vue", src)])
        .resolve("/ChangelogVersions.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ChangelogVersions.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_changelog_versions: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_changelog_versions: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
