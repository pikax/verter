...
//! runtime component at `/prose/Tr.vue`. The assertion posture is
...
fn corpus_audit_prose_tr_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Tr.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Tr.vue", src)])
        .resolve("/prose/Tr.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_prose_tr: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
