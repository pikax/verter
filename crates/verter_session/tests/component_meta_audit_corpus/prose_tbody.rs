...
//! runtime component at `/prose/Tbody.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_tbody_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Tbody.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Tbody.vue", src)])
        .resolve("/prose/Tbody.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Tbody.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_tbody: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_tbody: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
