...
//!
//! Corpus coverage for the vendored Vue fixture at `/Input.vue`.
...
#[test]
fn corpus_audit_input_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Input.vue");
    let result = AuditedRequest::builder()
        .files([("/Input.vue", src)])
        .resolve_component_meta("/Input.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Input.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_input: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_input: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
