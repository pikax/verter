...
//! runtime component at `/content/ContentToc.vue`. The assertion posture is
...
#[test]
fn corpus_audit_content_content_toc_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/content/ContentToc.vue"
    );
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/content/ContentToc.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_content_content_toc: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_content_content_toc: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
