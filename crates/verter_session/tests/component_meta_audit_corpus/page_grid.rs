...
//! runtime component at `/PageGrid.vue`. The assertion posture is
...
fn corpus_audit_page_grid_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/PageGrid.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_page_grid: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
