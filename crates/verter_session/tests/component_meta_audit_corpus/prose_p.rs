...
//! runtime component at `/prose/P.vue`. The assertion posture is
...
fn corpus_audit_prose_p_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/P.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_prose_p: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
