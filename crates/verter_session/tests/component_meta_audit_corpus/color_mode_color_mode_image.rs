...
//! runtime component at `/color-mode/ColorModeImage.vue`. The assertion posture is
...
#[test]
fn corpus_audit_color_mode_color_mode_image_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/color-mode/ColorModeImage.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/color-mode/ColorModeImage.vue", src)])
        .resolve("/color-mode/ColorModeImage.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/color-mode/ColorModeImage.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_color_mode_color_mode_image: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_color_mode_color_mode_image: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
