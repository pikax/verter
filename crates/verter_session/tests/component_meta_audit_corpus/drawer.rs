...
//!
//! Corpus coverage for the vendored Vue fixture at `/Drawer.vue`.
...
#[test]
fn corpus_audit_drawer_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Drawer.vue");
    let result = AuditedRequest::builder()
        .files([("/Drawer.vue", src)])
        .resolve_component_meta("/Drawer.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Drawer.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_drawer: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_drawer: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
