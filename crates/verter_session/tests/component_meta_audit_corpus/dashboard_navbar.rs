...
//! runtime component at `/DashboardNavbar.vue`. The assertion posture is
...
#[test]
fn corpus_audit_dashboard_navbar_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/DashboardNavbar.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/DashboardNavbar.vue", src)])
        .resolve("/DashboardNavbar.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/DashboardNavbar.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_dashboard_navbar: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_dashboard_navbar: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
