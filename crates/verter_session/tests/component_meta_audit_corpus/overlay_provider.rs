...
//! runtime component at `/OverlayProvider.vue`. The assertion posture is
...
fn corpus_audit_overlay_provider_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/OverlayProvider.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_overlay_provider: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
