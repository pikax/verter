...
//! runtime component at `/prose/Card.vue`. The assertion posture is
...
fn corpus_audit_prose_card_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Card.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_prose_card: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
