...
//!
//! Corpus coverage for the vendored Vue fixture at `/User.vue`.
...
#[test]
fn corpus_audit_user_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/User.vue");
    let result = AuditedRequest::builder()
        .files([("/User.vue", src)])
        .resolve_component_meta("/User.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/User.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_user: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_user: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
