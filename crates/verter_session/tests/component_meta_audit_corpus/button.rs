...
//!
//! Corpus coverage for the vendored Vue fixture at `/Button.vue`.
...
#[test]
fn corpus_audit_button_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Button.vue");
    let result = AuditedRequest::builder()
        .files([("/Button.vue", src)])
        .resolve_component_meta("/Button.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Button.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_button: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_button: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
