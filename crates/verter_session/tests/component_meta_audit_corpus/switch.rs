...
//! runtime component at `/Switch.vue`. The assertion posture is
...
fn corpus_audit_switch_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Switch.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_switch: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
