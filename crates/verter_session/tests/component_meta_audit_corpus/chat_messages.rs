...
//! runtime component at `/ChatMessages.vue`. The assertion posture is
...
#[test]
fn corpus_audit_chat_messages_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ChatMessages.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ChatMessages.vue", src)])
        .resolve("/ChatMessages.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ChatMessages.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_chat_messages: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_chat_messages: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
