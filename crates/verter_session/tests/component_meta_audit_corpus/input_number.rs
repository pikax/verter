...
//! runtime component at `/InputNumber.vue`. The assertion posture is
...
#[test]
fn corpus_audit_input_number_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/InputNumber.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/InputNumber.vue", src)])
        .resolve("/InputNumber.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/InputNumber.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_input_number: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_input_number: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
