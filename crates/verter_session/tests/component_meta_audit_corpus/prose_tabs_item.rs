...
//! runtime component at `/prose/TabsItem.vue`. The assertion posture is
...
#[test]
fn corpus_audit_prose_tabs_item_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/TabsItem.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/TabsItem.vue", src)])
        .resolve("/prose/TabsItem.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/TabsItem.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_tabs_item: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_tabs_item: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
