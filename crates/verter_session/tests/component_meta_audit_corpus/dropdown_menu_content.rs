...
//! runtime component at `/DropdownMenuContent.vue`. The assertion posture is
...
#[test]
fn corpus_audit_dropdown_menu_content_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/DropdownMenuContent.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/DropdownMenuContent.vue", src)])
        .resolve("/DropdownMenuContent.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/DropdownMenuContent.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_dropdown_menu_content: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_dropdown_menu_content: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
