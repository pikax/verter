...
//! runtime component at `/Breadcrumb.vue`. The assertion posture is
...
#[test]
fn corpus_audit_breadcrumb_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Breadcrumb.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/Breadcrumb.vue", src)])
        .resolve("/Breadcrumb.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Breadcrumb.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_breadcrumb: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_breadcrumb: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
