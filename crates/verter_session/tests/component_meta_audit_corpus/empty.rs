...
//!
//! Corpus coverage for the vendored Vue fixture at `/Empty.vue`.
...
#[test]
fn corpus_audit_empty_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Empty.vue");
    let result = AuditedRequest::builder()
        .files([("/Empty.vue", src)])
        .resolve_component_meta("/Empty.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Empty.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_empty: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_empty: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
