...
//!
//! Corpus coverage for the vendored Vue fixture at `/PageCTA.vue`.
...
#[test]
fn corpus_audit_page_cta_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/PageCTA.vue");
    let result = AuditedRequest::builder()
        .files([("/PageCTA.vue", src)])
        .resolve_component_meta("/PageCTA.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/PageCTA.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_page_cta: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_page_cta: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
