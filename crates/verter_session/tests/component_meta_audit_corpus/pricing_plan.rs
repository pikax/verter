...
//! runtime component at `/PricingPlan.vue`. The assertion posture is
...
#[test]
fn corpus_audit_pricing_plan_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/PricingPlan.vue"
    );
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/PricingPlan.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_pricing_plan: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_pricing_plan: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
