...
//! runtime component at `/Page.vue`. The assertion posture is
...
fn corpus_audit_page_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Page.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_page: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
