...
//!
//! Corpus coverage for the vendored Vue fixture at `/Slider.vue`.
...
#[test]
fn corpus_audit_slider_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Slider.vue");
    let result = AuditedRequest::builder()
        .files([("/Slider.vue", src)])
        .resolve_component_meta("/Slider.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Slider.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_slider: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_slider: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
