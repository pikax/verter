...
//! runtime component at `/DashboardGroup.vue`. The assertion posture is
...
fn corpus_audit_dashboard_group_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/DashboardGroup.vue"
    );
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_dashboard_group: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
