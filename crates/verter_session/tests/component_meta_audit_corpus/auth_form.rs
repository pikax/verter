...
//! runtime component at `/AuthForm.vue`. The assertion posture is
...
#[test]
fn corpus_audit_auth_form_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/AuthForm.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/AuthForm.vue", src)])
        .resolve("/AuthForm.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/AuthForm.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_auth_form: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_auth_form: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
