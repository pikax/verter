...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/A.vue`.
...
#[test]
fn corpus_audit_prose_a_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/prose/A.vue");
    let result = AuditedRequest::builder()
        .files([("/prose/A.vue", src)])
        .resolve_component_meta("/prose/A.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/A.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_a: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_a: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
