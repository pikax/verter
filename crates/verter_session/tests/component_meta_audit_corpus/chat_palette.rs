...
//! runtime component at `/ChatPalette.vue`. The assertion posture is
...
fn corpus_audit_chat_palette_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/ChatPalette.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/ChatPalette.vue", src)])
        .resolve("/ChatPalette.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_chat_palette: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
