...
//! runtime component at `/prose/Ol.vue`. The assertion posture is
...
fn corpus_audit_prose_ol_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/prose/Ol.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/prose/Ol.vue", src)])
        .resolve("/prose/Ol.vue");
    ...
    match result {
        ...
        Err(err) => {
            eprintln!(
                "corpus_audit_prose_ol: resolution returned an error — documenting skip: {err:?}",
            );
        }
    }
}
