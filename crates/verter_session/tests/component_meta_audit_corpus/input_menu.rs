...
//! runtime component at `/InputMenu.vue`. The assertion posture is
...
fn corpus_audit_input_menu_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/InputMenu.vue"
    );
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/InputMenu.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_input_menu: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_input_menu: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
