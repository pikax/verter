...
//! runtime component at `/EditorDragHandle.vue`. The assertion posture is
...
#[test]
fn corpus_audit_editor_drag_handle_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/EditorDragHandle.vue"
    );
    let result = AuditedRequest::builder()
        .files([("/EditorDragHandle.vue", src)])
        .resolve("/EditorDragHandle.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/EditorDragHandle.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_editor_drag_handle: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_editor_drag_handle: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
