...
//! runtime component at `/Separator.vue`. The assertion posture is
...
#[test]
fn corpus_audit_separator_produces_audit_record_or_documents_skip() {
    let src = include_str!(
        "../../../../.integration-tests/repos/nuxt-ui/src/runtime/components/Separator.vue"
    );
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Separator.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_separator: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_separator: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
