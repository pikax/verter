...
//! the `gen-typeinfo-manifest` cargo bin (`pnpm gen:typeinfo-manifest`).
...
// DAG are generated by the `gen-typeinfo-manifest` cargo bin. The
...
// static Rust maps, NOT from the partition. The guards below only diff/fail,
...
/// The list is authored in the `gen-typeinfo-manifest` cargo bin
...
/// in the `gen-typeinfo-manifest` cargo bin does (substring up
...
fn every_manifest_row_unblocker_matches_live_ignore_reason() {
    ...
    assert!(
        ...
        "ignored-test manifest: {} row(s) have an `unblocker` that \
         ...
         Re-run `cargo run -p verter_session --bin gen-typeinfo-manifest` \
...
fn registry_entry_count_matches_declared() {
    for m in oracle_registry::LIFTED_ROW_MIGRATIONS {
        ...
        // `IgnoredTestRow` side, generated by `gen-typeinfo-manifest`) matches the retained metadata.
        let row = EXPECTED_IGNORE_MANIFEST
            .iter()
