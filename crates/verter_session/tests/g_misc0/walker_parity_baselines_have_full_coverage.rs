...
fn baseline_source() -> String {
    ...
    let direct = tests_dir.join("legacy_walker_parity_baseline.rs");
    // The baseline file may be consolidated into a group subdirectory
    // (tests/g_<group>/legacy_walker_parity_baseline.rs); fall back to
    // locating it by basename. This test reads the file's SOURCE to
    // count `fn fixture_NN_` entries, so its on-disk location is
    // irrelevant to what it discriminates.
    let p = if direct.exists() {
        direct
    } else {
        walkdir::WalkDir::new(&tests_dir)
            .into_iter()
            .flatten()
            .find(|e| e.file_name() == "legacy_walker_parity_baseline.rs")
