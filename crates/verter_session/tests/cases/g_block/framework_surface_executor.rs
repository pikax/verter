...
/// a call of `notDeclared`, a name declared nowhere (TS2304), whose error
/// type is recovery for a program that does not type-check, which the
/// flow-return lane does not model (TypeScript 7.0.2 `tsc`: `{ label:
/// string; made: any }`).
///
