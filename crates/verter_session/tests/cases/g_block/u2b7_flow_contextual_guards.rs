...
fn flow_narrowing_at_identity_covers_substitution_axis() {
    ...
    let other =
        flow_narrowing_key_with_subst("a.ts", 0, SubstitutionCanonicalHash::distinct_for_test(9));
    assert_distinct_identity(&base, &other);
}
...
fn contextual_type_at_identity_covers_substitution_axis() {
    ...
    let other =
        contextual_type_key_with_subst("a.ts", 0, SubstitutionCanonicalHash::distinct_for_test(9));
    assert_distinct_identity(&base, &other);
}
