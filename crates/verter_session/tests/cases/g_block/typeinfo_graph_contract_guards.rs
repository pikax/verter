...
/// The closed `GraphTypeNode.kind` oneof — exactly these 33 arms. The
...
const NODE_TAXONOMY_ARMS: &[&str] = &[
    ...
    "opaque",
    "cycle",
    // Schema 8: the applied compiler-native type operation. Carries a
    // closed op enum and operand node ids — never a declaration symbol.
...
pub(crate) fn node_taxonomy_complete() {
    ...
    // guard pins the EXACT arm set (33 arms) plus the `reserved 35 to 100`
    ...
    assert_eq!(
        arms.len(),
        33,
        "GraphTypeNode.kind must have exactly 33 oneof arms; found {}",
        arms.len(),
    );
    assert_eq!(
        arms,
    ...
    assert!(
        body.contains("reserved 35 to 100;"),
        "GraphTypeNode must reserve the additive `oneof kind` tag window \
         (`reserved 35 to 100;`) at the enclosing message level",
    );
}
...
/// directives sit beside `reserved 35 to 100;` at message level).
///
