...
//!
//! - `IgnoredTestRow` — EXACTLY 348 test-site rows total. 302 carry
...
//!   count-guarded at 348; the live-ignore count is count-guarded at 302.
...
//! resolves to 302 (348 rows total − 46 lifted).
//!
//! Guards:
//!
...
//!
//! - `ignored_test_row_table_holds_exactly_348_rows` (binding total 348 +
...
//
// `IgnoredTestRow` carries EXACTLY 348 test-site rows total (count-guarded):
// 302 `Ignored` (bijective with the source `#[ignore]`s) + 46 `Lifted`.
...
// DAG are generated by `scripts/gen-typeinfo-ignore-manifest.mjs`. The
...
/// One manifest row per typeinfo test-site — EXACTLY 348 rows total
/// (302 `Ignored` + 46 `Lifted`). 14 fields.
...
/// The list is authored in `scripts/gen-typeinfo-ignore-manifest.mjs`
...
/// literal. It tracks the live ignore set — currently 302 (the 348-row
...
///
/// Why the 348 TABLE total: the substrate's macro-driven test
...
/// 302 expanded test-site ignores + 46 lifted rows = the 348-row table
...
fn manifest_length_matches_documented_total() {
    // The table length is the documented total 362 (ORACLE-lifted rows STAY
    ...
    assert_eq!(
        ...
        348,
        "the documented IgnoredTestRow table total is 348 (oracle-lifted rows \
         stay in the table)",
    );
}
...
/// The `IgnoredTestRow` table holds EXACTLY 348 rows (the
...
/// tables. An `IgnoredTestRow` count other than 348, an
/// `AdditionalProofRow` in the ignored set, or a `(file, function)` in
/// both, FAILS.
#[test]
fn ignored_test_row_table_holds_exactly_348_rows() {
    assert_eq!(
        EXPECTED_IGNORE_MANIFEST.len(),
        348,
        "the `IgnoredTestRow` table must hold EXACTLY 348 binding rows \
         ...
         lift. It DECREASES only when a row's `#[ignore]` is removed because \
         its contract landed as an always-running test — the row then leaves \
         the partition entirely (the 6 typeinfo footprint-attach / \
         cache-invalidation edit-cycle rows landed this way, the \
         relation-activation optional-to-required rejection row landed as an \
         always-running contract row backed by the parity-enforced relation \
         oracle, and the 4 unique-symbol branded/key-access rows landed as \
         always-running typed-key contract rows).",
        EXPECTED_IGNORE_MANIFEST.len(),
    );

    // The derived live-ignore count is 302: the table holds 348 rows, of which
    ...
    // U2.QUERY_VALUE_DOMAIN — plus the 4 module-augmentation-era lifts:
    // 3 indexed-member projection rows (the `as const` typeof indexed member +
    // the `typeof import(...)["default"]` default-export value projection +
    // the `typeof import(...)["leafName"]` named-value projection) at
    // U2.INDEXED_ACCESS and 1 namespace alias-chain row at
    // U2.QUERY_VALUE_DOMAIN — plus the 2 JSX-era lifts: the two
    // `IntrinsicPropsFor<"div">` / `IntrinsicPropsFor<"span">` parametric
    ...
    // U2.MAPPED_TEMPLATE) and 302 remain `Ignored` (the 6 guard-backed
    // footprint-attach / cache-invalidation edit-cycle rows, the
    // relation-activation rejection row, the 4 unique-symbol
    // branded/key-access rows, and the 14 U6.FLOW_RETURN_SUBSTRATE
    // producer-routing / value-inference rows left the partition when their
    // `#[ignore]`s were removed — see the table-length assertion above).
    // `EXPECTED_TOTAL_IGNORED_COUNT` is DERIVED as count(status == Ignored),
    // so it tracks lifts automatically.
    assert_eq!(
        EXPECTED_TOTAL_IGNORED_COUNT, 302,
        "EXPECTED_TOTAL_IGNORED_COUNT is DERIVED as count(status == \
         Ignored); with 46 rows lifted out of 348 it must equal 302. Got {}.",
        EXPECTED_TOTAL_IGNORED_COUNT,
    );
    // Exactly 46 rows are `Lifted` (the 19 pre-class-surface lifts + the 19
    // class-surface-era lifts + the 4 module-augmentation-era lifts + the 2
    // JSX-era lifts + the 2 mapped-template-era lifts enumerated above).
    ...
    assert_eq!(
        lifted, 46,
        "exactly 46 IgnoredTestRows are Lifted (the 19 pre-class-surface lifts \
         + the 19 class-surface-era lifts + the 4 module-augmentation-era \
         lifts + the 2 JSX-era lifts + the 2 mapped-template-era lifts); got {lifted}",
    );
