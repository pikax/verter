...
//!
//! - `IgnoredTestRow` — EXACTLY 351 test-site rows total. 305 carry
...
//!   count-guarded at 351; the live-ignore count is count-guarded at 305.
...
//! resolves to 305 (351 rows total − 46 lifted).
//!
//! Guards:
//!
...
//!
//! - `ignored_test_row_table_holds_exactly_351_rows` (binding total 351 +
...
//
// `IgnoredTestRow` carries EXACTLY 355 test-site rows total (count-guarded):
// 305 `Ignored` (bijective with the source `#[ignore]`s) + 46 `Lifted`.
...
/// One manifest row per typeinfo test-site — EXACTLY 355 rows total
/// (305 `Ignored` + 46 `Lifted`). 14 fields.
...
/// literal. It tracks the live ignore set — currently 305 (the 351-row
...
///
/// Why the 355 TABLE total (not 378): the substrate's macro-driven test
...
/// 305 expanded test-site ignores + 46 lifted rows = the 351-row table
...
fn manifest_length_matches_documented_total() {
    // The table length is the documented total 351 (ORACLE-lifted rows STAY
    ...
    assert_eq!(
        ...
        351,
        "the documented IgnoredTestRow table total is 351 (oracle-lifted rows \
         stay in the table)",
    );
}
...
/// The `IgnoredTestRow` table holds EXACTLY 355 rows (the
...
/// tables. An `IgnoredTestRow` count other than 355, an
/// `AdditionalProofRow` in the ignored set, or a `(file, function)` in
/// both, FAILS.
#[test]
fn ignored_test_row_table_holds_exactly_351_rows() {
    assert_eq!(
        EXPECTED_IGNORE_MANIFEST.len(),
        351,
        "the `IgnoredTestRow` table must hold EXACTLY 351 binding rows \
    ...
    // The derived live-ignore count is 305: the table holds 351 rows, of which
    ...
    // U2.MAPPED_TEMPLATE) and 309 remain `Ignored` (the 6 guard-backed
    ...
    assert_eq!(
        EXPECTED_TOTAL_IGNORED_COUNT, 305,
        "EXPECTED_TOTAL_IGNORED_COUNT is DERIVED as count(status == \
         Ignored); with 46 rows lifted out of 351 it must equal 305. Got {}.",
