...
fn corpus_generator_output_matches_committed_files() {
    ...
    let committed_entry = root.join("crates/verter_session/tests/cases/corpus_audit_tests.rs");
...
fn runtime_components_dir(root: &Path) -> PathBuf {
    root.join("crates/verter_session/tests/cases/component_meta_audit_corpus/fixtures")
}
...
fn commit_7_snapshots_stable_against_current_incidental_event_names_list() {
    ...
    let authored_fixtures_dir =
        root.join("crates/verter_session/tests/cases/component_meta_audit/corpus_representatives");
    assert!(
