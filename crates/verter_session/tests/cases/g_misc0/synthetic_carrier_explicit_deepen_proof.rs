...
//! at `tests/cases/g_misc2/no_carrier_verdict_db.rs` bans any production
//! source that constructs `SemanticNodeId(<ident>.value_node)` as a cache
//! key — the `value_node` ordinal is provenance, never identity.
//!
