...
//!
//! Corpus coverage for the vendored Vue fixture at `/Select.vue`.
...
#[test]
fn corpus_audit_select_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Select.vue");
    let result = AuditedRequest::builder()
        .files([("/Select.vue", src)])
        .resolve_component_meta("/Select.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Select.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_select: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_select: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
