...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/Script.vue`.
...
#[test]
fn corpus_audit_prose_script_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/prose/Script.vue");
    let result = AuditedRequest::builder()
        .files([("/prose/Script.vue", src)])
        .resolve_component_meta("/prose/Script.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Script.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_script: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_script: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
