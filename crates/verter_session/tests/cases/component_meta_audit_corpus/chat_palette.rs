...
//!
//! Corpus coverage for the vendored Vue fixture at `/ChatPalette.vue`.
...
#[test]
fn corpus_audit_chat_palette_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/ChatPalette.vue");
    let result = AuditedRequest::builder()
        .files([("/ChatPalette.vue", src)])
        .resolve_component_meta("/ChatPalette.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ChatPalette.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_chat_palette: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_chat_palette: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
