...
//!
//! Corpus coverage for the vendored Vue fixture at `/Collapsible.vue`.
...
#[test]
fn corpus_audit_collapsible_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Collapsible.vue");
    let result = AuditedRequest::builder()
        .files([("/Collapsible.vue", src)])
        .resolve_component_meta("/Collapsible.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Collapsible.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_collapsible: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_collapsible: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
