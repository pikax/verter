...
//!
//! Corpus coverage for the vendored Vue fixture at `/Page.vue`.
...
#[test]
fn corpus_audit_page_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Page.vue");
    let result = AuditedRequest::builder()
        .files([("/Page.vue", src)])
        .resolve_component_meta("/Page.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Page.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_page: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_page: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
