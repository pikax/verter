...
//!
//! Corpus coverage for the vendored Vue fixture at `/Error.vue`.
...
fn corpus_audit_error_produces_audit_record_or_documents_skip() {
    ...
    match result {
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_error: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_error: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
