...
//!
//! Corpus coverage for the vendored Vue fixture at `/Toast.vue`.
...
#[test]
fn corpus_audit_toast_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Toast.vue");
    let result = AuditedRequest::builder()
        .files([("/Toast.vue", src)])
        .resolve_component_meta("/Toast.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Toast.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_toast: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_toast: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
