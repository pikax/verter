...
//!
//! Corpus coverage for the vendored Vue fixture at `/Tree.vue`.
...
fn corpus_audit_tree_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Tree.vue");
    let result = AuditedRequest::builder()
        .files([("/Tree.vue", src)])
        .resolve_component_meta("/Tree.vue");
    ...
    match result {
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_tree: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_tree: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
