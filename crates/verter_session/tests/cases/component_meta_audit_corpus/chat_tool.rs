...
//!
//! Corpus coverage for the vendored Vue fixture at `/ChatTool.vue`.
...
#[test]
fn corpus_audit_chat_tool_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/ChatTool.vue");
    let result = AuditedRequest::builder()
        .files([("/ChatTool.vue", src)])
        .resolve_component_meta("/ChatTool.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/ChatTool.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_chat_tool: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_chat_tool: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
