...
//!
//! Corpus coverage for the vendored Vue fixture at `/PricingTable.vue`.
...
#[test]
fn corpus_audit_pricing_table_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/PricingTable.vue");
    let result = AuditedRequest::builder()
        .files([("/PricingTable.vue", src)])
        .resolve_component_meta("/PricingTable.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/PricingTable.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_pricing_table: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_pricing_table: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
