...
//!
//! Corpus coverage for the vendored Vue fixture at `/Chip.vue`.
...
#[test]
fn corpus_audit_chip_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Chip.vue");
    let result = AuditedRequest::builder()
        .files([("/Chip.vue", src)])
        .resolve_component_meta("/Chip.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Chip.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_chip: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_chip: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
