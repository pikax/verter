...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/Tr.vue`.
...
#[test]
fn corpus_audit_prose_tr_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/prose/Tr.vue");
    let result = AuditedRequest::builder()
        .files([("/prose/Tr.vue", src)])
        .resolve_component_meta("/prose/Tr.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Tr.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_tr: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_tr: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
