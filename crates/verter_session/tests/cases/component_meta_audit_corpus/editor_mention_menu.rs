...
//!
//! Corpus coverage for the vendored Vue fixture at `/EditorMentionMenu.vue`.
...
#[test]
fn corpus_audit_editor_mention_menu_produces_audit_record_or_documents_skip() {
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/EditorMentionMenu.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_editor_mention_menu: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_editor_mention_menu: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
