...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/P.vue`.
...
#[test]
fn corpus_audit_prose_p_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/prose/P.vue");
    let result = AuditedRequest::builder()
        .files([("/prose/P.vue", src)])
        .resolve_component_meta("/prose/P.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/P.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_p: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_p: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
