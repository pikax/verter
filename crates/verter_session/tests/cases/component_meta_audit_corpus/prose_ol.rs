...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/Ol.vue`.
...
#[test]
fn corpus_audit_prose_ol_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/prose/Ol.vue");
    let result = AuditedRequest::builder()
        .files([("/prose/Ol.vue", src)])
        .resolve_component_meta("/prose/Ol.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Ol.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_ol: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_ol: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
