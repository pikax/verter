...
//!
//! Corpus coverage for the vendored Vue fixture at `/Banner.vue`.
...
#[test]
fn corpus_audit_banner_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Banner.vue");
    let result = AuditedRequest::builder()
        .files([("/Banner.vue", src)])
        .resolve_component_meta("/Banner.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Banner.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_banner: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_banner: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
