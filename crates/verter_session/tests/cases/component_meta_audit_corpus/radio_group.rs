...
//!
//! Corpus coverage for the vendored Vue fixture at `/RadioGroup.vue`.
...
#[test]
fn corpus_audit_radio_group_produces_audit_record_or_documents_skip() {
    ...
    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/RadioGroup.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_radio_group: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_radio_group: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
