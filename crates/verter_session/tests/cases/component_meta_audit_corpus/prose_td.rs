...
//!
//! Corpus coverage for the vendored Vue fixture at `/prose/Td.vue`.
...
#[test]
fn corpus_audit_prose_td_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/prose/Td.vue");
    let result = AuditedRequest::builder()
        .files([("/prose/Td.vue", src)])
        .resolve_component_meta("/prose/Td.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/prose/Td.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_prose_td: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_prose_td: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
