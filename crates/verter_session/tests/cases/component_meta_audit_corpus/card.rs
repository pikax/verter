...
//!
//! Corpus coverage for the vendored Vue fixture at `/Card.vue`.
...
#[test]
fn corpus_audit_card_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Card.vue");
    let result = AuditedRequest::builder()
        .files([("/Card.vue", src)])
        .resolve_component_meta("/Card.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Card.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_card: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_card: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
