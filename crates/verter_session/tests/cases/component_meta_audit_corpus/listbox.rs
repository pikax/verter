...
//!
//! Corpus coverage for the vendored Vue fixture at `/Listbox.vue`.
...
#[test]
fn corpus_audit_listbox_produces_audit_record_or_documents_skip() {
    let src = include_str!("fixtures/Listbox.vue");
    let result = AuditedRequest::builder()
        .files([("/Listbox.vue", src)])
        .resolve_component_meta("/Listbox.vue");

    match result {
        Ok((_, _, record)) => {
            assert_eq!(
                record.canonical_id, "/Listbox.vue",
        ...
        Err(AuditedRequestError::ResolutionFailed) => {
            ...
            eprintln!(
                "corpus_audit_listbox: hermetic resolution returned None (missing deps) — documenting skip",
            );
        }
        Err(other) => panic!(
            "corpus_audit_listbox: unexpected audit error — this indicates an audit-wiring regression, not a hermetic-dep gap: {other:?}",
        ),
    }
}
