...
//! * A supported Svelte carrier satisfies `ensure_ide_compiled` + `get_ide`
//!   while retaining its runtime `Main` — and the IDE output is Svelte-specific (the
...
// ── Svelte: a legacy component retains both IDE and runtime surfaces ─────────
//
// T4 added legacy-script lowering, so a legacy (non-runes) component is a supported
// runtime surface. The carrier must preserve both that `Main` node and the IDE
// projection; runes coverage remains separate below so the two modes cannot mask one
// another.

const SVELTE_SRC: &str = "<script lang=\"ts\">let count = 0;</script>\n<button onclick={() => count++}>{count}</button>\n";

#[test]
fn svelte_ensure_ide_compiled_preserves_supported_legacy_main() {
    let host = host();
    upsert(
        &host,
        "/src/Counter.svelte",
    ...
    // … and the same carrier retains its legacy runtime `Main`. These assertions are
    // deliberately mode-specific: a runes-only implementation would omit the legacy
    // side-effect flag, while an accidentally routed Vue result would expose
    // `_sfc_main` instead of the Svelte client namespace.
    let main = main_code(&host, "/src/Counter.svelte", &profile)
        .expect("a supported legacy Svelte component must produce a runtime Main node");
    assert!(
        main.contains("import * as $ from 'svelte/internal/client';"),
        "legacy Main must be a Svelte client module:\n{main}"
    );
    assert!(
        main.contains("import 'svelte/internal/flags/legacy';"),
        "legacy Main must retain Svelte's legacy-mode flag:\n{main}"
    );
    assert!(
        !main.contains("_sfc_main"),
        "legacy Main must not contain Vue runtime residue:\n{main}"
    );
}
...
#[test]
fn svelte_virtual_inventory_exposes_compiled_style_nodes() {
    let host = host();
    let source = "<script>let count = $state(0);</script>\n<style>button{color:red}</style>\n<button>{count}</button>\n";
    upsert(
        &host,
        "/src/Inventory.svelte",
        source,
        FileLanguage::svelte(),
    );

    assert_eq!(
        host.list_virtual_files("/src/Inventory.svelte"),
        vec![VirtualNodeKind::Main, VirtualNodeKind::Style { index: 0 },],
        "the public virtual-file inventory must advertise every Svelte style node"
    );
}

#[test]
fn svelte_template_and_style_edit_invalidates_runtime_artifacts() {
    let host = host();
    ...
    let main_before = main_code(&host, canonical, &profile).expect("initial Svelte Main");
    let style_before = host
        .get_virtual_file(VirtualQuery {
            raw_id: None,
            canonical_id: Some(canonical.to_string()),
            node_kind: Some(VirtualNodeKind::Style { index: 0 }),
        ...
        .expect("initial Svelte style");

    let update = host
        .upsert(UpsertRequest {
            canonical_id: Some(canonical.to_string()),
            input_id: canonical.to_string(),
            source: Arc::from(after),
            file_language: FileLanguage::svelte(),
            aliases: Vec::new(),
        })
        .expect("edited Svelte upsert");
    assert!(
        update.changed,
        "a template/style edit with an unchanged script must invalidate the carrier"
    );
    assert_eq!(update.slice_changes.style_indices_changed, vec![0]);

    let main_after = main_code(&host, canonical, &profile).expect("updated Svelte Main");
    let style_after = host
        .get_virtual_file(VirtualQuery {
            raw_id: None,
            canonical_id: Some(canonical.to_string()),
            node_kind: Some(VirtualNodeKind::Style { index: 0 }),
        ...
        .expect("updated Svelte style");

    assert_ne!(
        main_before, main_after,
        "the runtime Main must be recomputed"
    );
    assert!(
        main_after.contains("after"),
        "updated markup must reach Main: {main_after}"
    );
    assert_ne!(
        style_before.code, style_after.code,
        "the style node must be recomputed"
    );
    assert!(
        style_after.code.contains("blue"),
        "updated CSS must reach the style node: {}",
        style_after.code
    );
}

#[test]
fn svelte_slice_changes_keep_block_domains_distinct_and_invalidate_style_dependents() {
    let host = host();
    let canonical = "/src/Slices.svelte";
    let initial = "<script>let count = $state(0);</script>\n<style>button{color:red}</style>\n<button>{count}</button>\n";
    let script_edited = "<script>let count = $state(0); const unused = 1;</script>\n<style>button{color:red}</style>\n<button>{count}</button>\n";
    let style_edited = "<script>let count = $state(0); const unused = 1;</script>\n<style>button{color:rebeccapurple}</style>\n<button>{count}</button>\n";

    upsert(&host, canonical, initial, FileLanguage::svelte());

    let script_update = host
        .upsert(UpsertRequest {
            canonical_id: Some(canonical.to_string()),
            input_id: canonical.to_string(),
            source: Arc::from(script_edited),
            file_language: FileLanguage::svelte(),
            aliases: Vec::new(),
        })
        .expect("script-only Svelte upsert");
    assert!(script_update.slice_changes.script_changed);
    assert!(!script_update.slice_changes.template_changed);
    assert!(script_update.slice_changes.style_indices_changed.is_empty());

    let style_update = host
        .upsert(UpsertRequest {
            canonical_id: Some(canonical.to_string()),
            input_id: canonical.to_string(),
            source: Arc::from(style_edited),
            file_language: FileLanguage::svelte(),
            aliases: Vec::new(),
        })
        .expect("style-only Svelte upsert");
    assert!(!style_update.slice_changes.script_changed);
    assert!(!style_update.slice_changes.template_changed);
    assert_eq!(style_update.slice_changes.style_indices_changed, vec![0]);
    assert_eq!(
        style_update.changed_virtual_nodes,
        vec![VirtualNodeKind::Main, VirtualNodeKind::Style { index: 0 }],
        "Svelte's runtime Main carries the CSS scope hash and must invalidate with its style"
    );
}
