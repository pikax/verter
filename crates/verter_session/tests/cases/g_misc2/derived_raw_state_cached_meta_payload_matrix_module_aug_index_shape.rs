...
//! must carry `FactKey::ModuleAugmentationIndexShape` facts in its
//! `fact_versions: Arc<[FactVersionRef]>` signature so warm-hit
//! validation invalidates when a `declare module 'vue'` augmentation
//! shape flips (e.g., a third-party plugin re-declares global
//! component types).
//!
//! Pre-1A: wrapper holds `Vec<FactVersionRef>` and the source-grep
//! arch guard FAILS. Post-1A: substrate is `Arc<[FactVersionRef]>`
//! and a `ModuleAugmentationIndexShape` fact-kind validates through
...
#[test]
fn cached_meta_payload_signature_carries_module_aug_index_shape() {
