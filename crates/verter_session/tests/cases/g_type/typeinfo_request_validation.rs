...
#[test]
fn schema_versions_one_and_six_are_rejected_with_typed_errors() {
    // 1 is below the floor; 6 is the first version ABOVE the current supported
    // set ([2, 3, 4, 5]). Both are outside the closed set and rejected.
    for version in [1u32, 6u32] {
...
#[test]
fn supported_schema_version_set_is_exactly_two_three_four_and_five() {
    assert_eq!(
        ...
        "the supported set holds every version some operation still accepts: \
         ...
         floor), schema 4 (member default/origin fields), and schema 5 \
         (current — retires the tag-28 relation-proof arm, adds the payload-side \
         relation_proofs table)",
    );
    assert_eq!(MIN_TYPEINFO_GRAPH_SCHEMA_VERSION, 2);
    assert_eq!(TYPEINFO_GRAPH_SCHEMA_VERSION, 5);
...
/// source. It reports `[2, 3, 4, 5]`.
#[test]
fn unknown_schema_version_error_advertises_two_three_four_and_five() {
    // Version 6 is the first version OUTSIDE the supported set — it triggers
    // the UnknownSchemaVersion rejection (5 is now supported).
    let err = validate_type_info_graph_request(&legacy_envelope(wire::Operation::ResolveSymbol, 6))
        .unwrap_err();
    ...
    assert_eq!(
        ...
        vec![2, 3, 4, 5],
        "the advertisement surface must report exactly [2, 3, 4, 5]",
    );
    assert_eq!(
        payload.server_supported_versions,
    ...
    assert_eq!(wire_payload.server_supported_versions, vec![2, 3, 4, 5]);
}
...
fn op_minimum_gate_walks_every_operation_discriminant() {
    ...
    for &operation in EVERY_OPERATION {
        ...
        // floor; 6 is the first version above the current set ([2, 3, 4, 5]).
        for version in [1u32, 6u32] {
            let err = validate_schema_version_for_operation(operation, version).unwrap_err();
            assert_eq!(
                err_variant_label(&err),
