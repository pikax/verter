...
/// only THAT it did.
///
/// A test behind a comma sequence (`(touch(), typeof x === "string")`), a
/// form the guard vocabulary does not carry, retains a superset and
/// records the typed guard-narrowing gap, so the value is usable but never
/// warm: both calls recompute cold. Each record must
...
fn flow_return_audit_explains_partial_cold_recompute() {
    ...
    upsert(
        ...
        canonical,
        "function touch() { }\n\
         export function gapped(x: string | number) { if ((touch(), typeof x === \"string\")) return x; return 0; }\n\
