//! Matrix slice: `component_meta` × `import_ref`.
//!
//! Discrimination: `ComponentMetaResultEntry.read_set_signature.facts`
//! MUST be able to carry `FactKey::ImportRef` facts so the
//! final-result cache invalidates when an importer's binding-to-export
...
#[test]
fn component_meta_signature_carries_import_ref() {
    ...
    assert!(
        ...
        "component_meta matrix slice: the constructed entry MUST \
         carry the `ImportRef` fact in its `read_set_signature.facts`."
    );
    ...
    assert!(
        ...
        "component_meta matrix slice: the permissive view MUST accept \
         an `ImportRef` fact in the entry's `read_set_signature.facts`."
    );
}
