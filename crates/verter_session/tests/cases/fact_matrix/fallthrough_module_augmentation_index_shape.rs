//! Matrix slice: `fallthrough` × `module_augmentation_index_shape`.
//!
//! Discrimination: `CachedFallthroughEntry.fact_versions` MUST be
//! able to carry `FactKey::ModuleAugmentationIndexShape` facts so a
//! fallthrough consumer that depends on a globally-augmented module
//! surface is invalidated when the augmentation index fingerprint
//! shifts.
...
#[test]
fn fallthrough_signature_carries_module_augmentation_index_shape() {
    ...
    assert!(
        ...
        "fallthrough matrix slice: the permissive view MUST accept a \
         `ModuleAugmentationIndexShape` RouteSurface fact in the \
         Arc-stored fallthrough signature."
    );
}
