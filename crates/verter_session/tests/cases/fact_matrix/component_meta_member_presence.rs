//! Matrix slice: `component_meta` × `member_presence`.
//!
//! Discrimination: `ComponentMetaResultEntry.read_set_signature.facts`
//! MUST be able to carry `FactKey::MemberPresence` facts so the
//! final-result cache invalidates when a referenced member is
//! renamed / added / removed on a dep (path-precise per R28).
...
#[test]
fn component_meta_signature_carries_member_presence() {
    ...
    assert!(
        ...
        "component_meta matrix slice: the constructed entry MUST \
         carry the `MemberPresence` fact in its \
         `read_set_signature.facts` after the carrier consolidation."
    );
    ...
    assert!(
        ...
        "component_meta matrix slice: the permissive view MUST accept \
         a `MemberPresence` fact in the entry's `read_set_signature.facts`."
    );
}
