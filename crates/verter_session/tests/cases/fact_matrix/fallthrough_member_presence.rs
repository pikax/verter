//! Matrix slice: `fallthrough` × `member_presence`.
//!
//! Discrimination: `DerivedRawState.cached_fallthrough`
//! (`CachedFallthroughEntry`) MUST carry `FactKey::MemberPresence`
//! facts in its `fact_versions: Arc<[FactVersionRef]>` signature so
//! a fallthrough consumer is invalidated when a referenced member is
//! renamed / added / removed on a dep.
...
#[test]
fn fallthrough_signature_carries_member_presence() {
    ...
    assert!(
        ...
        "fallthrough matrix slice: the permissive view MUST accept a \
         `MemberPresence` fact in the Arc-stored fallthrough signature."
    );
}
