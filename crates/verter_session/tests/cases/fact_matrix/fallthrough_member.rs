//! Matrix slice: `fallthrough` × `member`.
//!
//! Discrimination: `CachedFallthroughEntry.fact_versions` MUST be
//! able to carry `FactKey::Member` body facts so a fallthrough
//! consumer is invalidated when a referenced member's body fingerprint
//! changes on a dep.
...
#[test]
fn fallthrough_signature_carries_member() {
    ...
    assert!(
        ...
        "fallthrough matrix slice: the permissive view MUST accept a \
         `Member` fact in the Arc-stored fallthrough signature."
    );
}
