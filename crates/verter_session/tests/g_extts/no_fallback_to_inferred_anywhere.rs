...
// Carrier feature admission is verified through typed `TypeProvider` calls in
// `verter_lsp/tests/cases/owned_binding_gate.rs`. That downstream behavioral suite
// instantiates the production composite, proves every denied carrier skips the
// managed provider, and proves bound/non-carrier paths delegate as specified.
