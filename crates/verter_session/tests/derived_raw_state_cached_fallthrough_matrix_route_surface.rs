...
//! (`CachedFallthroughEntry`) must carry
//! `FactKey::EffectiveExportSet` route-surface facts in its
...
//! `RouteSurface` fact-kind validates through the per-domain fast-
//! path.
...
#[test]
fn cached_fallthrough_signature_carries_route_surface() {
    ...
    assert!(
        ...
        "permissive view must accept a RouteSurface fact in the Arc-stored signature"
    );
}
