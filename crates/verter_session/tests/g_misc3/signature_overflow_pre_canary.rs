//! Pre-canary — `signature_overflow_count == 0` over the
//! steady-state baseline + path-precise corpus.
//!
//! The full canary asserts the same invariant under the full
//! `repo_first_pass` / `repo_warm_second_pass` loop; this pre-canary
//! runs at unit-test speed against the hermetic baseline and
//! path-precise archetypes so a producer that flattens transitive
//! facts into an over-1024 signature surfaces here, not in the full
//! canary.
//!
...
//! downstream materialiser's `semantic_hash`.
...
/// Both fixture corpora exist on disk — the pre-canary depends on them
/// as the steady-state load.
#[test]
fn pre_canary_fixture_corpora_are_in_tree() {
    ...
    assert!(
        baseline.is_dir(),
        "cache_baseline fixture corpus missing at {:?}",
        baseline
    );
    assert!(
        path_precise.is_dir(),
        "path_precise fixture corpus missing at {:?}",
        path_precise
    );
}
...
/// version of the full steady-state assertion: an over-cap
...
/// `ValidatedFactCache`s today. The same accessors are consumed by
/// the full canary.
#[test]
fn pre_canary_signature_overflow_count_is_zero_on_basic_resolver_flow() {
    ...
    assert_eq!(
        ...
        0,
        "pre-canary: RouteDb signature_overflow_count must be 0 on the basic \
         resolver flow. A non-zero count means a producer flattened transitive facts \
         where it should have folded a downstream materialiser's semantic_hash."
    );
    ...
    assert_eq!(
        ...
        0,
        "pre-canary: ImportedRootDb signature_overflow_count must be 0 on \
...
/// `FACT_SIGNATURE_CAP`.
///
/// The baseline corpus is the canonical workload for the
...
fn pre_canary_signature_overflow_count_is_zero_under_steady_state_loop() {
    ...
    assert_eq!(
        routes.signature_overflow_count(),
        0,
        "pre-canary (steady state): RouteDb signature_overflow_count must \
    ...
    assert_eq!(
        imported_roots.signature_overflow_count(),
        0,
        "pre-canary (steady state): ImportedRootDb signature_overflow_count \
