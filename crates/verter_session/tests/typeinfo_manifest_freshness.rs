...
//! produced from the pure-Rust generator bin `gen-typeinfo-manifest`
//! (`crates/verter_session/src/bin/gen_typeinfo_manifest/main.rs`, run via
...
//! generator's own static maps, NOT from §10.4.1. Whenever the generator
//! or its inputs change, the committed files must be regenerated and
//! committed in the same change.
//!
//! The guard invokes the generator bin in `--check` mode, which
//! regenerates each tracked output in memory and runs an LF-normalized
//! content compare against the committed file (CRLF checkouts normalize
//! to LF per the Cross-Platform rule) WITHOUT writing the tree, exiting
//! non-zero (status 6) on any drift and naming the stale file(s). A
//! hand-edit to ANY generated manifest file — or a generator change
//! without regen — makes this test FAIL.
//!
//! The check runs UNCONDITIONALLY: the generator is a cargo bin of this
//! package, so cargo builds it for the integration-test run and exposes
//! its path through `CARGO_BIN_EXE_gen-typeinfo-manifest` — there is no
//! external interpreter to locate and no skip branch.
...
/// LF-normalized content-compare freshness discriminator: run the manifest
/// generator bin in `--check` mode (regenerate-in-memory + LF-normalized
/// content compare, no tree write) and
...
fn typeinfo_manifest_files_are_byte_equal_to_regenerated_generator_output() {
    let bin = env!("CARGO_BIN_EXE_gen-typeinfo-manifest");
    let output = Command::new(bin)
        .arg("--check")
        .current_dir(workspace_root())
        .output()
        .unwrap_or_else(|err| panic!("invoke `{bin} --check`: {err}"));
    ...
    assert!(
        ...
        "the committed typeinfo manifest data is STALE w.r.t. the \
         `gen-typeinfo-manifest` generator bin. The generator is the SOLE \
