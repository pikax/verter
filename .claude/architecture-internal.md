...
└────────┬─────────┘
         ...
         ▼
┌──────────────────┐
│   verter-lsp     │ ← Rust LSP binary: documents, diagnostics, completions
│  (Rust binary)   │
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│   verter_host    │ ← File host, compilation, caching
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│   verter_core    │ ← Template compilation (Rust)
└──────────────────┘
```
...
The LSP is implemented as a standalone Rust binary (`verter-lsp`) in `crates/verter_lsp/`. It communicates with VS Code over stdio.

### Module Structure

```
crates/verter_lsp/src/
├── main.rs          # Entry point: stdio transport, CLI args
...
├── features/        # LSP feature handlers (completions, diagnostics, hover, go-to-definition)
...
```
Completion Request (stdio)
       │
       ▼
┌──────────────────┐
│  verter-lsp      │ ← Rust binary
│  server.rs       │
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│ Find document    │ ← Document tracking
│ in host cache    │
└────────┬─────────┘
         │
         ▼
┌──────────────────┐
│ Feature handler  │ ← completions, hover, diagnostics, etc.
└────────┬─────────┘
         │
         ▼
Completion Response (stdio)
```
