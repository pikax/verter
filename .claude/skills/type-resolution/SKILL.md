...
**Deferred carrier-arg descent (owed before wiring the lowerer).** The structural `type_args` carrier class is BOUNDED, not fully closed. The sites that genuinely descend a carrier's args split two ways: the SCAN/CLASSIFY walkers — the absorb infer-scan, the open-node value-body / enumeration-domain walk, and the `subtree_references` reachability scan (the `build_mapped_type` key-independence hoist) — read them through the shared `SemanticNodeData::carrier_type_args` accessor, while the ALLOWLISTED reconstruction / render / rewrite / identity BOUNDARIES (raise/materialize, display, the substitute re-intern arm, and the structural `eq`/`hash` arms) DIRECT-BIND `type_args` by design. A NEW hand-rolled DIRECT `type_args` bind naming a carrier variant by its FINAL path segment — qualified (`SemanticNodeData::BareRef`) OR bare/imported (`BareRef`), in normal code or inside a `matches!`/other macro body — outside BOTH the accessor AND that allowlist is statically forbidden (`no_new_direct_carrier_type_args_traversal_outside_accessor_or_allowlist`: syn variant inspection over pat/expr structs + macro token-scan, keyed by exact `(path, fn)`). A `use SemanticNodeData::BareRef as Alias; … Alias { type_args, .. }` aliased-RENAME at the path tail is NOT statically caught — it relies on the exhaustive non-wildcard `carrier_type_args` accessor + the dormant-wiring guard + review instead. `is_deferred` is a ROOT-KIND classifier (it recognizes `BareRef`/`ImportType` as deferred-roots by kind, with no arg descent) — NOT an accessor-descent site. A NEW scanner that root-kind-matches a carrier and silently IGNORES its args is NOT statically caught; the defenses are the exhaustive non-wildcard `carrier_type_args` match (a new carrier variant fails to COMPILE at the accessor) plus the dormant-wiring guard `structural_lowerer_has_no_production_caller_until_carrier_resolution`, which pins `lower_type_expr_structural` to ZERO production callers so non-empty carriers cannot reach the still-naive consumer walkers until the descent lands. OWED before the lowerer may be wired: the carrier-arg descent for the `meta_resolve` ref/cycle/dep walkers, the `build.rs` type-param collector, and the carrier-aware exactness / free-type-param classifiers — recorded as the carrier-resolution owed-before-wiring debt in `docs/arch/parselower-design.md` (deferred per lead-architect ruling ARCH-CONSULT-3 LAND-WITH-DEFERRED-DEBT).
