...
The `TypeExpr`→handle migration replaces stored `TypeExpr` bodies on the hot parse/shallow/macro/lazy-body/prepared path with interned graph handles; `TypeExpr` materialises ONLY at compat/output boundaries. Carrier production reach: ALL THREE arg-carrying carriers (`TypeOf`/`BareRef`/`ImportType`) are dormant-producer-only at the node level today. The eager `lower.rs` `TypeExpr::TypeOf` arm RESOLVES typeof EAGERLY and mints no `TypeOf` carrier; the sole production `new_typeof` mint is the query-free structural lowerer (`structural_lower.rs`), and every other `new_typeof` / `new_bare_ref` / `new_import_type` call site is test-only (a `#[cfg(test)]` source module or an integration test under `crates/verter_session/tests/`). That structural lowerer (`lower_type_expr_structural`) has ZERO production callers (held dormant by `structural_lowerer_has_no_production_caller_until_carrier_resolution`), so no carrier flows through production yet. The carrier reduction arms (`TypeOf`) and head-resolution helpers (`BareRef`/`ImportType`) are wired and exercised today by directly-constructed carriers and the dormant producer — the LIVE path resolves eagerly (no carrier node), the DORMANT structural path emits carriers. The demand-time dispatch can resolve a `BareRef`/`ImportType` carrier head (direct-constructed today, or query-subject) through the shared `resolve_*_head` helpers consuming `CarrierResolverContext`. The carrier types:
