...
**Deferred carrier-arg descent (owed before wiring the lowerer).** The structural `type_args` carrier class is BOUNDED, not fully closed. The sites that genuinely descend a carrier's args split two ways: the SCAN/CLASSIFY walkers — the absorb infer-scan and the open-node value-body / enumeration-domain walk — read them through the shared `SemanticNodeData::carrier_type_args` accessor, while the ALLOWLISTED reconstruction / render / rewrite / identity BOUNDARIES (raise/materialize, display, the substitute re-intern arm, the `subtree_references` reachability scan, and the structural `eq`/`hash` arms) DIRECT-BIND `type_args` by design. A NEW hand-rolled DIRECT `type_args` bind outside BOTH the accessor AND that allowlist is forbidden (`no_new_direct_carrier_type_args_traversal_outside_accessor_or_allowlist`, keyed by exact `(path, fn)`). `is_deferred` is a ROOT-KIND classifier (it recognizes `BareRef`/`ImportType` as deferred-roots by kind, with no arg descent) — NOT an accessor-descent site. A NEW scanner that root-kind-matches a carrier and silently IGNORES its args is NOT statically caught; the defenses are the exhaustive non-wildcard `carrier_type_args` match (a new carrier variant fails to COMPILE at the accessor) plus the dormant-wiring guard `structural_lowerer_has_no_production_caller_until_carrier_resolution`, which pins `lower_type_expr_structural` to ZERO production callers so non-empty carriers cannot reach the still-naive consumer walkers until the descent lands. OWED before the lowerer may be wired: the carrier-arg descent for the `meta_resolve` ref/cycle/dep walkers, the `build.rs` type-param collector, and the carrier-aware exactness / free-type-param classifiers — recorded as the carrier-resolution owed-before-wiring debt in `docs/arch/parselower-design.md` (deferred per lead-architect ruling ARCH-CONSULT-3 LAND-WITH-DEFERRED-DEBT).
...
- `SemanticNodeData::TypeOf { value_root, path, type_args }` — deferred typeof
...
- **New `SemanticNodeData` carriers** — `BareRef { name, scope, type_args }` (unresolved bare name; `type_args` holds an applied `Foo<Arg>`'s structurally-lowered arguments, empty for a bare `Foo`), `ImportType { specifier, qualifier, type_args, typeof_query }` (unresolved dynamic-import), `RawFallback { raw }` (the ONLY carrier holding raw type text — display-only, round-trips to `Unknown { raw }`), `ConstructorType { signature }` (constructor-type fidelity), and `SyntheticBinding { id, value_node }`. There is NO standalone `SemanticNodeData::Rest` carrier — a bare `...T` outside a tuple slot is scratch-only (§7.18, guard `solver_scratch_only_nodes_never_enter_semantic_graph_store`); tuple-rest fidelity rides on `TupleElement.rest`, not a dedicated node. `RecursiveRef` stays demand-time-minted as `Opaque(QueryError::RecursiveRef)`.
