...
The `TypeExpr`→handle migration replaces stored `TypeExpr` bodies on the hot parse/shallow/macro/lazy-body/prepared path with interned graph handles; `TypeExpr` materialises ONLY at compat/output boundaries. The Stage-1 foundation is additive and DORMANT (nothing emits the carriers yet — a query-free structural lowerer emits them later, demand-time dispatch resolves them):
