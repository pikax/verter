...
- **`Navigate`** — do the minimum semantic work to continue a requested path. Unwrap aliases transparently (aliases have no independent structural shell; `Navigate(X)` where `type X = Y` returns the same node as `Navigate(Y)`), follow member / index hops already materialized in the graph, reduce closed conditionals, stop at undecidability barriers. Does NOT recursively materialize subtrees; does NOT expand sibling members. Used by path projection to step one hop at a time. **Open-enumeration-domain carrier-stop (L1):** under `Navigate`, the `Pick`/`Omit` object-filter carrier-stop. An object-filter utility (`Pick`/`Omit`) whose enumeration domain (argument 0) is OPEN stays a shallow carrier instead of materialising the source — materialising an open generic source (`Pick<PropsBase<T>, …>` over an SFC `generic="T"`) degenerates into full cross-file generic expansion. Mapped utilities (`Partial`/`Required`/`Readonly`) are NOT in the L1 family — they are guarded by their `MappedType` deferred-shell fail-closed behaviour; `Record` is an index-signature key domain, not finite enumeration. Openness is a bounded typed-IR walk (`raise.rs::utility_enumeration_domain_is_open_or_unknown`): OPEN ⇒ reaches an unsubstituted `TypeParam` / open conditional (`check`/`extends` depends on a `TypeParam`) / open `IndexedAccess`/`KeyOf`/`Mapped` / instantiation over an open arg / unresolved or open-bodied `DeclRef` alias chain / under-applied or unsatisfied `InstantiationRef` / `Opaque` / budget exhaustion; CLOSED ⇒ a finite object surface / concrete instantiation / finite union/intersection / a bounded alias chain terminating at a closed object (`Foo→Bar→{bar:string}`). `infer` is a binding placeholder, NOT an open generic. Closed `Pick<Foo,'bar'>` still materialises path-precisely. The L1 carrier-stop is the PRIMARY defense for this class on the `Navigate` projector reduce route; the per-request projection budget (`request_budget.rs`) is an ARMED-by-default runaway fuse (`projection_op_budget == 0` ⇒ effective cap 2000) — the projection keys plus `Instantiate`/`Conditional` count toward it, and an armed fuse that trips returns `BudgetExceeded` as a genuine partial (refused warm admission). L1 currently guards the `Navigate` route only; open-generic surfaces that recurse on the structural decl-body-lowering route (a deep `extends`-heritage such as `Table.vue`'s `Omit<CoreOptions<T>, …>`) are not yet L1-stopped and rely on the armed budget as the runaway backstop (they resolve degraded) pending a route/mode-independent L1 carrier-stop (a tracked follow-up).
