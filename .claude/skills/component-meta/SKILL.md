...
- **Bounded declaration-body hot-read path (Stage 6 Option B, LANDED).** Internal decl-body reads at the migrated graph-backed publication sites resolve via the shared `decl_body_hot_ref` accessor (over the `SemanticGraphStore` `Instantiate` memo, handles minted at the graph-bearing dispatch producer over the RESOLVING lowerer — not a re-lowering) feeding the session `HotPrepared*` carriers (`NoTypeExpr`-guarded); the public `TypeExpr::Ref` (and the compat DTO surface) stays the materialised output of `materialize_type_expr`. `DeclBodyMemo`, the lower-crate `verter_semantic` `Prepared*` DTOs, and three named residual `TypeExpr` reader classes (authored-shape / graph-free-DTO / graph-backed-PENDING, per `docs/arch/authored-shape-graph-native-migration-deferral.md`) **stay `TypeExpr`** for now (decl bodies still LOWER to typed IR via `DeclBodyMemo`; only the hot READ surface is handle-native).
