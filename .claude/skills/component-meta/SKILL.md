...
See `/type-resolution` for the full carrier set + the output-materialization boundaries (`materialize_output_type_expr` shell-only; `reduce_and_raise_for_projection_output` reduce-then-raise projection output) + the handle-capable-consumer entry rules, and `/type-cache-architecture` for the no-cache-key handle-identity contract.
