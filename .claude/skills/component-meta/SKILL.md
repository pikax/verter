...
- The slot-binding graph publisher's no-parser branch (`publish_merged_bindings` in `crates/verter_session/src/meta_resolve/slot_binding_graph.rs`) mints exactly one shape: the first-class source arm `SemanticTypeSource::SyntheticSlotBinding(Arc<SyntheticCarrierKey>)` (raised through the one dispatch as `SemanticNodeData::SyntheticBinding`, shelling to `TypeExpr::SyntheticSlotBinding`). Parser-path bindings (payload-present) publish the authored payload position and NEVER mint a synthetic carrier.
