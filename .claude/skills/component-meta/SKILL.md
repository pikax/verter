...
**Structural materialisation** has no separate entry or store: published surfaces come from the shared query route reduced by the projector pipeline. The former whole-surface materialiser (a walker with a private in-flight table and its own admission rail) and its `MaterializeStructureDb` slot are deleted; only the wire mirrors (`materialize_structure_*` request counters, `MaterializeStructure*` audit events, `MaterializationScopeAudit`) remain as zero-valued schema pending a wire-contract retirement (`roadmap/0.1.0-tama/decisions/2026-09-14-materialize-structure-wire-mirror-defer.md`). The legacy walker's per-shape materialiser DB is likewise retired — see `tests/cases/g_misc0/no_legacy_walker.rs::RETIRED_SYMBOLS`.
