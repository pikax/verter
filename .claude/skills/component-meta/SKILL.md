...
**Per-member projection** routes through the graph-native member reducer + the per-member slot of `ShapeCacheDb` indexed by `ShapeSubject::MemberValueNode` (built via `ShapeCacheKey::surface_member_value_whole_with_context(scope, &SurfaceMember, context)` — keyed by the sealed `MemberShapeNodeSubject(SemanticNodeId)` newtype over the member's `SurfaceMember.value`): each per-member shape query peeks the cache BEFORE any `raise_node_to_type_expr(member.value)` round-trip; cache writes record the observed `ReadSetSignature.facts` + `validated_at_generation`; warm reads validate both gates before return. This slot is a generation/store-scoped graph-instance memo, NOT a durable content-free R6 query-identity key: single-entry (not multi-candidate), not persistent, fact-validated + generation-gated; warm reuse requires `validated_at_generation` + strict `ReadSetSignature` self-root validation over the exact `SurfaceMember.value` graph instance. The split `MaterializeMemoDb`/`MemberShapeCacheDb` shape stores are RETIRED — the static guard `crates/verter_session/tests/cases/g_block/block_6i_static_guards.rs::shape_cache_db_replaces_split_caches` asserts neither may be re-introduced.
...
- Internal publication carriers become `HotTypeRef` handles into the semantic graph; the public `TypeExpr::Ref` (and the rest of the compat DTO surface) is the MATERIALISED output of the two output-boundary fns — `materialize_output_type_expr` (plain SHELL raise) / `materialize_reduced_output_type_expr` (REDUCE-then-raise for the per-member publication projectors) — not a stored body. The raw `SemanticNodeId → TypeExpr` raise primitive `raise_node_to_type_expr` is MODULE-PRIVATE in `project_semantic_dispatch/raise.rs` (a private INHERENT METHOD, so an out-of-module call is a compile error, E0624 "method is private" — NOT the E0603 used for a private free item / module); the reduce-then-raise orchestrator `raise_and_reduce_with_context` is confined to the `project_semantic_dispatch` dispatch subsystem (`pub(super)`; an out-of-subsystem call is a compile error, E0624). Output/compat seams route through the two boundary fns; the durable wrapper-class output-materialization FENCE (token + sealed carrier) is DEFERRED to block 8-A3 (see `docs/arch/parselower-design.md`), so the current confinement is the E0624 compile fence plus the anti-regression structural tests in `handle_capable_consumer_guards.rs`. The `no_hot_path_materialize_type_expr_bridge` guard polices only the test-only `materialize_type_expr` boundary, NOT the wrapper class.
