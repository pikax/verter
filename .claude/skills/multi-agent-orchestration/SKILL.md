...
For Rev11 unified nodes after trusted-local ORC0, the node's explicit per-role low/medium/high minima/defaults and deterministic admission escalation govern harness effort. The normal path has no manual assessment step; an explicit override can only raise effort. Architecture consultation alone uses the read-only Codex CLI fixed to OpenAI `gpt-5.6-sol` at `xhigh`; implementation, fix, review, verification, and confirmation are fresh provider-neutral harness tasks whose exact task/provider/model/effort/prompt/report bindings are recorded. This is honest-operator local audit evidence, not proof against a malicious repository or filesystem owner.

Two review/fix cycles are the soft maximum. If round two still reports a P0/P1, stop automatic cycling and obtain the neutral Architect's continue/stop decision plus an explicit additional-round cap. Keep reports honest and terse. Remove a disposable agent/train worktree immediately once its result is safely committed or recorded and no longer needed, and report cleanup state.
