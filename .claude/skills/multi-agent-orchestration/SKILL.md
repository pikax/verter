...
**Never**: write/edit production code; run test suite; investigate a bug by reading source; review a diff line-by-line; originate a repository-content fact from memory. The instant the orchestrator does the work, its context fills with detail and it loses the plot.

**One bounded probe.** Before a claim changes a brief, baseline, verdict, disposition, or reopen, the orchestrator may run exactly one bounded read-only probe per pending decision — git metadata, path existence, exact search, or count — with capped output and the input/result persisted. Chained exploration, full-file/diff reading, source interpretation, reproduction, and gate execution go to a diagnostic agent. This is not a licence to investigate; it is the minimum check that stops an unverified premise becoming an instruction.
...
**Decision admission — trust but verify.** Every report states *intent*, not *fact*, and every decision-bearing claim is unverified until admitted. Before a claim changes a brief, baseline, verdict, disposition, or reopen, record it as `VERIFIED` with input-bound evidence (commit/worktree, build identity, command, result, locator) or as `HYPOTHESIS` with the deciding check. A report or review never verifies its own premise — a review finding's factual claim is a hypothesis until independently re-derived, so brief it as "locate and reproduce; if verified, fix; else STOP", never as an established correction. Only verified facts and ratified decisions become unconditional instructions. The orchestrator may run one bounded read-only probe per pending decision (git metadata, path existence, exact search, count); anything chained or interpretive goes to a diagnostic agent. The trigger is the state change, not felt doubt — an optional check never fires when you are confident and wrong.
...
**A plant that fails to apply reports a pass.** When proving a test discriminates by planting a defect, `git diff` the file and confirm the mutation is actually in the source before trusting the run — `perl`/`sed`/`grep` all exit 0 on a non-match, so a zero exit means the tool ran, not that the edit landed. A verification search must prove the planted string is *unique and new*, not merely present: a pre-existing occurrence is a false positive. If the planted tree comes back green, the first hypothesis is that the plant failed — never that the code is correct.
...
- **Intent contract** — the ratified necessity, required/forbidden observable outcomes, authority/fallback order, and performance bounds, with a stable acceptance ID per outcome and its mapped public-boundary test. No brief dispatches without it.
...
- **Verification** — declared gate IDs and the required receipt state. Copied counts and direct/surgical test invocations are diagnostic evidence, not gates: a gate must prove it actually ran its intended surface (`CLAUDE.md` → Verification Must Prove Execution).
...
Output is large — grep the tail to orient, but a grep is DIAGNOSTIC ONLY and never establishes a verdict: `LAND`/`CHANGES`/`__DONE__` appear in prompt echoes and prose, so a keyword scan can hand back a verdict the leg never rendered. A leg whose output artifact you cannot produce did not run: the leg counts only on a clean exit under its timeout, a matching banner, and an output file whose final verdict you READ (`mom-cto-orchestration/reference/PROTOCOL.md` → codex Invocation). Don't run concurrently with a heavy test suite (memory contention). The verdict is authoritative on architecture; still verify surprising findings.
...
- Pausing between approved blocks without a defined evidence gate, or asking "should I continue?" instead of routing the gate autonomously — execute the approved plan through.
- Relaying a reviewer's or your own unverified premise into a brief as established fact — the costliest errors originate in the orchestrator, not the sub-agents.
- Commissioning a third fix cycle on a seam already reopened twice without asking whether the design should exist at all.
- Letting a block land with re-review flagging a real issue as "deferred" without an explicit disposition (`ADOPT-NOW`/`DEFER`/`REJECT`) and a tracked owner.
