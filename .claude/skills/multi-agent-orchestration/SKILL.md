...
Admission freezes the authorized block/train manifest: exact node and scope, base and candidate-start SHA/tree, authority/charter/profile bindings, conflict domains, stop conditions, and ownership. Implementation, fix, review, verification, and confirmation are fresh provider-neutral harness tasks. The node's static per-role low/medium/high minima/defaults plus deterministic authority signals compute each role's effective tier automatically. An optional override may only raise it. Persist the exact task, provider, model, effort, prompt, and report bindings; do not add a manual assessment ceremony.
...
- The parent chooses train order, owns the integration branch and lifecycle lease, schedules gate-bearing reviews/verification/confirmation, accepts transfers, lands, and reports external status.
...
- STOP on missing authority, scope ambiguity, invalid manifest, unexpected dependency, inability to produce discriminating tests, immutable-target change, gate failure, or a requested action outside the brief. Report the concrete blocker tersely; do not invent a workaround or a new train.
...
Create one worktree/branch for the train. At finalization, record one frozen review-target manifest containing the exact base/candidate SHA/tree and worktree. The frozen train/block worktree must remain unchanged for the entire review round.
...
All reviewers inspect the full cumulative diff, tests, scope completeness, fail-closed behavior, performance implications, and anti-rogue integrity. The author never reviews its own work. A profile-required clean current-round set is mandatory; high-risk work therefore requires clean 3/3.
...
4. If fixes are needed, invalidate the round. Dispatch one fix agent to address the whole consolidated set in one comprehensive new commit, including class-wide siblings and discriminating regressions.
5. Freeze the successor target and rerun the complete risk-scaled review set. A changed SHA/tree never inherits a verdict.

Two review/fix cycles are the soft maximum. If round two retains any P0/P1, stop automatic cycling and invoke the neutral Architect with the round-two-cap template. Continuing requires its explicit additional-round cap. If total rounds exceed five, the Architect prompt must additionally recommend and ask whether to split the work into smaller independently reviewable sub-subblocks. P2/P3 follow the owning profile; never silently drop a real finding.

## Verification, landing, and confirmation

Run targeted tests during implementation/fixes and the owning canonical gate only on the final frozen target. Any content, rebase, regeneration, conflict resolution, or tree change invalidates review and gate evidence.
