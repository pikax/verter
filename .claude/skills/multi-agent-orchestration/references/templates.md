...
Before squash or review, transition each included node's predeclared ledger line in authority/state/implemented.toml from pending to implemented, using the single-line form `"ID" = { status = "implemented", commit_message = "...", commit_date = "...", pull_request = N }` (`pull_request` optional) with the planned squash commit message, approximate date with timezone, and optional PR number. These are loose locator hints and must not be validated against Git or GitHub.
