...
- Test-Tightening 80/20 Bound (full rule: `reference/PROTOCOL.md` → Test-Tightening 80/20 Bound): after two auditable substantive TEST-ONLY tightening rounds for the same behavioral test-coverage claim, a third-round decision belongs to neutral/unprimed codex. Applies only to test/assertion/fixture/harness coverage (never production code, rule/plan guards, architecture-guard mechanisms, or Structural-Confinement-First scanners); codex must find all COMMON/contract/prior-defect/severity-bearing paths covered and only EDGE residuals left ("80/20" is a heuristic label, never a numeric alternative to COMMON-case coverage). Normal §1a/review/confirm gates, no-stub/discrimination, production rigor, Structural-Confinement-First, and valid-debt rules still bind.
