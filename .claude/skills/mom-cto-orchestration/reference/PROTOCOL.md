...
Repo root: `/Users/carlosrodrigues/Documents/dev/verter`. Integration branch: `refactor/semantic-db-overhaul`; never commit directly to it. Worktree/branch/train id come from the brief. All edits use absolute paths in the worktree; all build/test commands start `cd <worktree> && ...`. Every sub-agent brief repeats this.
...
```bash
cd <worktree> && cargo nextest run --workspace --no-fail-fast 2>&1 | tee /tmp/mom/<train>/gate-nextest.txt
cd <worktree> && cargo test -p verter_session --tests 2>&1 | tee /tmp/mom/<train>/gate-session.txt
...
Write/overwrite `/tmp/mom/<train>/status.json` at milestones:
