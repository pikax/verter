...
2. Read durable state: `git log <base>..HEAD`, `git status --short`, and for managers review/verdict dirs plus report files.
...
- `PHASE ...`
- `COMMIT <sha> — <what>`
- `DISPATCH <role> mechanism=agent brief=<path> report=<path> input-id=<hash> model=<id> effort=<level>` — EVERY gate-bearing dispatch records these five (gate-bearing roles persist brief+report to files, never inline). Mechanism-specific evidence is EXTRA only and never substitutes for the input identity: `agent-id=<id>` on the Agent path.
- `CAPABILITY mechanism=agent isolation=<pass|fail> identity=<pass|fail> stopcontinue=<pass|fail> childspawn=<pass|fail> result=PASS|FAIL` — recorded once before the first Agent dispatch; absent or stale (inputs changed) ⇒ "unknown" ⇒ STOP/escalate (no fallback).
