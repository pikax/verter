...
Guard: `synthetic_binding_identity_is_content_free`. See `/type-resolution` for the full carrier set + the two output-boundary fns (`materialize_output_type_expr` plain SHELL raise / `materialize_reduced_output_type_expr` REDUCE-then-raise); the raw `raise_node_to_type_expr` primitive is module-private in `project_semantic_dispatch/raise.rs` (a private INHERENT METHOD, so an out-of-module call is a compile error, E0624 "method is private" — NOT the E0603 used for a private free item / module) and the `raise_and_reduce_with_context` orchestrator is confined to the `project_semantic_dispatch` dispatch subsystem (`pub(super)`; an out-of-subsystem call is a compile error, E0624), so output/compat seams route through the two boundary fns. The durable wrapper-class output-materialization FENCE (token + sealed carrier) is DEFERRED to block 8-A3 (see `docs/arch/parselower-design.md`); the `no_hot_path_materialize_type_expr_bridge` guard polices only the test-only `materialize_type_expr` boundary, NOT the wrapper class.
