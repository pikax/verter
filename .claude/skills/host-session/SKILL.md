...
When a carrier script imports through a non-carrier barrel (for example `components/index.ts`), tsserver performs no barrel publication: its plugin resolves already-published framework carriers through TypeScript's configured-project filesystem/module resolver, so rewritten barrel buffers add no capability and only starve the single engine thread. TSGO has no equivalent host plugin and retains explicit publication: it seeds from shallow import facts, walks only `ExportFrom` references, syncs terminal carrier dependencies first, and publishes only barrel projections whose bytes differ from disk. That TSGO walk is cycle-terminated and complete (no depth/node cap may mint a false `DependencyReady` receipt); it yields periodically instead of truncating. Ordinary imports, dynamic imports, `require` edges, and unchanged compiled output remain provider-resolved from disk.
...
**Demand-driven workspace-symbol completeness**: ordinary tsserver hover/completion/definition keep only editor-active/import-observed carriers in the Program. References and rename are different: TypeScript cannot prove a workspace-wide result unless every framework source in the initiating carrier's configured project is represented. On the first such request, Verter reads the immutable configured-project membership, requires a current store advertisement for every Vue/Svelte carrier, and promotes that full project frontier through one `activate_carrier_members` transaction plus one interactive plugin refresh. Only one contentless project bootstrap is opened; all other sources remain plugin-owned external roots. If background publication has not made the frontier complete, the request signals scanner priority and fails closed instead of joining compilation or returning partial references/edits. The activated frontier stays warm for later workspace-symbol requests.
