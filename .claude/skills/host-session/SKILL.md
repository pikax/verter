...
- **KNOWN OPEN — untyped Svelte `$props()` destructuring**: no current producer owns the exact destructured public-key spans. The parse-domain `$props()` call span and the shallow analysis's `$props`-initialized leaf-binding spans cannot supply either structural bound of the destructuring pattern, so rename derives NO construct hull from them. Until a producer-owned destructured-key span inventory exists, these cursors follow the ordinary classifier: a shorthand key may be treated as its local binding; an aliased public key with no name anchor may pass through to the provider as `NotChildProp`; and a key sharing a name with an unrelated import may be misclassified as that import and produce a native same-file edit. This is an explicit unsound known-open hole, pinned by the `$props()` rename tests rather than hidden behind an unprovable refusal.
