...
**Post-scan completion** (`VerterLanguageServer::complete_post_scan`, `background_init.rs`): the post-scan drain advances the diagnostics generation of every carrier it settles, after their earlier publications completed, so each settled open document is republished there. Pending provider work gates only what it actually blocks: a document that is itself still queued in `pending_snapshot_provider_sync` is skipped (no completion receipt), and `typeProviderSyncComplete` stays suppressed while ANY retry is queued or once the init generation is superseded — but an unrelated pending carrier never starves the healthy open documents of their refresh.
