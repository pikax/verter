...
- `SemanticGraphStore` — host-owned semantic-query memo, dispatched through `ProjectSemanticDispatch::execute`. The canonical lazy semantic layer and sole authority for reusable type-resolution work. Two parallel memos:
  - **Node memo** (mode-erased `FamilyKey` → `FamilySlots`) for single-node queries (`ResolveDecl`, `Instantiate`, `KeyOf`, `MappedType`, `Conditional`, `ProjectPath`, `TypeOf`, `ReduceUnion`, `ReduceIntersection`, `ResolvedNamedType`).
...
**Post-scan completion** (`VerterLanguageServer::complete_post_scan`, `background_init.rs`): the post-scan drain advances the diagnostics generation of every carrier it settles, after their earlier publications completed, so each settled open document is owed a fresh publication there. `background_init` owns NO diagnostics publisher: both its post-init and post-scan steps call `rearm_open_document_diagnostics`, which retires each document's completion receipt and hands it to the debounced coordinator with receipts stamped in MRU order. A sweep that pulled inline served the open documents one after another in arbitrary order (tens of seconds after a restart that replays every editor) while discarding the publication the coordinator already had in flight for them. Pending provider work gates only what it actually blocks: a document that is itself still queued in `pending_snapshot_provider_sync` is skipped (no completion receipt), and `typeProviderSyncComplete` stays suppressed while ANY retry is queued or once the init generation is superseded — but an unrelated pending carrier never starves the healthy open documents of their refresh.

**One meaning per readiness notification.** `$/verter/typeProviderSyncComplete { gen }` is sent ONLY by `complete_post_scan`, with the init generation, and means level 2 of the readiness ladder. The per-publication ping an editor-owned TypeScript service needs (refresh your view of the carrier store) is the separate, payload-free `$/verter/carrierStoreChanged`, sent after every coordinator sync and store write. They were once one method: the ping carried an unrelated content generation as `gen`, so every gate keyed on level 2 (the VS Code E2E harness, dx-harness startup gate) was released by the first carrier write. Init generations also restart at 1 with each server process, so a log-scanning gate must only read lines written after the restart it is waiting on (`e2e/lib/typeProviderSyncLog.ts`).

**Watched dependency changes** (`handle_did_change_watched_files`, `refresh_open_importers_after_disk_change`): a disk create/change/delete of a file that is not open in the editor (a) reads the transitive importer closure BEFORE applying the change — an unresolved import is reachable only through the extension-stripped stem that query unions in (`EdgeStore::remove_file` removes the file's OWN outgoing edges and keeps the reverse buckets that name it as a target, so its importers stay reachable after a delete and a later re-create); (b) applies the `WorkspaceChange` to the VFS whether or not the host ever tracked the file, so a cached presence/absence answer moves with the disk; then (c) once the provider state has committed (after the awaited delete/carrier work, or at the end of the spawned non-carrier resync) re-records every importer's import edges and, for the OPEN ones only, advances the diagnostics generation, drops the cached document half, and arms a diagnostics-only publication — which retires the prior completion receipt. Closed and unrelated documents are never scheduled; an editor-open file ignores disk events entirely. Non-carrier changes are also FORWARDED to the provider before the importer refresh (`forward_watched_files_to_provider` → `TypeProvider::notify_watched_files_changed`, default no-op): an owned TSGO has no file watcher of its own under Verter, declares `workspace.didChangeWatchedFiles.dynamicRegistration`, and only re-reads a changed or deleted dependency when it receives `workspace/didChangeWatchedFiles`. Carrier paths are excluded — the carrier store is their authority.

**Retries are for states a retry can change.** `pending_snapshot_provider_sync` gates `typeProviderSyncComplete`, so an entry that can never settle suppresses level 2 for the whole session. Three such states are terminal instead: a non-carrier script with ownership ready and no nearest config (`SyncOutcome::Terminal`); a carrier publication that carries no `CarrierIde` companion (nothing to activate — only a source change can produce one, and that re-enters through the ordinary sync); and a content-transition heal whose generation did not move (`record_content_transition_at` is strictly increasing, so a heal is always observable). A carrier whose IDE output is cold is COMPILED by the gateway (`ensure_ide_compiled` then `get_ide`) rather than published API-only: publishing the narrower set would shrink the carrier's advertised companions and strand the open document's IDE member.

**TSGO owned provider: one ordering barrier per API query.** The `--lsp` and `--api` sides are separate connections; an API query must observe every editor write already sent on the LSP side. Writes only mark `LspWriteBarrier::note_lsp_write`; `settle_lsp_writes` runs lazily before an API query, at most once per batch of writes, and the barrier request is the syntax-only `textDocument/foldingRange` — a diagnostic pull there costs a full semantic check per write.

**SHARED (editor-owned) overlay injection uses the same kind of barrier.** `CarrierInjectionChannel::sync_overlay` awaits `CARRIER_SYNC_BARRIER_METHOD` (`textDocument/foldingRange`), the only request the relay allowlist admits besides `custom/initializeAPISession`. Known limit: injection is still one control round-trip per overlay on a strictly serial control channel, and the editor's engine rebuilds its program per injected file (~240 ms each on the parity fixture). After a workspace scan the import closure of one open file is ~80 overlays, so the first request can exceed `SHARED_OVERLAY_TIMEOUT` (20 s) and activate the sticky managed fallback. The fix is batched injection (N `didOpen`s, ONE barrier), which needs a control-protocol addition. The shared engage path logs its timings at debug (`verter_lsp::tsgo`).

**Reverse edges mirror the importers' forward state** (`verter_workspace::exact_resolution::EdgeStore::remove_file`): removing a file as an OWNER retracts only its own forward edges. The buckets naming it as a dependency TARGET stay, because its importers still name it; each importer retracts its own entry when its edges change or it is removed. This is what keeps `affected_canonicals` correct across the ordinary editor ordering — importer recorded first, dependency then created or changed on disk (its watcher resync is a `host.remove` + reload) — where re-recording the importer's unchanged edges is an idempotent no-op that could never restore a dropped bucket.

Publication admission reserves its epoch before capturing document/host snapshots. A suspended old capture cannot invalidate newer work when it resumes.
...
**The node release is gated on activity.** Semantic node ids are read by id from many places, often twice within one computation (classify, then take a shape-specific accessor). A released node reads as `Opaque(Miss)`, so the switch must never land between two reads of one computation. `VerterHost::evict` therefore drains the memo immediately (safe: readers hold results by `Arc`) and queues the payload release with the arena size as a watermark; the queue is applied only when no `SemanticActivityGuard` is alive (`project_type_store::semantic_activity`, a Dekker pair on two `SeqCst` atomics), inline when nothing is in flight, otherwise by the last guard to drop. Nodes interned after the close (the reload) are above the watermark and stay. In `verter_lsp` the host is reachable only through `documents::SharedHost`, whose sole accessor returns a guard-carrying `HostRef`; a borrowed `&VerterHost` cannot outlive its guard, so every host call is covered by construction. New host readers outside `verter_lsp` (an embedder calling host methods concurrently with closes) must take `VerterHost::semantic_activity()` per call.

**Node ids are monotonic, node storage is chunked.** A `SemanticNodeId` is never reused (memo entries, relation proofs and the close watermark hold ids), but the arena stores slots in 256-id chunks keyed by `id >> CHUNK_BITS` and drops a chunk once every slot in it is released, so its memory follows the live set rather than the ids ever handed out (`node_slot_count` is the id space, `node_storage_slots` the physical slots the retention snapshot reports as `semanticNodeSlots`). A released id reads as `Opaque(Miss)` whether its chunk is gone or not. The relation-proof and relate-key tables are sparse ordinal maps (`semantic_query_memo/intern_table.rs`) for the same reason. `$/verter/getStatistics.retention.heapInUseBytes` is the platform allocator's exact in-use figure (HeapSummary / mallinfo2 / malloc_zone_statistics), the churn lane's retention instrument for the server: unlike RSS it carries no allocator settling.

**Framework surface DTOs are windowed per owner.** `FrameworkSurfaceStore` (the per-content props/emits/slots cache behind `vue_macro_dtos_with_ctx`) keeps the newest `CONTENT_VERSIONS_PER_OWNER` content versions per owner file, refreshed on every warm read; an older version's entries are evicted on insert and recomputed on demand. Before the window every edited version kept its full normalized surface for the life of the session.
...
1. **Latest-value SyncCoordinator** (`sync_coordinator.rs`): dispatch order is part of the contract, because a restart replays every open editor at once. (a) ONE document per dispatch — the provider sync is awaited inline, so a batch taken at once is a batch nothing can overtake; deposits are absorbed before every choice. (b) The choice is the document the USER touched most recently: `PendingSignal::user_received_at` (set by `signal`, never by `signal_diagnostics_only` / semantic-ready / refresh / importer / init re-arms, which restamp `received_at` for every open document) maxed with a live `touch` (`SyncCoordinatorHandle::touch`, fed by `touch_mru` on every interactive request); background-only work follows, newest first. (c) Provider diagnostic pulls are bounded (`max_inflight_diagnostics`: tsserver-family 2, TSGO 8): a pull handed to the provider sits in a queue this side cannot reorder, so its depth is the floor on how long the next document the user turns to must wait. With every slot taken no timer is armed; a `PullSlot` guard frees the slot on completion AND cancellation and wakes the loop. `didChange` performs no provider I/O. It commits the registry/host snapshot, replaces the file's pending signal in a map, and best-effort wakes a capacity-one channel. The single coordinator debounces the latest value per canonical file; edits cancel only detached stale diagnostic tasks, never a provider-state commit that may be mid-transaction.
2. **Version-fenced staged push diagnostics**: LSP uses push diagnostics exclusively (no pull/`diagnostic_provider`). After the quiet window, the coordinator computes the provider-free Verter/ownership batch and publishes it immediately if URI, source identity, and exact document version still match. Provider diagnostics run afterward on a cancellable detached task and replace that batch with the merged result under the same exact-version/source fence. A slow or hung provider therefore cannot starve Verter-owned errors/hints, while a newer edit cannot publish an older batch. Completion of optional native semantic enrichment broadcasts a versioned `SemanticReady` event that invalidates only the Verter diagnostic cache and schedules a diagnostics-only pass: it never repeats provider file sync or graph refresh. Broadcast lag recovers by scheduling every open document once; channel closure terminates the coordinator instead of spinning.
   ...
   **Cross-file importer re-arm** (`arm_open_importer_republish`): after a tick processes a settled REAL edit (`requires_sync`), it arms a debounced diagnostics-only republish for every OPEN importer in the edited file's transitive reverse closure (`WorkspaceRead::affected_canonicals` — the R22 read-only consumer; never cache invalidation). A child edit moves neither the parent's document version nor the parent's diagnostics epoch, so the arm ADVANCES that epoch (`VerterHost::bump_diagnostics_generation`) and drops the importer's cached document-half entry. The advance is the fence, not the drop: the drop cannot stop a parent computation already in flight from landing its pre-arm write after it, and with version and epoch both unmoved every later read warm-hits that write. Every computation stamps the epoch it sampled at entry into its write and every read re-validates against the live value, so a computation that began before the arm can never satisfy a read after it. That requires the advance to be OBSERVABLE for every open importer — including one with no `ProfileState` row (never compiled, or the row dropped by the `set_workspace` / `close` authority-reset cascade) and one flagged evicted (a close/reopen pair leaves the flag set, because a byte-identical reopen takes the `upsert` quintuple-unchanged fast path): the epoch is reported independently of that flag, `evict` advances it, `bump_diagnostics_generation` materialises the row for any canonical the host knows, and the LSP stores the epoch as `Option<u64>` so "none recorded" can never collide with a real stamp. This is what makes a parent re-report/clear `verter/unknown-prop` and provider type errors when a child's prop surface changes. Fan-out bounds: once per settled quiet window (rides the tick, never `did_change`), open-editor count only, pending-map coalescing, `requires_sync: false` on the armed signal (no provider sync, warm IDE refresh; the tick appends to `settled_edits` only for a signal carrying `requires_sync`, so armed publishes never re-arm further — no cascade).
   tsserver diagnostics come from the three synchronous pull commands, issued as one ordered background transaction under one editor-idle admission; the pull succeeds only when the semantic, syntactic and suggestion responses ALL succeed, so a failed category fails the whole pull (an error, never a partial or empty success) while interactive preemption restarts the idempotent transaction. Its pushed `semanticDiag` / `syntaxDiag` / `suggestionDiag` event bodies carry a file but no ScriptInfo version in supported TypeScript protocol versions, so those events are progress/health signals only and never enter the result cache. No earlier pull is ever reused as a fallback after a transport or category failure: unchanged local source bytes do not prove that imported dependencies are unchanged, so a failed pull yields no completion receipt and the next scheduled publication pulls again. The LSP's exact authored document-version plus immutable source-identity fence remains the final publication authority, including same-version edits and close/reopen ABA.
...
**Rename fail-closed until ownership is authoritative:** hover / definition / completion / references / diagnostics may serve from a single resolved owner, but a provider rename covers only that one project. A symbol that escapes the owner (exported + imported by a sibling configured project) would rename partially; escape detection needs the cross-project fan-out (future block). `handle_rename` / `handle_prepare_rename` therefore FAIL CLOSED (a clear `verter:` error, no `WorkspaceEdit`) through the shared `rename_request_admission` gate (`server/rename_plan.rs`) over the tri-state `carrier_multi_claimancy` snapshot decision (`server/provider_state.rs`). A missing root or a non-authoritative bootstrap root returns `NotReady` without inferring uniqueness from absent claimants. A rebuild leaves the previous ready root published and rename may continue against it while the provider still uses that same generation. Immediately before background init assigns the rebuilt provider authority, `OwnershipGenerationFence` advances the authority-assignment marker; admission then returns `NotReady` until the rebuilt root is republished, while an already-admitted request revalidates its reusable `OwnershipGenerationWitness` after provider work and refuses if the marker/root pair moved. A matching pair establishes only that assignment of that generation's configured-owner authority was observed and that generation's root was republished. It does NOT establish that `resync_open_files` completed successfully or that every open provider file was rebound to that authority: a coalesced follower can return before the sweep finishes, and resync errors are discarded. The fence therefore narrows rebuild windows but does not close the remaining per-file rebinding window; an overlap still returns the terminal multi-project refusal, while a non-overlap merely passes this admission check. No other per-file feature consumes the rename-only classification, but the witness primitive is feature-neutral and reusable by other provider-await proofs.

**Never-ready limitation:** rename admission can observe only whether an authoritative root and the authority-assignment marker name the same generation. `spawn_background_init` records no running/failed terminal state: a background error is logged (and a config-discovery panic also sends a warning), while an init with no roots can return without ever publishing readiness. The gate therefore cannot distinguish "still initializing" from "failed or never started", and the client performs no automatic rename retry. Its refusal states only that authority is unavailable and advises retry after initialization, then reload/log inspection if authority remains unavailable; it does not claim initialization is definitely still progressing.
...
- **Classified public component props refuse before provider rename** (`RenameTargetClass::PublicComponentProp`): a prop cursor is positional, never inferred from provider non-emptiness. Vue macro/runtime prop fields, Options-API prop keys, and component-usage prop-name spans come from the existing shallow analysis; the usage check intentionally does NOT require child resolution, so an unresolved prop-shaped cursor cannot fall through `NotChildProp`. Svelte typed `$props()` members and legacy `export let` names use the existing public-API projector source map, whose mapped runs are byte-verified prop-name anchors only. Prepare projects this class to `Decline`; direct rename returns the clear incomplete-cross-file-proof `RequestFailed` error immediately after the shared resolution, before current-file provider synchronization and before `get_rename_locations`. No `WorkspaceEdit` is constructed. The reason is workspace completeness, not a discovered-parent count: every current reverse edge, open-document set, configured project, and provider graph is a positive cache and cannot prove that no unseen parent exists. The late `gate_cross_file_child_prop_rename` also unconditionally drops any `Confirmed` result as defense in depth.
- **Untyped Svelte `$props()` destructuring is exact-span fail-closed**: the Svelte script-fact producer carries every `$props()` call's exact `public_keys`, `local_bindings`, and `has_rest` inventory into `rename_plan.rs`. A cursor inside an exact public-key span always refuses; a local-only binding refuses only for an open/rest call, so a closed aliased local binding remains renameable. Classification never derives a construct hull from `call_span`, source order, spelling equality, or a widened miss fallback. The same exact public-key spans also guard the EMITTED native transaction: if the native same-name occurrence scan proposes any edit intersecting a public key — for example an unrelated import with the same spelling — the WHOLE rename refuses before a `WorkspaceEdit` is built, so it cannot silently change the component API. `resolve_svelte_script_facts == None` is not itself a refusal because the producer also returns `None` for an ordinary Svelte file whose capture inventory is empty (including a file with no `$props()`); those files retain the ordinary typed rename path.
...
- Every `FileArtifactStore` keyed insert/replace/evict/GC/per-canonical-retention sweep and every augmentation-index mutation bumps `artifact_generation`. `IndexedReady` is the single per-file artifact: route facts come from the current-content `IndexedReady` surface (no separate route-owned artifact), gated by `indexed_surface_is_current` and exact resolution facts/store-view fencing. `project_generation` identifies project configuration, environment, identity, and authority resets; route publication/restoration advances its exact resolution witnesses without changing project shape. A route-resolution mutation drives an edge-refresh materialise that reuses the content-addressed payload and rebuilds the route surface.
