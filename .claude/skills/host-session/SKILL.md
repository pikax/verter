...
**Watched dependency changes** (`handle_did_change_watched_files`, `refresh_open_importers_after_disk_change`): a disk create/change/delete of a file that is not open in the editor (a) reads the transitive importer closure BEFORE applying the change — removing a target drops its reverse-dependency bucket, and an unresolved import is reachable only through the extension-stripped stem that query unions in; (b) applies the `WorkspaceChange` to the VFS whether or not the host ever tracked the file, so a cached presence/absence answer moves with the disk; then (c) once the provider state has committed (after the awaited delete/carrier work, or at the end of the spawned non-carrier resync) re-records every importer's import edges and, for the OPEN ones only, advances the diagnostics generation, drops the cached document half, and arms a diagnostics-only publication — which retires the prior completion receipt. Closed and unrelated documents are never scheduled; an editor-open file ignores disk events entirely.
