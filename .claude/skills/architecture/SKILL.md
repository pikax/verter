...
Plugins transform parsed SFC items into TSX. Each can hook into `pre`/`post` phases, transform specific `ScriptTypes` via `transformXxx` methods, and add items to `context.items` for downstream plugins.
