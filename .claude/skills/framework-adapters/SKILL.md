...
| `mcp_routing_has_no_hardcoded_vue_gate` | no hardcoded Vue gate in the `verter_mcp` tool surface: no executable `.vue`/`"vue"` GATE (`.is_vue(`, `.vue`-suffix classifiers, `.vue`/`"vue"` equality + `matches!`, bare `"vue"` language-id classifiers, `.vue.ts/.tsx/.jsx` literals) in the carrier-NEUTRAL analysis tools, so `.svelte` reaches parity — outside the function-scoped Vue-intrinsic tool allowlist (Provide/Inject, migration targets, Pinia/Vuex store analysis, Vue-lifecycle SSR) / test / comment / `is_svelte()` |
| `session_resolution_routing_has_no_hardcoded_vue_gate` | no hardcoded Vue gate in the `verter_session` resolution/routing tree (scan scoped TIGHT to `resolver_core/`, where a carrier-neutral `.vue`/`is_vue()` gate is a genuine parity bug — the fallthrough / child-resolution carrier gate is carrier-generic): same banned `.vue`/`"vue"` GATE shapes, outside the Vue-MACRO-intrinsic `direct_macro.rs` / test / comment / `is_svelte()` allowlist |
