...
- **A refusal's stage is the space the refusing stage was handed, recorded PER STAGE by the runner that handed the bytes over** (`CascadeStageSpaces`). It is not derived from one earlier stage's rewrite flag, and not from the output state: every stage can rewrite on its own, so the CSS-Modules stage's answer is not the scoped-selector stage's answer (that stage parses the modules output), and a refusing plain-CSS-only stage also clears the output after reporting.
- **A cleared output is a refusal, not a rewrite that emitted nothing.** The runner's output state is the three-way `CascadeOutput` (`Passthrough` / `Rewritten` / `ClearedByRefusal`), and a wiped output mints `QualifiedStyleResult::refused(...)` — `is_refused()` true, no bytes, and no claim that a rewrite produced them. Deriving "was this rewritten?" from "are there owned bytes?" labelled those zero bytes `FrameworkRewritten` + `StyleProducer::Verter`. `cascade_output_is_publishable` reads the recorded refusal rather than `code().is_empty()`, which is a shape a legitimately-empty authored block shares.
...
- Consumers pick their map FROM that stage. `compile::push_style_diagnostic` shifts a span by the block start only for an `Authored` span; a later-space span anchors on the block, because no later-space → SFC map exists at that boundary and the authored arithmetic would place it past the construct it names. The VS Code client's `preprocessorDiagnostics` applies the same rule.
- **The cascade never infers its input's provenance.** `run_vue_style_cascade_verified` / `transform_vue_style` take a `CascadeInput` (`Authored`, or `Preprocessed(StyleProducer)`): preprocessed plain CSS and authored plain CSS are the same bytes, so only the caller knows. `standalone::apply_selected_runtime_styles` reads it from `RuntimeBlockContentInput.producer`, which the host fills from the supplied artifact's processor identity — never from a dialect comparison, which missed every tool that ran over an already-plain-CSS block. The authored-`v-bind()` stage still runs on preprocessed bytes: a preprocessor leaves `v-bind()` in its output for exactly that stage.
- `prepare_supplied_style(PreprocessedStyle<'_>)` is the sole admission point for caller-preprocessed bytes. The witness borrows the bytes (no per-block copy at admission) and carries the producer; it is minted only by `QualifiedStyleResult::as_preprocessed` or `PreprocessedStyle::admitted` (the boundary that accepted the tool's output). `as_preprocessed` gates on all three recorded facts, because the witness asserts all three: stage `Preprocessed`, an EXTERNAL producer, and not a refusal. `QualifiedStyleResult::refused` is public, keeps the refusing stage's stage and sets the producer to `Verter` over zero bytes — a stage-only gate mints a witness from it and `prepare_supplied_style` admits a silently empty stylesheet no preprocessor ever produced. What the signature enforces is exactly this: bytes cannot arrive without a stated space AND a stated producer — `&str` is neither, pinned by `tests/cases/compile-fail/prepare_supplied_style_rejects_unqualified_bytes.rs`. Read that as a rejection of the SPELLING, not of the assertion: `PreprocessedStyle::admitted` and `StyleProducer::ExternalAnonymous` are both public, so a caller willing to assert provenance can always mint the witness — which is exactly what the pre-process admission boundary needs. It does NOT prove the bytes left SCSS behind; plain CSS is a subset of every dialect this crate parses, so no grammar check could. That assertion belongs to the admitting boundary, made once, with the tool's identity attached. The tool's version, config fingerprint and reported diagnostics stay on `SuppliedContentArtifact`, where this host's consumers read them.
- `CssDialect::from_lang` is the single authority for the `lang="…"` spelling → `CssDialect` identity, and `CssDialect::requires_external_preprocessing()` for "is this already CSS". `LANG_SPELLINGS` is its table: byte-exact `scss`/`sass`/`less`/`stylus`/`styl`, so `lang="SCSS"` fails closed — nothing downstream can compile it. `css` is the ONE case-insensitive spelling, on both this owner and the carrier parser's `StyleLang::from_bytes`: plain CSS reaches no preprocessor table at all, so no downstream tool can fail on its casing, and refusing `lang="CSS"` would cost an unambiguously-CSS block every CSS diagnostic, hover and colour the editor serves while buying agreement with nothing. `a_case_variant_css_style_lang_stays_native` pins it against the `lang="SCSS"` control that keeps the widening step from returning. Every route reaches that owner with the bytes as authored: `block_content::role_lang` reports a STYLE block's `lang` verbatim, `StyleAnalysisLang::from_lang` classifies it, and `native_language` / `optional_native_style_preprocessor` ask that classification rather than a list. Byte-exact reporting is scoped to the style role, because it is the role whose owners are byte-exact: `role_lang` keeps the ASCII-lowercased spelling for template/script/custom blocks, whose own classifier (`ScriptLanguage::from_bytes` → `sfc_script_dialect`) resolves an unrecognised spelling to a language rather than refusing it. Reporting those verbatim made block-content the ONLY route calling `<script lang="TS">` non-native — demanding preprocessed content for a script no preprocessor claims — while every other route still typed and served it. `carrier_style_dialect` is the matching single owner for the carrier-level classification, shared by the Vue and Svelte arms of `project_style`. The carrier parser's `StyleLang::from_bytes` is the deliberately WIDER universe: it also names `postcss`, which has no native grammar here. The two answers differ on purpose — the editor's CSS intelligence serves a `lang="postcss"` block while the rewrite pipeline refuses it — and collapsing `postcss` into "unrecognised" made both answer the same way, so a mainstream Vue configuration was served nothing at all. `style_stage_identity.rs`'s `the_dialect_owner_accepts_exactly_what_the_carrier_parser_classifies` pins both the agreement and that one named divergence.
