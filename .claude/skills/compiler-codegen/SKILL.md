...
### Script-setup unused-binding liveness (`ide/script/setup.rs`)

The `<script setup>` lowering exposes setup bindings to the template through a `___VERTER___unwrapped = ___VERTER___shallowUnwrapRef({ … })` object plus a block-scoped destructure. Whether a setup binding PARTICIPATES in that object + destructure decides whether TypeScript considers the user's original `const foo` declaration "used":

- A binding USED somewhere emits a value-read entry `foo: foo as unknown as typeof foo` (TSX) / `foo: foo` (JSX) AND a destructured local `const/let { foo } = ___VERTER___unwrapped` — the value read keeps the decl live (correct; no false TS6133) and the template can read the unwrapped local.
- A binding PROVEN-unused is **OMITTED ENTIRELY** from the unwrapped object AND the destructure block. The user's original `const foo` is then its sole remaining occurrence, so TypeScript surfaces TS6133 ("declared but never read") at the SOURCE decl's MAPPED range. Template IntelliSense / `shallowUnwrapRef` typing is unaffected because an omitted binding is, by definition, referenced nowhere (template included), so nothing reads its unwrapped property.

**Why omission, NOT a type-only entry (the position bug).** A type-only entry (`foo: undefined as unknown as typeof foo`) does NOT work: `typeof foo` is itself a TYPE-QUERY REFERENCE to `foo`, so it keeps the SOURCE decl live and TypeScript never flags it. The TS6133 instead falls on the SYNTHESIZED `const { foo } = ___VERTER___unwrapped` destructure copy, which has NO per-binding source mapping — so the diagnostic collapses to line 1 of the `.vue`. Omission is the only shape that leaves the source decl genuinely unused and lands TS6133 on its mapped declaration line. When EVERY setup binding is omitted, the destructure reads nothing from `___VERTER___unwrapped`, so a `void ___VERTER___unwrapped;` keep-alive is emitted (inside the block scope) to avoid a spurious TS6133 on the temp; when at least one live binding is destructured, the destructure already reads the temp and the guard is omitted (so the common-case output is byte-identical to before).

**Conservative safety invariant — `unknown => used` (CRITICAL for soundness).** Omission is allowed ONLY on a PROVEN-unused binding. The gate is `all_sources_complete && !used` where `all_sources_complete = template_used_vars.is_some() && style_usage_complete && script_complete`: if usage cannot be determined COMPLETELY and SOUNDLY across template + script + style, EVERY binding keeps its value-read entry (is NOT omitted) and NO TS6133 fires. Every usage source must be COMPLETE or mark itself INCOMPLETE so the gate fails open. A false negative (a real unused binding left undiagnosed) is the correct tradeoff for a diagnostic gate; a false positive (a used binding flagged unused) is a regression strictly worse than the original no-diagnostic bug.

"Used somewhere" is a typed-IR/AST union, NOT a string scan, drawn from three COMPLETE/SOUND sources:

- **Template** (`IdeScriptOptions.template_used_vars`): expression bindings ∪ v-for/v-slot references ∪ component-tag candidates, built in `compile/mod.rs` via `template_expr_overlay::collect_template_used_vars` (which returns `(used, complete)`). It is built from the **`ide_completion = false`** overlay — NOT the IDE template-codegen lane's `ide_completion = true` overlay, which `BindingContext::completion_prefixes` intentionally SUPPRESSES references for (so a used binding would mis-report as unused). This abandons the zero-extra-parse optimisation: liveness builds/reuses the `false` overlay (an extra parse in IDE-only compiles is the accepted correctness cost). Component-tag matching uses the shared `component_tag_candidates` helper — `<my-comp/>` counts `my-comp`, `MyComp`, AND `myComp`; member-style tags count the root. **EVERY liveness-feeding reference path routes through the ONE complete `oxc_ast_visit::Visit` collector (`bindings/setup_refs.rs`), never a hand-rolled partial walker** — this is the structural invariant that prevents a `_ => {}` arm from silently dropping a reference inside a nested callback / statement family and omitting a USED binding. Concretely: (a) main template expressions (interpolations, v-if, directive values, dynamic args) draw references via `collect_expression_free_ref_spans(expr.expression)` over the parsed expression — NOT the runtime `BindingVisitor` binding facts (whose `_ => {}` arms on `visit_statement`/`visit_expression` dropped a `switch` inside an IIFE, etc.); (b) v-for source refs and v-slot default/type refs are owned NAMES on `VForWithBindings::liveness_reference_names` / `VSlotWithBindings::liveness_reference_names` (`Vec<String>`), populated at extraction time by `collect_expression_free_refs` + `collect_pattern_default_free_ref_names` (value defaults) + `collect_type_references` (type annotations). Liveness carries NAMES (not spans) so it NEVER depends on the partial wrapped→file-relative span shift (`span_shift.rs::shift_default_expression_spans` ALSO has the `_ => {}` callback-body gap — it would leave a callback-body span off by the wrapper byte). The complete collector RETAINS global-named identifiers (`{{ Date }}` / `v-for="x in Date"` / `#default="{ row = Map }"` for a `const Date`/`Map` that shadows the JS global) — the `is_global` filter is a completion/`_ctx`-prefixing heuristic that must NOT gate liveness — while still excluding genuine v-for/v-slot scope locals (filtered by name; the directive locals are declared outside the source/default expression). The runtime `references` field keeps the `is_global` filter and its partial walker (correct: a genuine global must NOT be `_ctx`-prefixed, and runtime never collects callback-body refs). The partial `collect_expression_reference_spans_for_liveness` / `collect_pattern_reference_spans_for_liveness` / `BindingExtractionResult::liveness_reference_names()` are DELETED. **Empty interpolation is COMPLETE, not a parse error:** a whitespace-only interpolation (`{{ }}`) is short-circuited to an empty expression in `parse_expression` (`source_slice.trim().is_empty()`), so it references nothing and does NOT mark liveness incomplete — the gate stays closed and a genuinely-unused binding is still omitted. **Per-expression parse-error fail-open:** if ANY template expression fails to parse (`expr.errors.is_some()`, e.g. `{{ count + }}`) or any v-for/v-slot expression `has_errors()`, the result is INCOMPLETE; the caller maps incomplete → `None`. An SFC-level tokenizer error (`has_parse_errors`) and a template-less SFC also yield `None`. `None` ⇒ fail open.
- **Script body** (`collect_setup_binding_refs`, in `verter_parser`): a COMPLETE `oxc_ast_visit::Visit`-based free-reference collector (`bindings/setup_refs.rs`) — it visits member roots + computed keys of assignment/update targets (`foo.value++`, `foo.x = y`, `foo[key] = y`), destructuring defaults, class bodies, static blocks, labeled statements; nothing is silently skipped. It models lexical shadowing with a scope-frame stack (program frame intentionally empty so top-level decls don't suppress their own references — `const doubled = computed(() => count.value)` correctly marks `count` used). `record_use` is scoped to `setup_names` and applies NO `is_global` filter (a setup binding named like a global shadows it — `const Map = ref(0)` used via `Map.value++` stays live). **Script-recovery fail-open:** `setup.rs` derives `script_complete = parser_ret.errors.is_empty()`; on a `<script setup>` parse error OXC hands the collector an empty/recovered program that UNDER-COUNTS references, so the gate adds `script_complete` and fails open. The shared `collect_expression_free_refs(expr)` reuses the same `Visit` machinery with no name filter for the style scan.
- **Style** (`IdeScriptOptions.style_v_bind_vars` + `style_usage_complete`): SOUND `v-bind()` usage parsed from the raw `<style>` bodies by `compile/style_usage.rs` — each `v-bind()` expression is parsed and run through the COMPLETE `collect_expression_free_refs` collector (NOT the partial `collect_expression_references`, which dropped global-named roots, assignment LHS targets, and `_ => {}` construct families; NOT the host's `.split('.')` heuristic). So `v-bind(obj[key])`, a spread, and an LHS ref are all captured. A single unparseable `v-bind()` flips `style_usage_complete = false` ⇒ fail open.
