...
- **Style diagnostics have ONE publication route: `outcome.result.diagnostics()`.** `facts.refusals` and `stage_failures` are each authority's own record of what it reported; the carrier is where those records are projected into the shared vocabulary exactly once. `StyleRewriteFailure::to_diagnostic` is crate-private so no consumer outside `verter_compiler` can mint the second shape from the first and report every refusal twice.
