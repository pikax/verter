...
- **CAT0-AC1 — ownership contract:** enumerate every in-scope outcome, consumer and displaced production route. Each outcome/consumer has exactly one implementation owner; each displaced production route has exactly one later production-capable deletion/rejection owner. Bind owners to existing DAG node IDs, valid successor paths and receiving acceptance IDs under contracts/successor-charter-quality.md. The contract-owned schema/validator must reject missing inventory members, unknown/pathless owners, missing receiving acceptance IDs, conflicting assignments and production obligations assigned to a known owner whose charter authorizes no production changes. Include a negative fixture substituting such a contract-only or documentation-only owner while its node, path and acceptance ID remain valid, so refusal discriminates production authority. CAT0's own contract artifacts remain CAT0-owned; production deletion remains acceptance of the later owner.
...
**Predecessors:** `UAK1`, `VID0`.
**Subblocks:** (1) specify typed carrier/profile/project/embedded/interoperability/capability/rule tables; (2) inventory `FrameworkAdapterRegistry` and descriptor-data migrations and bind later implementation owners; (3) specify descriptor-generated client manifest byte-pinning; (4) produce exhaustive registration/capability fixture matrices; (5) prove deterministic contract fixtures and duplicate-owner rejection; (6) assign later deletion owners for per-framework client wiring. All are contract/schema/fixture work within CAT0's zero-production boundary.
**Acceptance:** the contract and fixtures define byte/fact-equivalent Vue/Svelte registry, session, LSP, MCP, NAPI, WASM and client outcomes, including a dormant test row requiring data/manifest changes but no switch in neutral routing. CAT0 validates complete ownership bindings; the assigned successor acceptance IDs require the actual production migrations and equivalence evidence.
**Forbidden:** runtime plugin loading, `Any` in public registration, hardcoded Vue/Svelte branching, or a second “universal” catalog.
**Deletion/abort:** inventory displaced registry constructors/generated mirrors and bind their concrete successor deletion owners and receiving acceptance IDs under CAT0-AC1. Those owners remove the displaced routes in their production cutover and reject dual active registration authorities. CAT0 changes no production route and rejects missing or conflicting ownership assignments.
