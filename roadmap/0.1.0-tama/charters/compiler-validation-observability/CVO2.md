...
- **Solution and architecture decisions:**
  - capture through the raw adapter: cold and warm compile samples per case, sample counts, and workload size/case count, with selection throughput derived from the cold rows rather than measured as its own execution; the existing suites' native statistics are not imported because they are computed under threshold policies;
...
- **CVO2-AC4 — bounded work:** the only hot path this node adds is artifact retrieval, bounded by the archive and count ceilings above and proven by the oversized-archive fixture; no new counters or instrumentation paths are created.
...
- raw observation adapter in the probe crate for cold/warm samples and workload size, throughput derived from cold rows; no reuse of threshold-bearing benchmark scripts;
