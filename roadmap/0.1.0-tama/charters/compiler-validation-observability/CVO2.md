...
- Test/CI homes: the CVO1 lane's test-only crate, existing benchmark homes (`crates/verter_bench`, `packages/benchmark`), `.github/workflows` observation lane.
...
- **CPER0M:** implemented ledger row for "NAPI memory-audit snapshot coherence"; the native memory-audit peak is coherent, so any reported memory/allocation number rests on a trustworthy high-water basis. Ledger presence alone satisfies the predecessor. Its locator metadata remains non-authoritative.
- **External requirements:** agents check `pinned_vue_benchmarks_checkout`; tooling does not validate external state.
...
- **Solution and architecture decisions:**
  ...
  - memory/allocation data only via the existing trustworthy measurement path; no new instrumentation invented solely to increase the metric count;
  ...
  - correctness labeling: each result retains its correctness status; a workload that is not correctness-equivalent may record timing as observation but must not be ranked as evidence that Verter is faster or slower for equivalent work (conceptual `comparison_eligible: false`).
...
- **CVO2-AC2 — positive contract:** an observation run emits one artifact with the full reproducibility metadata block and per-case correctness labeling derived from the CVO1 classification; repeated runs on identical inputs are structurally comparable.
...
- memory/allocation only via the trustworthy existing path (CPER0M basis);
- full reproducibility metadata on every artifact;
- correctness status and `comparison_eligible` labeling on every result;
