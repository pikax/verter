...
- **CVO4-AC2 — positive contract:** `JoinRecord::validate` passes on the published record and fails closed on six planted negatives kept as fixtures: a record omitting one manifest entry, a record omitting one observation `{ artifact_id, row_id }`, a duplicated manifest entry id, a duplicated observation identity, a `known-fail -> gate` promotion with no deciding authority, and a record whose observation entries carry identities but no `disposition` (the identity-only false green); `validate-probe-authorities.mjs` refuses a promotion citing an unimplemented owner or a wrong-framework authority (its own fixtures, owned by CVO0). After the join, every probe-state manifest entry is internally consistent — gates are enforced as gates, canaries keep their expected classes, skips still name owners, XPASS candidates are either promoted with an owner or left `deferred` with a reason.
...
3. Bind the preflight evidence selection and terse rationale in the review report. The six planted negatives named in CVO4-AC2 are the TDD artifact for the validator.
