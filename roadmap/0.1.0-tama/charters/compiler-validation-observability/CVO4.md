...
- Named API/data boundaries: the promotion join record `crates/verter_validation_probe/join/<date>.toml` — per probe, its current state, candidate transition (`skip -> canary`, `canary -> known-fail`, `known-fail -> gate`, `canary -> gate`) or `deferred`, deciding authority (`{ authority, atom }`; never a roadmap identifier), and rationale, plus one disposition record per inventoried observation — `{ artifact_id, row_id, disposition: deferred | adopted | discarded, rationale, basis? }`, where `adopted` (the number becomes a baseline or gate candidate for the performance authority) requires `basis = { authority: compiler.equivalent-work-ledger, atom }` with an implemented owner and is admitted only for a source row whose `comparison_eligible` is `true` and whose semantic and equivalent-work bases are still implemented at join time (an ineligible row can never become a baseline or gate candidate) and `discarded` requires a rationale naming the defect (an observation from a superseded corpus revision is `deferred` or `discarded` with the rationale `superseded corpus revision <sha>`, never dropped from the inventory) — never identities alone, so an omitted or duplicated row inside a multi-row artifact is detectable and a record that lists observations without reviewing them is invalid; and the `retrieval_window` reported by `ObservationInventory::fetch`; the join validator `crates/verter_validation_probe/src/join.rs` (`JoinRecord::validate(manifests: &BTreeMap<Framework, ProbeStateManifest>, observations)`, which refuses to run unless every pinned framework's manifest is supplied), which proves total inventory — every manifest cell `{ probe_id, dimension }` from both CVO1/CVO1S manifests and every retrievable observation `{ artifact_id, row_id }` appears exactly once in the record — that every non-deferred entry cites an `{ authority, atom }`, and that every transition is one of the four allowed; `validate-probe-authorities.mjs --join crates/verter_validation_probe/join` (CVO0) then proves each cited authority's owning node is implemented and its catalog row covers the probe's framework and dimension through the cited atom, so a promotion to `gate` cannot rest on an unimplemented or wrong-framework authority; updated probe-state manifest entries.
- Mutation boundary: the join record, the validator, manifest entries, and — only for an owner-supplied Svelte comparator binding — the `comparator` field of that framework's row in `roadmap/0.1.0-tama/catalogs/validation-authorities.toml` (a `validation_observability` root) together with the Svelte manifest's `comparison`/`comparator` header; production LOC is zero.
...
- **CVO4-AC2 — positive contract:** `JoinRecord::validate` passes on the published record and fails closed on nine planted negatives kept as fixtures: a record omitting one manifest cell, a record omitting one dimension of a probe while retaining its sibling dimension, a record omitting one observation `{ artifact_id, row_id }`, a duplicated manifest cell identity, a duplicated observation identity, a `known-fail -> gate` promotion with no deciding authority, a record whose observation entries carry identities but no `disposition` (the identity-only false green), an `adopted` disposition on a `comparison_eligible = false` row, and a validation attempted with one framework's manifest missing; `validate-probe-authorities.mjs` refuses a promotion citing an unimplemented owner or a wrong-framework authority (its own fixtures, owned by CVO0). After the join, every probe-state manifest entry is internally consistent — gates are enforced as gates, canaries keep their expected classes, skips still name owners, XPASS candidates are either promoted with an owner or left `deferred` with a reason.
...
3. Bind the preflight evidence selection and terse rationale in the review report. The nine planted negatives named in CVO4-AC2 are the TDD artifact for the validator.
