...
- **Solution and architecture decisions:**
  ...
  - the PR smoke slice gains at most 24 Svelte cases in the same single job (combined smoke inventory at most 48 cases, one `compileRequests` call per case); the main lane stays within the single combined 200-case limit shared with Vue (the summary validator enforces the total, so the two selections are sized together, not added); elapsed time is an observation only;
