...
- Named API/data boundaries: the test-only workspace crate `crates/verter_validation_probe` (not a member of any production dependency graph) with modules `outcome` (`ProbeOutcomeClass`; the precedence fold `ProbeOutcomeClass::terminal(observed: &[ProbeOutcomeClass]) -> Result<Terminal, InvalidObservation>` where `Terminal { class, secondary: Vec<ProbeOutcomeClass> }` is the single terminal class plus retained secondary evidence and `InvalidObservation` names the impossible observed combination), `manifest` (`ProbeStateManifest`, `ProbeEntry { probe_id, framework, case, expected_state, expected_class, owner, reason, external_revision }`, `ProbeStateManifest::validate`), `evaluate` (`ProbeEntry::evaluate(&self, terminal: &Terminal) -> Evaluation` with the closed enum `Evaluation { GatePass, GateRegression, CanaryExpected, KnownFailExpected, Xpass, UnrelatedRegression, Skipped }`, the only place an expectation meets an observation), and the rejection fixtures in the crate's single integration-test binary `tests/main.rs`, cases under `tests/cases/contract.rs`. Successor nodes plug the runner, corpus adapters, controls, and join validator into this crate; none of them re-declares these types.
...
- Test homes: `crates/verter_validation_probe/tests/main.rs` (the crate's only integration-test binary, per the repository's single-`tests/main.rs` rule) with cases under `tests/cases/contract.rs`; this charter for the contract text.
