...
- **Solution and architecture decisions — seed exactly four control classes:**
  ...
  2. **Timeout/refusal distinction:** a timed-out or signaled process cannot satisfy an expected-refusal/negative-control assertion; injected as raw `ExecutionEvent::TimedOut` and `ExecutionEvent::Signaled` through the runner's in-process `runner::classify_execution`, whose classes then flow through the per-dimension fold into `ProbeEntry::evaluate`; the control asserts the classes `timeout` and `crash` and the evaluation `UnrelatedRegression` against an entry expecting `verter_diagnostic` or `unsupported`; the same table carries a real-union gate row — a driver line whose entry is `{ response: null, failure: { kind: "runtimeSurfaceRefused", ... } }` against a `Route` gate expecting `unsupported` evaluates to `GatePass`, while `TimedOut` and a `binding` failure against that gate evaluate to `GateRegression` — and a mutation of the event-to-class mapping (for example `TimedOut` → `verter_diagnostic`) must turn it red.
