...
- **Solution and architecture decisions — seed exactly four control classes:**
  ...
  2. **Timeout/refusal distinction:** a timed-out or signaled process cannot satisfy an expected-refusal/negative-control assertion; injected as raw `ExecutionEvent::TimedOut` and `ExecutionEvent::Signaled` through the runner's in-process `runner::classify_execution`, whose classes then flow through the per-dimension fold into `ProbeEntry::evaluate`; the control asserts the classes `timeout` and `crash` and the evaluation `UnrelatedRegression` against an entry expecting `verter_diagnostic` or `unsupported`; the same table carries real-union rows that keep the three boundary classes apart — a driver line whose entry is `{ response: null, failure: { kind: "runtimeSurfaceRefused", ... } }` against a `Route` gate expecting `unsupported` evaluates to `GatePass`; `{ kind: "refused" }` against a gate expecting `request_refused` evaluates to `GatePass` and against one expecting `unsupported` to `GateRegression`; `{ kind: "host", diagnostics: <one error> }` folds to `host_failure` with the diagnostic secondary and evaluates a gate expecting `verter_diagnostic` to `GateRegression`; a driver-level `{ error }` line folds to `harness_failure`; and `TimedOut` against any refusal gate evaluates to `GateRegression` — and a mutation of the event-to-class mapping (for example `TimedOut` → `verter_diagnostic`) must turn it red.
