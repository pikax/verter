...
- **Solution and architecture decisions — seed exactly four control classes:**
  1. **Diagnostic propagation:** construct a case where compilation fails after initial ingestion, exercised at the runner's ingestion boundary rather than at the classifier alone: a constructed driver response line in the driver's own envelope — `{ probe_id, elapsed_ns, entries: [{ canonicalId, response: { canonicalId, diagnostics: <one error-severity entry>, products: [] }, failure: null }], reference: [{ code }] }` — is fed to `runner::ingest_response_line` in-process (no test-only envelope shape exists), and the terminal must be `verter_diagnostic`, never `product_not_produced` or generic empty output.
