...
- **SCP0-AC1 — ownership contract:** every inventoried outcome, consumer and displaced route has exactly one declared implementation/deletion owner. Validate contract artifacts and their negative controls; production deletion is acceptance of the later owner, not this documentation-only node.
...
- Inventory and assign the later deletion/rejection owner for: **legacy Svelte emitter route**. This node changes no production route.
