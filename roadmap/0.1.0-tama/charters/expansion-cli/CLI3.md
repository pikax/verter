...
- **CLI5:** implemented ledger row for “Base packaging, watch mode, compatibility wrappers, and promotion”; ledger presence alone satisfies the predecessor. Its commit message, approximate timezone-bearing date, and optional PR are locator hints only.
- **CLIF0:** implemented ledger row for “Formatter CLI adapter”; ledger presence alone satisfies the predecessor. Its commit message, approximate timezone-bearing date, and optional PR are locator hints only.
- **CLIL0:** implemented ledger row for “Lint CLI adapter”; ledger presence alone satisfies the predecessor. Its commit message, approximate timezone-bearing date, and optional PR are locator hints only.
...
| **Solid 2** | Same OXC/TypeScript TSX geometry, separate semantic profile for one exact 2.x release | Signals, async memos, split effects, batching, stores, control flow, and component semantics; prevents React-shaped kernel | Immediate React counterfixtures, JSX binding/event differences, Solid 1 unsupported-major controls, SolidStart later | `NotApplicable` |
...
| **htmx 4** | Neutral HTML with attribute-level request/target/trigger/swap claims for one exact 4.x release | Low-cost navigation from selectors and captured project routes; underserved HTML-first DX | explicit attribute inheritance and configuration overrides, standardized events, selectors, partials, extensions, route metadata as optional project input, CE consumption | `NotApplicable` |
...
| **HTML** | Independent neutral parser initially forked from Vue and de-Vue’d | Standards source tooling and substrate for Angular/Alpine/htmx 4/WC | standards recovery, a11y, links/assets/selectors, formatter, full applicable LSP | `NotApplicable` |
...
Solid 2 and htmx 4 are the selected majors for their existing portfolio and proof scopes. Each profile pins one exact release, package roles, oracle, and corpus; prerelease evidence retains its exact version and maturity. Solid 1 support, compatibility, and migration are outside current scope, and older htmx majors are not implied by htmx 4 support. Unsupported majors never silently select these profiles. This selection adds no production implementation train or compiler commitment. See [the framework-major selection decision](../../decisions/2026-09-08-solid2-htmx4-targets.md).

The user’s “htmlx” item is interpreted as **htmx 4**, consistent with the supplied examples and `hx-*` semantics. If a separate language named HTMLX was intended, it requires its own feasibility dossier rather than silently sharing the htmx 4 profile.
...
| Deferred; unranked | SolidStart | unscored | routing, server functions, islands/hydration, data/cache boundaries | Solid 2 vertical plus independent feasibility/score lock against a compatible exact project-profile release |
