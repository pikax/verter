...
- No authored-operand cutover of the conditional family, no conditional-`infer` binder frame on TE1 authored identity, and no `verter_type_expr` locator vocabulary change: those are a TE1 completeness gap that any node needing an isolated authored binder-crossing force records as its own DAG amendment before mutation.
...
- Stop before mutation if either predecessor lacks an implemented ledger row, if the lowering-site decision cannot reuse `conditional_branch_selection` without a second selector, or if the pending-substitution pairing cannot be sealed content-free on TE1's runtime-node arm plus positional substitution axis.
