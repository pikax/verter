...
- **TE2:** implemented ledger row for "Conditional branch forcing at the lowering and instantiation sites"; ledger presence alone satisfies the predecessor. TE2's remaining cutover supplies lowering-site selection before branch lowering and instantiation-site pending-substitution deferral, preserving the delivered dispatch-boundary dependency facts, exact infer scoping, open conditional residual projection, and distributivity ownership. A conditional mapper value forced under a demanded key substitutes and forces only its winning branch; TE4 reuses TE2's sealed materialized-handle-plus-substitution pairing for demanded-key value forcing rather than introducing a second one.
