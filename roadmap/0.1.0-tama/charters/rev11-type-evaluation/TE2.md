...
properties of the landed tree put that outside this node's boundary:

- TE1's authored operand identity is `(locator, lexical scope, binder
  visibility, positional header substitution, split env)`, and TE1's own
  sealed design states that runtime binder handles — `InferBinderId`
  included — are identity only on the runtime-node arm and never on the
  authored arm. A conditional-`infer` frame on an authored true-branch
  operand is therefore not representable by TE1 identity; making it so is a
  TE1 amendment, not a TE2 specialization of the force-request vocabulary.
- Forcing an authored `ConditionalTrue` operand whose sibling `extends`
  declares an `infer` is a binder-crossing step (`step_crosses_binder_scope`),
  and the crossing deref route re-lowers the whole enclosing conditional,
  false branch included, before navigating.
- Several conditional positions (array element, rest inner, `keyof` operand,
  template expression, `import()`/`typeof` type argument, non-header
  type-parameter bounds) have no `TypeBodyPathStep` spelling without a
  `verter_type_expr` vocabulary change.

The excluded scope is owned by `TE2B` and is not silently absorbed here.
...
branch, the other branch contributes no dependency fact. An absorbed result
(the fast-reject table) roots only on the operands its row read. This charter
accepts one conditional dependency-fact cutover at the dispatch boundary and
contains no independently dispatchable subblocks.

## Concrete surfaces and APIs

- Production surfaces: `crates/verter_session/src/project_semantic_dispatch`.
- Production files: `build.rs` and `absorb.rs`; tests are non-production and live in `project_semantic_dispatch/tests.rs`.
- Named boundaries: `SemanticQueryKey::Conditional`, `SemanticNodeData::Conditional`, `ProjectSemanticDispatch::build_conditional`, `absorb_conditional`, `conditional_branch_selection`, `conditional_infer_route`, `distributive_check_union_members`, `observed_self_roots_from_nodes`, `SemanticQueryKey::Relate`, `InferBinderId`, `ConditionalSelect`, `InferBind`, `ReadSetSignature`, and `SignatureAdmission`.
- Mutation boundary: the observed self-roots and dependency facts of decided, distributed, and absorbed conditional results, and exact tests. No key or family-identity change, no lowering change, no public `TypeInfo`, native-checker, flow-product, relation-policy, truthiness, canonical-algebra, or wire change.

## Exact predecessor contracts

- **TE1:** implemented ledger row for “Sealed semantic operands and forcing boundary”; ledger presence alone satisfies the predecessor. This node does not consume the force capability or mint an operand; it keeps the materialized `SemanticNodeId` operands the conditional family already carries. TE1's materialized-handle arm and positional substitution axis are consumed by `TE2B`, not here.
...
- `absorb_conditional` roots each row on what it read: the `error`-dominance row and the distributive naked-`never` row read neither branch and root on `check` and `extends` only; the `any` row publishes the union of both branches and keeps all four roots.
...
- **TE2-AC1 — select before branch force at the dispatch boundary:** a decided true/false conditional returns the winning branch while the losing branch receives no force, nested dispatch, relation read, origin edge, or fact read, and contributes no self-root. Restoring the four-node root set on a decided result must fail `decided_conditional_roots_only_on_check_extends_and_the_winner`; the decided-false leg mirrors the decided-true leg so the proof cannot pass by dropping one fixed branch. One `Conditional` family and one relation authority remain structurally evident, and the unselected branch is neither materialised nor projected (`closed_conditional_selects_and_emits_edges`, `closed_conditional_does_not_materialise_losing_branch_body`, `conditional_unselected_branch_not_projected`, `expansion_imported_projection_loads_selected_but_not_unselected_branch`).
- **TE2-AC2 — infer, distributivity, and open projection:** infer bindings substitute only into the selected true branch through the shared oracle (`bare_infer_extends_selects_true_through_the_shared_oracle`, `relate_infer_binds_substituted_type_for_true_branch`, and the `conditional_binds_*_infer_in_true_branch` suite); a distributed conditional's dependency facts follow member selections for all-true, all-false, mixed, and open member sets, for a raw union check and a global alias over it (`distributed_conditional_dependencies_follow_member_selections`); an open conditional with a residual path projects that path through both branches through the walker (`root_conditional_still_distributes`); root `Expanded` semantics are unchanged.
- **TE2-AC3 — admission and incremental equivalence:** a warm repeat of the distributed parent hits without rebuilding members and yields the same read set (the warm leg of `distributed_conditional_dependencies_follow_member_selections`); absorbed `error`/`never` rows root on no branch file and the `any` row roots on both (the absorbed arms of `decided_conditional_roots_only_on_check_extends_and_the_winner`); strict self-root validation is neither weakened nor refined; cancelled, budgeted, or partial work stays `ReturnOnly` and never warms.
...
- Mutation controls: the three declared recipes — restoring the four-node root set on the decided arm, restoring the unconditional root set on the distributed early return, and restoring the branch roots on the `error`/`never` absorb rows — must each fail the named test and pass its control.
...
- The unconditional four-node observed-self-root set on the decided, distributed, and branch-free absorbed arms is deleted; the replacement is the selection-observed root set in `build_conditional` and the row-read root set in `absorb_conditional`.
...
- Do not implement `TE2B`, TE4, or TE5 scope here: no lowering-site or instantiation-site change, no authored branch operand, no projection-context axis in `FamilyKey::Conditional` (conditional reduction reads no projection context; adding the axes to a context-free reduction only fragments warm candidates).
...
1. Run the discriminating conditional root, infer, distributivity, absorb, and walker projection cases and the three declared mutation controls.
