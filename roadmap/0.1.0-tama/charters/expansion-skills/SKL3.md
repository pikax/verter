...
- **SKL2:** implemented ledger row for “Skill forward tests and independent review receipt”; ledger presence alone satisfies the predecessor. Its commit message, approximate timezone-bearing date, and optional PR are locator hints only.
...
**Predecessors:** `SKL2`.
**Subblocks:** (1) verify the `SKL2` semantic and test evidence; (2) stage the complete skills+AGENTS+discovery+old-workflow-retirement cutover candidate; (3) run fresh routing/negative tests and independent Codex Architect review of the candidate patch; (4) obtain explicit maintainer adoption; (5) land one atomic commit; (6) verify routing and rollback restoration.
**Acceptance:** exactly one lifecycle-paired workflow is active before and after cutover; review and adoption cover the complete cutover patch; a material fix triggers proportionate fresh review without receipt restamping; rollback restores the old routing atomically.
...
**Deletion/abort:** retire only the old invocable entry point and duplicate routing after zero-consumer proof; abort and keep the old workflow active on any routing mismatch.
