...
class=successor
owner=governance.release-control:ledger-derived milestone readiness and existing release rehearsal
...
Implement a milestone release planner whose DAG-managed readiness comes exclusively from implementation-ledger rows and whose rehearsal invokes the existing release workflow graph. Release PR/tag/publication remain REL2.
...
- **REL0:** implemented ledger row for “Milestone release-planning overlay and DAG scheduling”; ledger presence alone satisfies the predecessor. Its commit message, approximate timezone-bearing date, and optional PR are locator hints only.
...
- Inspect all milestone items and derive DAG-managed readiness from implemented-ledger rows, not issue closure, labels, Project status, or milestone progress.
- Report every blocking node/item and missing predecessor row without silent waiver.
...
- Abort on missing predecessor rows, missing required workflow, unexpected skip, ambiguous waiver, open P0/P1, or a policy-blocking lower finding.
