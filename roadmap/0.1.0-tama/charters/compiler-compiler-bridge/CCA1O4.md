...
- Own framework/product request construction for `compileMany`, `getVirtualFile`, IDE materialization/read paths used by unplugin, and conversion from the stable external unplugin option vocabulary into the typed native request.
...
- Each old native call becomes one typed call with no source copy or duplicate compilation; malformed/cross-framework fields fail closed.
