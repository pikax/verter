...
- Keep opt-in issue title/body ordinary and human-readable. The body contains the current useful block description and ends with `Model: <model name>`; it contains no effort or DAG metadata block. Protected issues remain untouched.
- Define the agent flow: create a PR with the expected final conventional-commit title and exact `Closes #<gh_issue>` body link so merge closes the mapped issue. Update the useful issue description only when `sync_to_github = true`; never edit a protected issue.
