<!-- unified-charter-v2
id=STP2
name=Public Vue constructor compatibility and specialization feasibility
predecessors=STP1
phase=compiler
...
class=compiler
owner=compiler.sfc-ts-projection:abi-proof
conflict_domains=vue_product
...
charter=charters/compiler-sfc-ts-projection/STP2.md
size=M
...
# STP2 - Public Vue constructor compatibility and specialization feasibility

> **Status: pending implementation.** This charter is part of final plan revision 2, not a record of completed engine or product qualification.

## 1. Independently acceptable outcome

Deliver **Public Vue constructor compatibility and specialization feasibility** as one reviewable boundary. The acceptance product is `ConstructorCompatibilityEvidence`, `InstanceTypeCompatibilityTable`. A failed mandatory case blocks completion; rescoping may split work but may not reduce this obligation.

## 2. Current owner, final owner and architectural role

Current authority lives in the listed existing compiler/provider/LSO surfaces. Final responsibility stays with `compiler.sfc-ts-projection:abi-proof` through the existing ProjectionBackend/CodeTransform/CertifiedTypeEngineBinding/LSO division. This node has role `delivery` and kind `proof`. It creates no parallel framework resolver, checker, CSS or mapping authority.
...
- `tests/sfc-projection/STP2/contract.ts`

- `tests/sfc-projection/STP2/`

- `tests/sfc-projection/STP2/manifest.json`
...
**Produces:** `ConstructorCompatibilityEvidence`; `InstanceTypeCompatibilityTable`. These are named logical contracts. Existing names remain upstream-owned; a new concrete signature must match the STP8 accepted ABI and the product catalog.
...
### STP2.1 - Establish the source contract


Exercise constructor-only, real Vue-compatible candidates using actual Vue types, not a reduced fake ComponentPublicInstance. Assert import Comp from a .vue module; InstanceType<typeof Comp>; ref<InstanceType<typeof Comp> | null>; $props/$emit/$slots; exposed-only setup members; ComponentPublicInstance/Component assignability; and public options/static behavior.


**Discriminating cases:** `STP2-instance-concrete`, `STP2-constructor-escape`. Start with the failing or characterized baseline and keep the corresponding clean/control twin.


### STP2.2 - Implement the owned product


Distinguish unbound extraction, user-authored explicit specialization, props-inferred construction, and template-use specialization. Require `InstanceType<typeof Comp<number>>`, `const NumberComp = Comp<number>`, and `new Comp({ test: 0 })` from a real imported generic .vue module on every admitted engine profile; these are compatibility obligations, not optional capabilities. Observe the selected T through actual generic-dependent $props/exposed members and event/slot contracts. Use the user-supplied `declare class Foo<T = unknown> { constructor(props?: { test: T }) }` as the syntax control, and an observable twin with `readonly value: T`; an empty structural instance alone cannot prove inference precision. Assert defaults, constraints, wrong explicit inputs, and the distinct unbound/zero-argument behaviors against the pinned TypeScript contract. Treat construction here as typechecking-only, not runtime Vue instantiation.


**Discriminating cases:** `STP2-instance-generic`, `STP2-vue-utilities`. Start with the failing or characterized baseline and keep the corresponding clean/control twin.


### STP2.3 - Close boundary integration


Attack constructor intersections and overload order with wrong required props, wrong callback payloads, and foreign Vue utility types. Reject any broad construct overload, erased generic relation, callable default export, or checker-only constructor parameter unless a future explicit ABI decision proves full consumer equivalence.


**Discriminating cases:** `STP2-instance-explicit`, `STP2-not-callable`. Start with the failing or characterized baseline and keep the corresponding clean/control twin.


**Additional specialization regression rows:** `STP2-constructor-inferred`, `STP2-explicit-input-mismatch`. Implement these in the existing work packages; they create no new scheduler nodes or precedence over other required semantics.
...
| `STP2-instance-concrete` | InstanceType of a normal imported component exposes reset(): void and exact $props | accept |

| `STP2-instance-generic` | Unbound generic InstanceType retains non-generic members and constraints without any/never collapse | accept |

| `STP2-instance-explicit` | For a real generic .vue import, InstanceType<typeof Comp<number>> and Comp<number> preserve the selected number in generic-dependent public members; the Foo<number> control is also accepted | accept |

| `STP2-constructor-escape` | A broad overload accepts wrong props or changes InstanceType extraction | reject |

| `STP2-vue-utilities` | createApp/h/TSX and existing component utility consumers retain their admitted behavior | accept |

| `STP2-not-callable` | A generated Vue default export can only be consumed by switching to a callable SFC type | reject |


| `STP2-constructor-inferred` | new Comp({ test: 0 }) infers the same generic-dependent public member types as the observable Foo control; a sibling string use remains independent | accept |

| `STP2-explicit-input-mismatch` | new Comp<number>({ test: "wrong" }) or new NumberComp({ test: "wrong" }) is accepted despite the explicit number argument | reject |


**STP2-AC1 - completeness:** all cases and applicable RequiredCurrent rows selected; no zero-test pass. **STP2-AC2 - observability:** diagnostics/types/source targets and representative hover, definition/references and edit participation for touched semantics. **STP2-AC3 - state safety:** incremental=fresh and stale/cancelled publication controls for owned state. **STP2-AC4 - bounded work:** no hidden duplicate parse/check/emit/helper expansion or unbounded retention in touched hot paths. A not-applicable axis needs an explicit untouched-owner rationale; required features cannot be waived.
...
Use owning Rust/TypeScript tests and the existing `targeted-domain` gate profile. Reviews follow `public-3` (independent reviewers under the existing policy). Canonical full gates are owned by CI/orchestration, not repeated locally by every implementer. Commands in this section are future implementation obligations, not claimed executions in the planning session.
