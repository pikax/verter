...
- **D3C:** implemented ledger row for “Product worklist cutover”; ledger presence alone satisfies the predecessor. Its commit message, approximate timezone-bearing date, and optional PR are locator hints only.
...
- Truthiness narrowing CONSUMES the demand-scoped `ClassifyTruthinessDomain` fact owned by TA2 (`decisions/2026-08-31-canonical-type-algebra-predecessor.md`). The narrowing frames read the fact; they never carry a private truthiness rule and never fork a second classification for their own arms. D4 owns flow participation, provenance, and the narrowing EFFECT — it does not own truthiness classification.
- Every narrowed or structural return COMPOSITE — the narrowed arm, the reunited branch result, the structural return object — is constructed through TA1A's canonical union/intersection algebra. D4 chooses contributors and preserves provenance; it never defines union or intersection laws and never applies `T | never = T`, `T | T = T`, flattening, or disjoint-intersection reduction privately.

The 2026-09-08 D3 scope review identified two existing narrowing defects for this
owner. Restoring a clause-entry guard after `try { x = 1; } finally {}` can revive
the earlier `typeof x === 'string'` fact. A switch case can refine reaching type
to `'a'` while an older, higher-precedence `string` guard masks it. Both behaviors
predate the product migration; D3C transports the existing state without accepting
these semantic corrections. D4 must define coherent updates through the shared
product and canonical-algebra owners, preserving unrelated member facts and the
existing clause-scoped assertion policy. Discriminate strengthening without a write,
an unchanged write, and a write followed by an assertion equal to the entry guard.
The unlanded entry-guard projection prototype is design evidence, not a required API
or accepted predecessor. See the [stack repair scope clarification](../../decisions/2026-09-07-flow-product-execution-and-stack-repair.md).
