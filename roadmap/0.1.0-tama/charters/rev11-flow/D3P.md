...
Execution and proof follow the [2026-09-08 amendment](../../decisions/2026-09-08-flow-product-execution-contract.md).
...
- Named API/data boundaries: `FlowProductKey`, `FlowProductValue`, `FlowProductStore`, `FlowProductExecution`, `ReachingValueProduct`, `DefiniteAssignment::{Unassigned, Assigned, MaybeAssigned}`, `FlowTransferOutcome::{Unchanged, Changed, Gap, BudgetExceeded}`, `transfer_product`, `join_product`, the closed `FlowDomain` registry (`DeclaredType`, `DefiniteAssignment` added), `ResultContractId`.
...
- Named types (ruling §3): `FlowProductKey`, `FlowProductValue`, `FlowProductStore`, `FlowProductExecution`, `ReachingValueProduct`, `DefiniteAssignment::{Unassigned, Assigned, MaybeAssigned}`, `FlowTransferOutcome::{Unchanged, Changed, Gap, BudgetExceeded}`, exhaustive `transfer_product` and `join_product` per live domain. Extend the closed `FlowDomain` registry with `DeclaredType` and `DefiniteAssignment`; do not create a second domain enum.
...
- Discriminating substrate proof: existing domain-join tests cover reaching-definition sets, canonical semantic unions delegated to the semantic algebra owner, definite assignment, and narrowing intersection. `product_snapshots_and_evidence_are_predecessor_permutation_invariant` compares all five domain snapshots, exact executed-domain/subject evidence and iteration counts across equivalent actual predecessor and transfer orders. Existing budget tests prove completion at the actual fixed-point cap, typed exhaustion on an additional round, and failed execution refusing evidence; warm admission is tested in D3C. Selection is never evidence that a transfer executed.
...
- Test homes: `crates/verter_semantic/tests`, `crates/verter_session/tests/cases`, and existing owner unit-test modules. Graph enumeration uses the owner test home allowed by the enumeration amendment.
...
- Planning reference: 800 production LOC, 8 production files, 2 related crates/packages (ruling estimate: 450–650 production LOC, at most 5 files, `verter_session` only).
