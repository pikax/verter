...
- Never introduce a second domain enum, a second product store, a private product join, or a public product query; `FlowReturn` remains the only proof-enabled root and `FlowNarrowingAt` / `ContextualTypeAt` remain pending typed gaps.
