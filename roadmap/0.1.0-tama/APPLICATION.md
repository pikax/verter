...
Run `githubctl sync-issues` for initial issue creation, when explicitly adding more trains/nodes, or to reconcile the versioned label and milestone catalogs, direct blocked-by edges, explicit `gh_milestone` assignments, and managed labels. Before initial creation or explicit `--refresh-content`, author or update the selected node's stable `catalogs/github-issue-content.toml` entry from the charter and current source. The renderer validates that reviewed entry against the human issue standard from `contracts/github-control-plane.md`, never extracts charter sections, and ends the body with `AI-Generated` without a model name. Missing or invalid content aborts before GitHub mutation. A selection that depends on unresolved nodes outside its boundary fails without mutation and reports the required nodes. Pass `--create-blockers` to include that predecessor closure and create any missing issues, or `--ignore-blockers` to keep the requested boundary and leave those relationships untouched. Do not pass both. Automatic blocker expansion never includes a locally complete predecessor. Closed GitHub issues are no-touch and are never recreated or refreshed. The command writes separate `[[github_issue]]` mappings with `sync_to_github = true` for newly created issues; commit that local mapping patch. For an already mapped opt-in node, a normal run leaves title/body untouched. Use `--refresh-content` only after a rescope or deliberate content change. A `false` mapping is reported and skipped without reading or mutating the issue. The command never imports GitHub edits into local authority and does not run continuously.

A named train may have one parent GitHub issue. Each independently landable block is a sub-issue of that parent. `githubctl sync-issues` maps and labels the block issues; it does not create the parent. Create the parent separately and attach the mapped block issues as sub-issues so the train stays one human coordination surface. Project lifecycle discovers that native parent but uses locally mapped nodes in the train as the expected child set, so GitHub omissions cannot mark the train Done early.
