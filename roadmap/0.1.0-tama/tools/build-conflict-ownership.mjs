...
import { PACKAGE_ROOT, loadAuthority, normalizeProductionSurface } from "./lib.mjs";
...
const scalar = (block, key) =>
  JSON.parse(block.match(new RegExp(`^${key} = (.+)$`, "m"))?.[1] || "null");
const domains = new Map(
  blocks.map((block) => {
    const id = scalar(block, "id");
    const roots = scalar(block, "path_roots").map((root) =>
      normalizeProductionSurface(root, `conflict domain ${id}`),
    );
    return [id, { block, roots }];
  }),
);
...
for (const node of authority.nodes.filter((candidate) => candidate.review_profile !== "history")) {
  ...
  for (const rawSurface of [...line.matchAll(/`([^`]+)`/g)].map((match) => match[1])) {
    const surface = normalizeProductionSurface(rawSurface, node.id);
    const owned = node.conflict_domains.map((id) => domains.get(id)).filter(Boolean);
    if (
      owned.some((domain) =>
        domain.roots.some((root) => surface === root || surface.startsWith(`${root}/`)),
      )
    )
      continue;
    const target =
      owned.find((domain) => domain.roots.some((root) => intersects(surface, root))) || owned[0];
    if (!target) throw new Error(`${node.id}: no domain for ${surface}`);
    if (!target.roots.includes(surface)) {
      target.roots.push(surface);
      additions += 1;
    }
  }
}
...
if (check) {
  if (output !== original) {
    console.error("STALE conflict ownership catalog");
    process.exit(1);
  }
  ...
  console.log(
    `build-conflict-ownership: wrote ${domains.size} domains; ${additions} uncovered surfaces assigned`,
  );
}
