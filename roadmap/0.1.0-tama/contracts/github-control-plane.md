...
## GitHubTrainIssueMapping

Every active train has one GitHub coordination issue whose identity is stored separately from its children:
...
`GitHubTrainIssueMapping` identity is exactly `{train, gh_issue}`. Train names are unique, and issue numbers are unique across both train and node mappings. The row is coordination identity only: it is not a DAG node, does not satisfy readiness, and never marks any child implemented. Train-parent prose lives in `catalogs/github-train-issues.toml`; each reviewed entry supplies a human title, problem, expected outcome, acceptance results, problem label, and completion-horizon milestone. Normal sync preserves existing parent prose. `--refresh-content` is the only parent-prose refresh and ends the rendered body with `AI-Generated` without a model name.

The train parent and every active mapped child are native GitHub parent/sub-issues and members of Project 3. Sync creates a missing parent, writes its mapping immediately, and attaches each child with GitHub's native sub-issue relationship. A child already attached to a different parent is a conflict: the complete sync fails before mutation and never silently reparents it. GitHub permits at most 100 native sub-issues per parent. Preflight counts existing manual and completed children as well as planned additions, so a train whose projected membership exceeds the limit must first be split into independently reviewable trains. Locally completed/history children and closed issues are no-touch; when no active child is selected, sync does not mutate the parent.
...
After the GH train lands, `githubctl sync-issues` owns occasional explicit one-way synchronization from local authority to GitHub for a named train or node set. A normal run reconciles the versioned repository label and milestone catalogs, managed labels, explicit `gh_milestone` assignments, native blocked-by relationships, train parents, native sub-issues, and Project 3 membership on selected active opt-in issues. It does not render, compare, or PATCH an existing issue's title or body. `--refresh-content` is the sole existing-issue prose refresh; it renders the reviewed node and train descriptions with the stable `AI-Generated` footer, compares them with mapped issues, and PATCHes only when different. Neither refresh nor missing-issue creation requires or stores a model name. Locally completed/history nodes and closed GitHub issues are reported or skipped and receive no issue, label, milestone, relationship, or Project mutation, even when explicitly selected.

The check report remains the named `IssueCreateOrUpdatePlan` (`missing`, `drift`, `protected`, `closed`, `current`) and adds `label_catalog`, `milestone_catalog`, Project disposition, and `train_parents` status plus per-issue `milestone`, `add_blocked_by`, `remove_blocked_by`, and unresolved/protected predecessor fields. Label, relationship, milestone, Project, and, only under `--refresh-content`, prose drift place an opt-in mapping in `drift`; apply executes that plan. A selection with unresolved predecessor nodes outside its boundary returns `ok: false`, lists `required_blocker_issues`, and performs no mutation. `--create-blockers` explicitly expands the selection through the unresolved predecessor closure, creates missing issues in topological order, and writes their mappings before its relationship pass. `--ignore-blockers` explicitly keeps the original boundary and preserves omitted relationships without adding or removing them. The two flags are mutually exclusive. Before an opt-in existing-issue operation the command GETs the mapped issue through the unambiguous issue read (`parseIssuePayload`, including rejection of PR-shaped `pull_request != null`) and aborts `UnstructuredGitHubOutputError` before mutation—including a GET 404. A node create records `{node_id, gh_issue, kind, mapping_written: false}` and a parent create records `{train, gh_issue, kind, mapping_written: false}` as soon as GitHub returns a number; writing the local mapping flips `mapping_written`. Any later failure is `PartialFailureError` including that identity row even when it is the first operation. CLI identity is node/train plus number and `mapping_written`, never title or body.
...
`roadmap/0.1.0-tama/catalogs/github-milestones.toml` owns milestone titles and descriptions; the exact title is the stable catalog identity. The optional node field `gh_milestone` owns a child assignment. Absence means unmanaged/preserve, allowing a gradual maintainer sweep; it never means clear. Each train catalog entry requires a parent `gh_milestone` representing the train's completion horizon; individual children may target earlier milestones. Sync creates a missing exact-title milestone and corrects the description of an existing exact-title milestone without deleting, renaming, opening, closing, or changing due dates. It assigns selected active child and parent issues from those explicit local fields.
...
`roadmap/0.1.0-tama/catalogs/github-issue-labels.toml` is the sole vocabulary, presentation, and classification authority for issue-sync labels. Classification is a deterministic lookup over existing train and kind fields; it never reads issue prose, invokes a model, or lets an agent invent labels during sync. Every selected opt-in child receives exactly one `area:*`, exactly one `problem:*`, zero or more cataloged `framework:*` labels, and `origin:ai`. Each train parent receives its cataloged problem label plus the same area/framework classification and `origin:ai`; compiler bridge therefore carries `framework:vue`, `framework:svelte`, and `framework:shared`. The provenance label means the initial title and description were generated by AI; later maintainer edits or explicit refreshes do not change that historical fact.
...
Issue dependencies use GitHub's native `/issues/{n}/dependencies/blocked_by` endpoints. List responses retain the owner, repository, issue number, and numeric database id, and reconciliation compares that complete identity so an unrelated repository's same-number issue is preserved. Add resolves the exact blocker database id and POSTs `{issue_id}`; remove DELETEs that blocker database id. Both endpoint mappings must be opt-in. Native train attachment uses GraphQL `addSubIssue` with `replaceParent: false`, verifies the returned parent and child identities, is idempotent for the expected parent, and fails closed for a conflicting parent. Project lifecycle reads the native issue parent/sub-issue graph and Project 3 item statuses through GraphQL, then writes only the Status single-select field.
...
`githubctl schedule --check|--apply` is the maintainer-owned scheduling overlay. READY comes only from `deriveState` / `programctl` (loadAuthority in-process). GitHub Project Status, labels, and milestones never change DAG readiness. Local preflight resolves selection and mapping policy before doctor or adapter access, so a protected selection aborts with zero Project traffic. Check plans without adding project items or writing milestones. Apply is doctor-gated. For each selected node that is READY and has an opt-in local `[[github_issue]]` mapping, apply ensures that issue and its eligible same-repository native parent are in GitHub Project 3, the one long-lived project, idempotently if already members, and initializes each newly added item to Todo. It never resets an existing item's status. This duplicates only the safe membership invariant already established by `sync-issues`; schedule remains the READY-only admission overlay. A protected parent or protected mapped child suppresses parent scheduling and later parent roll-up. Explicit `--nodes` aborts when any selected node is not READY, including COMPLETE and BLOCKED. `--train` keeps only READY nodes from that train, in deterministic topological order among READY, and aborts when that set is empty. A selected READY node without a local mapping aborts. Missing Project 3 aborts. Schedule never writes milestones or parent/sub-issue relationships.
...
Inspect milestone issues through structured adapter JSON (`listMilestoneIssues`). Map child items to the DAG through the local `[[github_issue]]` table. A mapped child is `ReleaseReadiness` iff its node has an `[[implemented]]` row. An issue named by `[[github_train_issue]]` is a coordination container and is excluded from release readiness and unmapped-item blockers; its milestone communicates the train completion horizon but never adds a second implementation requirement. Check and apply compute the same plan. Apply does not write GitHub. It records rehearsal identity `{workflow: "release-check.yml", uses: "release.yml", dry_run: true}` from the job in `.github/workflows/release-check.yml` that `uses: ./.github/workflows/release.yml` and whose `with.dry_run` is YAML-true; a comment mentioning `dry_run: true` is not identity. The same identity requires `.github/workflows/release.yml` to host the clean-room published-artifact check (`scripts/githubctl/clean-room.mjs`) in a job that is not skipped when `dry_run` is true; rehearsal evidence records `clean_room: {kind: "CleanRoomPublishedArtifact", hosted: true, skipped: false}` and a skipped check cannot be PASS. Live `workflow_dispatch` requires explicit `--dispatch`, is doctor-gated for `actions`, and is never the default. A 204 dispatch records `terminal_result: "pending"` and does not make `plan.ok` true; `plan.ok` is ledger-blocker emptiness. Live job poll is not default. Do not create a duplicate release validator.
