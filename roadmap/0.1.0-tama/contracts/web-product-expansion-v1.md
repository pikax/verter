...
- Current top candidates by evidence (full rows in the product): `verter_scheduler/src/scheduler.rs` (20,743 LOC; admission + batching + pool routing + cancellation in one file — three responsibilities, coupling 105-pair with `verter_session`), `verter_session/src/project_semantic_dispatch/flow_return.rs` (15,021; flow-return dispatch + tests-adjacent fixtures), `verter_session/src/semantic_query.rs` (10,634; query envelope assembly), `verter_session/src/flow_slice_content.rs` (9,427). `verter_semantic/src/analysis/html_intrinsics_data.rs` (9,971) is a generated data source and is explicitly not a god-module candidate.
