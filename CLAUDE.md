...
# `node scripts/gate.mjs` as the sole Rust gate regardless: it is the only path that also runs the small
# shipped-`cfg(debug_assertions)` guard, which bare `--workspace --tests` and plain `cargo nextest run
# --workspace` both skip.
cargo test --package verter_compiler 2>&1 | tail -60  # Full suite with truncated output
```

The bare gate is the local fail-fast policy: nextest stops scheduling after its first failure, and a hard
Surface-1 failure skips the shipped-cfg guard with a loud incomplete FAIL. A bare PASS still means every
required stage completed. Use `--exhaustive` when the red run itself must cover every test and the shipped
guard; CI and release always pass it explicitly.
