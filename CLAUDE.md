...
Guards: `verter_audit_no_upward_deps`, `audit_substrate_isolation`, `audit_observer_single_accessor`. The former single-engine `no_new_*` shrinking-ledger scanners retired with the deleted parser-owned `type_surface` engine; the single-engine invariant is now held structurally (the compiler consumes only the dependency-neutral macro DTO) plus the Vue macro boundary suite guarded under "Vue Macro Semantic Boundary" in `/compiler-codegen`.
...
The fact-based cache architecture splits cache keys across five orthogonal env-hash dimensions (`parse_env_hash`, `resolve_env_hash`, `type_env_hash`, `lib_env_hash`, `project_identity`). Each cache layer keys only on the dimensions it actually depends on (R21 scoping rule — a single bundled `project_config_hash` is forbidden). `lib_env_hash` enters a key only when the value depends on lib data: `ResolvedImportFacts` does NOT include it; `RouteDb`, typed-IR resolve, `SemanticGraphStore`, `ComponentMetaResultDb` DO.

Two cache families: **content-addressed artifact caches** (`FileArtifactStore`, `ResolvedImportFacts`, typed-IR resolve, `MemberSemanticFactStore`, `MemberDisplayFactStore`, `ModuleAugmentationIndex`) carry `content_hash` or `parse_stable_hash` in the key; **query-identity caches** (`RouteDb`, `SemanticGraphStore` query nodes, `ComponentMetaResultDb`) exclude version hashes from the key — concurrent variants coexist as candidates in one slot, with version rooting on the cached value (the semantic-graph caches — `SemanticGraphStore` memo, `ShapeCacheDb` — root via `ReadSetSignature.facts` + `self_root_canonicals`; `RouteDb` via its value-side `ValidatedFactCache` fact signature; `ComponentMetaResultDb` via the owner whole-hash candidate discriminant + `ReadSetSignature.facts`). Cache keys never include `fact_dep_signature`. There is NO separate structural-materialiser store: every structural surface is produced by the one shared query route and reduced by the projector pipeline (the `materialize_structure_*` audit counters and `MaterializeStructure*` events stay on the wire as zero-valued schema pending a wire-contract retirement). The materialization cycle gate is the sealed `SemanticQueryKey::ClassifyMaterializationCycleGate` family (`MaterializationCycleGateKey`: `ResolvedDeclSlotIdentity` root slot + `parse_env_hash` + `resolve_env_hash`, fixed `StructuralTransit`/`Skeleton`/empty-args axes) — the SOLE authority for "does this declaration transitively reach a cycle through a complex helper surface"; only its `Decided` outcome admits (`LegacyFallback` always suppresses), and consumers branch on the carried `Stop | Continue` verdict from both arms.
...
Cache runtime hard rules — three always in force: cache correctness is read-side authoritative; `ReturnOnly` (overflow, budget exhaustion, cancellation, generation supersession, incomplete self-rooting, unresolved provenance) never publishes entries, reverse-index metadata, or persistent artifacts; overlay/session results never populate base-only or persistent caches. Full 21-rule list: `/type-cache-architecture` → Cache Runtime Hard Rules.

**Aggregate retention is one process-local byte account.** `SemanticRetentionAccount` (`crates/verter_session/src/semantic_retention_account.rs`) is the SOLE authority for how many semantic bytes the process retains — one account per PROCESS, shared by every `ProjectTypeStore`, so N loaded projects do not multiply the ratified ceiling by N. Per-family COUNT caps (`GlobalRetentionBudget`, `FamilyKey::candidate_cap`) still hold; a store must never carry a private BYTE quota beside the account. Reservations are RAII `RetentionCharge` tokens whose `Drop` is the only release path (exactly-once across cancellation, failed builds, ownership transfer and eviction; shared payloads share one `Arc`-held charge so an allocation is charged once). `Retained`/`Active` reservations are refusable under a CAS that concurrent admitters cannot jointly overshoot; a `Pinned` charge (a lease-pinned retained parse snapshot, or any allocation a live handle owns) is charged UNCONDITIONALLY and consumes headroom rather than being refused — refusing one would force a live artifact to re-parse or revoke a live handle. A refused admission returns its COMPLETE value uncached under the `LocalOnly` `NonAdmissionReason::RetentionPressure`, never a stale candidate and never a fabricated partial; that reason is distinct from `BudgetExceeded`, which means the compute stopped early. Byte figures are `RetainedFootprint` ESTIMATES used only for admission — never a validity input. A candidate-retaining store holds its account as a `StoreAccount` — a handle with NO account-less variant, whose `Default` is `process_local()` (never a freshly minted private account, which would be the forbidden per-store byte quota). `SemanticGraphStore` and `ComponentMetaResultDb` therefore charge the one account through every reachable constructor, and their reserve paths have no uncharged arm. The shared candidate substrate beneath them is closed the same way: `BoundedCandidateMap::admit` takes a `RetentionCharge` by value, not an `Option`, so a retained candidate cannot exist without a granted reservation on ANY caller path, production or test. Full contract (limits, classes, participants): `/type-cache-architecture` → Aggregate retention account.

Guards: `many_individually_legal_entries_stop_at_the_aggregate_ceiling`, `concurrent_admissions_never_jointly_overshoot_the_ceiling`, `a_charge_releases_exactly_once_across_every_ending`, `a_shared_charge_counts_once_and_frees_when_the_last_owner_drops`, `a_result_cache_under_pressure_stores_nothing_and_disturbs_nothing`, `a_retained_parse_snapshot_charges_its_pin_once_per_snapshot`, `every_project_store_shares_the_one_process_local_account`, `no_reachable_candidate_store_retains_off_the_aggregate_account`.

Guards: `cache_satisfaction_is_materialized_point_not_nominal_demand`, `cache_satisfaction_requires_path_exact_not_prefix`, `backfill_writes_only_recorded_materialized_points`, `no_off_store_host_caches`, the `r6_*` cluster, plus the four migrated-query-identity-key guards in `tests/cases/g_cache/r6_r21_query_identity_keys.rs` (`component_meta_result_key_*`, `route_name_key_*`/`barrel_surface_key_*`, `materialization_cycle_gate_key_*`, `materialization_cache_key_*`).

See `/type-cache-architecture` skill for the full rule set (R1–R31, two-fact `MemberPresence`/`Member` model, multi-candidate substrate, signature-overflow contract, module augmentation completeness, heuristic-cache-semantics prevention, exact policy identity) and `.claude/skills/type-cache-architecture/SKILL.md` for the per-field audit table + per-cache-layer key composition.
...
Guards: `no_macro_string_heuristics_in_resolver_core`, `no_format_then_reparse`, `no_role_inference_from_name_suffix`, `no_node_modules_substring_outside_workspace_api`, `no_pick_or_omit_string_prefix_check`, `lazy_decl_lowering_uses_scheduler_snapshot_not_reparse`, plus the rest of the typed-IR guard cluster.
...
pnpm build                    # Host developer build: native → lsp → ts packages (NO wasm, NO wasm-opt)
...
pnpm run build:wasm           # Runnable developer WASM: bindgen only, no wasm-opt, no playground copy
pnpm run build:ts             # Build all TypeScript packages
pnpm run build:playground     # Build the playground for deployment
pnpm dist                     # Publication-ready artifacts: native → lsp → wasm (bindgen + wasm-opt) → ts
```

`pnpm build` is the ordinary host developer build — native bindings first (needed by unplugin), then the
LSP binary (shares compiled Rust deps with native, avoids recompilation), then all TS packages. It never
builds WASM and never runs `wasm-opt`. `pnpm build:wasm` produces a separate, directly runnable developer
WASM artifact (`packages/wasm/wasm/verter_wasm_bg.wasm`, unoptimized) for local iteration; it is not part
of `pnpm build` and does not write into the playground package.

Only `pnpm dist` produces the complete publication-ready artifact set (native release, LSP, optimized WASM,
TS packages) — this is what CI's WASM/nightly/release lanes and `packages/playground`'s own `build` script
invoke (via `pnpm --filter @verter/wasm build`, not the root `pnpm build:wasm`). `wasm-opt` runs through a
content-addressed cache (`scripts/wasm-opt-cache.mjs`, keyed on the post-wasm-bindgen `.wasm` bytes +
`wasm-opt --version` + its exact arguments + a cache schema version, at `.cache/wasm-opt/`) — an unchanged
input reuses the cached optimized output instead of re-running Binaryen. Neither `pnpm test` nor the Rust
gate (`node scripts/gate.mjs`) invokes `pnpm build` or `pnpm dist` as a prerequisite.
...
# suite (~4404 tests) because a `session_metrics` Cargo feature unified differently standalone vs in the
# workspace, dropping those binaries from the workspace test set. That feature was removed (metrics are now
...
`min(cpuCount, memory tier)` where the effective memory ceiling selects 12 jobs at `>=16GiB`, 8 jobs at
`>=12GiB`, otherwise 4), `--test-threads <N>` (nextest/libtest thread count, gate-only, default
`min(12, cpuCount)`), `--memory-limit <SIZE>` (active child-tree RSS ceiling, e.g. `8GiB`/`12288MiB`,
...
invocation. Build jobs and test threads are independently owned defaults; the omitted build value is CPU-
and memory-clamped, the omitted test value is CPU-clamped, and explicit positive overrides are preserved
exactly. A breach kills the active child tree fast
and exits 123 (`ABORTED — memory ceiling`) — this
...
**MANDATORY: behavioral code changes use TDD.** Documentation, generated projections, formatting, and mechanical metadata changes use their owning freshness/validation evidence unless executable behavior also changes.

1. Name a plausible regression or contract boundary not already discriminated
2. Write or extend the smallest test and verify it fails before implementing
3. Implement minimum code to pass
4. Run tests, verify green
5. Refactor while keeping tests green

Tests are evidence, not quota. Prefer existing coverage or extending/table-driving one test. Existing behavioral coverage, type/compiler/capability enforcement, static validation, canonical gates, bounded inspection, and benchmarks are valid with a terse rationale. Do not add prose/format assertions unless those bytes are public contract, implementation mirrors, duplicate permutations, or universal negative/mutation companions. Negative/mutation tests are for plausible critical fail-closed/correctness boundaries or reproduced defects. Incremental/performance evidence applies only when touched authority or hot paths make it relevant. See `/testing` for the owning policy.

Negative assertions must discriminate a plausible forbidden public outcome not already covered. Codegen tests check removed syntax only when its survival is a realistic regression of the changed transform. Type tests use `@ts-expect-error` when it guards a plausible widening such as `any`/`never`, not by universal rule.

**Public-boundary acceptance**: every changed user-visible IDE, API, or compiler outcome needs evidence through the real public boundary. Prefer mapped existing coverage; extend/table-drive it when a plausible new boundary is missing. Add a new public-boundary test only when no existing proof discriminates the required outcome. Add a forbidden/fail-closed leg only for a plausible critical boundary or reproduced defect. Provider-selection, status, unit, type/compiler/static, gate, inspection, and benchmark evidence may supplement or establish non-behavioral boundaries with a terse rationale. A substrate block may inherit a parent boundary proof by recording the acceptance-ID mapping and executing the applicable evidence in its gate. Enforcement is judgment; selected mutation recipes follow §1a.

**Architecture guards for critical rules**: every new `CRITICAL` architecture rule lands with primary executable enforcement in the same change — a type/capability boundary, a dependency or compiler check, or a discriminating regression test (subject to the landed-scanner bar below — a "static guard" is never a new name-keyed file scanner); if enforcement cannot be automated yet, the rule text names the planned guard/test and the gap is tracked in the owning skill/doc. Each rule section names its guards inline. There is no central rule-to-guard registry: membership in a list was never proof, and a registry that scanned doc headings for guard names only enforced spelling. A rule's durability is the assertions its named guards actually make.

**Landed guards are structural, never name-keyed file scanners (forward-only)**: a heuristic file-scanner guard/test that keys on a specific tool, function, or identifier name (any spelled source name/path/token — type, module, import/path-segment, and string identities included; `syn`/AST-based scanning included) is a transient plan artifact — WIP-only (scratch branches, squashed out before landing), never a full-fledged landed guard. LANDED enforcement of an invariant is structural — compiler/type-system/tool-based (privacy/visibility/`E0603`, type-state, sealed traits, marker-trait derives, a real used tool or function) — never a name/text/grep scanner over the source tree. The active `/multi-agent-orchestration` anti-rogue review strengthens Structural-Confinement-First: even a residual scanner that older policy would permit (justified, recorded, supplement to a structural primary) does not land — keep it WIP, replace it structurally, or accept the residue uncovered by any landed scanner. Review/governance-enforced by design, NOT guard-enforced — a guard that detects "name-keyed scanner guards" would itself be a name-scanner. An already-landed scanner is not protected by its age: it is retained only while it still carries enforcement value its structural rail does not, and retiring it together with its exclusive parser, allowlists, synthetic cases and wiring is a normal change, not a policy exception. Where a structural rail already holds the invariant, the scanner is residue — delete it rather than keep it for symmetry. The former high-risk example (illustrative, not an exhaustive inventory) was the hot-materialize syntactic tripwire (`hot_materialize_syntactic_tripwire_residual_backstop` + its `HOT_TERMINAL_SINKS`/`HOT_DECIDE_TAINTED_GATE_IDENTS`/`HOT_EXTRACTING_GATE_IDENTS`/`HOT_MAT_DIRECT_IDENTS` name-lists in `crates/verter_source_policy_gate/tests/cases/output_projector_residual_guards.rs`); SIMP5 retired that tripwire and its scanner file once its STRUCTURAL rail (the `NoTypeExpr` marker + the sealed `OutputProjector` capabilities) covered the residue. Post-SIMP5 remaining enforcement is the accidental-regression `assert_not_impl_any!` carrier-escape canary in `crates/verter_session/src/project_semantic_dispatch/output_materialization_guards.rs`, compile-time witnesses in `crates/verter_source_policy_gate/tests/cases/semantic_capability_witnesses.rs`, and the trybuild fixtures run by `scripts/compile-contracts.mjs`.
...
**This rule currently fails its own test, and says so.** A rule whose thesis is "a gate that cannot prove it ran is a failure" is, today, a rule with no executable enforcement of its own: nothing in the tree fails when a required job silently disappears from a gate's universe. So the gap is named rather than hidden — the deferral, its owner (the gate-integrity block), its resolution gate (that block's landing), and the live in-tree instances are recorded in [`docs/contributing/gate-integrity-ledger.md`](docs/contributing/gate-integrity-ledger.md). Promotion to `(CRITICAL)` belongs in the same change that lands `gate_contract_integrity`, never before it, and is an ACCEPTANCE CRITERION of that block (ledger row GI-4). It is never folded into `Stub Prevention` — a related but distinct invariant whose guards do not enforce these semantics.
...
**One-pass execution applies only while the approved design remains valid.** Two actual review/fix cycles are the soft maximum: if cycle two retains P0/P1, pause and obtain the neutral Architect ruling and exact additional-round cap defined by `/multi-agent-orchestration`. This is not a checkpoint — one-pass governs *executing an approved design*, and the escalation fires when the current design still has critical defects. STOP, failed verification, rule conflict, and verified plan-invalidating discoveries pause at their prescribed evidence gate without creating a discretionary user checkpoint. Breadth, breakage, effort, or migration size never lapses approval; approved scope changes only through the recorded ruling or explicit user re-scope.
...
Guards: `macro_impacting_constructs_fail_lowering_not_silent_skip`, `every_consumer_has_production_call_site`.
