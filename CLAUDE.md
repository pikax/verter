...
- Canonical post-parse artifact = `IndexedReady`: canonical imports/exports + compact owned symbol indexes, spans, operator tags, interned names, shallow bodies safe for host-owned `Send + Sync` caches. Component-meta and later analysis layers both build from it; symbol expansion populates and reuses the same shared resolver caches — no separate expansion paths.
- Parse each live file version once through the scheduler; lower only the shallow syntax later passes need. Transient OXC parse arenas are per-file/per-version only — drop after lowering, never leak into host-owned shared caches.
...
`FileArtifactStore` is the authoritative per-file storage layer; the overlay-aware `augmentation_index` (module-augmentation inverse lookup) lives on it. See `/type-cache-architecture` for its key composition, `AugmentationTargetKey`/`AugmentationPopulation` semantics, and the `parse_stable_hash` definition.
...
Guards: `cache_satisfaction_is_materialized_point_not_nominal_demand`, `cache_satisfaction_requires_path_exact_not_prefix`, `backfill_writes_only_recorded_materialized_points`, `no_off_store_host_caches`, the `r6_*` cluster — full list in `CRITICAL_RULE_GUARDS`.
...
OXC lowering happens once during shallow analysis via `lower_ts_type(ts_type, source)`; the lowered `TypeExpr` is stored alongside `Analyzed*Field` (and on `ResolvedLocalType.type_expr`, `ProjectedMacroSurfaces.*_expr`) and survives all caches. If a new requirement appears to need text manipulation inside the resolver, fix the producer (lower the right OXC node, store the right typed field, extend `@verter/type-ir` with a missing variant) rather than reparsing or pattern-matching on text.

Guards: `no_macro_string_heuristics_in_resolver_core`, `no_format_then_reparse`, `no_role_inference_from_name_suffix`, `no_node_modules_substring_outside_workspace_api`, `no_pick_or_omit_string_prefix_check`, plus the rest of the typed-IR guard cluster — full list in `CRITICAL_RULE_GUARDS`.
