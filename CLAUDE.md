...
cargo nextest run --workspace                # SURFACE 1 — every workspace test target INCLUDING the verter_session integration binaries (`main` + the allowlisted `allocator_canaries`, consolidated per Anti-Binary-Growth Integration-Test Layout), per-test process isolation
...
# `node scripts/gate.mjs` as the sole Rust gate regardless: it is the only path that also runs the
# shipped-`cfg(debug_assertions)`-off SURFACE 3, which neither `--workspace --tests` nor the two-surface pair
# above covers.
