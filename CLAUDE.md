...
### Single Resolution Engine Rule (CRITICAL)

There is exactly ONE type-resolution engine: the canonical typed-IR dispatch `SemanticQueryKey` → `ProjectSemanticDispatch::execute` → `SemanticGraphStore`. The redundant eager OXC `resolve_type` engine (`crates/verter_parser/src/utils/oxc/vue/script/resolve_type/`) and its query-time rail — the prepared-surface walker (`PreparedSurfaceProjection`), the eager macro-surface producer (`from_eager_meta`), the engine output type (`ResolvedElements`), and the duplicate `read_surface_members` reader — are being deleted across the consolidation stages. OXC survives ONLY as the lowering front-end (`verter_type_expr_oxc::lower_ts_type`, TS→`TypeExpr` once during shallow analysis); QUERY-time OXC resolution is forbidden. No NEW production site of a doomed symbol may be added while the second engine is torn down. The shrinking-ledger guards in `single_resolution_engine_guards` (`crates/verter_session/tests/architecture_guards.rs`) pin this: `no_new_from_eager_meta_production_site`, `no_new_duplicate_read_surface_members_definition`, `no_new_resolve_type_engine_path_production_file`, `no_new_resolved_elements_production_file`, `no_new_prepared_surface_projection_production_file` — each fails on a new production site and on a stale allowlist entry, so every stage that removes a site shrinks the allowlist toward empty. The canonical macro shape is `shared-resolve + normalise`: resolve the macro's generic-parameter or object-argument type through the one resolver, then apply a thin per-kind normaliser (props/emits/options/slots/`withDefaults`). This rule complements the `Macro Type Traversal Rule` and `Typed-IR-Only Resolver Rule` above.
