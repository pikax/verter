...
- The declaration-body **hot READ path is handle-native at the one migrated graph-backed site** (`lower_decl_body_to_node`) through `decl_body_hot_ref`; prepared declarations and `LoweredValueDecl` are fact+locator `NoTypeExpr`, and `LoweredTypeDecl.body` is content-free. `DeclBodyMemo` is not yet wholly `NoTypeExpr`: memoized `LoweredTypeDecl.type_parameters: Vec<TypeParam>` retains constraint/default `TypeExpr` and feeds locator/binder lowering; the external frontier also derives narrow type-parameter output from that field instead of the existing `narrow_type_parameters` mirror. A second stored pocket lives beside it: `TypeParamBinding.constraint/default` (`resolver_core/prepared_decl.rs:185` (fields `:193-194`)) keeps `Arc<TypeExpr>` bounds inside cached prepared decl bundles, read by query-time lowering (`project_semantic_dispatch/lower.rs:328`). Both violate the terminal storage target and are owned by the type-parameter-bound confinement block, which must retain only facts/locators and re-borrow authored bounds lease-only at the producer boundary. The residual-reader ledger is not a terminal-zero mechanism: `ProducerLowering` is permanent transient ingress, the five `GraphFreeDto` rows remain named until the separate producer-boundary-confinement cutover, and a hot consumer must never take `HotTypeRef → TypeExpr → semantic decision`.
