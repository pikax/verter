...
const Foo: CircularA;
` + Array.from({length: 10000}).map((_, i)=> i === 0 ? `type Var0 = { a: 1 }` : `type Var${i} = ${Array.from({length: i}).map((_, b)=> `Var${b}`).join('&')}`).join('\n');
...
function describe(descriptor: TypeDescriptor | undefined): string {
  ...
  if (descriptor.kind === "indexedAccess") return `indexedAccess(${describe(descriptor.objectType)}[${describe(descriptor.indexType)}])`;
  if (descriptor.kind === "recursiveRef") return `recursiveRef(${descriptor.conditionalContext}${descriptor.name})`;
...
function play(): void {
  ...
  try {
    ...
    for (const sym of session.listSymbols(REAL_FILE)) {
      console.log(
        `    ${sym.name.padEnd(16)} ${sym.kind.padEnd(14)} exported=${sym.isExported}`,
      );
    }
