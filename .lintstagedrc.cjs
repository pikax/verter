...
// lint-staged chunks a large staged-file set before it invokes function-form
// rules on Windows. The Rust rule formats the whole workspace and deliberately
// ignores those file arguments, so scheduling it for every chunk would run the
// same package matrix concurrently. Keep the command global to this config
// evaluation and make later chunks no-ops.
let rustWorkspaceFmtScheduled = false;

module.exports = {
  "*.rs": () => {
    if (rustWorkspaceFmtScheduled) return [];
    rustWorkspaceFmtScheduled = true;
