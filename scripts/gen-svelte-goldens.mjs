...
function normalize(slug, backend, compiled, semanticCommentSignature) {
  ...
  return {
    ...
    // Raw official output is classified by the SAME Rust/OXC extractor as
    // Verter's structural comparator. This survives clientModule's cosmetic
    // comment stripping without introducing a second JS classifier.
...
function buildAllGoldens(compiler, goldensDir = GOLDENS_DIR) {
  ...
  for (const abs of fixtures) {
    ...
    for (const backend of BACKENDS) {
      ...
      pending.push({
        path: goldenPathFor(goldensDir, slug, backend),
        slug,
        backend,
        compiled,
      });
  ...
  const clientRows = pending.filter((row) => row.backend === "client");
  const signatures = semanticCommentSignaturesOf(
    clientRows.map((row) => row.compiled.js.code),
    REPO_ROOT,
  );
  let signatureIndex = 0;
  for (const row of pending) {
    const signature = row.backend === "client" ? signatures[signatureIndex++] : [];
    const golden = normalize(row.slug, row.backend, row.compiled, signature);
    result.set(row.path, serializeGolden(golden));
  }
  return result;
}
...
function buildConformanceGoldens(compiler, plan) {
  ...
  for (const planCase of plan.cases) {
    ...
    for (const backend of planCase.backends) {
      ...
      if (oracleRejected) {
        ...
        pending.push({
          path: join(CONFORMANCE_GOLDENS_DIR, `${planCase.slug}.${backend}.json`),
          slug: planCase.slug,
          backend,
          compiled,
        });
  ...
  for (const row of pending) {
    ...
    result.set(
      row.path,
      serializeGolden(normalize(row.slug, row.backend, row.compiled, signature)),
    );
  }
  return result;
}
