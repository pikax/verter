...
test("STP10 node manifest is runnable without STP1 mandatory cases", () => {
  const node = loadNodeManifest(REPO_ROOT, "tests/sfc-projection/STP10/manifest.json");
  assert.equal(node.errors.length, 0, JSON.stringify(node.errors));
...
test("STP14 node manifest is runnable without STP1 mandatory cases", () => {
  const node = loadNodeManifest(REPO_ROOT, "tests/sfc-projection/STP14/manifest.json");
  assert.equal(node.errors.length, 0, JSON.stringify(node.errors));
...
test("STP14 verify: binder-aware capture on both engines", async () => {
  const result = await verifyNode({
    repoRoot: REPO_ROOT,
    node: "STP14",
    engine: "all",
  ...
  assert.equal(result.ok, true, JSON.stringify(result.errors, null, 2));
  assert.deepEqual([...STP14_MANDATORY_CASES].sort(), [...result.mandatoryCases].sort());
  for (const id of STP14_MANDATORY_CASES) {
    assert.ok(selectedCaseIds(result).includes(id), `missing selected case ${id}`);
  }
  assert.equal(result.harnessRuns.length, 2);
  for (const run of result.harnessRuns) {
    assert.equal(run.positiveDiagnostics.length, 0, JSON.stringify(run));
    // Lifting keeps the authored member type, so the wrong key stays a
    // customer diagnostic rather than a hidden one.
...
test("STP17 node manifest is runnable without STP1 mandatory cases", () => {
  const node = loadNodeManifest(REPO_ROOT, "tests/sfc-projection/STP17/manifest.json");
  assert.equal(node.errors.length, 0, JSON.stringify(node.errors));
...
test("STP17 verify: attribute operations and consumer channels on both engines", async () => {
  const result = await verifyNode({
    repoRoot: REPO_ROOT,
    node: "STP17",
    engine: "all",
  ...
  assert.equal(result.ok, true, JSON.stringify(result.errors, null, 2));
  assert.deepEqual([...STP17_MANDATORY_CASES].sort(), [...result.mandatoryCases].sort());
  for (const id of STP17_MANDATORY_CASES) {
    assert.ok(selectedCaseIds(result).includes(id), `missing selected case ${id}`);
  }
  assert.equal(result.harnessRuns.length, 2);
...
test("STP10 verify: emission correspondence on both engines", async () => {
  ...
  assert.equal(result.ok, true, JSON.stringify(result.errors, null, 2));
  assert.deepEqual([...STP10_MANDATORY_CASES].sort(), [...result.mandatoryCases].sort());
  for (const id of STP10_MANDATORY_CASES) {
    assert.ok(selectedCaseIds(result).includes(id), `missing selected case ${id}`);
  }
  assert.equal(result.engines.length, 2, JSON.stringify(result.engines));
