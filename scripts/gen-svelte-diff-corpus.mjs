...
function enumerateCases() {
  ...
  for (const ev of EVENTS) {
    ...
    push(`event_${ev.label}_on_component`, mergeNeeds(fooComp, ev), `<Foo ${ev.markup} />`);
  }
...
function pushValueFidelity(push) {
  ...
  push("nsprop_input_default_value_mixed", { props: ["x"] }, '<input defaultValue="a {x} b" />');
  ...
  push("dir_bind_value_quoted", { funcs: ["let dv = $state('');"] }, '<input bind:value="{dv}" />');
  push(
