 ...
 * Rust/OXC extractor the structural comparator also calls. The whole corpus is
 * sent through one process: regeneration never forks the classifier in JS and
 * never pays a subprocess per fixture.
 */
export function semanticCommentSignaturesOf(modules, repoRoot) {
  if (!Array.isArray(modules) || !modules.every((module) => typeof module === "string")) {
    throw new Error("semanticCommentSignaturesOf expects an array of raw JavaScript modules");
  }
  if (modules.length === 0) return [];
  const run = spawnSync(
    "cargo",
    ["run", "--quiet", "-p", "verter_compiler", "--bin", "svelte-semantic-comment-sig"],
    {
      cwd: repoRoot,
      encoding: "utf8",
  ...
  if (run.status !== 0) {
    throw new Error(
      `Rust semantic-comment extractor failed (status ${run.status}):\n${run.stderr || run.error}`,
    );
  }
  let signatures;
  try {
    signatures = JSON.parse(run.stdout);
  } catch (error) {
    throw new Error(`Rust semantic-comment extractor returned invalid JSON: ${error.message}`);
  }
  if (
    !Array.isArray(signatures) ||
    signatures.length !== modules.length ||
    !signatures.every(
      (signature) =>
        Array.isArray(signature) && signature.every((entry) => typeof entry === "string"),
    )
  ) {
    throw new Error(
      `Rust semantic-comment extractor returned a malformed batch: expected ${modules.length} signatures`,
    );
  }
  return signatures;
}
