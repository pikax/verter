...
if (!__input) {
  console.error("usage: node _deep-categorize.mjs <mismatches.json>");
  process.exit(1);
}
  ...
  if (!m.verter || m.verter.trim() === "" || !m.verter.includes("ssrRender")) {
    ...
    if (cat === "Z-other") {
      ...
      for (const diff of diffs) {
        ...
        if (
          vueCtx.includes("_mergeProps") ||
          verterCtx.includes("_mergeProps") ||
          vueCtx.includes("mergeProps") ||
          verterCtx.includes("mergeProps")
        ) {
          cat = "D-mergeProps";
          break;
        }
        if (
          vueCtx.includes("onUpdate:") ||
          verterCtx.includes("onUpdate:") ||
          vueCtx.includes("v-model") ||
          verterCtx.includes("v-model")
        ) {
          cat = "E-v-model";
          break;
        }
    ...
    if (cat === "Z-other") {
      for (const diff of diffs) {
        ...
        // Check for v-show
        if (
          vueCtx.includes("v-show") ||
          verterCtx.includes("v-show") ||
          vueCtx.includes("display:") ||
          verterCtx.includes("display:")
        ) {
          cat = "K-v-show";
          break;
        }
        ...
        if (
          vueCtx.includes("_ssrRenderSlot") ||
          verterCtx.includes("_ssrRenderSlot") ||
          vueCtx.includes("$slots") ||
          verterCtx.includes("$slots")
        ) {
          cat = "L-slot-diff";
          break;
        }
        ...
        if (
          vueCtx.includes("_ssrRenderClass") ||
          verterCtx.includes("_ssrRenderClass") ||
          vueCtx.includes("_ssrRenderStyle") ||
          verterCtx.includes("_ssrRenderStyle")
        ) {
          cat = "N-class-style";
          break;
        }
