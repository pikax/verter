...
if (!__input) {
  console.error("usage: node _drill-other.mjs <mismatches.json>");
  process.exit(1);
}
...
for (const m of mm) {
  ...
  // Component casing
  if (
    /[a-z][A-Z]/.test(vueCtx.slice(vueCtx.length / 2 - 10, vueCtx.length / 2 + 10)) &&
  ...
  if (subcat === "unknown") {
    ...
    // Conditional slot rendering
      ...
      vueCtx.includes("? {") ||
      verterCtx.includes("? {") ||
      vueCtx.includes(": {name:") ||
      verterCtx.includes(": {name:")
    ) {
      subcat = "conditional-slot";
    }
    // v-for fragment markers
    else if (
      vueCtx.includes("<span>") ||
      verterCtx.includes("<span>") ||
      vueCtx.includes("<!--[-->") ||
      verterCtx.includes("<!--[-->")
    ) {
    ...
    // Binding prefix
      ...
      vueCtx.includes("$data.") ||
      verterCtx.includes("$data.") ||
      vueCtx.includes("$props.") ||
      verterCtx.includes("$props.") ||
      vueCtx.includes("$setup.") ||
      verterCtx.includes("$setup.")
    ) {
