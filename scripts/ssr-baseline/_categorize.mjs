...
if (!__input) {
  console.error("usage: node _categorize.mjs <mismatches.json>");
  process.exit(1);
}
...
for (const m of mm) {
  ...
  if (cat === "Z-other") {
    if (
      firstDiffVue.includes("_imports_") ||
      firstDiffVerter.includes("_imports_") ||
      (firstDiffVue.includes("import ") && firstDiffVue.includes("_plugin_vue_export_helper"))
    ) {
      cat = "B-asset-imports";
    } else if (
      firstDiffVue.includes("_mergeProps") ||
      firstDiffVerter.includes("_mergeProps") ||
      firstDiffVue.includes("mergeProps") ||
      firstDiffVerter.includes("mergeProps")
    ) {
      cat = "D-mergeProps";
    } else if (
      firstDiffVue.includes("ssrGetDirectiveProps") ||
      firstDiffVerter.includes("Directive")
    ) {
      cat = "C-custom-directive";
    } else if (
      firstDiffVue.includes("_ssrRenderVNode") ||
      firstDiffVerter.includes("_ssrRenderVNode")
    ) {
      cat = "F-ssrRenderVNode";
    } else if (
      firstDiffVue.includes("v-model") ||
      firstDiffVerter.includes("v-model") ||
      firstDiffVue.includes("onUpdate:model") ||
      firstDiffVerter.includes("onUpdate:model")
    ) {
      cat = "E-v-model";
    } else if (diff.includes("Teleport") || diff.includes("teleport")) {
      cat = "G-teleport";
    } else if (
      firstDiffVue.includes("_ssrRenderSlotInner") ||
      firstDiffVerter.includes("_ssrRenderSlotInner")
    ) {
      cat = "H-slotInner";
  ...
  if (examples[cat].length < 3) {
    examples[cat].push({
      file: m.file,
      diffVue: firstDiffVue.slice(0, 120),
      diffVerter: firstDiffVerter.slice(0, 120),
    });
  }
}
