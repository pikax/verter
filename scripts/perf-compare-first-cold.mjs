...
function main() {
  ...
  for (const name of COMPONENTS) {
    ...
    if (!within) {
      failed.push(`${name}: ${fmtMs(cur)}ms exceeds baseline ${fmtMs(base)}ms by ${fmtPct(deltaPct)}`);
    }
  }
  ...
  if (!totalWithin) {
    failed.push(`aggregate: ${fmtMs(currentTotal)}ms exceeds baseline ${fmtMs(baselineTotal)}ms by ${fmtPct(totalDeltaPct)}`);
  }
