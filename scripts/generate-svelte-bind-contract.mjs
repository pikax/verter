...
/*
  ...
  crates/verter_compiler/tests/cases/svelte_generated_artifacts_freshness.rs — a registry
...
export function generateSvelteBindContract(root, outPathOverride) {
  const lines = [];
  ...
  lines.push("// gate (svelte_generated_artifacts_freshness.rs) fails.");
  lines.push("");
  ...
  // The combined freshness checker passes a temp output explicitly. The env
  // override remains available to direct CLI callers and focused tests.
  const outPath =
    outPathOverride ||
...
if (isMain) {
  ...
  const out = generateSvelteBindContract(root);
