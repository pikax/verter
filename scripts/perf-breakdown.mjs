...
const VERTER_TSC = join(
  resolve(import.meta.dirname, ".."),
  "target",
  "release",
  `verter-tsc${process.platform === "win32" ? ".exe" : ""}`,
);
...
for (const p of PROJECTS) {
  ...
  if (!existsSync(join(root, "node_modules"))) {
    console.log(`  ${p.name}: SKIP (no node_modules)`);
    continue;
  }

  const tsconfig = join(root, p.tsconfig);
  if (!existsSync(tsconfig)) {
    console.log(`  ${p.name}: SKIP (no ${p.tsconfig})`);
    continue;
  }
...
console.log(
  '  If overhead >> 0, the .tsc.tsx files add significant tsc work (import("vue") generic evaluation)',
);
console.log(
  "  If verter-tsc < vue-tsc, verter wins. If verter-tsc > vue-tsc, Volar is more efficient.\n",
);
