...
const ACKNOWLEDGED_GAPS = [{ file: PS_FILE, anchor: "could not be checked — git could not resolve" }];
...
function applied(base, mutated, label) {
  assert.notEqual(mutated, base, `mutation ${label} did not change the fixture text — vacuous mutation`);
  return mutated;
}
...
function runPS(dagPath, statePath, mode, extraArgs = [], { cwd = gitRoot, env } = {}) {
  ...
  const res = spawnSync(
    process.execPath,
    [PS_FILE, "--dag", dagPath, "--state", statePath, "--mode", mode, ...extraArgs],
    opts,
  );
...
function dagBlock({ id, name = id, class: cls = "foundational", predecessors, conditional }) {
  const predLine =
    predecessors === undefined ? "" : `predecessors = [${predecessors.map((p) => `"${p}"`).join(", ")}]\n`;
...
function header({ status = "ACTIVE", current, repoSha = SHA, dagDigest = DAG1_DIGEST, extraTop = "", orchestration = "" }) {
  return `schema = 1
revision = 11
status = "${status}"
...
test("[PS] tree field is not the tree of its paired sha", () => {
  ...
  const state = write(
    "state-git5.toml",
    header({ current: "A0" }) + block("A0", "LOCKED", { candidate_sha: SHA, candidate_tree: TREE_BASE }),
  );
  const r = runPS(dag, state, "live");
...
test("[PS] ACCEPTED with a dangling (unreachable) accepted_sha", () => {
  ...
  const state = write(
    "state-git8.toml",
    header({ current: "A0" }) + acceptedBlock("A0", { accepted_sha: SHA_DANGLING, landing_equivalence_digest: DIGEST }),
  );
  const r = runPS(dag, state, "live");
...
test("[PS] merge-base subprocess failure", () => {
  ...
  const state = write(
    "state-git9.toml",
    header({ current: "A0" }) +
      acceptedBlock("A0", { base_sha: SHA_BASE, accepted_sha: SHA, landing_equivalence_digest: DIGEST }),
  );
...
test("[PS] base_sha is not an ancestor of accepted_sha", () => {
  ...
  const state = write(
    "state-git10.toml",
    header({ current: "A0" }) +
      acceptedBlock("A0", { base_sha: SHA, accepted_sha: SHA_BASE, landing_equivalence_digest: DIGEST }),
  );
  const r = runPS(dag, state, "live");
...
test("[PS] state schema/revision mismatched against DAG", () => {
  ...
  const state = write("state-hdr2.toml", header({ current: "A0", dagDigest }) + block("A0", "LOCKED"));
  const r = runPS(dag, state, "live");
...
test("[PS] live top-level status is not ACTIVE", () => {
  ...
  const state = write("state-hdr3.toml", header({ current: "A0", status: "PAUSED" }) + block("A0", "LOCKED"));
  const r = runPS(dag, state, "live");
...
test("[PS] empty program_dag_digest in live mode", () => {
  ...
  const state = write("state-hdr4.toml", header({ current: "A0", dagDigest: "" }) + block("A0", "LOCKED"));
  const r = runPS(dag, state, "live");
...
test("[PS] program_dag_digest mismatched against the real DAG file hash", () => {
  ...
  const state = write("state-hdr5.toml", header({ current: "A0", dagDigest: DIGEST2 }) + block("A0", "LOCKED"));
  const r = runPS(dag, state, "live");
...
test("[PS] DAG duplicate block id", () => {
  ...
  const state = write(
    "state-hdr6.toml",
    header({ current: "A0", dagDigest: createHash("sha256").update(readFileSync(dag)).digest("hex") }) +
      block("A0", "LOCKED"),
  );
  const r = runPS(dag, state, "live");
...
test("[PS] DAG block has no predecessors array", () => {
  ...
  assert.ok(!readFileSync(dag, "utf8").includes("predecessors"), "predecessors line must be absent");
  const state = write(
    "state-hdr7.toml",
    header({ current: "A0", dagDigest: createHash("sha256").update(readFileSync(dag)).digest("hex") }) +
      block("A0", "LOCKED"),
...
test("[PS] state block status outside the declared enum", () => {
  ...
  const state = write("state-status2.toml", header({ current: "A0" }) + block("A0", "BOGUS_STATUS"));
  const r = runPS(dag, state, "live");
...
test("[PS] PASS mandate with no reviewed sha bound", () => {
  ...
  const state = write(
    "state-verdict1.toml",
    header({ current: "A0" }) + block("A0", "LOCKED", { conformance_review: "PASS", conformance_reviewed_sha: "" }),
  );
  const r = runPS(dag, state, "live");
...
test("[PS] PASS mandate whose candidate_sha is not well-formed", () => {
  ...
  const state = write(
    "state-verdict2.toml",
    header({ current: "A0" }) +
      block("A0", "LOCKED", { conformance_review: "PASS", conformance_reviewed_sha: SHA, candidate_sha: "" }),
  );
  const r = runPS(dag, state, "live");
...
test("[PS] opened conditional predecessor not accepted", () => {
  ...
  const state = write(
    "state-seq2.toml",
    header({ current: "A1", dagDigest: createHash("sha256").update(readFileSync(dag)).digest("hex") }) +
      acceptedBlock("A0") +
      "\n" +
...
test("[PS] stacked-work exception: mismatched snapshot digest", () => {
  ...
  const state = write(
    "state-seq5.toml",
    header({ current: "A0", dagDigest: DAG3_DIGEST }) +
      block("A0", "IN_PROGRESS", { stack_id: "S1", stack_snapshot_digest: DIGEST2, stack_layer: 0 }) +
      "\n" +
      block("A1", "REVIEW", {
        stack_id: "S1",
        stack_snapshot_digest: DIGEST,
...
test("[PS] ACCEPTED with diverged identity and no landing-equivalence digest", () => {
  ...
  const state = write(
    "state-gate5.toml",
    header({ current: "A0" }) +
      acceptedBlock("A0", { accepted_sha: SHA_BASE, accepted_tree: TREE_BASE, landing_equivalence_digest: "" }),
  );
  const r = runPS(dag, state, "live");
  expectCheck(
    PS_FILE,
...
test("[PS] IN_PROGRESS block disagrees with current_block", () => {
  ...
  const state = write(
    "state-cur3.toml",
    header({ current: "B0", dagDigest: createHash("sha256").update(readFileSync(dag)).digest("hex") }) +
      block("A0", "IN_PROGRESS") +
      "\n" +
      block("B0", "LOCKED"),
  );
...
test("[PS] orchestration.evidence_root is not a resolvable directory", () => {
  ...
  const r = runPS(dag, state, "live");
  expectCheck(PS_FILE, "is not a resolvable directory — evidence_digest bindings cannot be verified", r);
});
...
function windowText({ mode = "ATOMIC_REVIEW", acceptanceBlockId = "", status = "ACTIVE", overrides = {}, layers = [layerText()] } = {}) {
  const fields = {
    schema: 1,
    revision: 11,
    status,
...
test("[SWL] unknown top-level mode value", () => {
  const p = write("swl-7.toml", windowText({ mode: "LANDABLE", overrides: { mode: "SOMETHING_ELSE" } }));
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] top-level digest field malformed", () => {
  const p = write("swl-9.toml", windowText({ mode: "LANDABLE", overrides: { authority_package_digest: "not-a-digest" } }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "top-level ${field} is not a ", r);
});

test("[SWL] previous_stack_snapshot_digest malformed", () => {
  const p = write("swl-10.toml", windowText({ mode: "LANDABLE", overrides: { previous_stack_snapshot_digest: "bogus" } }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "previous_stack_snapshot_digest is not", r);
});

test("[SWL] root_base_sha malformed", () => {
  const p = write("swl-11.toml", windowText({ mode: "LANDABLE", overrides: { root_base_sha: "not-a-sha" } }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "top-level root_base_sha is not a resolved 40-char lowercase git object id:", r);
});

test("[SWL] root_base_tree malformed", () => {
  const p = write("swl-12.toml", windowText({ mode: "LANDABLE", overrides: { root_base_tree: "not-a-sha" } }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "top-level root_base_tree is not a resolved 40-char lowercase tree object id:", r);
});

test("[SWL] max_open_layers outside [2, 6]", () => {
  const p = write("swl-13.toml", windowText({ mode: "LANDABLE", overrides: { max_open_layers: 7 } }));
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] layer required string field is empty", () => {
  const p = write("swl-16.toml", windowText({ mode: "LANDABLE", layers: [layerText({ worker: "" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "${where} ${field} is not a non-empty string:", r);
});

test("[SWL] layer optional string field is not a string", () => {
  const p = write("swl-17.toml", windowText({ mode: "LANDABLE", layers: [layerText({ notes: 5 })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] duplicate layer_id", () => {
  const p = write(
    ...
    windowText({
      mode: "LANDABLE",
      layers: [layerText({ index: 1, layer_id: "L1", block_id: "B1" }), layerText({ index: 2, layer_id: "L1", block_id: "B2" })],
    }),
  );
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] layer index is not a positive integer", () => {
  const p = write("swl-19.toml", windowText({ mode: "LANDABLE", layers: [layerText({ index: 0 })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] duplicate layer index", () => {
  const p = write(
    ...
    windowText({
      mode: "LANDABLE",
      layers: [layerText({ index: 1, layer_id: "L1", block_id: "B1" }), layerText({ index: 1, layer_id: "L2", block_id: "B2" })],
    }),
  );
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] layer kind outside the declared enum", () => {
  const p = write("swl-21.toml", windowText({ mode: "LANDABLE", layers: [layerText({ kind: "BOGUS" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] pr_number is not a non-negative integer", () => {
  const p = write("swl-22.toml", windowText({ mode: "LANDABLE", layers: [layerText({ pr_number: -1 })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "pr_number is not a non-negative integer:", r);
});

test("[SWL] mergeable is not a boolean", () => {
  const p = write("swl-23.toml", windowText({ mode: "LANDABLE", layers: [layerText({ mergeable: "notabool" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "mergeable is not a boolean:", r);
});

test("[SWL] layer charter_digest malformed", () => {
  const p = write("swl-24.toml", windowText({ mode: "LANDABLE", layers: [layerText({ charter_digest: "not-a-digest" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "${where} charter_digest is not a resolved SHA-256", r);
});

test("[SWL] layer base_sha malformed", () => {
  const p = write("swl-25.toml", windowText({ mode: "LANDABLE", layers: [layerText({ base_sha: "not-a-sha" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "40-char lowercase git object id or empty", r);
});

test("[SWL] layer base_tree malformed", () => {
  const p = write("swl-26.toml", windowText({ mode: "LANDABLE", layers: [layerText({ base_tree: "not-a-sha" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
  expectCheck(SWL_FILE, "tree object id or empty", r);
});

test("[SWL] layer patch_digest malformed", () => {
  const p = write("swl-27.toml", windowText({ mode: "LANDABLE", layers: [layerText({ patch_digest: "not-a-digest" })] }));
  const r = runSW(["--window", p, "--mode", "live"]);
...
test("[SWL] LANDABLE mode with a duplicate block_id across layers", () => {
  const p = write(
    ...
    windowText({
      mode: "LANDABLE",
      layers: [layerText({ index: 1, layer_id: "L1", block_id: "B1" }), layerText({ index: 2, layer_id: "L2", block_id: "B1" })],
    }),
  );
  const r = runSW(["--window", p, "--mode", "live"]);
...
function atomicWindow({ acceptanceBlockId = "D2", d1Kind = "NON_MERGEABLE_PRIVATE_LAYER", d2Kind = "mergeable", d1Index = 1, d2Index = 2, overrides = {} } = {}) {
  return windowText({
    mode: "ATOMIC_REVIEW",
    acceptanceBlockId,
    overrides: { max_open_layers: 2, ...overrides },
    layers: [
      layerText({ index: d1Index, layer_id: "D1", block_id: "D1", kind: d1Kind, mergeable: d1Kind === "mergeable", base_branch: "main", branch: "d1" }),
      layerText({ index: d2Index, layer_id: "D2", block_id: "D2", kind: d2Kind, mergeable: d2Kind === "mergeable", base_branch: "d1", branch: "d2" }),
    ],
  });
}
...
test("[SWL] --dag: private layer names a non-checkpoint block", () => {
  ...
  expectCheck(SWL_FILE, "a private ATOMIC_REVIEW layer must repeat the acceptance block's own id", r);
});

function ledgerText({ d1Snapshot, d2Snapshot, d1StackId = "S1", d2StackId = "S1", d1Layer = 1, d2Layer = 2, d1Status = "PRIVATE_CHECKPOINT" } = {}) {
...
test("[SWL] --current-program-state: stack_id mismatch", () => {
  ...
  const state = write("swl-34-state.toml", ledgerText({ d1Snapshot: snap, d2Snapshot: snap, d1StackId: "OTHER" }));
  const r = runSW(["--window", p, "--mode", "live", "--current-program-state", state]);
...
test("[SWL] --current-program-state: stack_layer mismatch", () => {
  ...
  const snap = createHash("sha256").update(readFileSync(p)).digest("hex");
  const state = write("swl-36-state.toml", ledgerText({ d1Snapshot: snap, d2Snapshot: snap, d1Layer: 5 }));
  const r = runSW(["--window", p, "--mode", "live", "--current-program-state", state]);
...
function cpWindowText({ acceptanceId = "A2", a1Kind = "NON_MERGEABLE_PRIVATE_LAYER", a1Index = 1, a2Index = 2 } = {}) {
  return atomicWindow({ acceptanceBlockId: acceptanceId, d1Kind: a1Kind, d1Index: a1Index, d2Index: a2Index })
...
test("[SWL via PS] window mode is not ATOMIC_REVIEW", () => {
  ...
  const windowText_ = cpWindowText().replace('mode = "ATOMIC_REVIEW"', 'mode = "LANDABLE"').replace('acceptance_block_id = "A2"', 'acceptance_block_id = ""');
  ...
  expectCheck(SWL_FILE, "not ATOMIC_REVIEW — a PRIVATE_CHECKPOINT predecessor is legalized only", r);
});
...
test("[SWL via PS] window declares no layer for the predecessor block", () => {
  ...
  const windowText_ = applied(base, base.replace('block_id = "A1"', 'block_id = "RENAMED"'), "rename A1 layer's block_id");
...
test("[SWL via PS] predecessor layer is independently mergeable", () => {
  ...
  const r = runPS(dag, state, "live", ["--stack-window", windowPath]);
  expectCheck(SWL_FILE, "a checkpoint predecessor's layer must never be independently mergeable", r);
});
...
test("coverage: every derived check was tripped by at least one mutation", () => {
  ...
  assert.ok(registry.checks.length > 100, `expected a large derived inventory, got ${registry.checks.length} — extraction may be broken`);
});
