...
function detectPackageManager(repoRoot, rootPackageRaw) {
  if (rootPackageRaw) {
    ...
    if (packageManager.startsWith('pnpm@')) return { packageManager: 'pnpm', lockfile: 'pnpm-lock.yaml' };
    if (packageManager.startsWith('npm@')) return { packageManager: 'npm', lockfile: 'package-lock.json' };
    if (packageManager.startsWith('yarn@')) return { packageManager: 'yarn', lockfile: 'yarn.lock' };
...
function extractSurfaceFlags(repoRoot, scan, rootPackageRaw) {
  ...
  for (const packageJson of scan.packageJsonFiles) {
    ...
    if (/"vue-tsc"\s*:/u.test(raw) || /(?:^|[^A-Za-z0-9_-])vue-tsc(?:$|[^A-Za-z0-9_-])/u.test(raw)) {
      hasVueTsc = true;
    }
...
function pickCommands(rootPackageRaw) {
  ...
  const buildCmd = typeof scripts.build === 'string' && scripts.build.trim() ? scripts.build.trim() : null;
  let testCmd = null;
...
function classifyRecipe({
  ...
  const hasExecutableTypecheck = typecheckSurface && Boolean(packageManager) && Boolean(chosenTsconfig);
  const hasExecutableBuild = buildSurface && Boolean(buildBundler) && Boolean(packageManager) && Boolean(buildCmd);
...
function createReplacementSteps(surfaceFlags, recipe) {
  ...
  if (surfaceFlags.hasVueTypeScriptPlugin || surfaceFlags.hasVueLanguageTools || surfaceFlags.hasVueTsc) {
...
function buildTier1Entries(matrixProjects) {
  return matrixProjects.map((project) => ({
    ...
    replacementSteps: [
      project.bundler === 'nuxt' ? 'nuxt-module' : 'build-plugin',
      'verter-tsc',
    ],
...
export function analyzeRepo(repoRoot, { discoveryRoot } = {}) {
  ...
  const editorSurface = surfaceFlags.workspaceMentionsVolar || surfaceFlags.workspaceMentionsVuePlugin;
  const typecheckSurface = surfaceFlags.hasVueTsc
  ...
  const rawRelativeRoot = discoveryRoot ? normalizePath(path.relative(discoveryRoot, repoRoot)) : normalizePath(repoRoot);
  const relativeRoot = rawRelativeRoot && rawRelativeRoot !== '.' ? rawRelativeRoot : path.basename(repoRoot);
  const id = relativeRoot.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || path.basename(repoRoot);

  const reasons = [];
  if (recipe === 'manual_review') {
    if (surfaceFlags.isToolchainRepo) reasons.push('repo is verter toolchain');
    if (surfaceFlags.buildSurfaceKinds.length > 1) reasons.push(`ambiguous build surface: ${surfaceFlags.buildSurfaceKinds.join(', ')}`);
    if (typecheckSurface && !chosenTsconfig) reasons.push('missing root tsconfig');
    if ((typecheckSurface || buildSurface) && !packageManager) reasons.push('missing package manager');
...
export function renderDiscoveryMarkdown(inventory) {
  ...
  for (const project of inventory.tier1Projects) {
    lines.push(`| ${project.name} | ${project.buildBundler} | ${project.packageManager} | ${project.buildCmd ?? '-'} | ${project.testCmd ?? '-'} |`);
  }
  lines.push('');
  ...
  for (const recipe of ['full_stack', 'typecheck_only', 'editor_only', 'build_only', 'manual_review']) {
    ...
    for (const project of items) {
      ...
      lines.push(`| ${project.relativeRoot} | ${project.executionTier} | ${steps} | ${project.packageManager ?? '-'} | ${project.surfaces.buildBundler ?? '-'} | ${project.buildCmd ?? '-'} | ${project.testCmd ?? '-'} | ${project.chosenTsconfig ?? '-'} | ${reasons} |`);
    }
    lines.push('');
  }
