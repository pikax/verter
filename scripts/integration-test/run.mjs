...
function runTypeChecks(project, repoDir) {
  ...
  try {
    ...
    if (!hasFiles && !hasInclude && hasRefs) {
      ...
      if (tsconfig === rootTsconfig) {
        log(
          project.name,
          `[type-check] root tsconfig is references-only, no alternative found — skipping`,
        );
        return null;
      }
    }
  } catch {
    /* parse error — try with the root tsconfig anyway */
  }
  ...
  const fmtTcMs = (r) =>
    r.timedOut ? ">5min" : formatDuration(r.ms) + (r.exitCode !== 0 ? "(err)" : "");
...
function prepareLocalSandbox(project, runContext) {
  ...
  copyRecursive(
    project.repoRoot,
    sandboxDir,
    LOCAL_SANDBOX_SKIP_ALWAYS,
    LOCAL_SANDBOX_SKIP_TOP_ONLY,
  );
...
function writeProjectSummary(project, artifactDir, result, diff, queue) {
  ...
  if (project.replacementSteps?.length)
    lines.push(`- Steps: ${project.replacementSteps.join(", ")}`);
  ...
  if (queue?.items?.length) {
    ...
    for (const item of queue.items.slice(0, 25)) {
      lines.push(
        `- ${item.status} ${item.classification} ${item.code ?? "-"} ${item.file ?? "-"}:${item.line ?? "-"} ${item.message}`,
      );
    }
    lines.push("");
  }
  ...
  const candidates = [
    ...findFiles(
      ...
      (name, full) =>
        ...
        (["settings.json", "extensions.json"].includes(name) &&
          full.includes(`${path.sep}.vscode${path.sep}`)),
    ),
  ];
...
function replaceTypeScriptTooling(project, repoDir) {
  ...
  for (const filePath of configFiles) {
    ...
    const updated = content
      .split("@vue/typescript-plugin")
      .join("@verter/typescript-plugin")
      .split("vue-tsc")
      .join("verter-tsc")
      .split("Vue.vscode-typescript-vue-plugin")
      .join(VERTER_EXTENSION_ID);
...
function cloneProject(project) {
  ...
  if (fs.existsSync(path.join(repoDir, ".git"))) {
    ...
    const depthFlag = project.shallow === false ? "" : "--depth 1";
    const result = run(
      `git clone ${depthFlag} --branch ${project.branch} https://github.com/${project.repo}.git "${repoDir}"`,
      ROOT,
    );
    if (!result.ok) {
...
function fixWindowsScripts(project, repoDir) {
  ...
  for (const [name, script] of Object.entries(pkg.scripts)) {
    if (
      ...
      envPrefixRe.test(script) &&
      !script.startsWith("cross-env ")
    ) {
...
function expectedNativeBinaries() {
  ...
  switch (platform) {
    case "win32":
      switch (arch) {
        case "x64":
          return ["verter-native.win32-x64-msvc.node", "verter.win32-x64-msvc.node"];
        case "ia32":
          return ["verter-native.win32-ia32-msvc.node", "verter.win32-ia32-msvc.node"];
        case "arm64":
          return ["verter-native.win32-arm64-msvc.node", "verter.win32-arm64-msvc.node"];
      }
      break;
    case "darwin":
      switch (arch) {
...
function installVerterTarballs(project, repoDir) {
  ...
  const relUnplugin = path
    .relative(repoDir, path.join(TARBALLS_DIR, unpluginTgz))
    .replace(/\\/g, "/");
  const relNuxt = nuxtTgz
    ? path.relative(repoDir, path.join(TARBALLS_DIR, nuxtTgz)).replace(/\\/g, "/")
    : null;
  ...
  if (project.packageManager === "pnpm") {
    ...
    // Delete lockfile so pnpm generates a fresh one matching the new overrides.
    // Without this, `pnpm run` commands may re-check lockfile consistency and fail
  ...
  for (const { dist: destDist, pkg } of distsToOverwrite) {
    ...
    if (pkg === "native") {
      if (!validateNativeDist(srcDist)) {
        log(
          project.name,
          `  WARNING: @verter/native source dist has no .node binary. Build native bindings first. Skipping overwrite.`,
        );
        continue;
      }
    } else {
      ...
      if (missing.length > 0) {
        log(
          project.name,
          `  WARNING: @verter/${pkg} source dist is missing: ${missing.join(", ")}. Build @verter/${pkg} first. Skipping overwrite.`,
        );
        continue;
      }
    }
  ...
  // Also overwrite root-level files (package.json, index.js, index.ts) for each
  // @verter package whose dist was successfully overwritten.  package.json is
  // needed so that entry-point fields (main, module, exports) match the actual
  // dist filenames produced by the current build toolchain.
  const rootFiles = ["package.json", "index.js", "index.ts"];
  for (const { dist: destDist, pkg } of distsToOverwrite) {
...
function ensureVerterAccessible(project, repoDir) {
  ...
  if (project.bundler === "nuxt") {
    ...
    ensureNuxtDepsResolvable(project, repoDir, nuxtDir, unpluginDir);
  }
...
/**
 * Ensure that @verter/nuxt's dependencies (@nuxt/kit, @verter/unplugin)
 * are resolvable from its manually-copied location.  pnpm strict isolation
 * means these won't be in the Node resolution path unless symlinked.
 */
function ensureNuxtDepsResolvable(project, repoDir, nuxtDir, unpluginDir) {
  const localNM = path.join(nuxtDir, "node_modules");

  // @nuxt/kit — find in top-level or .pnpm store
  const nuxtKitTarget = path.join(localNM, "@nuxt", "kit");
  if (!fs.existsSync(path.join(nuxtKitTarget, "package.json"))) {
    let kitSource = null;
    const topKit = path.join(repoDir, "node_modules", "@nuxt", "kit");
    if (fs.existsSync(path.join(topKit, "package.json"))) {
      kitSource = topKit;
    } else {
      const pnpmDir = path.join(repoDir, "node_modules", ".pnpm");
      if (fs.existsSync(pnpmDir)) {
        for (const entry of fs.readdirSync(pnpmDir)) {
          if (entry.startsWith("@nuxt+kit@")) {
            const candidate = path.join(pnpmDir, entry, "node_modules", "@nuxt", "kit");
    ...
    if (kitSource) {
      fs.mkdirSync(path.join(localNM, "@nuxt"), { recursive: true });
      try {
        fs.symlinkSync(kitSource, nuxtKitTarget, "junction");
      } catch {
        copyRecursive(kitSource, nuxtKitTarget);
      }
      log(project.name, `  Linked @nuxt/kit from ${path.relative(repoDir, kitSource)}`);
    }
  }

  // @verter/unplugin — symlink to the already-resolved copy
  const verterUnpluginTarget = path.join(localNM, "@verter", "unplugin");
  if (!fs.existsSync(path.join(verterUnpluginTarget, "package.json"))) {
    if (fs.existsSync(path.join(unpluginDir, "package.json"))) {
      fs.mkdirSync(path.join(localNM, "@verter"), { recursive: true });
      try {
        fs.symlinkSync(unpluginDir, verterUnpluginTarget, "junction");
      } catch {
        copyRecursive(unpluginDir, verterUnpluginTarget);
      }
      log(project.name, `  Linked @verter/unplugin into @verter/nuxt`);
    }
  }
}
...
function ensureUnpluginResolvable(project, repoDir, unpluginDir) {
  ...
  try {
    fs.rmSync(localUnpluginTarget, { recursive: true, force: true });
  } catch {}
  ...
  if (!unpluginSource) {
    ...
    if (project.packageManager === "pnpm") {
      ...
      if (!installResult.ok) {
        log(
          project.name,
          `  Warning: failed to install unplugin via pnpm, falling back to source copy`,
        );
      }
    } else {
      const installResult = run("npm install --legacy-peer-deps unplugin", repoDir, {
        timeout: 60_000,
      });
      if (!installResult.ok) {
        log(
          project.name,
          `  Warning: failed to install unplugin via npm, falling back to source copy`,
        );
      }
    }
    if (fs.existsSync(path.join(topLevel, "package.json"))) {
...
function patchTsdownConfigs(project, repoDir) {
  ...
  inputOptions: (defaults) => {
    ...
    if (fromViteIdx >= 0) {
      ...
      if (lineEnd >= 0) {
        modified =
          modified.slice(0, lineEnd + 1) + inputOptionsBlock + "\n" + modified.slice(lineEnd + 1);
      }
    }
  ...
  for (const configPath of nuxtConfigs) {
    ...
    if (modulesMatch) {
      ...
      if (configMatch) {
        ...
        log(
          project.name,
          `  WARNING: Could not inject module into ${path.relative(repoDir, configPath)}`,
        );
        continue;
      }
    }
...
async function processLocalProject(project, opts, runContext) {
  ...
  try {
    ...
    const replaceTypeScript =
      replacementSteps.has("typescript-plugin") || replacementSteps.has("verter-tsc");
    ...
    if (runBuilds) {
      const buildProject = {
        ...project,
        bundler: project.surfaces?.buildBundler ?? project.bundler,
      };
      installVerterTarballs(buildProject, repoDir);
      const modified =
      ...
      if (!results.replacement.verified) {
        ...
        writeProjectSummary(
          ...
          artifactDir,
          results,
          typeCheckArtifacts.diff,
          typeCheckArtifacts.queue,
        );
        return results;
      }
    } else {
...
async function processProject(project, opts) {
  ...
  try {
    const repoDir = opts.noClone ? path.join(REPOS_DIR, project.name) : cloneProject(project);
    ...
    if (!results.replacement.verified) {
      ...
      writeProjectSummary(
        { ...project, executionTier: "tier1", replacementRecipe: "full_stack" },
        logDir,
        results,
        typeArtifacts.diff,
        typeArtifacts.queue,
...
function printSummary(allResults) {
  ...
  if (tcResults.length > 0) {
    ...
    for (const r of tcResults) {
      ...
      const vtscActuallyRan =
        vtscDone && (tc.verterTsc.warm.exitCode === 0 || tc.verterTsc.warm.ms > 2000);
...
function writeDiscoveryArtifacts(runContext, inventory) {
  ...
  fs.writeFileSync(
    path.join(runContext.runDir, "discovery.md"),
    renderDiscoveryMarkdown(inventory),
  );
}
...
function sortLocalProjects(projectsToRun) {
  ...
  return [...projectsToRun].sort((a, b) => {
    const recipeDelta =
      recipeOrder.indexOf(a.replacementRecipe) - recipeOrder.indexOf(b.replacementRecipe);
...
async function main() {
  ...
  if (shouldDiscoverLocal) {
    ...
    const localMatches = inventory.localProjects.filter((project) =>
      matchesSelection(project, opts.projectNames),
    );
    manualReviewSelected = localMatches.filter(
      (project) => project.replacementRecipe === "manual_review",
    );
    localSelected = sortLocalProjects(
      localMatches.filter(
        (project) =>
          project.executionTier === "tier2" && project.replacementRecipe !== "manual_review",
      ),
    );
    ...
    if (manualReviewSelected.length > 0) {
      console.log(
        `  Manual review required: ${manualReviewSelected.map((project) => project.relativeRoot).join(", ")}`,
      );
    }
    console.log("");
  }
  ...
  const unknown = opts.projectNames.filter((name) => {
    ...
    const inLocal =
      inventory?.localProjects.some((project) => matchesSelection(project, [name])) ?? false;
  ...
  if (shouldRunMatrix || shouldRunLocal) {
    ...
    if (!opts.skipBuild) {
      ...
      const needsTarballs =
        ...
        localSelected.some((project) =>
          ["full_stack", "build_only"].includes(project.replacementRecipe),
        );
      const needsTypecheckBinary =
        localSelected.some((project) =>
          ["full_stack", "typecheck_only"].includes(project.replacementRecipe),
        ) ||
        (shouldRunMatrix && selected.length > 0);
  ...
  if (shouldRunLocal) {
    ...
    if (opts.concurrency <= 1) {
      for (const project of localSelected) {
        ...
        log(
          project.name,
          `Starting local (${project.packageManager ?? "unknown"}, ${project.surfaces?.buildBundler ?? "n/a"})`,
        );
