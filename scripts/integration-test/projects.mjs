 ...
 * @property {boolean}        [shallow]      - Clone with --depth 1 (default true). Set false for projects needing full git history.
 */
...
export const projects = [
  ...
  },
  {
    ...
    e2eCmd:
      'DEV_LOG=/tmp/slidev-dev.log; pnpm -C cypress/fixtures/basic run dev > "$DEV_LOG" 2>&1 & DEV_PID=$!; sleep 15 && npx cypress run --headless; EXIT=$?; echo "--- Dev server log ---"; cat "$DEV_LOG" 2>/dev/null || true; kill $DEV_PID 2>/dev/null; wait $DEV_PID 2>/dev/null; exit $EXIT',
    packageManager: "pnpm",
    bundler: "vite",
  },
  {
  ...
  },
  {
    ...
    buildCmd:
      "pnpm run build:metadata && pnpm run build:resolver && pnpm run build:core && pnpm run build:lib",
    testCmd: "",
    packageManager: "pnpm",
    bundler: "rollup",
  },
