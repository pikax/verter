...
function listCurrentTestBinaries() {
  ...
  if (run.status !== 0) {
    ...
    fail(
      `\`cargo nextest list\` exited with status ${run.status}.\n${stderrTail}`,
    );
  }
...
function main() {
  ...
  const report = {
    ...
    currentTestBinariesSizeHuman:
      currentTestBinariesSizeBytes == null
        ? null
        : humanSize(currentTestBinariesSizeBytes),
    staleExecutables,
  };
...
function emit(report, opts, extra) {
  ...
  if (report.currentTestBinaries != null) {
    ...
    lines.push(
      `  stale execs  : ${report.staleExecutables} (dep execs not in current set)`,
    );
  } else {
    lines.push(
