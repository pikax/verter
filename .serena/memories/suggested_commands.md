...
Testing:
- `cargo nextest run --workspace` - canonical completeness gate for every Rust workspace test target.
- `cargo test -p verter_session --tests` - canonical shared-process session surface; required alongside Nextest.
...
- `pnpm exec vitest run path/to/test.spec.ts` - specific TS test file.
...
- `cargo clippy --workspace -- -D warnings` - warning-denied Rust lint pass.
