...
- Default Rust verification is the canonical pair: `cargo nextest run --workspace` for every workspace test target, plus `cargo test -p verter_session --tests` for the shared-process session surface. Do not use bare `cargo test --workspace` unless doctests or examples are relevant or explicitly requested.
...
- TypeScript packages are adapters and distribution surfaces; parsing, SFC lowering, IDE code generation, and runtime code generation are Rust-owned.
...
- Common scopes: `compiler`, `session`, `napi`, `wasm`, `play`, `unplugin`, `lsp`, `types`, `meta`, `ci`, `*`.
