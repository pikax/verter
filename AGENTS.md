...
- Default Rust verification gate is `node scripts/gate.mjs` (see `CLAUDE.md` → Testing / `/testing`). It builds the test universe once via `cargo nextest archive`, then runs `cargo nextest run --workspace` (process isolation) and the in-process `verter_session` integration surface (`cargo test -p verter_session --tests`) from that same archive, plus a shipped-`cfg(debug_assertions)`-off SURFACE 3 for `verter_session`/`verter_scheduler`. A contributor without Node, or debugging one surface in isolation, runs `cargo nextest run --workspace` then `cargo test -p verter_session --tests` directly. Bare `cargo test --workspace --tests` historically silently skipped the verter_session integration suite due to a now-removed `session_metrics` Cargo feature (metrics are a runtime `HostConfig.metrics_enabled` toggle today); the skip no longer reproduces, but `node scripts/gate.mjs` stays the sole Rust gate regardless — it is the only path that also runs SURFACE 3.
