...
- Default local Rust verification is `node scripts/gate.mjs` (see `CLAUDE.md` → Testing / `/testing`). It builds/lists the workspace archive once, then runs provider-free core Surface 1 plus the required shipped-`cfg(debug_assertions)` lane (`cargo check --workspace --all-targets --profile no-debug-assertions`, then the `verter_shipped_cfg_contract` nextest run under that profile); a PASS requires complete receipts from both, and the lane has no skip disposition. CI, release, complete diagnostics, and comparable benchmarks use `node scripts/gate.mjs --exhaustive`, which changes failure collection only (`--no-fail-fast`). Real providers, Svelte conformance, compile contracts, and proto freshness have dedicated lanes and are not part of the canonical nextest run. Bare `cargo test --workspace --tests` is not the canonical gate.
