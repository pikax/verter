...
- Default local Rust verification is `node scripts/gate.mjs` (see `CLAUDE.md` → Testing / `/testing`). It builds/lists the test universe once, then overlaps archive-backed Surface 1 with the isolated serial shipped-`cfg(debug_assertions)` check/contract lane. A hard Surface-1 receipt cancels any live shipped step or prevents its remaining contract, so a local red is always an incomplete FAIL. CI, release, complete diagnostics, and comparable benchmarks use `node scripts/gate.mjs --exhaustive`, which changes failure collection only: both nextest invocations collect all failures and both lanes are awaited after ordinary test failures. A contributor without Node, or debugging Surface 1 in isolation, runs `cargo nextest run --workspace` directly. Bare `cargo test --workspace --tests` is not the canonical gate because it omits the shipped-cfg guard.
